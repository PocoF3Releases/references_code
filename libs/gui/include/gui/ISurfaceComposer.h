...
public:
    ...
    virtual status_t setFrameRateVideoToDisplay(
            uint32_t cmd, Parcel *data) = 0;
...
#ifdef CURTAIN_ANIM
    virtual status_t setCurtainAnimRate(float rate) = 0;
    virtual status_t enableCurtainAnim(bool isEnable) = 0;
#endif

#ifdef MI_SF_FEATURE
    /* MIUI ADD: HDR Dimmer
     * Sets the global configuration of HDR Dimmer.
     * enable: true means open HDR Dimmer, and false means close.
     * factor: brightening factor, used to adjust dimming level.
     */
    virtual status_t enableHdrDimmer(bool enable, float factor) = 0;
#endif
    /*
    ...
    //MIUI ADD: START
    virtual status_t checkLayerNum(bool* outNumber) const = 0;
    //END

    //MIUI ADD: START
    virtual status_t producerFrameDropped(int32_t droppedFrameCount,
                                          int64_t intendedVsyncTime, String8 windowName) = 0;
    //END
};
...
public:
    enum ISurfaceComposerTag {
        ...
        SET_VIDEO_DECODER_INFO,
        // MIUI ADD: START
        CHECK_LAYER_NUMBER,
        // END
#if MI_SCREEN_PROJECTION
        // MIUI ADD:
        SET_MIUI_TRANSACTION_STATE,
#endif
        // MIUI ADD:
        PRODUCER_FRAME_DROPPED,

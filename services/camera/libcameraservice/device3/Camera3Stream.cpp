...
status_t Camera3Stream::getBuffer(camera_stream_buffer *buffer,
    ...
    ALOGV("%s: Stream[%d]: numOutstandingBuffers %zu, max_buffers %d, numCachedBuffers %zu, maxNumCachedBuffers %zu",
          __FUNCTION__, mId, numOutstandingBuffers, camera_stream::max_buffers, numCachedBuffers, maxNumCachedBuffers);
...
status_t Camera3Stream::getBuffers(std::vector<OutstandingBuffer>* buffers,
    ...
    size_t numCachedBuffers = getCachedOutputBufferCountLocked();
    size_t maxNumCachedBuffers = getMaxCachedOutputBuffersLocked();
    ALOGV("%s: Stream[%d]: numBuffersRequested %zu, numOutstandingBuffers %zu, max_buffers %d, numCachedBuffers %zu, maxNumCachedBuffers %zu",
         __FUNCTION__, mId, numBuffersRequested, numOutstandingBuffers, camera_stream::max_buffers, numCachedBuffers, maxNumCachedBuffers);
    // Wait for new buffer returned back if we are running into the limit. There
    // are 2 limits:
    // 1. The number of HAL buffers is greater than max_buffers
    // 2. The number of HAL buffers + cached buffers is greater than max_buffers
    ...
    while (numOutstandingBuffers + numBuffersRequested > camera_stream::max_buffers ||
        ...
        numCachedBuffers = getCachedOutputBufferCountLocked();
    }

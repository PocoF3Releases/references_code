...
static bool interop_dynamic_database_match_( interop_db_entry_t *entry);
...
void interop_database_name_add(const uint16_t feature, const char *name)
{
  assert(name);
  interop_database_add_name(feature,name);
}
...
static const char* interop_feature_string_(const interop_feature_t feature)
{
  switch (feature) {
    ...
    CASE_RETURN_STR(INTEROP_ENABLE_ABSOLUTE_VOLUME)
    ...
    CASE_RETURN_STR(INTEROP_NEED_NOT_RETRY_SCO_AFTER_REMOTE_REJECT_SCO)
    CASE_RETURN_STR(INTEROP_DELAY_SCO_FOR_MO_CALL)
    CASE_RETURN_STR(INTEROP_A2DP_HIDE_AA_SPECIFIC_CODEC)
    CASE_RETURN_STR(INTEROP_A2DP_MAKE_REMOTE_AS_MASTER)
    ...
    CASE_RETURN_STR(INTEROP_WEARABLE_WATCH_DEVICE_WHILTELIST)
    CASE_RETURN_STR(INTEROP_A2DP_DONOT_MAKE_REMOTE_AS_MASTER)
    CASE_RETURN_STR(INTEROP_HFP_DISABLE_INBAND_RINGTONE)
    CASE_RETURN_STR(INTEROP_HID_DISABLE_AUTOCONNECTION)
  }
...
void interop_dynamic_database_clear()
{
    assert(config_dynamic != NULL);
    pthread_mutex_lock(&file_lock);
    int fd = open (INTEROP_DYNAMIC_FILE_PATH, O_RDWR);
    LOG_DEBUG(LOG_TAG, "interop_dynamic_database_clear");
    if (fd != -1) {
        ftruncate(fd,0);
        lseek(fd,0,SEEK_SET);
        close (fd);
    ...
    config_dynamic = config_new(INTEROP_DYNAMIC_FILE_PATH);
                          ...
                          bool persist)
{
  ...
  if (!interop_database_match_(db_entry, &ret_entry,
    ...
    if (db_entry->bl_entry_type == INTEROP_ENTRY_TYPE_DYNAMIC) {
      if (interop_dynamic_database_match_(db_entry)) {
        LOG_DEBUG(LOG_TAG, "Dynamic entry is already present in the dynamic list");
        return;
      } else {
        LOG_DEBUG(LOG_TAG, "Dynamic entry is not present in the dynamic list");
      }
    } else {
...
static bool interop_dynamic_database_match_( interop_db_entry_t *entry)
{
  ...
  const list_node_t *node = list_begin(config_dynamic->sections);

  while (node != list_end(config_dynamic->sections)) {
    interop_db_entry_t *db_entry = (interop_db_entry_t *)list_node(node);
    assert(db_entry);

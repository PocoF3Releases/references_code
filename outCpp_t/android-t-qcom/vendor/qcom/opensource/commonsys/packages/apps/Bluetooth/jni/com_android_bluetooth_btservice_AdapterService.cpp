...
// MIUI ADD: START
static void interopDatabaseAddAACAddressNative(JNIEnv* env, jobject obj,
                                     jbyteArray address, int length) {
  ALOGV("%s", __func__);
  if (!sBluetoothInterface) return;
  ...
  sBluetoothInterface->interop_database_add(INTEROP_ENABLE_AAC_CODEC, (RawAddress*)addr, length);
  env->ReleaseByteArrayElements(address, addr, 0);
}

static void interopDatabaseAddNameNative(JNIEnv* env, jobject obj, int feature,
                                     jstring name) {
  ALOGV("%s", __func__);
  if (!sBluetoothInterface) return;

  sBluetoothInterface->interop_database_name_add(feature, env->GetStringUTFChars(name, NULL));
}

static void interopDatabaseAddAACNameNative(JNIEnv* env, jobject obj, jstring name) {
  ALOGV("%s", __func__);
  if (!sBluetoothInterface) return;

  sBluetoothInterface->interop_database_name_add(INTEROP_ENABLE_AAC_CODEC, env->GetStringUTFChars(name, NULL));
}
// END
...
static void miConnectLeAudioNative(JNIEnv* env, jobject obj, jbyteArray address) {
  ALOGV("%s", __func__);
  if (!sBluetoothInterface) return;
  jbyte* addr = env->GetByteArrayElements(address, NULL);
  if (addr == NULL) {
  ...
  sBluetoothInterface->connectLeAudio(addr_obj);
#endif
  env->ReleaseByteArrayElements(address, addr, 0);
}
...
static JNINativeMethod sMethods[] = {
    ...
    // MIUI ADD: START
    {"interopDatabaseAddAACAddressNative", "([BI)V", (void*)interopDatabaseAddAACAddressNative},
    {"interopDatabaseAddNameNative", "(ILjava/lang/String;)V", (void*)interopDatabaseAddNameNative},
    {"interopDatabaseAddAACNameNative", "(Ljava/lang/String;)V", (void*)interopDatabaseAddAACNameNative},
    {"miConnectLeAudioNative", "([B)V", (void*)miConnectLeAudioNative},
    // END

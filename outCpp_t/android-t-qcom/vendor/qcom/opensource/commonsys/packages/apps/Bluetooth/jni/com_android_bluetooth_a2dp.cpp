...
static void bta2dp_multicast_state_callback(btav_mulicast_state_t state) {
  ALOGI("%s", __func__);
...
static jboolean enableMultiA2dpNative(JNIEnv* env, jobject object,
                                      jbyteArray address) {
  ALOGI("%s: sBluetoothA2dpInterface: %p", __func__, sBluetoothA2dpInterface);
  ...
  #ifdef MULTI_A2DP_ENABLE
  status = sBluetoothA2dpInterface->enable_multi_a2dp(bd_addr);
  #endif
  if (status != BT_STATUS_SUCCESS) {
    ALOGE("%s: Failed A2DP enable_multi_a2dp, status: %d", __func__, status);
  }
  env->ReleaseByteArrayElements(address, addr, 0);
  return (status == BT_STATUS_SUCCESS) ? JNI_TRUE : JNI_FALSE;
}

static jboolean disableMultiA2dpNative(JNIEnv* env, jobject object) {
  ALOGI("%s: sBluetoothA2dpInterface: %p", __func__, sBluetoothA2dpInterface);
  bt_status_t status = BT_STATUS_FAIL;
  std::shared_lock<std::shared_timed_mutex> lock(interface_mutex);
  if (!sBluetoothA2dpInterface) {
  ...
  #ifdef MULTI_A2DP_ENABLE
  status = sBluetoothA2dpInterface->disable_multi_a2dp();
  #endif
  if (status != BT_STATUS_SUCCESS) {
    ALOGE("%s: Failed A2DP disable_multi_a2dp, status: %d", __func__, status);
  }
  return (status == BT_STATUS_SUCCESS) ? JNI_TRUE : JNI_FALSE;
}

static jboolean setMultiA2dpDeviceNative(JNIEnv* env, jobject object,
                                      jbyteArray address,jint value,jint type) {
  ALOGI("%s: sBluetoothA2dpInterface: %p", __func__, sBluetoothA2dpInterface);
  bt_status_t status = BT_STATUS_FAIL;
  std::shared_lock<std::shared_timed_mutex> lock(interface_mutex);
  if (!sBluetoothA2dpInterface) {
  ...
  #ifdef MULTI_A2DP_ENABLE
  status = sBluetoothA2dpInterface->set_multi_a2dp_device(bd_addr,value,type);
  #endif
  if (status != BT_STATUS_SUCCESS) {
    ALOGE("%s: Failed A2DP set_multi_a2dp_device, status: %d", __func__, status);
  }
  env->ReleaseByteArrayElements(address, addr, 0);
  return (status == BT_STATUS_SUCCESS) ? JNI_TRUE : JNI_FALSE;
}

static int getMultiA2dpDeviceNative(JNIEnv* env, jobject object,
  ...
  #ifdef MULTI_A2DP_ENABLE
  value = sBluetoothA2dpInterface->get_multi_a2dp_device(bd_addr,type);
  #endif
  env->ReleaseByteArrayElements(address, addr, 0);
  return value;
}
...
static JNINativeMethod sMethods[] = {
    ...
    {"enableMultiA2dpNative", "([B)Z", (void*)enableMultiA2dpNative},
    {"disableMultiA2dpNative", "()Z", (void*)disableMultiA2dpNative},
    {"setMultiA2dpDeviceNative", "([BII)Z", (void*)setMultiA2dpDeviceNative},
    {"getMultiA2dpDeviceNative", "([BI)I", (void*)getMultiA2dpDeviceNative},

    ...
    if (bta_av_co_audio_is_aac_wl_enabled(&p_ccb->peer_addr)) {
      if (bta_av_co_audio_device_addr_check_is_enabled(&p_ccb->peer_addr)) {
        if (btif_storage_get_stored_remote_name(p_ccb->peer_addr, remote_name)) {
          char aac_configure_default[255] = "null";
          int retval = property_get("persist.vendor.bt.a2dp.aac.whitelist",
                aac_configure_default, "null");
          APPL_TRACE_DEBUG("%s: device=%s, persist.vendor.bt.a2dp.aac.whitelist=%s, retval=%d",
                __func__, p_ccb->peer_addr.ToString().c_str(), aac_configure_default, retval);
          if ((!strcmp(aac_configure_default, p_ccb->peer_addr.ToString().c_str())) ||
            ...
            aac_support = true;
              ...
              APPL_TRACE_DEBUG("%s: RD not matched for name/address based WL check, skip AAC advertise\n",
                                    __func__);
          }
        } else {
...
void avdt_ccb_hdl_discover_cmd(tAVDT_CCB* p_ccb, tAVDT_CCB_EVT* p_data) {
  ...
  bool lhdc_v5_support = false;
  bool lhdc_v3_support = false;
  bool lhdc_v2_support = false;
  bool lhdc_v1_support = false;
  ...
  if (p_ccb != NULL) {
    ...
    if (btif_config_get_str(bd_addrstr, BTIF_STORAGE_KEY_FOR_SUPPORTED_CODECS, value, &size)) {
      ...
      while (tok != NULL)
      {
       ...
       } else if (strcmp(tok,"LHDC V5") == 0) {
         lhdc_v5_support = true;
       } else if (strcmp(tok,"LHDC_V3") == 0) {
         lhdc_v3_support = true;
       } else if (strcmp(tok,"LHDC_V2") == 0) {
         lhdc_v2_support = true;
       } else if (strcmp(tok,"LHDC_V1") == 0) {
         lhdc_v1_support = true;
       }
  ...
  for (; i < AVDT_NUM_SEPS; i++, p_scb++) {
    ...
    if ((p_ccb != NULL) && (p_scb->allocated) && (!p_scb->in_use)) {
      ...
      if ((soc_type != BT_SOC_TYPE_SMD && soc_type != BT_SOC_TYPE_ROME)) {
        // cherokee chipset check aptX-adaptive IOT and remote may state error if parse new specific codec
        if (strcmp(codec_name, "aptX-adaptive") == 0) {
          if (interop_match_addr(INTEROP_A2DP_HIDE_AA_SPECIFIC_CODEC, &p_ccb->peer_addr) ) {
            AVDT_TRACE_WARNING("%s: RD matched address based BL check, skip aptX-Adaptive advertise\n",
                                  __func__);
            continue;
          }
        }
      ...
      if (codecs_cached) {
        ...
        } else if ((strcmp(codec_name,"LHDC V5") == 0) && lhdc_v5_support) {
          APPL_TRACE_DEBUG("show support of LHDC_V3");
        } else if ((strcmp(codec_name,"LHDC_V3") == 0) && lhdc_v3_support) {
          APPL_TRACE_DEBUG("show support of LHDC_V3");
        } else if ((strcmp(codec_name,"LHDC_V2") == 0) && lhdc_v2_support) {
          APPL_TRACE_DEBUG("show support of LHDC_V2");
        } else if ((strcmp(codec_name,"LHDC_V1") == 0) && lhdc_v1_support) {
          APPL_TRACE_DEBUG("show support of LHDC_V1");
        } else {
          continue;
        }
      }

...
/* BLE static random device list for first booting connection */
#define BTIF_STORAGE_SETCTION_INTERNEL_SBA_IOT_LIST "InternalBleSRAList"
#define BTIF_STORAGE_KEY_BLE_IOT_SRA "BleConnDirectStaticRandomDevices"

/* Quick Connect: XiaoMi's device list for att's secure level 0*/
#define BTIF_STORAGE_SETCTION_QUCIK_CONNECT_LIST "QuickConnectDeviceList"
#define BTIF_STORAGE_KEY_ATT_SECURE_LEVEL_0 "AttSecurelevel0Devices"
...
bt_status_t btif_storage_is_device_bonded(RawAddress *remote_bd_addr) {
  ...
  if((btif_config_exist(bdstr, "LinkKey")) &&
    ...
    if (btm_sec_is_a_bonded_dev_by_transport(*remote_bd_addr, BT_TRANSPORT_BR_EDR))
      return BT_STATUS_SUCCESS;
    else
      return BT_STATUS_FAIL;
  }
...
bt_status_t btif_storage_load_bonded_devices(void) {
  ...
  /* Now send the adapter_properties_cb with all adapter_properties */
  {
    ...
    /* DISC_TIMEOUT */
    BTIF_STORAGE_GET_ADAPTER_PROP(status, BT_PROPERTY_ADAPTER_DISCOVERABLE_TIMEOUT,
...
*
**
** Function         btif_storage_save_ble_static_ramdon_conn_device
**
** Description      Stores specified ble static ramdon device info
**
** Returns          BT_STATUS_SUCCESS
**
*******************************************************************************/
bt_status_t btif_storage_save_ble_static_ramdon_conn_device(RawAddress *remote_bd_addr)
{
    std::string addrstr = remote_bd_addr->ToString();
    const char* bdstr = addrstr.c_str();
    ...
    if(btif_config_get_str(BTIF_STORAGE_SETCTION_INTERNEL_SBA_IOT_LIST,
                           BTIF_STORAGE_KEY_BLE_IOT_SRA, value, &value_size))
    {
        if (strcasestr(value, bdstr) == NULL)
        {
            if ((unsigned int)value_size < BTIF_STORAGE_MAX_LINE_SZ - strlen(bdstr))
            {
               memcpy(value + value_size - 1, ",", 1);
               memcpy(value + value_size, bdstr, strlen(bdstr));
            }
            else
            {
                /* cached address need overwrite the last one */
                memcpy(value + value_size - strlen(bdstr), bdstr, strlen(bdstr));
            }
            btif_config_set_str(BTIF_STORAGE_SETCTION_INTERNEL_SBA_IOT_LIST,
                                BTIF_STORAGE_KEY_BLE_IOT_SRA, value);
            return BT_STATUS_SUCCESS;
        }
    }
    else
    {
        memcpy(value, bdstr, strlen(bdstr));
        btif_config_set_str(BTIF_STORAGE_SETCTION_INTERNEL_SBA_IOT_LIST,
                            BTIF_STORAGE_KEY_BLE_IOT_SRA, value);
        return BT_STATUS_SUCCESS;
    }
...
*
**
** Function         btif_storage_is_ble_static_ramdon_conn_device
**
** Description      BTIF storage API  Checks if the given device is ble specified device
**
** Returns          BT_STATUS_SUCCESS
**
*******************************************************************************/
bool btif_storage_is_ble_static_ramdon_conn_device(RawAddress *remote_bd_addr)
{
    std::string addrstr = remote_bd_addr->ToString();
    const char* bdstr = addrstr.c_str();
    char value[BTIF_STORAGE_MAX_LINE_SZ];
...
*
**
** Function         btif_storage_save_quick_connct_device
**
** Description      Stores specified quick connect device info
**
** Returns          BT_STATUS_SUCCESS
**
*******************************************************************************/
bt_status_t btif_storage_save_quick_connct_device(const RawAddress *remote_bd_addr)
{
    std::string addrstr = remote_bd_addr->ToString();
    const char* bdstr = addrstr.c_str();
    char value[BTIF_STORAGE_MAX_LINE_SZ];
    int value_size = sizeof(value);
    memset(value, 0x0, sizeof(value));
    if (btif_config_get_str(BTIF_STORAGE_SETCTION_QUCIK_CONNECT_LIST,
                            BTIF_STORAGE_KEY_ATT_SECURE_LEVEL_0, value, &value_size))
    {
        if (strcasestr(value, bdstr) == NULL)
        {
            if ((unsigned int)value_size < BTIF_STORAGE_MAX_LINE_SZ - strlen(bdstr))
            ...
            btif_config_set_str(BTIF_STORAGE_SETCTION_QUCIK_CONNECT_LIST,
                                BTIF_STORAGE_KEY_ATT_SECURE_LEVEL_0, value);
            return BT_STATUS_SUCCESS;
        }
    }
    else
...
*
**
** Function         btif_storage_is_att_secure_level_0_device
**
** Description      BTIF storage API  Checks if the given device
**                  is att secure level 0 device
**
** Returns          BT_STATUS_SUCCESS
**
*******************************************************************************/
bool btif_storage_is_att_secure_level_0_device(const RawAddress *remote_bd_addr)
{
    std::string addrstr = remote_bd_addr->ToString();
    const char* bdstr = addrstr.c_str();
    char value[BTIF_STORAGE_MAX_LINE_SZ];

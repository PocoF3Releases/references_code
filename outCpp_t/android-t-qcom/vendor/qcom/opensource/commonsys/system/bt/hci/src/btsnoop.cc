...
// property
#define DEFAULT_BTSNOOP_SIZE 10*1024*1024
...
static void capture(const BT_HDR* buffer, bool is_received) {
  ...
  /* BT magic code logging stoped and need stop capture snoop,
   * do not consider settings snoop logging switch
   */
  property_get("persist.bluetooth.btsnoopenable", prop, NULL);
  if(!strcasecmp(prop, "false"))
    return;
...
static void btsnoop_write_packet(packet_type_t type, uint8_t* packet,
  ...
  if (logfile_fd != INVALID_FD) {
    // Consider limited snoop data length but no packet count
    if (is_btsnoop_enabled || is_vndbtsnoop_enabled) {
      packet_counter += sizeof(btsnoop_header_t) + length_he - 1;

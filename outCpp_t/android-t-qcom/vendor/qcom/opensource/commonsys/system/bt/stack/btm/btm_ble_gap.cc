...
// MIUI ADD: START
void appearance_to_cod(uint16_t appearance, uint8_t* dev_class)
{
  btm_ble_appearance_to_cod(appearance, dev_class);
}
// END
...
void btm_ble_update_inq_result(tINQ_DB_ENT* p_i, uint8_t addr_type,
  ...
  if (!data.empty()) {
    ...
    p_uuid16 = AdvertiseDataParser::GetFieldByType(
        data, BTM_BLE_AD_TYPE_16SRV_DATA, &len);
    if (p_uuid16 != NULL) {
      uint8_t i;
      for (i = 0; i + 2 <= len; i = i + 2) {
         /* if this BLE device support ASHA over LE, set HEARING_AID Major in class of
           * device */
        if ((p_uuid16[0] | (p_uuid16[1] << 8)) == UUID_SERVCLASS_ASHA) {
          p_cur->dev_class[0] = 0;
          p_cur->dev_class[1] = BTM_COD_MAJOR_HEARING_AID;
          p_cur->dev_class[2] = 0;
          break;
        }
      }
  ...
  if (!data.empty()) {
    const uint8_t* p_device_id =
        AdvertiseDataParser::GetFieldByType(data, 0x10, &len); //ad type:0x10=device id
    //0x038F = Xiaomi Inc
    if (p_device_id != NULL && len == 2
         && (0x038F == ((uint16_t)p_device_id[0] | (p_device_id[1] << 8)))
         && (p_cur->dev_class[1] == BTM_COD_MAJOR_ADV_AUDIO)) {
        p_cur->dev_class[1] = 0;
    }
  }
...
void btm_ble_process_adv_addr(RawAddress& bda, uint8_t* addr_type) {
  ...
  BTM_TRACE_EVENT("%s : bda=%s", __func__, bda.ToString().c_str());
...
static void btm_ble_process_adv_pkt_cont(
  ...
  if (is_active_scan && is_scannable && !is_scan_resp) {
    ...
    BTM_TRACE_EVENT("%s :Waiting for scan response bda=%s", __func__, bda.ToString().c_str());
    return;
  }

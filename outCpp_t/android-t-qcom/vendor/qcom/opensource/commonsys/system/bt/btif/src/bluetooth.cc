...
void btif_debug_fw_version_dump(int fd)
{
  FILE *filep;
  char line[100] = {0};
  dprintf(fd, "\nBluetooth FW:\n");
  filep =  fopen("/data/vendor/bluetooth/bt_fw_version.txt", "r" );
  if(filep == NULL) {
    dprintf(fd, "Open Fail\n");
  }
  else {
    dprintf(fd, ">>>>\n");
...
static void connectLeAudio(const RawAddress& address) {
  LOG_INFO(LOG_TAG, "%s", __func__);
  btif_dm_connectLeAudio(address);
}
#endif

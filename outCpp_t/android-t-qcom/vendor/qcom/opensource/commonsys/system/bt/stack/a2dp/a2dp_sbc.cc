...
static const tA2DP_ENCODER_INTERFACE a2dp_encoder_interface_sbc = {
    ...
    a2dp_sbc_set_transmit_queue_length
};
...
bool A2dpCodecConfigSbc::setCodecConfig(const uint8_t* p_peer_codec_info,
  ...
  if(codec_user_config_.codec_specific_3 == 1){
    result_config_cie.min_bitpool = 2;
    result_config_cie.max_bitpool = 37;
    LOG_DEBUG(LOG_TAG, "%s:min/max bitpool = 0x%x/0x%x",__func__,
                       result_config_cie.min_bitpool,result_config_cie.max_bitpool);
  }
  ...
  //if (codec_user_config_.codec_specific_3 != 0) //modify for sbc ll feature

...
*
 *
 * Function         btm_set_PL10_br_edr_max_power
 *
 * Description      xiaomi-bt control pl10 max power level
 *
 * Returns          void
 *
******************************************************************************/
void btm_set_PL10_br_edr_max_power(uint8_t br_idx, uint8_t edr_idx) {
  uint8_t param[6] = {0};
  uint8_t *p_param = param;
  uint8_t count = 0;

  BTM_TRACE_WARNING("%s, br_idx=%d, edr_idx=%d", __func__, br_idx, edr_idx);

  UINT8_TO_STREAM(p_param, HCI_VS_SET_TECH_BASED_MAX_RADIATED_POWER);
  if (br_idx <= 0 || edr_idx <= 0) {
    return;
  }
  if (br_idx < 0x50 && edr_idx < 0x50) {
    count = 2;
    UINT8_TO_STREAM(p_param, count);
    UINT8_TO_STREAM(p_param, 0x00);  //BR
    UINT8_TO_STREAM(p_param, br_idx);
    UINT8_TO_STREAM(p_param, 0x01);  //EDR
    UINT8_TO_STREAM(p_param, edr_idx);
  } else if (br_idx < 0x50 && edr_idx >= 0x50) {
    count = 1;
    UINT8_TO_STREAM(p_param, count);
    UINT8_TO_STREAM(p_param, 0x00);  //BR
    UINT8_TO_STREAM(p_param, br_idx);
  } else if (edr_idx < 0x50 && br_idx >= 0x50) {
    count = 1;
    UINT8_TO_STREAM(p_param, count);
    UINT8_TO_STREAM(p_param, 0x01);  //EDR
    UINT8_TO_STREAM(p_param, edr_idx);
  } else {
      /* both br/edr index invaild */
      return;
  }

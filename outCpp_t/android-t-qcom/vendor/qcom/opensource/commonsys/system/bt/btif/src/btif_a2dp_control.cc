    ...
    switch (cmd) {
      ...
      case A2DP_CTRL_GET_OUTPUT_AUDIO_CONFIG: {
        ...
        if (current_codec != nullptr) {
          ...
          if (codec_index == BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV2
            || codec_index == BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV3
            || codec_index == BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV5) { /*|| codec_index == BTAV_A2DP_CODEC_INDEX_SOURCE_LHDC_LL){*/
              isLLEnabled = (codec_config.codec_specific_3 & 1) != 0 ? true : FALSE;
          } else {
              isLLEnabled = false;
          }
#endif
    ...
    switch (cmd) {
      ...
      case A2DP_CTRL_CMD_START:
        ...
        if(btif_av_is_split_a2dp_enabled() && btif_av_get_multicast_state() == false) {
            APPL_TRACE_WARNING("%s,current codec is split a2dp %d", __func__, __LINE__);
            btif_a2dp_command_ack(A2DP_CTRL_ACK_FAILURE);
            break;
        }
#endif

...
//MIUI ADD
// this device can support lhd 4.0 now
static std::string device_list[] = {"zeus","cupid","mona","odin","vili","ingres","thor","loki","taoyao","zijin",
    "zizhan","star","mars","venus","haydn","haydnin","alioth","aliothin","thyme","cas","cmi","umi","apollo","lmi",
    "psyche","unicorn","cetus","mayfly","diting","ziyi","caiwei","nuwa","fuxi","munch","mondrian", "socrates","liuqin",
    "marble","marblein","ishtar","ishtarin"};
//END
...
static const tA2DP_LHDC_FEATURE_POS a2dp_lhdc_source_caps_JAS = {
    ...
    LHDC_EXTEND_FUNC_A2DP_SPECIFIC4_INDEX,
...
static tA2DP_LHDC_CIE a2dp_lhdc_source_caps = {
...
static bool is_support_lhdc4()
{
   ...
   for(auto device_name : device_list) {
        if (!strcmp(device_prop_value, device_name.c_str())) {
             LOG_DEBUG(LOG_TAG, "%s: can support lhdc 4.0", __func__);
            return true;
        }
   }
   LOG_DEBUG(LOG_TAG, "%s: can not support lhdc 4.0", __func__);
   return false;
}
//END
...
bool A2DP_VendorGetSrcCapVectorLhdcv3(uint8_t *capVector)
{
    ...
    if(capNumber <= 0)
    {
      LOG_WARN(LOG_TAG, "%s: no capabilities, nothing to do!", __func__);
      return true;
    }
    LOG_DEBUG(LOG_TAG, "%s: capNumber=%d", __func__, capNumber);
...
uint8_t A2DP_VendorGetVersionLhdcV3(const uint8_t* p_codec_info){
  ...
  LOG_INFO(LOG_TAG, "%s: version = 0x%02x, result = 0x%02x", __func__, lhdc_cie.version, result);
...
bool A2DP_VendorDumpCodecInfoLhdcV3(const uint8_t* p_codec_info) {
  tA2DP_STATUS a2dp_status;
  tA2DP_LHDC_CIE lhdc_cie;
...
const char* A2DP_VendorCodecIndexStrLhdcV3(void) { return "LHDC_V3"; }

bool A2DP_VendorInitCodecConfigLhdcV3(tAVDT_CFG* p_cfg) {
...
A2dpCodecConfigLhdcV3::A2dpCodecConfigLhdcV3(
    ...
    : A2dpCodecConfig(BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV3, "LHDC_V3",
                      codec_priority) {
  ...
  if (!is_support_lhdc4()) {
        a2dp_lhdc_source_caps.version &= ~ A2DP_LHDC_VER6;
        a2dp_lhdc_source_caps.hasFeatureLHDCV4 = false;
        LOG_DEBUG(LOG_TAG, "%s: close lhdc v4 on not support device", __func__);
  }
}
...
bool A2dpCodecConfigLhdcV3::init() {
  ...
  if (A2DP_IsCodecEnabledInOffload(BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV3)) {
    LOG_DEBUG(LOG_TAG, "%s: LHDC V3 enabled in HW mode", __func__);
    return true;
  }
  // Load the encoder
...
bool A2dpCodecConfigLhdcV3::setCodecConfig(const uint8_t* p_peer_codec_info,
  ...
  if (is_support_lhdc4()) {
    ...
    if (sink_info_cie.version == A2DP_LHDC_VER6) {
        sink_info_cie.version = 0;
        LOG_DEBUG(LOG_TAG, "%s: disable lhdc4.0",__func__);
    }
  }
  ...
  result_config_cie.version = sink_info_cie.version;
    LOG_DEBUG(LOG_TAG, "%s: VERSION LOG! peer(0x%02x), host(0x%02x)",
              __func__, sink_info_cie.version, a2dp_lhdc_source_caps.version);
...
/*
  ...
  */
  ...
  result_config_cie.isLLSupported = (sink_info_cie.isLLSupported ? 1 : 0) & codec_user_config_.codec_specific_3;
  LOG_DEBUG(LOG_TAG, "%s: isLLSupported, Sink(0x%02x) Set(0x%02x)", __func__, sink_info_cie.isLLSupported, result_config_cie.isLLSupported);

  //
  // Select the sample frequency
  //
  sampleRate = a2dp_lhdc_source_caps.sampleRate & sink_info_cie.sampleRate;
  LOG_DEBUG(LOG_TAG, "%s: sample rate = 0x%x", __func__, sampleRate);
  ...
  do {
    LOG_DEBUG(LOG_TAG, "%s: Select the sample frequency, there is no user preference", __func__);
    ...
    LOG_DEBUG(LOG_TAG, "%s: try the codec audio config", __func__);
    // No user preference - try the codec audio config
    if (select_audio_sample_rate(&codec_audio_config_, sampleRate,
                                 &result_config_cie, &codec_config_)) {
      break;
    }
    LOG_DEBUG(LOG_TAG, "%s: try the default config", __func__);
    ...
    LOG_DEBUG(LOG_TAG, "%s: use the best match", __func__);
  ...
  codec_config_.codec_max_target_bitrate = result_config_cie.maxTargetBitrate;
  ...
  codec_config_.codec_channel_split_mode = result_config_cie.channelSplitMode;
  ...
  if ((codec_user_config_.codec_specific_1 & A2DP_LHDC_VENDOR_CMD_MASK) != A2DP_LHDC_QUALITY_MAGIC_NUM) {
      /* For earbud reconnecting with sample rate 96k, reconfigure with ABR will cause
       * sample rate be modified to 48K by "Final Custom Rules",
       * For this case, we use bit rate LOW as default.
       **/
      if(result_config_cie.sampleRate == A2DP_LHDC_SAMPLING_FREQ_96000 || result_config_cie.sampleRate == A2DP_LHDC_SAMPLING_FREQ_88200){
          codec_user_config_.codec_specific_1 = (A2DP_LHDC_QUALITY_MAGIC_NUM | A2DP_LHDC_QUALITY_LOW) - A2DP_LHDC_QUALITY_OFFSET;
              LOG_DEBUG(LOG_TAG, "%s: reconfig quality mode: LOW", __func__);
          } else{
              codec_user_config_.codec_specific_1 = (A2DP_LHDC_QUALITY_MAGIC_NUM | A2DP_LHDC_QUALITY_ABR) - A2DP_LHDC_QUALITY_OFFSET;;
              LOG_DEBUG(LOG_TAG, "%s: reconfig quality mode: ABR", __func__);
          }
  }

  quality_mode = ((codec_user_config_.codec_specific_1 & 0xff) + A2DP_LHDC_QUALITY_OFFSET);
  ...
  if(maxBitRate_Qmode < 0xFF) {
    if(quality_mode != A2DP_LHDC_QUALITY_ABR && quality_mode > maxBitRate_Qmode){
      LOG_DEBUG(LOG_TAG,"%s: adjust quality_mode:0x%x to 0x%x by maxTargetBitrate:0x%x", __func__,
       ...
       codec_user_config_.codec_specific_1 = (A2DP_LHDC_QUALITY_MAGIC_NUM | quality_mode) - A2DP_LHDC_QUALITY_OFFSET;
    }
  }
*/
  LOG_DEBUG(LOG_TAG, "%s: Config bitrate before offset(0x%02x), after offset(0x%02x)", __func__, (uint32_t)(codec_user_config_.codec_specific_1 & 0xff), quality_mode);
  /*******************************************
   * LHDC features: safety tag check
   * to specific 3
   *******************************************/
  if ((codec_user_config_.codec_specific_4 & A2DP_LHDC_VENDOR_FEATURE_MASK) != A2DP_LHDC_FEATURE_MAGIC_NUM)
  {
      ...
      codec_user_config_.codec_specific_4 = A2DP_LHDC_FEATURE_MAGIC_NUM;
  ...
  */
   ...
   *     codec_config_.codec_specific_4 &= ~A2DP_LHDC_LLAC_ENABLED;
   *     codec_selectable_capability_.codec_specific_4 &= ~A2DP_LHDC_LLAC_ENABLED;
   *     codec_capability_.codec_specific_4 &= ~ A2DP_LHDC_LLAC_ENABLED;
   * */
  A2DP_UpdateFeatureToA2dpConfigLhdcV3(
      &allCfgPack,
      LHDC_EXTEND_FUNC_A2DP_LHDC_LLAC_CODE,
  ...
  if (hasFeature /*&& 
    ...
    //codec_config_.codec_specific_4 |= A2DP_LHDC_LLAC_ENABLED;
    //codec_user_config_.codec_specific_4 |= A2DP_LHDC_LLAC_ENABLED;
    A2DP_UpdateFeatureToA2dpConfigLhdcV3(
        &allCfgPack,
        LHDC_EXTEND_FUNC_A2DP_LHDC_LLAC_CODE,
  ...
  /* *
   * reset bit in A2DP configs
   *     codec_config_.codec_specific_4 &= ~A2DP_LHDC_V4_ENABLED;
   *     codec_selectable_capability_.codec_specific_4 &= ~A2DP_LHDC_V4_ENABLED;
   *     codec_capability_.codec_specific_4 &= ~ A2DP_LHDC_V4_ENABLED;
   * */
  A2DP_UpdateFeatureToA2dpConfigLhdcV3(
      &allCfgPack,
      LHDC_EXTEND_FUNC_A2DP_LHDC_V4_CODE,
  ...
  if (hasFeature /*&& 
    ...
    //codec_config_.codec_specific_4 |= A2DP_LHDC_V4_ENABLED;
    //codec_user_config_.codec_specific_4 |= A2DP_LHDC_V4_ENABLED;
    A2DP_UpdateFeatureToA2dpConfigLhdcV3(
        &allCfgPack,
        LHDC_EXTEND_FUNC_A2DP_LHDC_V4_CODE,
        (A2DP_LHDC_TO_A2DP_CODEC_CONFIG_| A2DP_LHDC_TO_A2DP_CODEC_USER_),
   ...
   *******************************************/
  {
    ...
    /* *
     * reset bit in A2DP configs
     *     codec_config_.codec_specific_4 &= ~A2DP_LHDC_JAS_ENABLED;
     *     codec_selectable_capability_.codec_specific_4 &= ~A2DP_LHDC_JAS_ENABLED;
     *     codec_capability_.codec_specific_4 &= ~ A2DP_LHDC_JAS_ENABLED;
     * */
    A2DP_UpdateFeatureToA2dpConfigLhdcV3(
        &allCfgPack,
        LHDC_EXTEND_FUNC_A2DP_LHDC_JAS_CODE,
    ...
    //if ((codec_user_config_.codec_specific_4 & A2DP_LHDC_JAS_ENABLED) && hasFeature) {
    ...
    LOG_DEBUG(LOG_TAG, "%s: JAS update:[config:(0x%016llX) cap:(0x%016llX) selcap:(0x%016llX) user:(0x%016llX)] ", __func__,
                                  ...
                                  (codec_capability_.codec_specific_4 & A2DP_LHDC_JAS_ENABLED),
                                  (codec_selectable_capability_.codec_specific_4 & A2DP_LHDC_JAS_ENABLED),
                                  (codec_user_config_.codec_specific_4 & A2DP_LHDC_JAS_ENABLED));
  }
  ...
  /* *
   * reset bit in A2DP configs
   *     codec_config_.codec_specific_4 &= ~A2DP_LHDC_AR_ENABLED;
   *     codec_selectable_capability_.codec_specific_4 &= ~A2DP_LHDC_AR_ENABLED;
   *     codec_capability_.codec_specific_4 &= ~ A2DP_LHDC_AR_ENABLED;
   * */
  A2DP_UpdateFeatureToA2dpConfigLhdcV3(
      &allCfgPack,
      LHDC_EXTEND_FUNC_A2DP_LHDC_AR_CODE,
  ...
  //if ((codec_user_config_.codec_specific_4 & A2DP_LHDC_AR_ENABLED) && hasFeature) {
  if( hasFeature && A2DP_IsFeatureInUserConfigLhdcV3(&allCfgPack, LHDC_EXTEND_FUNC_A2DP_LHDC_AR_CODE) ){
    ...
    {
        LOG_DEBUG(LOG_TAG, "%s: Limit current sample rate(0x%02X) to 48Khz when AR feature turnned on", __func__, codec_user_config_.sample_rate);
  ...
  LOG_DEBUG(LOG_TAG, "%s: AR update:[config:(0x%016llX) cap:(0x%016llX) selcap:(0x%016llX) user:(0x%016llX)] ", __func__,
                                ...
                                (codec_capability_.codec_specific_4 & A2DP_LHDC_AR_ENABLED),
                                (codec_selectable_capability_.codec_specific_4 & A2DP_LHDC_AR_ENABLED),
                                (codec_user_config_.codec_specific_4 & A2DP_LHDC_AR_ENABLED));
  ...
  /* *
   * reset bit in A2DP configs
   *     codec_config_.codec_specific_4 &= ~A2DP_LHDC_META_ENABLED;
   *     codec_selectable_capability_.codec_specific_4 &= ~A2DP_LHDC_META_ENABLED;
   *     codec_capability_.codec_specific_4 &= ~ A2DP_LHDC_META_ENABLED;
   * */
  A2DP_UpdateFeatureToA2dpConfigLhdcV3(
      &allCfgPack,
      LHDC_EXTEND_FUNC_A2DP_LHDC_META_CODE,
  ...
  //if ((codec_user_config_.codec_specific_4 & A2DP_LHDC_META_ENABLED) && hasFeature) {
  ...
  LOG_DEBUG(LOG_TAG, "%s: META update:[config:(0x%016llX) cap:(0x%016llX) selcap:(0x%016llX) user:(0x%016llX)] ", __func__,
                                ...
                                (codec_capability_.codec_specific_4 & A2DP_LHDC_META_ENABLED),
                                (codec_selectable_capability_.codec_specific_4 & A2DP_LHDC_META_ENABLED),
                                (codec_user_config_.codec_specific_4 & A2DP_LHDC_META_ENABLED));
  ...
  /* *
   * reset bit in A2DP configs
   *     codec_config_.codec_specific_4 &= ~A2DP_LHDC_MBR_ENABLED;
   *     codec_selectable_capability_.codec_specific_4 &= ~A2DP_LHDC_MBR_ENABLED;
   *     codec_capability_.codec_specific_4 &= ~ A2DP_LHDC_MBR_ENABLED;
   * */
  A2DP_UpdateFeatureToA2dpConfigLhdcV3(
      &allCfgPack,
      LHDC_EXTEND_FUNC_A2DP_LHDC_MBR_CODE,
  ...
  LOG_DEBUG(LOG_TAG, "%s: MBR update:[config:(0x%016llX) cap:(0x%016llX) selcap:(0x%016llX) user:(0x%016llX)] ", __func__,
                                ...
                                (codec_capability_.codec_specific_4 & A2DP_LHDC_MBR_ENABLED),
                                (codec_selectable_capability_.codec_specific_4 & A2DP_LHDC_MBR_ENABLED),
                                (codec_user_config_.codec_specific_4 & A2DP_LHDC_MBR_ENABLED));
  ...
  /* *
   * reset bit in A2DP configs
   *     codec_config_.codec_specific_4 &= ~A2DP_LHDC_LARC_ENABLED;
   *     codec_selectable_capability_.codec_specific_4 &= ~A2DP_LHDC_LARC_ENABLED;
   *     codec_capability_.codec_specific_4 &= ~ A2DP_LHDC_LARC_ENABLED;
   * */
  A2DP_UpdateFeatureToA2dpConfigLhdcV3(
      &allCfgPack,
      LHDC_EXTEND_FUNC_A2DP_LHDC_LARC_CODE,
  ...
  //if ((codec_user_config_.codec_specific_4 & A2DP_LHDC_LARC_ENABLED) && hasFeature) {
  ...
  LOG_DEBUG(LOG_TAG, "%s: LARC update:[config:(0x%016llX) cap:(0x%016llX) selcap:(0x%016llX) user:(0x%016llX)] ", __func__,
                                ...
                                (codec_capability_.codec_specific_4 & A2DP_LHDC_LARC_ENABLED),
                                (codec_selectable_capability_.codec_specific_4 & A2DP_LHDC_LARC_ENABLED),
                                (codec_user_config_.codec_specific_4 & A2DP_LHDC_LARC_ENABLED));
  ...
  else
  {
      ...
      if(maxBitRate_Qmode < 0xFF) {
          if(quality_mode != A2DP_LHDC_QUALITY_ABR && quality_mode > maxBitRate_Qmode){
              LOG_DEBUG(LOG_TAG,"%s: adjust quality_mode:0x%x to 0x%x by maxTargetBitrate:0x%x", __func__, quality_mode, maxBitRate_Qmode, result_config_cie.maxTargetBitrate);
              quality_mode = maxBitRate_Qmode;
              codec_user_config_.codec_specific_1 = (A2DP_LHDC_QUALITY_MAGIC_NUM | quality_mode) - A2DP_LHDC_QUALITY_OFFSET;
          }
      }
  ...
  /*
   * Rules of conflict between capabilities and version
   */
  if (result_config_cie.hasFeatureLLAC && result_config_cie.hasFeatureLHDCV4) {
    //LHDCV4 + LLAC
      ...
      if (quality_mode == A2DP_LHDC_QUALITY_ABR) {
        ...
        LOG_DEBUG(LOG_TAG,"%s: [LLAC + LHDC V4]: LHDC", __func__);
          ...
          if (quality_mode < A2DP_LHDC_QUALITY_MID) {
            codec_user_config_.codec_specific_1 = (A2DP_LHDC_QUALITY_MAGIC_NUM | A2DP_LHDC_QUALITY_MID) - A2DP_LHDC_QUALITY_OFFSET;
    ...
    if (result_config_cie.sampleRate == A2DP_LHDC_SAMPLING_FREQ_96000 ) {
        ...
        if (result_config_cie.hasFeatureMinBitrate) {
            if (quality_mode < A2DP_LHDC_QUALITY_LOW4) {
                codec_user_config_.codec_specific_1 = (A2DP_LHDC_QUALITY_MAGIC_NUM | A2DP_LHDC_QUALITY_LOW4) - A2DP_LHDC_QUALITY_OFFSET;
    ...
    if (quality_mode > A2DP_LHDC_QUALITY_LOW && quality_mode != A2DP_LHDC_QUALITY_ABR) {
      codec_user_config_.codec_specific_1 = (A2DP_LHDC_QUALITY_MAGIC_NUM | A2DP_LHDC_QUALITY_LOW) - A2DP_LHDC_QUALITY_OFFSET;
      quality_mode = A2DP_LHDC_QUALITY_LOW;
      LOG_ERROR(LOG_TAG,"%s: [LLAC only]: reset Qmode (0x%x)", __func__, quality_mode);
    }

    /* LLAC: prevent quality mode using 64kbps */
    if (result_config_cie.hasFeatureMinBitrate) {
        if (quality_mode < A2DP_LHDC_QUALITY_LOW1) {
          codec_user_config_.codec_specific_1 = (A2DP_LHDC_QUALITY_MAGIC_NUM | A2DP_LHDC_QUALITY_LOW1) - A2DP_LHDC_QUALITY_OFFSET;
    ...
    if (result_config_cie.sampleRate == A2DP_LHDC_SAMPLING_FREQ_96000) {
      ...
      if (result_config_cie.hasFeatureMinBitrate) {
        if (quality_mode < A2DP_LHDC_QUALITY_LOW4) {
          codec_user_config_.codec_specific_1 = (A2DP_LHDC_QUALITY_MAGIC_NUM | A2DP_LHDC_QUALITY_LOW4) - A2DP_LHDC_QUALITY_OFFSET;
          quality_mode = A2DP_LHDC_QUALITY_LOW4;
          LOG_ERROR(LOG_TAG,"%s: [LHDCV3 only]: reset Qmode (0x%x), ", __func__, quality_mode);
        }
      }
    }
  }
  ...
  //if (codec_user_config_.codec_specific_3 != 0 || sink_info_cie.isLLSupported == true)
//    codec_config_.codec_specific_3 = ; //codec_user_config_.codec_specific_3 | sink_info_cie.isLLSupported;
  codec_config_.codec_specific_3 = result_config_cie.isLLSupported == true ? 1 : 0;
  codec_selectable_capability_.codec_specific_3 = sink_info_cie.isLLSupported ? 1 : 0;
  // (codec_user_config_.codec_specific_4 != 0)

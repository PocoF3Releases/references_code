...
static bool snoop_enabled = false;
static int max_lhdc_raw_size = 0;
#define DEFAULT_MAX_LHDC_RAW_SIZE 5*1024*1024
#define MAX_LHDC_RAW_SIZE_PROPERTY "persist.bluetooth.lhdcrawsize"
#define LHDC_RAW_PATH "/data/misc/bluedroid/lhdc_v3.raw"
#define LAST_LHDC_RAW_PATH "/data/misc/bluedroid/lhdc_v3.raw.last"
#endif
...
void a2dp_vendor_lhdcv3_encoder_init(
    ...
    a2dp_source_read_callback_t read_callback,
    a2dp_source_enqueue_callback_t enqueue_callback) {
    char btsnoop_enabled[100] = {0};
    osi_property_get("persist.bluetooth.btsnoopenable", btsnoop_enabled, "false");
    if(!strncmp(btsnoop_enabled, "true", 4)) {
      snoop_enabled = true;
    } else {
      snoop_enabled = false;
    }
...
#if defined(_RECODER_FILE_)
static void open_next_lhdc_raw_file(bool b_create) {
  if (RecFile == NULL && b_create == true) {
      if (rename(LHDC_RAW_PATH, LAST_LHDC_RAW_PATH) != 0 && errno != ENOENT)
          LOG_WARN(LOG_TAG, "%s: unable to rename %s", __func__, LHDC_RAW_PATH);

      RecFile = fopen(LHDC_RAW_PATH, "wb");
      LOG_INFO(LOG_TAG, "%s:Creat record file %p", __func__, RecFile);

      max_lhdc_raw_size = osi_property_get_int32(MAX_LHDC_RAW_SIZE_PROPERTY,
                                                      DEFAULT_MAX_LHDC_RAW_SIZE);
  } else if (RecFile != NULL && b_create == false) {
      fclose(RecFile);
      RecFile = NULL;
      ...
      RecFile = fopen(LHDC_RAW_PATH, "wb");
      if (RecFile == NULL) {
          LOG_WARN(LOG_TAG, "%s: unable to creat file %s", __func__, LHDC_RAW_PATH);
      }
  }
  return;
}
...
static void a2dp_vendor_lhdcv3_encoder_update(uint16_t peer_mtu,
  ...
  *p_config_updated = false;
  ...
  if ((codec_config.codec_specific_1 & A2DP_LHDC_VENDOR_CMD_MASK) == A2DP_LHDC_QUALITY_MAGIC_NUM) {
      ...
      if(newValue == 0){
        newValue = A2DP_LHDC_QUALITY_LOW3;
      }
      else{
        newValue += A2DP_LHDC_QUALITY_OFFSET;
      }
      ...
      codec_config.codec_specific_1 = A2DP_LHDC_QUALITY_MAGIC_NUM | A2DP_LHDC_QUALITY_LOW - A2DP_LHDC_QUALITY_OFFSET;
  }
  ...
  p_encoder_params->isLLEnabled = (codec_config.codec_specific_3 & 1ULL) != 0 ? true :false; //A2DP_VendorGetLowLatencyState(p_codec_info);
...
tA2DP_LHDC_ENCODER_PARAMS* p_encoder_params = &a2dp_lhdc_encoder_cb.lhdc_encoder_params;
  ...
  while (nb_frame) {
    ...
    do {
      ...
      if (a2dp_lhdcv3_read_feeding(read_buffer, &temp_bytes_read)) {
        ...
        out_len = lhdc_encode_func(a2dp_lhdc_encoder_cb.lhdc_handle, read_buffer, packet, (uint32_t*)&written, (uint32_t*)&out_frames);
#if defined(_RECODER_FILE_)
        if(snoop_enabled == true) {
            if (RecFile != NULL && out_len > 0) {
                fwrite(packet, sizeof(uint8_t), written, RecFile);
                raw_size += out_len;
                if(raw_size > max_lhdc_raw_size) {
                    open_next_lhdc_raw_file(false);
                    raw_size = 0;
                }
            }
        }
        ...
        LOG_VERBOSE(LOG_TAG, "nb_frame:%d, written:%d, out_frames:%d", nb_frame, written, out_frames);
    ...
    if (p_buf->len) {
      ...
      *( ( uint32_t*)( p_buf + 1)) = a2dp_lhdc_encoder_cb.timestamp;
      ...
      LOG_DEBUG(LOG_TAG, "nb_frame:%d, remain_nb_frame:%d, done_nb_frame:%d", nb_frame, remain_nb_frame, done_nb_frame);

      mtu_usage += ((float)p_buf->len) / max_mtu_len;
      mtu_usage_cnt++;

      LOG_VERBOSE (LOG_TAG, "%s: Read bytes(%d)", __func__, a2dp_lhdc_encoder_cb.bytes_read);
      LOG_VERBOSE (LOG_TAG, "%s: Send Frame(%d), length(%d)", __func__, written_frame, p_buf->len);
      bytes_read = a2dp_lhdc_encoder_cb.bytes_read;
      a2dp_lhdc_encoder_cb.bytes_read = 0;

      if (!a2dp_lhdc_encoder_cb.enqueue_callback(p_buf, 1,
                                                 bytes_read))
        return;
    } else {
  ...
  if (now_ms - time_prev >= 1000 ) {
      LOG_VERBOSE(LOG_TAG, "%s: Current data rate about %d kbps, packet usage %.2f%%", __func__, (allSendbytes * 8) / 1000, (mtu_usage * 100)/mtu_usage_cnt);
      allSendbytes = 0;
      mtu_usage_cnt = 0;
      mtu_usage = 0;
      time_prev = now_ms;
...
static bool a2dp_lhdcv3_read_feeding(uint8_t* read_buffer, uint32_t *bytes_read) {
  ...
  LOG_VERBOSE(LOG_TAG, "%s: want to read size %u, read byte number %u",

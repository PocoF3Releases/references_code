...
#define LHDC_RAW_PATH "/data/misc/bluedroid/lhdc_v2.raw"
#define LAST_LHDC_RAW_PATH "/data/misc/bluedroid/lhdc_v2.raw.last"
#endif
...
static void a2dp_vendor_lhdcv2_encoder_update(uint16_t peer_mtu,
  ...
  if (A2DP_IsCodecEnabledInOffload(BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV2)) {
...
*/
  ...
  p_encoder_params->isLLEnabled = (codec_config.codec_specific_3 & 1) != 0 ? true :false; //A2DP_VendorGetLowLatencyState(p_codec_info);
...
static void a2dp_lhdcv2_encode_frames(uint8_t nb_frame) {
    ...
    uint32_t bytes_read = 0;
        ...
        if (now_ms - time_prev >= 1000 ) {
            /* code */
            LOG_VERBOSE(LOG_TAG, "%s: Current data rate about %d kbps", __func__, (allSendbytes * 8) / 1000);
            allSendbytes = 0;
            time_prev = now_ms;
        }
        ...
        LOG_VERBOSE(LOG_TAG, "%s:btBufs.size() = %u", __func__, (uint32_t)btBufs.size());
        if ( btBufs.size() == 1) {
            p_buf = btBufs[0];
            ...
            *( ( uint32_t*)( p_buf + 1)) = a2dp_lhdc_encoder_cb.timestamp;
            ...
            LOG_VERBOSE (LOG_TAG, "%s: Read bytes(%d)", __func__, bytes_read);
            bytes_read = 0;

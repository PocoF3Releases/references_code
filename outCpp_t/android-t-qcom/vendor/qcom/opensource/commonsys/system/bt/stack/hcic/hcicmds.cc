...
void btsnd_hcic_write_scan_enable(uint8_t flag) {
  ...
  if(flag & HCI_PAGE_SCAN_ENABLED) {
    property_get("persist.bluetooth.enablepagescan", value, "true");
    if (!(strcmp(value,"false"))) {
      flag = flag & ~HCI_PAGE_SCAN_ENABLED;
      LOG_WARN(LOG_TAG, "%s,disable page scan", __func__);
    }
  }

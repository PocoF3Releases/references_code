...
void l2cu_no_dynamic_ccbs(tL2C_LCB* p_lcb) {
  ...
  if (start_timeout) {
    ...
    char timeStamp[PROPERTY_VALUE_MAX] = "";
    property_get("persist.vendor.service.bt.opp.highspeed.device", device, "");
    property_get("persist.vendor.service.bt.opp.highspeed.time", timeStamp, "");
    long sec = time(NULL);
    if (!strcasecmp(device, p_lcb->remote_bd_addr.ToString().c_str()) && (sec - atol(timeStamp)) < 2) {
      timeout_ms = 500;
      L2CAP_TRACE_DEBUG("%s set highspeed opp disconnect spp timeout: %d ms", __func__, timeout_ms);
    }

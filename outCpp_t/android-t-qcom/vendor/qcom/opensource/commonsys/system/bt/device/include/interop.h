...
typedef enum {
  ...
  // Some remotes reject SCO connect and no need retry connect.
  INTEROP_NEED_NOT_RETRY_SCO_AFTER_REMOTE_REJECT_SCO,

  //some remoted need a delay after outgoing call indicators
  //are sent before sco is opened
  INTEROP_DELAY_SCO_FOR_MO_CALL,

  //Some remotes can't parse AA codec and state error, it may set config with new codec which
  //itself can't support, hide those codec for it.
  INTEROP_A2DP_HIDE_AA_SPECIFIC_CODEC,

  // Make remote as Master when playing a2dp
  INTEROP_A2DP_MAKE_REMOTE_AS_MASTER,
  ...
  //some wearble watch device may report error cod, So we add WEARABLE_WATCH_DEVICE_WHILTELIST
  //if the device in WEARABLE_WATCH_DEVICE_WHILTELIST, even the cod is error , it can hang up call
  INTEROP_WEARABLE_WATCH_DEVICE_WHILTELIST,

  //don't Make remote as Master when playing a2dp which devices in INTEROP_A2DP_MAKE_REMOTE_AS_MASTER
  INTEROP_A2DP_DONOT_MAKE_REMOTE_AS_MASTER,

  // not support inband ringtone when devices in INTEROP_HFP_DISABLE_INBAND_RINGTONE
  INTEROP_HFP_DISABLE_INBAND_RINGTONE,

  //not support HID auto connect  when devices in INTEROP_HID_DISABLE_AUTOCONNECTION
  INTEROP_HID_DISABLE_AUTOCONNECTION,

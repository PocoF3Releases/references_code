...
#define A2DP_SBC_MIDDLE_BITPOOL 35
/* Low quality quality setting @ 44.1 khz */
#define A2DP_SBC_LOW_BITPOOL 25
/* High quality quality setting @ 44.1 khz */
#define A2DP_SBC_DEFAULT_BITPOOL 53
...
int16_t p_multicast_s16BitPool = A2DP_SBC_DEFAULT_BITPOOL;
static bool is_multia2dp_supported = false;
...
void a2dp_sbc_encoder_init(const tA2DP_ENCODER_INIT_PEER_PARAMS* p_peer_params,
                           ...
                           a2dp_source_read_callback_t read_callback,
                           a2dp_source_enqueue_callback_t enqueue_callback) {
  is_multia2dp_supported = btif_av_is_multicast_supported();
  LOG_INFO(LOG_TAG,"a2dp_sbc_encoder_init,is_multia2dp_supported:%d",is_multia2dp_supported);
  if (A2DP_IsCodecEnabledInOffload(BTAV_A2DP_CODEC_INDEX_SOURCE_SBC) && !is_multia2dp_supported) {
  ...
  p_multicast_s16BitPool = A2DP_SBC_DEFAULT_BITPOOL;
...
static void a2dp_sbc_encoder_update(uint16_t peer_mtu,
  ...
  *p_config_updated = false;
  ...
  if (a2dp_sbc_encoder_cb.sbc_encoder_params.s16BitPool > A2DP_SBC_MIDDLE_BITPOOL - 1) {
    p_multicast_s16BitPool = std::min(p_multicast_s16BitPool,
                                a2dp_sbc_encoder_cb.sbc_encoder_params.s16BitPool);
    a2dp_sbc_encoder_cb.sbc_encoder_params.s16BitPool = p_multicast_s16BitPool;
    LOG_DEBUG(LOG_TAG, "%s: save final bit pool %d", __func__, p_multicast_s16BitPool);
  }
}
...
void a2dp_sbc_set_transmit_queue_length(size_t transmit_queue_length) {
  if (A2DP_IsCodecEnabledInOffload(BTAV_A2DP_CODEC_INDEX_SOURCE_SBC) && !is_multia2dp_supported) {
    LOG_INFO(LOG_TAG,"a2dp_sbc_set_transmit_queue_length:"
                     "sbc is running in offload mode");
    return;
  }
  if (a2dp_sbc_encoder_cb.sbc_encoder_params.s16BitPool < A2DP_SBC_LOW_BITPOOL) {
    LOG_INFO(LOG_TAG,"a2dp_sbc_set_transmit_queue_length:"
                     "s16BitPool is low quality");
    return;
  }
  LOG_VERBOSE(LOG_TAG, "%s: transmit_queue_length %zu", __func__, transmit_queue_length);
  if (((a2dp_sbc_encoder_cb.sbc_encoder_params.s16ChannelMode == SBC_STEREO)
       || (a2dp_sbc_encoder_cb.sbc_encoder_params.s16ChannelMode == SBC_JOINT_STEREO))
       && transmit_queue_length > 0) {
    LOG_INFO(LOG_TAG, "%s: Auto Bitrate Enabled!,transmit_queue_length:%zu",
             __func__,transmit_queue_length);
    if (transmit_queue_length < 4)
        a2dp_sbc_encoder_cb.sbc_encoder_params.s16BitPool = A2DP_SBC_MIDDLE_BITPOOL;
    else
        a2dp_sbc_encoder_cb.sbc_encoder_params.s16BitPool = A2DP_SBC_LOW_BITPOOL;
  } else {
    a2dp_sbc_encoder_cb.sbc_encoder_params.s16BitPool = p_multicast_s16BitPool;
  }
}
...
void a2dp_sbc_send_frames(uint64_t timestamp_us) {
  ...
  LOG_INFO(LOG_TAG, "%s: Sending %d frames per iteration, %d iterations",
...
static void a2dp_sbc_get_num_frame_iteration(uint8_t* num_of_iterations,
  ...
  LOG_VERBOSE(LOG_TAG, "%s: frames for available PCM data projected_nof:%u,is_peer_edr:%d", __func__,
              projected_nof,a2dp_sbc_encoder_cb.is_peer_edr);
  ...
  if (a2dp_sbc_encoder_cb.is_peer_edr) {
    ...
    LOG_VERBOSE(LOG_TAG, "%s: headset BR, number of frames %u,projected_nof:%u", __func__, nof,projected_nof);
    if (projected_nof > MAX_PCM_FRAME_NUM_PER_TICK) {
...
void setvolumeStereo(int16_t* out, uint32_t count, int16_t* input,int level)
{
    float volume = (float)level/100;
...
static void a2dp_sbc_encode_frames(uint8_t nb_frame) {
  ...
  uint8_t done_nb_frame = 0;
  int i = 0;
  int index = 0;
  uint8_t* output;
  uint16_t output_len;
  int16_t* input;
  int16_t input_multicast[A2DP_MULTI_NUM_CB][SBC_MAX_PCM_BUFFER_SIZE]={{0}};
  uint8_t* output_multicast[A2DP_MULTI_NUM_CB];
  BT_HDR* p_buf_multicast[A2DP_MULTI_NUM_CB];
  BT_HDR* p_buf;
  uint16_t encoder_len = 0;
  uint16_t latency[A2DP_MULTI_NUM_CB] = {0};
  uint16_t delay = 0;
  uint16_t m_delay = 0;
  uint16_t layer_specific = 0;
  uint32_t timestamp_ms = time_get_os_boottime_ms();
  if(is_multia2dp_supported){
    if(max_multi_a2dp > 1){
       delay = std::min(btif_multi_a2dp_cb[max_multi_a2dp-1].latency, btif_multi_a2dp_cb[max_multi_a2dp-2].latency);
        if(max_multi_a2dp == 3){
            delay = std::min(btif_multi_a2dp_cb[max_multi_a2dp-3].latency, delay);
        } else if(max_multi_a2dp == 4){
            m_delay = std::min(btif_multi_a2dp_cb[max_multi_a2dp-3].latency, btif_multi_a2dp_cb[max_multi_a2dp-4].latency);
            delay = std::min(m_delay, delay);
        }
    }
    for (i = 0; i < max_multi_a2dp; i++) {
        if(timestamp_ms < btif_multi_a2dp_cb[i].start_a2dp_time_ms){
             timestamp_ms = time_get_os_boottime_ms();
        }
        latency[i] = btif_multi_a2dp_cb[i].latency - delay;
        if((btif_multi_a2dp_cb[i].start_a2dp_time_ms != 0)
           && timestamp_ms - btif_multi_a2dp_cb[i].start_a2dp_time_ms >= latency[i]){
           btif_multi_a2dp_cb[i].flags = 1;
           index = i;
        } else {
  ...
  while (nb_frame) {
    uint32_t bytes_read = 0;
         if(!is_multia2dp_supported){
        p_buf = (BT_HDR*)osi_malloc(A2DP_SBC_BUFFER_SIZE);
        p_buf->offset = A2DP_SBC_OFFSET;
        p_buf->len = 0;
        p_buf->layer_specific = 0;
    } else {
        for (i = 0; i < max_multi_a2dp; i++) {
           if(btif_multi_a2dp_cb[i].flags == 1){
               p_buf_multicast[i] = (BT_HDR*)osi_malloc(A2DP_SBC_BUFFER_SIZE);
               p_buf_multicast[i]->offset = A2DP_SBC_OFFSET;
               p_buf_multicast[i]->len = 0;
               p_buf_multicast[i]->layer_specific = 0;
           }
        }
    }
    ...
    do {
      ...
      if (a2dp_sbc_read_feeding(&num_bytes)) {
                 input = a2dp_sbc_encoder_cb.pcmBuffer;
         if(!is_multia2dp_supported){
            output = (uint8_t*)(p_buf + 1) + p_buf->offset + p_buf->len;
            #ifdef MULTI_A2DP_ENABLE
            output_len = SBC_Encode(p_encoder_params, input, output,0);
            #else
            output_len = SBC_Encode(p_encoder_params, input, output);
            #endif
            ...
            encoder_len = p_buf->len;
            layer_specific = p_buf->layer_specific;
         } else {
             for (i = 0; i < max_multi_a2dp; i++) {
                if (btif_multi_a2dp_cb[i].flags == 1) {
                    if(btif_multi_a2dp_cb[i].isAbsSupported == 1)
                      setvolumeStereo(input_multicast[i],num_bytes/2,input,btif_multi_a2dp_cb[i].volume);
                    else
                      memcpy((uint8_t*)input_multicast[i],(uint8_t*)input,sizeof(a2dp_sbc_encoder_cb.pcmBuffer));
                    SBC_AnalysisSets16X(i);
                    output_multicast[i] = (uint8_t*)(p_buf_multicast[i] + 1) + p_buf_multicast[i]->offset + p_buf_multicast[i]->len;                   
                    #ifdef MULTI_A2DP_ENABLE
                    output_len = SBC_Encode(p_encoder_params, input_multicast[i], output_multicast[i],btif_multi_a2dp_cb[i].mod);
                    #else
                    output_len = SBC_Encode(p_encoder_params, input_multicast[i], output_multicast[i]);
                    #endif
                    p_buf_multicast[i]->len += output_len;
                    p_buf_multicast[i]->layer_specific++;
                    SBC_AnalysisSaves16X(i);
                    if(i == index){
                       last_frame_len = output_len;
                       nb_frame--;
                       bytes_read += num_bytes;
                       encoder_len = p_buf_multicast[i]->len;
                       layer_specific = p_buf_multicast[i]->layer_specific;
                    }
                }
            }
         }
        ...
        nb_frame = 0;
                ...
                ((encoder_len + last_frame_len) < a2dp_sbc_encoder_cb.TxAaMtuSize) &&
        (layer_specific < 0x0F) && nb_frame);
    if(!is_multia2dp_supported){
          if (p_buf->len) {
            /*
             ...
             *((uint32_t*)(p_buf + 1)) = a2dp_sbc_encoder_cb.timestamp;
             ...
             done_nb_frame = remain_nb_frame - nb_frame;
        ...
        for (i = 0; i < max_multi_a2dp; i++) {
            if (btif_multi_a2dp_cb[i].flags == 1) {
              if(p_buf_multicast[i]->len){
                if(i == index){
                   done_nb_frame = remain_nb_frame - nb_frame;
                   remain_nb_frame = nb_frame;
                }
                *((uint32_t*)(p_buf_multicast[i] + 1)) = btif_multi_a2dp_cb[i].timestamp;
                btif_multi_a2dp_cb[i].timestamp += p_buf_multicast[i]->layer_specific * blocm_x_subband;
                sbc_encoder_index = i;
                //LOG_WARN(LOG_TAG, "%s: p_buf_multicast[%d]->len:%d,nb_frame:%d",__func__, i,p_buf_multicast[i]->len,nb_frame);
                if (!a2dp_sbc_encoder_cb.enqueue_callback(p_buf_multicast[i], done_nb_frame,bytes_read)) return;
              } else {
                  a2dp_sbc_encoder_cb.stats.media_read_total_dropped_packets++;
                  osi_free(p_buf_multicast[i]);
              }
            }
        }
    }

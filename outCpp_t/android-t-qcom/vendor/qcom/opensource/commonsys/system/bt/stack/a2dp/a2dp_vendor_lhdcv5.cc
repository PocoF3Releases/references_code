...
static const tA2DP_LHDC_FEATURE_POS a2dp_lhdcv5_source_spec_JAS = {
    ...
    LHDCV5_FEATURE_ON_A2DP_SPECIFIC_4,
...
static bool is_codec_version_supported(uint8_t version, bool is_source) {
  ...
  LOG_DEBUG(LOG_TAG, "%s: versoin unsupported! peer:%d local:%d",
...
static tA2DP_STATUS A2DP_BuildInfoLhdcV5(uint8_t media_type,
  ...
  *p_result++ = para; para = 0;
  ...
  LOG_DEBUG(LOG_TAG, "%s: codec info built = H0-H2{%02X %02X %02X} P0-P3{%02X "
...
static tA2DP_STATUS A2DP_ParseInfoLhdcV5(tA2DP_LHDCV5_CIE* p_ie,
  ...
  LOG_DEBUG(LOG_TAG, "%s: codec info parsed = H0-H2{%02X %02X %02X} P0-P3{%02X "
      "%02X %02X %02X} P4-P5{%02X %02X} P6{%02X} P7{%02X} P8{%02X} P9{%02X} P10{%02X}", __func__,
      tmpInfo[0], tmpInfo[1], tmpInfo[2], tmpInfo[3], tmpInfo[4], tmpInfo[5], tmpInfo[6], tmpInfo[7],
      tmpInfo[8], tmpInfo[9], tmpInfo[10], tmpInfo[11], tmpInfo[12], tmpInfo[A2DP_LHDCV5_CODEC_LEN]);

  LOG_DEBUG(LOG_TAG, "%s: %s isCap{%d} SR{%02X} BPS{%02X} Ver{%02X} FL{%02X} "
  ...
  if (!is_source) {
    if (!a2dp_lhdcv5_decoder_save_codec_info(p_codec_Info_save)) {
      LOG_DEBUG(LOG_TAG, "%s: save decoder parameters error", __func__);
    }
  }
#endif
...
int A2DP_VendorGetBitRateLhdcV5(const uint8_t* p_codec_info) {
  ...
  if (NULL == current_codec) {
    LOG_ERROR(LOG_TAG, "%s: failed to get current_codec", __func__);
    return 0;
  }
...
bool A2DP_VendorDumpCodecInfoLhdcV5(const uint8_t* p_codec_info) {
  tA2DP_STATUS a2dp_status;
  tA2DP_LHDCV5_CIE lhdc_cie;
...
bool A2DP_VendorInitCodecConfigLhdcV5(tAVDT_CFG* p_cfg) {
  if (p_cfg == nullptr) {
    LOG_ERROR(LOG_TAG, "%s: nullptr input", __func__);
    return false;
  }
...
bool A2dpCodecConfigLhdcV5Source::init() {
  ...
  if (A2DP_IsCodecEnabledInOffload(BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV5)) {
...
bool A2dpCodecConfigLhdcV5Base::setCodecConfig(const uint8_t* p_peer_codec_info,
  ...
  LOG_DEBUG(LOG_TAG, "%s: sampleRate Cap:{peer:0x%02X local:0x%02X result:0x%02X}",
      __func__, sink_info_cie.sampleRate, p_a2dp_lhdcv5_caps->sampleRate, sampleRate);

  codec_config_.sample_rate = BTAV_A2DP_CODEC_SAMPLE_RATE_NONE;
  LOG_DEBUG(LOG_TAG, "%s: codec_user_config_.sample_rate:0x%02X",
      __func__, codec_user_config_.sample_rate);
  ...
  do {
    ...
    if (codec_config_.sample_rate != BTAV_A2DP_CODEC_SAMPLE_RATE_NONE) {
      LOG_DEBUG(LOG_TAG, "%s: sample rate configured successfully 0x%02X",
    ...
    if (select_audio_sample_rate(&codec_audio_config_, sampleRate,
      ...
      LOG_DEBUG(LOG_TAG, "%s: select sample rate from audio: 0x%02X", __func__,
    ...
    if (select_best_sample_rate(
        ...
        &result_config_cie, &codec_config_)) {
      LOG_DEBUG(LOG_TAG, "%s: select sample rate from default: 0x%02X", __func__,
          result_config_cie.sampleRate);
      break;
    }
    ...
    if (select_best_sample_rate(sampleRate, &result_config_cie,
        &codec_config_)) {
      LOG_DEBUG(LOG_TAG, "%s: select sample rate from best match: 0x%02X", __func__,
          result_config_cie.sampleRate);
      break;
    }
  } while (false);
  ...
  LOG_DEBUG(LOG_TAG, "%s: => sample rate(0x%02X) = %s", __func__,
      result_config_cie.sampleRate,
  ...
  LOG_DEBUG(LOG_TAG, "%s: bitsPerSample:{peer:0x%02X local:0x%02X result:0x%02X}",
  ...
  do {
    ...
    if (codec_config_.bits_per_sample != BTAV_A2DP_CODEC_BITS_PER_SAMPLE_NONE) {
      LOG_DEBUG(LOG_TAG, "%s: bit_per_sample configured successfully 0x%02X",
    ...
    if (select_audio_bits_per_sample(&codec_audio_config_, bitsPerSample,
        &result_config_cie, &codec_config_)) {
      LOG_DEBUG(LOG_TAG, "%s: select bit per sample from audio: 0x%02X", __func__,
    ...
    if (select_best_bits_per_sample(
        ...
        &result_config_cie, &codec_config_)) {
      LOG_DEBUG(LOG_TAG, "%s: select bit per sample from default: 0x%02X", __func__,
          result_config_cie.bitsPerSample);
      break;
    }
  ...
  LOG_DEBUG(LOG_TAG, "%s: => bit per sample(0x%02X) = %s", __func__,
      result_config_cie.bitsPerSample,
  ...
  LOG_DEBUG(LOG_TAG, "%s: => frame length type(0x%02X) = %s", __func__,
  ...
  LOG_DEBUG(LOG_TAG, "%s: => peer Max Bit Rate(0x%02X) = %s", __func__,
  ...
  LOG_DEBUG(LOG_TAG, "%s: => peer Min Bit Rate(0x%02X) = %s", __func__,
  ...
  if ((codec_user_config_.codec_specific_4 & A2DP_LHDC_VENDOR_FEATURE_MASK) != A2DP_LHDC_FEATURE_MAGIC_NUM)
  {
    ...
    LOG_DEBUG(LOG_TAG, "%s: LHDC features tag check fail, reset UI status[AR] => %s", __func__, hasUserSet?"true":"false");
  }
  ...
  if ((codec_user_config_.codec_specific_1 & A2DP_LHDC_VENDOR_CMD_MASK) != A2DP_LHDC_QUALITY_MAGIC_NUM) {
    codec_user_config_.codec_specific_1 = (A2DP_LHDC_QUALITY_MAGIC_NUM | A2DP_LHDCV5_QUALITY_ABR) - A2DP_LHDC_QUALITY_OFFSET;
    LOG_DEBUG(LOG_TAG, "%s: tag not match, use default Quality Mode: ABR", __func__);
  }
  qualityMode = ((uint8_t)codec_user_config_.codec_specific_1 & A2DP_LHDCV5_QUALITY_MASK) + A2DP_LHDC_QUALITY_OFFSET;  
   ...
   *******************************************/
  {
    ...
    LOG_DEBUG(LOG_TAG, "%s: featureJAS: enabled? <%s> {Peer:0x%02X Local:0x%02X User:%s}", __func__,
   ...
   *******************************************/
  {
    ...
    if (hasFeature && (hasUserSet || sink_info_cie.featureOnAR)) {
      ...
      if (codec_user_config_.sample_rate != BTAV_A2DP_CODEC_SAMPLE_RATE_48000) {
        ...
        result_config_cie.sampleRate = A2DP_LHDCV5_SAMPLING_FREQ_48000;
        LOG_DEBUG(LOG_TAG, "%s: set 48KHz sample Rate for running AR", __func__);
      }
      ...
      if (codec_user_config_.bits_per_sample != BTAV_A2DP_CODEC_BITS_PER_SAMPLE_24) {
        ...
        result_config_cie.bitsPerSample = A2DP_LHDCV5_BIT_FMT_24;
        LOG_DEBUG(LOG_TAG, "%s: set 24 bits per sample 24 for running AR", __func__);
      }
    }
    LOG_DEBUG(LOG_TAG, "%s: featureAR: enabled? <%s> {Peer:0x%02X Local:0x%02X User:%s AR_ON:%s}", __func__,
   ...
   *******************************************/
  {
    ...
    LOG_DEBUG(LOG_TAG, "%s: featureMETA: enabled? <%s> {Peer:0x%02X Local:0x%02X User:%s}", __func__,
   ...
   *******************************************/
  {
    ...
    LOG_DEBUG(LOG_TAG, "%s: featureLL: enabled? <%s> {Peer:0x%02X Local:0x%02X User:%s}", __func__,
   ...
   *******************************************/
  {
    ...
    LOG_DEBUG(LOG_TAG, "%s: featureLLESS: enabled? <%s> {Peer:0x%02X Local:0x%02X User:%s}", __func__,
  ...
  if (qualityMode != A2DP_LHDCV5_QUALITY_ABR) {
    ...
    if (qualityMode > bitRateQmode) {
      codec_user_config_.codec_specific_1 = (A2DP_LHDC_QUALITY_MAGIC_NUM | bitRateQmode) - A2DP_LHDC_QUALITY_OFFSET;
      qualityMode = bitRateQmode;
      LOG_DEBUG(LOG_TAG, "%s: downgrade quality mode to 0x%02X", __func__, qualityMode);
    }
    ...
    if (qualityMode < bitRateQmode) {
      codec_user_config_.codec_specific_1 = (A2DP_LHDC_QUALITY_MAGIC_NUM | bitRateQmode) - A2DP_LHDC_QUALITY_OFFSET;
      qualityMode = bitRateQmode;
      LOG_DEBUG(LOG_TAG, "%s: upgrade quality mode to 0x%02X", __func__, qualityMode);
    }

#if 0 // do not excute because Xiaomi's UI bitrate is lowest at 256kbps.
    ...
    if (result_config_cie.sampleRate == A2DP_LHDCV5_SAMPLING_FREQ_96000 ||
      ...
      if (qualityMode < A2DP_LHDCV5_QUALITY_LOW3) {
        codec_user_config_.codec_specific_1 = (A2DP_LHDC_QUALITY_MAGIC_NUM | A2DP_LHDCV5_QUALITY_LOW4) - A2DP_LHDC_QUALITY_OFFSET;
        qualityMode = A2DP_LHDCV5_QUALITY_LOW4;
        LOG_DEBUG(LOG_TAG, "%s: upgrade quality mode to 0x%02X due to higher sample rates", __func__, qualityMode);
      }
    }
#endif

#if 0// do not excute because Xiaomi's UI bitrate is highest at 900kbps.
    ...
    if (result_config_cie.sampleRate == A2DP_LHDCV5_SAMPLING_FREQ_44100 ||
      ...
      if (qualityMode == A2DP_LHDCV5_QUALITY_HIGH1) {
        ...
        LOG_DEBUG(LOG_TAG, "%s: downgrade quality mode to 0x%02X due to lower sample rates", __func__, qualityMode);
      }
    }
#endif
  }
  ...
  codec_config_.codec_specific_3 = result_config_cie.hasFeatureLL == true ? 1 : 0;
  codec_selectable_capability_.codec_specific_3 = sink_info_cie.hasFeatureLL ? 1 : 0;
...
static bool swapInt64toByteArray(uint8_t *byteArray, int64_t integer64) {
  if (byteArray == nullptr) {
    LOG_DEBUG(LOG_TAG, "%s: null ptr", __func__);
    return false;
  }
...
static bool A2DP_LhdcV5GetFeatureInfoV2(uint8_t *featVector) {
  ...
  if(featVector == nullptr) {
    LOG_DEBUG(LOG_TAG, "%s: null buffer!", __func__);
    return false;
  }

  featNum = (sizeof(a2dp_lhdcv5_source_spec_all) / sizeof(tA2DP_LHDC_FEATURE_POS) );
  if (featNum <= 0) {
    //LOG_DEBUG(LOG_TAG, "%s: no any features recorded in specifics, nothing to do", __func__);
    return true;
  }
...
static bool A2DP_LhdcV5GetA2DPSpecificV2(btav_a2dp_codec_config_t *a2dpCfg,
  ...
  if (len < LHDCV5_EXTEND_API_A2DP_SPEC_TOTAL_SIZE_V2) {
    LOG_DEBUG(LOG_TAG, "%s: payload size too small %d ",__func__, len);
    return false;
  }
...
static int A2DP_getEncoderExtendFuncA2DPTypeLhdcV5(A2dpCodecConfig* codecCfg,
  ...
  if (buflen < (LHDCV5_EXTEND_API_A2DP_SPEC_VER_SIZE + LHDCV5_EXTEND_API_A2DP_SPEC_CODE_SIZE)) {
    LOG_DEBUG(LOG_TAG, "%s: buf len too small %d",  __func__, buflen);
  ...
  case LHDCV5_EXTEND_API_A2DP_SPEC_CODE:
    ...
    case LHDCV5_EXTEND_API_A2DP_SPEC_AUDIO_CFG:
      ...
      break;
    default:
      LOG_DEBUG(LOG_TAG, "%s: unknown a2dp codec config",  __func__);
      return BT_STATUS_FAIL;
    }
    break;
    default:
      LOG_DEBUG(LOG_TAG, "%s: invalid ext API command(0x%X)",  __func__, exFuncCode);
      return BT_STATUS_FAIL;
  }
  ...
  case LHDCV5_EXTEND_API_A2DP_SPEC_VER2:
    if (!A2DP_LhdcV5GetA2DPSpecificV2(&codec_config_tmp, buf, buflen)) {
      LOG_DEBUG(LOG_TAG, "%s: get specifics error",  __func__);
      return BT_STATUS_FAIL;
    }
    break;
  default:
    LOG_DEBUG(LOG_TAG, "%s: invalid ext API version(0x%X)",  __func__, exFuncVer);
    return BT_STATUS_FAIL;
  }
...
int A2dpCodecConfigLhdcV5Base::getLhdcExtendAPIVersion(A2dpCodecConfig* peerCodec,
  ...
  if (peerCodec == nullptr || buf == nullptr || clen <= 0) {
    LOG_DEBUG(LOG_TAG, "%s: invalid input clen %d", __func__, clen);
    return BT_STATUS_FAIL;
  }
  ...
  if (is_source_) {
    ...
    LOG_DEBUG(LOG_TAG, "%s: sink not supported", __func__);
    return BT_STATUS_FAIL;
  }
...
bool A2DP_VendorGetMaxBitRateLhdcV5(uint32_t *retval, const uint8_t* p_codec_info){
  ...
  LOG_ERROR(LOG_TAG, "%s: failed to get lhdcv5 Max bitrate, lhdc_cie.maxTargetBitrate: %d", __func__,
      lhdc_cie.maxTargetBitrate);
  return false;
}

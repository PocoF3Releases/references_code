...
uint16_t GATTS_AddService(tGATT_IF gatt_if, btgatt_db_element_t* service,
  ...
  GATT_TRACE_EVENT("%s: handles needed=%d, s_hdl=%s , e_hdl=%s , uuid=%s, is_primary=%d", __func__, num_handles,
                   loghex(list.asgn_range.s_handle).c_str(),
                   loghex(list.asgn_range.e_handle).c_str(),
                   list.asgn_range.svc_uuid.ToString().c_str(),
  ...
  GATT_TRACE_EVENT("%s: service parsed correctly, now starting", __func__);
  ...
  GATT_TRACE_EVENT("%s: allocated el s_hdl=%s, e_hdl=%s, type=%s, sdp_hdl=%s", __func__, loghex(elem.s_hdl).c_str(),
                    loghex(elem.e_hdl).c_str(), loghex(elem.type).c_str(),
                    loghex(elem.sdp_handle).c_str());
...
bool GATTS_DeleteService(tGATT_IF gatt_if, Uuid* p_svc_uuid,
  ...
  GATT_TRACE_EVENT("released handles s_hdl=%s, e_hdl=%s", loghex(it->asgn_range.s_handle).c_str(),
                    loghex(it->asgn_range.e_handle).c_str());
...
void GATTS_StopService(uint16_t service_handle) {
  GATT_TRACE_EVENT("%s: %s", __func__, loghex(service_handle).c_str());
...
tGATT_STATUS GATTC_ConfigureMTU(uint16_t conn_id, uint16_t mtu) {
  ...
  if (p_tcb->transport != BT_TRANSPORT_LE || mtu < GATT_DEF_BLE_MTU_SIZE ||
        mtu > GATT_MAX_MTU_SIZE) {
    VLOG(1) << __func__ << " current mtu = " << p_tcb->payload_size;
    tGATT_CMPL_CBACK* p_cmpl_cb =
...
tGATT_IF GATT_Register(const Uuid& app_uuid128, tGATT_CBACK* p_cb_info, bool eatt_support) {
  ...
  GATT_TRACE_EVENT("%s %s", __func__, app_uuid128.ToString().c_str());
  ...
  for (i_gatt_if = 0, p_reg = gatt_cb.cl_rcb; i_gatt_if < GATT_MAX_APPS;
    ...
    if (!p_reg->in_use) {
      ...
      GATT_TRACE_EVENT("allocated gatt_if=%d", gatt_if);
      return gatt_if;
    }
  }
...
void GATT_Deregister(tGATT_IF gatt_if) {
  ...
  GATT_TRACE_EVENT("%s gatt_if=%d", __func__, gatt_if);
 ...
 *
 * Description      Sets the trace level for GATT. If 0xff is passed, the
 *                  current trace level is returned.
 *
 *                  Input Parameters:
 *                      new_level:  The level to set the GATT tracing to:

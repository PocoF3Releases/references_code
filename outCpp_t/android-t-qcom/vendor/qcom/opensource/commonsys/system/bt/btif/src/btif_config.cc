...
static const char* CONFIG_DATABASE_DYNAMIC = "interop_database_dynamic.conf";
#else   // !defined(OS_GENERIC)
static const char* CONFIG_DATABASE_DYNAMIC = "/data/misc/bluedroid/interop_database_dynamic.conf";
...
// XIAOMI ADD:
thread_t* btconfig_async_thread;
sem_t btconfig_async_sem;
// XIAOMI END
...
static future_t* init(void) {
  ...
  // XIAOMI_ADD:
  if (!btconfig_async_thread) {
      btconfig_async_thread = thread_new("config_shutdown_thread");
      if (!btconfig_async_thread) {
          LOG_ERROR(LOG_TAG, "%s unable to create shutdown config thread.", __func__);
      }
      sem_init(&btconfig_async_sem, 0, 0);
  } else {
      LOG_ERROR(LOG_TAG, "%s shutdown config aync thread created.", __func__);
  }
  // XIAOMI_END
...
// XIAOMI ADD:
static void event_bt_config_flush(UNUSED_ATTR void* context) {
  alarm_cancel(config_timer);
  btif_config_write(0, NULL);
  sem_post(&btconfig_async_sem);
}

void btif_config_flush_async(void) {
  if (btconfig_async_thread) {
      CHECK(config != NULL);
      CHECK(config_timer != NULL);
      LOG_INFO(LOG_TAG, "%s async start", __func__);
      struct timespec ts = {0,0};
      clock_gettime(CLOCK_REALTIME, &ts);
      ts.tv_sec += 3;

      thread_post(btconfig_async_thread, event_bt_config_flush, NULL);
      int ret = sem_timedwait(&btconfig_async_sem, &ts);
      if (!ret) {
          thread_free(btconfig_async_thread);
      }
      btconfig_async_thread = NULL;
      LOG_INFO(LOG_TAG, "%s async end = : %d", __func__, ret);
  } else {
      LOG_INFO(LOG_TAG, "%s call leagcy flush function", __func__);
      btif_config_flush();
  }
}
// XIAOMI END
...
static void delete_config_files(void) {
  ...
  remove(CONFIG_DATABASE_DYNAMIC);

 ...
 */
// XM: Modified for LHDC from (MAX_PCM_FRAME_NUM_PER_TICK * 2)
// to (MAX_PCM_FRAME_NUM_PER_TICK * 6)
...
tBTIF_MULTI_A2DP_CB btif_multi_a2dp_cb[A2DP_MULTI_NUM_CB];
int sbc_encoder_index = 0;
...
extern int btif_av_get_multicast_device_idx_to_opened();
...
static void btif_a2dp_source_shutdown_delayed(UNUSED_ATTR void* context) {
  ...
  for (int i = 0; i < A2DP_MULTI_NUM_CB; i++) {
    if(btif_multi_a2dp_cb[i].tx_audio_queue != NULL){
        fixed_queue_free(btif_multi_a2dp_cb[i].tx_audio_queue, NULL);
        btif_multi_a2dp_cb[i].tx_audio_queue = NULL;
    }
  }
...
#ifdef SUPPORT_LHDC_CODEC
    } else if (codec_config.codec_type == BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV1
        || codec_config.codec_type == BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV2
        || codec_config.codec_type == BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV3
        || codec_config.codec_type == BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV5) {
      uint32_t bitrate = 0;
      bitrate = A2DP_GetTrackBitRate(p_codec_info);
      APPL_TRACE_DEBUG(LOG_TAG,"bitrate = %d", bitrate);
...
void btif_a2dp_source_start_audio_req(void) {
  ...
  BTIF_TRACE_DEBUG("%s: clear tx_flush when a2dp start.",__func__);
  if (btif_a2dp_source_cb.tx_flush == true)
...
static void btif_a2dp_source_audio_handle_timer(UNUSED_ATTR void* context) {
  size_t transmit_queue_length = 0;
  ...
  if (alarm_is_scheduled(btif_a2dp_source_cb.media_alarm)) {
    ...
    if (!btif_av_is_multicast_supported()) {
        ...
        transmit_queue_length = std::max(fixed_queue_length(btif_multi_a2dp_cb[0].tx_audio_queue),
                                fixed_queue_length(btif_multi_a2dp_cb[1].tx_audio_queue));
    }
#ifndef OS_GENERIC
...
#ifdef SUPPORT_LHDC_CODEC
    while(true) {
      bytes_read = UIPC_Read(UIPC_CH_ID_AV_AUDIO, &event, p_buf + offset, remain);
      offset += bytes_read;
      remain -= bytes_read;
      if(remain <= 0) {
        break;
      ...
      timeout--;
      LOG_DEBUG(LOG_TAG, "%s: timeout count(%d), remain(%d),  offset(%d), ",
            __func__, timeout, remain, offset);
    }
#else
...
static bool btif_a2dp_source_enqueue_callback(BT_HDR* p_buf, size_t frames_n,
  ...
  if(btif_av_is_multicast_supported()){
   for (int i = 0; i < max_multi_a2dp; i++) {
      //LOG_DEBUG(LOG_TAG,"%s: btif_multi_a2dp_cb[%d].flags %d,sbc_encoder_index:%d", __func__,i,btif_multi_a2dp_cb[i].flags,sbc_encoder_index);
      if(btif_multi_a2dp_cb[i].flags != 1){
         continue;
      }
      if (fixed_queue_length(btif_multi_a2dp_cb[i].tx_audio_queue) + frames_n >
          MAX_OUTPUT_A2DP_FRAME_QUEUE_SZ) {
        LOG_DEBUG(LOG_TAG, "%s: TX queue buffer size now=%u adding=%u max=%d",
                 __func__,
                 (uint32_t)fixed_queue_length(btif_multi_a2dp_cb[i].tx_audio_queue),
        ...
        size_t drop_n = fixed_queue_length(btif_multi_a2dp_cb[i].tx_audio_queue);
      ...
      if(sbc_encoder_index == i){
         CHECK(btif_a2dp_source_cb.encoder_interface != NULL);
         fixed_queue_enqueue(btif_multi_a2dp_cb[i].tx_audio_queue, p_buf);
      }
    }
  } else {
...
static void btif_a2dp_source_audio_tx_flush_event(UNUSED_ATTR BT_HDR* p_msg) {
  ...
  fixed_queue_flush(btif_a2dp_source_cb.tx_audio_queue, osi_free);
  for (int i = 0; i < A2DP_MULTI_NUM_CB; i++) {
       fixed_queue_flush(btif_multi_a2dp_cb[i].tx_audio_queue, osi_free);
  }
...
BT_HDR* btif_a2dp_source_audio_readbuf(RawAddress peer_addr) {
  uint64_t now_us = time_get_os_boottime_us();
  BT_HDR* p_buf = NULL;
  //uint16_t playing_num = 0;
  //uint16_t index = 0;
  if(!btif_av_is_multicast_supported()){
     p_buf = (BT_HDR*)fixed_queue_try_dequeue(btif_a2dp_source_cb.tx_audio_queue);
  } else {
     /*for (int i = 0; i < A2DP_MULTI_NUM_CB; i++) {
         if(btif_multi_a2dp_cb[i].flags == 1){
            playing_num++;
         }
         if(btif_multi_a2dp_cb[i].peer_bda == peer_addr){
            index = i;
         }
     }
     if (playing_num == max_multi_a2dp || isSlaveSwitch){
         p_buf = (BT_HDR*)fixed_queue_try_dequeue(btif_multi_a2dp_cb[index].tx_audio_queue);
     }*/
    for (int i = 0; i < A2DP_MULTI_NUM_CB; i++) {
        if((btif_multi_a2dp_cb[i].flags == 1)
            && (btif_multi_a2dp_cb[i].peer_bda == peer_addr)){
           p_buf = (BT_HDR*)fixed_queue_try_dequeue(btif_multi_a2dp_cb[i].tx_audio_queue);
        }
    }
  }
  ...
  if (p_buf != NULL) {
    //APPL_TRACE_EVENT("%s: p_buf is not null, updating queue statistics.", __func__);
    // Update the statistics
      ...
      if (btif_av_stream_started_ready()) {
        ...
        if (btif_a2dp_src_vsc.tx_start_initiated == TRUE) {
          ...
          if (remote_start_flag) {
            ...
            int multicast_idx = btif_av_get_multicast_device_idx_to_opened();
            if (multicast_idx != btif_max_av_clients) {
                APPL_TRACE_DEBUG("%s: multicast device is opened",__func__);
                btif_dispatch_sm_event(BTIF_AV_START_STREAM_REQ_EVT, (char *)&multicast_idx, 1);
                status = A2DP_CTRL_ACK_PENDING;
            } else {
                status = A2DP_CTRL_ACK_SUCCESS;
            }

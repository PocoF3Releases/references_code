...
// MIUI ADD: START
  uint8_t len;
  uint32_t cod = 0;
  bt_property_t propertie;
  DEV_CLASS dev_class_cache;
  #define D_PEN 0x051c
  const uint8_t* p_uuid16 = AdvertiseDataParser::GetFieldByType(
        value, BTM_BLE_AD_TYPE_APPEARANCE, &len);

  if (p_uuid16 && len == 2)
  {
    appearance_to_cod((uint16_t)p_uuid16[0] | (p_uuid16[1] << 8), dev_class_cache);
    cod = devclass2uint(dev_class_cache);
    BTIF_TRACE_DEBUG("%s cod is 0x%06x", __func__, cod);
    if(cod == D_PEN)
    {
      BTIF_STORAGE_FILL_PROPERTY(&propertie, BT_PROPERTY_CLASS_OF_DEVICE, sizeof(cod), &cod);
      btif_storage_set_remote_device_property(&bd_addr, &propertie);
    }
  }
// END
...
void bta_scan_results_cb(tBTA_DM_SEARCH_EVT event, tBTA_DM_SEARCH* p_data) {
  ...
  /* Cache specifiled devices */
  const uint8_t *p_manudata  = AdvertiseDataParser::GetFieldByType(
                                                    value, BTM_EIR_MANUFACTURER_SPECIFIC_TYPE, &len);
  if (p_manudata && (len >= 2)) {
    if ((p_manudata[0] == 0x57) && (p_manudata[1] == 0x01)
        && (p_data->inq_res.ble_addr_type == BLE_ADDR_RANDOM)) {
      if (!btif_storage_is_ble_static_ramdon_conn_device(&p_data->inq_res.bd_addr)) {
        BTIF_TRACE_WARNING("cache HUAMI Random device");
        btif_storage_save_ble_static_ramdon_conn_device(&p_data->inq_res.bd_addr);
      }
    }
  }

  if (p_manudata && (len >= 2)) {
    if ((p_manudata[0] == 0x8F) && (p_manudata[1] == 0x03)) {
      if (!btif_storage_is_att_secure_level_0_device(&p_data->inq_res.bd_addr)) {
          btif_storage_save_quick_connct_device(&p_data->inq_res.bd_addr);
      }
    }
  }

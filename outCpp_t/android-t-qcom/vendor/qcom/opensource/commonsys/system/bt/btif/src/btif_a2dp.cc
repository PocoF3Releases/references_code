  ...
  if (p_av_start == NULL) {
    ...
    if (btif_av_is_split_a2dp_enabled()) {
      if (bluetooth::headset::btif_hf_is_call_vr_idle())
        ...
        /* two headset connected, one device is lhdc device, the other is not,
         * when playing music through lhdc, disconect the lhdc device, then
         * active device change to the other device, a2dp start will be init
         * by audio, but stack did not send ack to audio. we send it here.
         */
        if (pending_cmd == A2DP_CTRL_CMD_START && btif_av_get_multicast_state() == false) {
  ...
  if (!btif_av_is_split_a2dp_enabled()) {
    ...
    if (btif_a2dp_audio_if_init || btif_a2dp_source_is_hal_v2_supported()) {
      if (btif_a2dp_source_is_hal_v2_supported()) {
        ...
        pending_cmd = bluetooth::audio::a2dp::get_pending_command();
        if (pending_cmd == A2DP_CTRL_CMD_START) {
          APPL_TRACE_EVENT("btif_a2dp_on_stopped, ACK disconnect in progress for pending start");
...
#ifdef SUPPORT_LHDC_CODEC
      if (btif_av_is_lhdc_codec() && !btif_av_is_split_a2dp_enabled()

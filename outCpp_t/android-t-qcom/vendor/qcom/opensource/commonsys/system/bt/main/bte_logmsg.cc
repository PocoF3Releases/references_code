...
// DEFAULT_CONF_TRACE_LEVEL},
    ...
    {BTTRC_ID_LATENCY_AUDIO, BTTRC_ID_LATENCY_AUDIO, AUDIO_Latency_SetTraceLevel, "TRC_LATENCY_AUDIO",
     DEFAULT_CONF_TRACE_LEVEL},
#endif
    /* LayerIDs for BTA, currently everything maps onto appl_trace_level.
     */
...
static void load_levels_from_config(const config_t* config) {
  ...
  if (!strncmp(btsnoop_enabled, "true", 4)) snoop_enabled = true;
  ...
  for (tBTTRC_FUNC_MAP* functions = &bttrc_set_level_map[0];
    ...
    if (snoop_enabled) {
        functions->trace_level = 6;  //full level debuging
    }else if (value != -1) {

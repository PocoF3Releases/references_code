...
config_t* config_new_clone(const config_t* src) {
  ...
  for (const list_node_t* node = list_begin(src->sections);
       node != list_end(src->sections) && i <= list_length(src->sections);
    ...
    if (sec) {
      ...
      for (const list_node_t* node_entry = list_begin(sec->entries);
           node_entry != list_end(sec->entries) && ii <= list_length(sec->entries);
           node_entry = list_next(node_entry), ii++) {
...
void config_dump(const config_t *config, int fd)
{
  assert(config != NULL);
  unsigned char is_key = 0;

  for (const list_node_t *node = list_begin(config->sections);
           node != list_end(config->sections); node = list_next(node)) {
      const section_t *section = (const section_t *)list_node(node);

      if (strchr(section->name, ':')) {
        //xiaomi-bt: only dump first 3 bytes of a BD Address.
        char addr_NAP_UAP[10] = {0};
        strncpy(addr_NAP_UAP, section->name, 8);
        if (dprintf(fd, "[%s]\n", addr_NAP_UAP) < 0) {
          LOG_ERROR(LOG_TAG, " section dump err, section=%s\n", addr_NAP_UAP);
        }
      } else {
        if (dprintf(fd, "[%s]\n", section->name) < 0) {
          LOG_ERROR(LOG_TAG, " section dump err, section=%s\n", section->name);
      ...
      for (const list_node_t *enode = list_begin(section->entries);
        ...
        if (strcasestr(entry->key, "KEY")) is_key = 1;

        unsigned char colon_offset = 0;
        unsigned char is_bt_addr = 0;
        char* frist_colon_pt = strchr(entry->value,':');
        if (frist_colon_pt != NULL) {
           do {
             colon_offset++;
             if (colon_offset == 5) {
               is_bt_addr = 1;
             ...
             if (size_t(frist_colon_pt - entry->value + colon_offset*3) > strlen(entry->value)) break;
           } while (*(frist_colon_pt + colon_offset*3) == ':');
        }

        if ( is_bt_addr == 1 && strcmp(entry->key, "Name") != 0) {
          //xiaomi-bt: only dump first 3 bytes of a BD Address.
          char* short_addr_buf = static_cast<char*>(osi_calloc(strlen(entry->value)));
          strncpy(short_addr_buf, entry->value, 8);
          char* current_pt = entry->value + 17;
          while ((current_pt = strchr(current_pt,',')) != NULL) {
            strncat(short_addr_buf,current_pt,9);
            current_pt += 1;
          }
          if (dprintf(fd, "%s = %s\n", entry->key, !is_key ? short_addr_buf : " ") < 0) {
            LOG_ERROR(LOG_TAG, " key dump err, key=%s, value=%s\n", entry->key, short_addr_buf);
          }
          osi_free(short_addr_buf);
        } else {
          if (dprintf(fd, "%s = %s\n", entry->key, !is_key ? entry->value : " ") < 0) {
...
static section_t* section_find(const config_t* config, const char* section) {
  size_t i = 0;
  for (const list_node_t* node = list_begin(config->sections);
       node != list_end(config->sections) && (i <= list_length(config->sections));
       node = list_next(node), i++) {
    section_t* sec = static_cast<section_t*>(list_node(node));
...
static entry_t* entry_find(const config_t* config, const char* section,
  ...
  size_t i = 0;
  for (const list_node_t* node = list_begin(sec->entries);
       node != list_end(sec->entries) && (i <= list_length(sec->entries));
       node = list_next(node), i++) {
    entry_t* entry = static_cast<entry_t*>(list_node(node));

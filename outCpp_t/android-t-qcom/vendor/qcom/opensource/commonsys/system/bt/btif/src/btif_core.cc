  ...
  switch (event) {
    ...
    case BTIF_CORE_STORAGE_ADAPTER_READ: {
      ...
      if (prop.type == BT_PROPERTY_LOCAL_LE_FEATURES) {
        ...
        if (btif_av_is_split_a2dp_enabled() == false) {
...
bt_status_t btif_set_adapter_property(const bt_property_t* property) {
  ...
  uint16_t name_len = 0;
  bool is_page_scan_type_changed = false;
  ...
  switch (property->type) {
    ...
    case BT_PROPERTY_ADAPTER_SCAN_MODE: {
      ...
      switch (mode) {
        ...
        case BT_SCAN_MODE_PAGE_SCAN_STANDARD:
          page_scan_type = BTA_DM_SCAN_TYPE_STANDARD;
          is_page_scan_type_changed = true;
          break;

        case BT_SCAN_MODE_PAGE_SCAN_INTERLACED:
          page_scan_type = BTA_DM_SCAN_TYPE_INTERLACED;
          is_page_scan_type_changed = true;
          break;
      ...
      if (is_page_scan_type_changed) {
        BTIF_TRACE_EVENT("set property scan mode : %x", mode);
        BTA_DmSetPageScanType(page_scan_type);
        storage_req_id = BTIF_CORE_STORAGE_ADAPTER_WRITE;
        break;
      }
    ...
    case BT_PROPERTY_SAR_DSI: {
      int dsi = *(int*)property->val;
      BTIF_TRACE_ERROR("btif_get_adapter_property : type %d, val %d",
                       property->type, dsi);
      BTA_DmSetSarLimited(dsi);
    } break;
    case BT_PROPERTY_RNR: {
       int power_status = *(int*)property->val;
       BTA_DmSetLimitRnr(power_status);
    } break;
    default:

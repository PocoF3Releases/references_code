...
void bta_hh_le_register_input_notif(tBTA_HH_DEV_CB* p_dev_cb, uint8_t srvc_inst,
  ...
  if (p_dev_cb != NULL) {
    p_dev_cb->register_notify = BTA_HH_LE_NOTIFY_REGISTER;
  }
#if (BTA_HH_DEBUG == TRUE)
...
static void bta_hh_gattc_callback(tBTA_GATTC_EVT event, tBTA_GATTC* p_data) {
  ...
  switch (event) {
    ...
    case BTA_GATTC_SEARCH_CMPL_EVT: /* 6 */
      if (p_data != NULL) {
        tBTA_HH_DEV_CB* p_dev_cb = bta_hh_le_find_dev_cb_by_conn_id(p_data->search_cmpl.conn_id);
        if (p_dev_cb != NULL && p_dev_cb->register_notify == BTA_HH_LE_NOTIFY_CLEAN) {
          bta_hh_le_register_input_notif(p_dev_cb, 0, p_dev_cb->mode, false);
        }
      }
    ...
    case BTA_GATTC_SRVC_CHG_EVT:
        if (p_data != NULL) {
            tBTA_HH_DEV_CB* p_dev_cb = bta_hh_le_find_dev_cb_by_bda(p_data->remote_bda);
            if (p_dev_cb != NULL && p_dev_cb->register_notify == BTA_HH_LE_NOTIFY_REGISTER) {
                p_dev_cb->register_notify = BTA_HH_LE_NOTIFY_CLEAN;
            }
        }
        break;
    ...
    case BTA_GATTC_SRVC_DISC_DONE_EVT:
        if (p_data != NULL) {
            tBTA_HH_DEV_CB* p_dev_cb = bta_hh_le_find_dev_cb_by_bda(p_data->enc_cmpl.remote_bda);
            if (p_dev_cb != NULL && p_dev_cb->register_notify == BTA_HH_LE_NOTIFY_CLEAN) {
                bta_hh_le_pri_service_discovery(p_dev_cb);

...
tBTM_STATUS BTM_SwitchRole(const RawAddress& remote_bd_addr, uint8_t new_role,
  ...
  VLOG(1) << __func__ << " p->link_role: " << p->link_role << "new_role:" <<new_role;

  if (interop_match_addr_or_name(INTEROP_A2DP_MAKE_REMOTE_AS_MASTER,&remote_bd_addr)) {

      BTIF_STORAGE_FILL_PROPERTY(&prop_name, BT_PROPERTY_BDNAME,
          sizeof(bt_bdname_t), bdname.name);

      btif_storage_get_remote_device_property(const_cast<RawAddress *>(&remote_bd_addr),
          &prop_name);
    ...
    if (p->link_role == BTM_ROLE_SLAVE) {
      VLOG(1) << __func__ << "[INTEROP_A2DP_MAKE_REMOTE_AS_MASTER]Remote is already as Master, return";
      return(BTM_SUCCESS);
    }
  } else {
    /* Finished if already in desired role */
    if ((p->link_role == new_role) ||
        (interop_match_addr_or_name(INTEROP_DISABLE_ROLE_SWITCH, &remote_bd_addr)) ||

...
*
**
** Function         l2c_link_hci_disc_comp_debug
**
** Description      This function is called when an HCI Disconnect Complete
**                  event is received for conn debug dump.
**
** Returns          void
**
*******************************************************************************/
void l2c_link_hci_disc_comp_debug (uint16_t handle, uint8_t reason) {
  tL2C_LCB    *p_lcb;
  ...
  if (p_lcb) {
    /* Check for BLE and handle that differently */
    if (p_lcb->transport == BT_TRANSPORT_LE) {
      btif_debug_conn_state(p_lcb->remote_bd_addr, BTIF_DEBUG_LE_DISCONNECTED, reason);
    } else if (p_lcb->transport == BT_TRANSPORT_BR_EDR) {
      btif_debug_conn_state(p_lcb->remote_bd_addr, BTIF_DEBUG_CLASSICAL_DISCONNECTED, reason);
    }
  }
}

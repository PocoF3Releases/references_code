                                         ...
                                         audio_policy_forced_cfg_t config)
{
    ...
    if (mEngine->getPhoneState() == AUDIO_MODE_IN_CALL && hasPrimaryOutput()) {
        ...
        //waitMs = updateCallRouting(newDevices, delayMs);
          waitMs = updateCallRouting(false, delayMs);
    }
                                               ...
                                               bool force)
{
    ...
    outputDesc->setVolume(volumeDb, muted, volumeSource, curves.getStreamTypes(), deviceTypes, delayMs, force);
    ...
    */
    ...
    if (profile != 0) {
        ...
        String8 address = outputDevices.size() > 0 ? String8(outputDevices.itemAt(0)->address().data())
                : String8("");
        status = outputDesc->open(config, nullptr /* mixerConfig */, devices, stream, *flags, &output);
...
non_direct_output:
    ...
    ALOGD("getOutputForDevice() returns output %d", output);

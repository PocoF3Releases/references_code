...
int get_fb_width() {
    return g_draw.dmode.resolution.getWidth();
}
int get_fb_height() {
    return g_draw.dmode.resolution.getHeight();
}
...
static void draw_buttons(layout * lay, SkCanvas * skcan, draw_control_t * pdraw) {
    ...
    for(iter = lay->button_list.begin(); iter != lay->button_list.end(); iter++) {
        ...
        if(btn->get_visibility()) {
            ...
            /**Draw button rect*/
            srect.setXYWH(rect->x, rect->y, rect->w, rect->h);
            ...
            skcan->drawSimpleText(btn->get_text(), strlen(btn->get_text()), SkTextEncoding::kUTF8, point.x, point.y, font /*SkFont()*/, pdraw->paint);
        ...
        while(*p != '\0') {
            ...
            *ptr = '\0';
            ...
            if((last_text_y + text_h) < (rect->y + rect->h)) {
                skcan->drawSimpleText(tmp, strlen(tmp), SkTextEncoding::kUTF8, last_text_x, last_text_y, SkFont(), pdraw->paint);
            }
...
#endif
        skcan->drawSimpleText(text, strlen(text), SkTextEncoding::kUTF8, point.x, point.y, SkFont(), pdraw->paint);
    }

    /**Draw indicator*/
    srect.setXYWH(indicator_x, indicator_y, indicator_w, indicator_h);
    pdraw->paint.setColor(SK_ColorWHITE);
    skcan->drawRect(srect, pdraw->paint);
}

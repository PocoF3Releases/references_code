...
static int erase_all_files() {
    IResultType result = mFactoryClient->eraseAllFiles(path);
...
static int dir_list_filename_to_file(const char *filename) {
    ...
    IResultType result = mFactoryClient->dirListFileNameToFile(path, name);
    MMI_ALOGE("dir_list_filename_to_file, result=%d\n", result);
    int res = (int)result;
    return res;
}

PACKED void *ftm_app_status(void *req_pkt, uint16 pkt_len) {
...
PACKED void *ftm_select_sequence(void *req_pkt, uint16 pkt_len) {
...
PACKED void *ftm_clear_results(void *req_pkt, uint16 pkt_len) {
...
PACKED void *ftm_execute_single_test(void *req_pkt, uint16 pkt_len) {
    ftm_common_response *rsp = NULL;
...
PACKED void *ftm_execute_all_tests(void *req_pkt, uint16 pkt_len) {
    ftm_common_response *rsp = NULL;
...
PACKED void *ftm_read_file(void *req_pkt, uint16 pkt_len) {
    ...
    mFactoryClient->readFile(path, readReq, [&](auto& readResult){
        rsp->sftm_header.cmd_code = 75;
        rsp->sftm_header.sub_sys_id = DIAG_SUBSYS_FTM;
        rsp->sftm_header.sub_sys_cmd_code = FTM_AP_C;
...
*/
PACKED void *ftm_write_file(void *req_pkt, uint16 pkt_len) {
    ftm_common_response *rsp = NULL;
    ...
    IResultType result = mFactoryClient->writeFile(path,writeReq);
...
PACKED void *ftm_test_list_to_file(void *req_pkt, uint16 pkt_len) {
    ftm_common_response *rsp = NULL;
...
PACKED void *ftm_erase_all_files(void *req_pkt, uint16 pkt_len) {
    ftm_common_response *rsp = NULL;
...
PACKED void *ftm_dir_to_file(void *req_pkt, uint16 pkt_len) {
    ftm_common_response *rsp = NULL;
...
PACKED void *ftm_exit_test(void *req_pkt, uint16 pkt_len) {
    ftm_common_response *rsp = NULL;
...
PACKED void *ftm_append_to_res(void *req_pkt, uint16 pkt_len) {
    ftm_common_response *rsp = NULL;
...
PACKED void *ftm_utility(void *req_pkt, uint16 pkt_len) {
    ftm_common_response *rsp = NULL;
...
PACKED void *ftm_read_app_version(void *req_pkt, uint16 pkt_len) {
...
int diag_init(void) {
    ...
    mFactoryClient = IFactory::getService();
    if (mFactoryClient == NULL) {

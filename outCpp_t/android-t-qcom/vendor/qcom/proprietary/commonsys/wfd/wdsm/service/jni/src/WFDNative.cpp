...
{
    LOGD(" AR %f original xin = %lf yin = %lf screen AR = %lf",gResolution.aspectRatio,
         xin, yin, gScreenDimensions.aspectRatio);
        ...
        const jobject& wfdSessionObj, const jobject& eventDispatcherObj)
{
    ...
    case WFD_UIBC_TOUCH:
     {
        ...
        if(uibcParam->multiMode) {
            if(uibcParam->prevEv->getPointerCount() < ev->parms.touch_event.num_pointers) {
               ...
               fillerEv->initialize(InputEvent::nextId(),MOTION_EVENT_DEVICE_ID,
                            AINPUT_SOURCE_TOUCHSCREEN,
                            // TODO(b/112430608): ADISPLAY_ID_DEFAULT is almost
                            // certainly the wrong value to pass for the
                            // display ID.
                            ADISPLAY_ID_DEFAULT,{},
                            fillerAction,
                            ...
                            MotionClassification::NONE,ui::Transform(),
                            2.0f, 2.1f,
                            AMOTION_EVENT_INVALID_CURSOR_POSITION, AMOTION_EVENT_INVALID_CURSOR_POSITION,
                            ui::Transform(), (uibcParam->prevEv)->getDownTime(), fillerEvTime,
        ...
        if (ev->parms.touch_event.num_pointers > 0 && ev->parms.touch_event.num_pointers <= UIBC_MAX_TOUCH_INPUTS) {
            ...
            newEv->initialize(InputEvent::nextId(),MOTION_EVENT_DEVICE_ID, AINPUT_SOURCE_TOUCHSCREEN,
                              // TODO(b/112430608): ADISPLAY_ID_DEFAULT is almost
                              // certainly the wrong value to pass for the
                              // display ID.
                              ADISPLAY_ID_DEFAULT,{},
                              ...
                              2.0f, 2.1f,
                              AMOTION_EVENT_INVALID_CURSOR_POSITION, AMOTION_EVENT_INVALID_CURSOR_POSITION,
                              ui::Transform(), uibcParam->downTime, eventTime,
    ...
    case WFD_UIBC_KEY:
      {
        ...
        newEvent->initialize(InputEvent::nextId(),KEY_EVENT_DEVICE_ID, AINPUT_SOURCE_KEYBOARD,
                             // TODO(b/112430608): ADISPLAY_ID_DEFAULT is almost
                             // certainly the wrong value to pass for the
                             // display ID.
                             ADISPLAY_ID_DEFAULT,{},
    ...
    /* scroll event: unsupported for now */
    case WFD_UIBC_SCROLL:
    {
      unsigned int num_pixels_scroll;
      int scroll_type;
      scroll_type = ev->parms.scroll_event.type;
      LOGD("Received WFD_UIBC_SCROLL event, Scroll type = %d",scroll_type);
      num_pixels_scroll = ev->parms.scroll_event.num_pixels_scrolled;
      LOGD(" Num_Pixels_Scrolled = %d", num_pixels_scroll);
      break;
    }
    default: //do nothing
...
static bool sendHIDEvent(const uint8* const HIDPack, uint8_t len,
...
JNIEXPORT jint JNICALL Java_com_qualcomm_wfd_WFDSession_nativeInit
  (JNIEnv * env, jobject wfdSessionObj, jint isSource)
{

    bool isSink = isSource? false: true;


    WiFiDisplayClient *wfdClient = WiFiDisplayClient::create(isSink, &jni_stringarray_callback,
  ...
  (JNIEnv *env, jobject wfdSessionObj, jobject jobj, jlong nativePtr)
{
    ...
    if(client)
    {
        if(client->getResolution((&gResolution.width), &(gResolution.height)) !=0)
        {
 ...
 */
JNIEXPORT jobjectArray JNICALL Java_com_qualcomm_wfd_WFDSession_nativeGetConfigItems
  (JNIEnv * env, jobject wfdSessionObj, jlong nativePtr)
{
    UNUSED(wfdSessionObj);
    WiFiDisplayClient* client = reinterpret_cast<WiFiDisplayClient*>(nativePtr);
    if (client)
    {
        std::vector<std::string> cfgItems;
        int ret = client->getConfigItems(cfgItems);
        ...
        jobjectArray oarr = env->NewObjectArray(
            cfgItems.size(), env->FindClass("java/lang/String"), NULL);
        int i = 0;
        for (auto const& elem: cfgItems) {
            env->SetObjectArrayElement(oarr, i++, env->NewStringUTF(elem.c_str()));
        }
        return oarr;
    }
    return NULL;
  ...
  (JNIEnv *env, jobject wfdSessionObj, jintArray result, jlong nativePtr)
{
    ...
    int ret = client->getCommonResolution(&comRes,&numProf);
 ...
 * Signature: ([IJ)[J
 */
JNIEXPORT jlongArray JNICALL Java_com_qualcomm_wfd_WFDSession_nativeGetNegotiatedRes
  (JNIEnv *env, jobject wfdSessionObj, jintArray result, jlong nativePtr)
{
    UNUSED(wfdSessionObj);
    ...
    jlongArray retArr = nullptr;
    if (client)
    {
        std::vector<uint64_t> negRes;
        int ret = client->getNegotiatedResolutionBitmap(negRes);

        LOGD("Calling WiFiDisplayClient::getNegotiatedResolution returned %d", ret);

        if (ret == 0 && negRes.size() > 0)
        {
            retArr = env->NewLongArray(negRes.size());
            if (retArr)
            {
                env->SetLongArrayRegion(retArr,0,negRes.size(),(jlong*)&negRes[0]);
                nativeResult[0] = 0;
            }
            else
            {
                LOGE("Unable to alloacate memory from JNI heap!");
                env->ExceptionClear();
            }
        }
    }
...
void overlay_callback(JNIEnv *env, char strArray[][256], const jobject& wfdSessionObj){
    jlong size = 0;
    void* buf = NULL;
    sscanf(strArray[2], "%lld", &size);
    sscanf(strArray[3], "%p", &buf);
    if (size == 0 || buf == NULL) {
        LOGE("invalid buffer for overlay");
        return;
    }
    jobject byteBuffer = env->NewDirectByteBuffer(buf, size);
    if (byteBuffer == NULL) {
        LOGE("failed to allocate ByteBuffer");
        return;
    }
    jstring eventName = env->NewStringUTF("OVEvent");
    jclass objArrCls = env->FindClass("java/lang/Object");
    jobjectArray oarr = env->NewObjectArray(4, objArrCls, NULL);
    jstring strArr[3];
    for (int i = 0; i < 4; i++) {
        if (i == 3) {
            env->SetObjectArrayElement(oarr, i, byteBuffer);
        } else {
    ...
    char strArray[][256],const jobject& wfdSessionObj)
{
    ...
    if(!strcmp(eName, "OVEvent") && (numObjects == 4)
            && !strcmp(strArray[0], "BUFFER")) {
        overlay_callback(env, strArray, wfdSessionObj);
        if (isAttached)
            gJavaVM->DetachCurrentThread();
        return;
    }

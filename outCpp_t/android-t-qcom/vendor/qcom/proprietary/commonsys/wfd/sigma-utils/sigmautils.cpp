    ...
    if (s != NULL)
    {
        ...
        if (s->client != NULL && s->client->initCheck() == NO_ERROR)
        {
            ...
            if(primaryDisplay)
            {
                ...
                android::ui::DisplayMode mainDpyMode;
                status_t ret1 = SurfaceComposerClient::getActiveDisplayMode(m_mainDpy, &mainDpyMode);
                ...
                if (ret == NO_ERROR && ret1 == NO_ERROR) {
                  if (mainDpyState.orientation == ui::ROTATION_90
                     ...
                     displayHeight = mainDpyMode.resolution.width;
                     displayWidth = mainDpyMode.resolution.height;
                  } else {
                     displayWidth = mainDpyMode.resolution.width;
                     displayHeight = mainDpyMode.resolution.height;
                  }
                }
               if (displayWidth == 0)
                   displayWidth = 800;
...
int configureVirtualDisplay(const sp<IBinder>& m_mainDpy,const ui::DisplayState& mainDpyState,
    ...
    */
    ...
    if (!deviceRotated) {
        sourceWidth = mainDpyMode.resolution.width;
        sourceHeight = mainDpyMode.resolution.height;
    } else {
        sourceHeight = mainDpyMode.resolution.width;
        sourceWidth = mainDpyMode.resolution.height;
    }
    ...
    */
    ...
    t.setDisplayLayerStack(dpy, ui::DEFAULT_LAYER_STACK);    // default stack
    t.apply();
    *pDisplayHandle = dpy;

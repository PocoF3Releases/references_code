...
#pragma clang diagnostic pop
      ...
      void parseVideoFormats(string);
      void parseVideoCodecHeader(string);
      void parseVideoCodec(string,int);
      void parseExtVideoFormats(string);
      void parseExtVideoCodec(string);
      void limitCapabilites(uint16&, uint16&, int&);
      void limitCodecParams(uint8&,string);
      ...
      void parseDisplayEdidSupport(string);
      void parseCursorSupport(string);
      ...
      void updateVar(const char* key, const string& tagValue);
...
int mnHW_ID;
int mnHW_Ver;
std::unordered_map<string,string> cfgItemsStr;
...
void  parseUIBCSupport(string uibcBuffer)
{
   ...
   if(tagValue.length())
   {
     updateVar(UIBC_VALID_KEY,tagValue);
   }
   if(bValid)
   {
      ...
      if(tagValue.length())
      {
        updateVar(UIBC_M14_KEY,tagValue);
      }
      tagValue = getValueForTag(uibcBuffer,UIBC_M4_ENABLE_KEY);
      if(tagValue.length())
      {
        updateVar(UIBC_M4_ENABLE_KEY,tagValue);
      }
      if(bGeneric)
      {
...
/*==============================================================================
         ...
         FUNCTION:    parseDisplayEdidSupport

         DESCRIPTION:
         @brief       Parse XML for Display EDID support
*//**
...
void  parseDisplayEdidSupport(string edidBuffer)
{
   bool valid = false;
   valid = (atoi(edidBuffer.c_str()) == 1) ? true : false;
   pReadCfgCapabilities->pCfgCapabilities->display_edid_support = valid;
}

/*==============================================================================

         FUNCTION:    parseCursorSupport

         DESCRIPTION:
         @brief       Parse XML for HWcursor support
*//**
...
void  parseCursorSupport(string cursorBuffer)
{
   bool valid = false;
   valid = (atoi(cursorBuffer.c_str()) == 1) ? true : false;
   pReadCfgCapabilities->pCfgCapabilities->cursor_support = valid;
}
...
void  parseRIRSupport(string buffer)
{
    ...
    if(tagvalue.length())
    {
        updateVar(PERIODIC_IDR_INTERVAL_VALID_KEY,tagvalue);
    }
    tagvalue = getValueForTag(buffer,PERIODIC_IDR_INTERVAL_KEY);
    if(tagvalue.length())
    {
        updateVar(PERIODIC_IDR_INTERVAL_KEY,tagvalue);
    }
}
...
bool initAuxStreamInfo()
{
    ...
    {
        memset(pReadCfgCapabilities->pAuxCfgCapabilities,0,
                sizeof(*(pReadCfgCapabilities->pAuxCfgCapabilities)));
    }
    else
    {
        /*Initialize*/
        pReadCfgCapabilities->pAuxCfgCapabilities = (WFD_MM_Aux_capability_t *)
            MM_Calloc(1, sizeof(WFD_MM_Aux_capability_t));
...
/*==============================================================================
         FUNCTION:    limitCodecParams

         DESCRIPTION:
         @brief        Limit codec capabilities based on chipset
*//**
...
*//*
         PARAMETERS:
*//**       @param      - uMaxLevel - Reference to level support
...
@par     SIDE EFFECTS:
                           None
*//*==========================================================================*/
void limitCodecParams(uint8& uMaxLevel,string codecName)
{
    /* Video Driver now restricts level setting based on chipset capability */
    switch(mnHW_ID)
    {
        /*--1--*/
        case 417:
        ...
        case 562:
        {
            size_t found = codecName.find("264");
        ...
        case 591:
        {
            if(wfdDevice == SOURCE)
            {
                size_t found = codecName.find("265");
                if(found != string::npos)
                {
                    if(uMaxLevel > 3)
                        uMaxLevel = 3;
...
/*==============================================================================
         FUNCTION:    limitCapabilites

         DESCRIPTION:
         @brief        Limit capabilities for source and sink based on chipset
*//**
...
*//*
         PARAMETERS:
*//**       @param      - uMaxHRes - Reference to horizontal resolution support
                        - uMaxVRes - Reference to vertical resolution support
...
@par     SIDE EFFECTS:
                           None
*//*==========================================================================*/
void limitCapabilites(uint16& uMaxHRes, uint16& uMaxVRes, int& fpsSupported)
{
    /* Check if the wlan is capable of supporting 4k */
    if((uMaxVRes > 1080) || (uMaxHRes > 1920))
    {
    ...
    /* Put cap on source capabilities based on chipset support*/
    if(wfdDevice == SOURCE)
    {
        switch(mnHW_ID)
        {
            ...
            case 591:
                 if(mnHW_Ver == 0)
                {
                    //Higher Spec is limited to UHDp30
                    if(uMaxHRes >= 3840 || uMaxVRes >= 2160)
                    {
                        uMaxHRes = 3840;
                        uMaxVRes = 2160;
                        if(fpsSupported > 30)
                ...
                {
                    //Lower Spec is limited to 1080p30
                    if(uMaxHRes >= 1920 || uMaxVRes >= 1080)
                    {
                        uMaxHRes = 1920;
                        uMaxVRes = 1080;
                        if(fpsSupported > 30)
            ...
            case 565:
                if(mnHW_Ver == 0)
                {
                    break; // Higher Spec hence not limiting [defaults UHD].
                }
                // No SKU's defined/Lower spec [1080p]
                if(uMaxHRes >= 1920 || uMaxVRes >= 1080)
                {
                    uMaxHRes = 1920;
                    uMaxVRes = 1080;
            ...
            case 459:
                if(mnHW_Ver == 0)
                {
                    //Higher Spec is limited to 1080p30
                    if(uMaxHRes >= 1920 || uMaxVRes >= 1080)
                    {
                        uMaxHRes = 1920;
                        uMaxVRes = 1080;
                ...
                {
                    //Lower Spec is limited to 720p30
                    if(uMaxHRes >= 1280 || uMaxVRes >= 720)
                    {
                        uMaxHRes = 1280;
                        uMaxVRes = 720;
                        if(fpsSupported > 30)
            ...
            case 561:
            case 562:
                //No Difference in different SKUs
                if(uMaxHRes >= 1920 || uMaxVRes >= 1080)
                {
                    uMaxHRes = 1920;
                    uMaxVRes = 1080;
    ...
    else /* Put cap on sink capabilities based on chipset support */
    {

        /* Put cap on max sink RESOLUTION SUPPORT based on chipset support */
        switch(mnHW_ID)
        {
            /*--1--*/
            case 366:
            case 365:
                if(mnHW_Ver == 0)
                {
                    break;//not limiting for higher spec
                }
            /*--2--*/
            case 336:
            case 337:
        ...
        /* Put cap on max sink FPS SUPPORT based on chipset support */
        switch(mnHW_ID)
        {
            /*--1--*/
            case 475:
            ...
            case 440:
            case 400:
                if(uMaxHRes >= 4096 || uMaxVRes >= 2160)
                {
                    if(fpsSupported > 30)
                    {
                        fpsSupported = 30;
    ...
    MM_MSG_PRIO3(MM_GENERAL,MM_PRIO_HIGH,
        "Update resolution to %d x %d  %d FPS",uMaxHRes,uMaxVRes, fpsSupported);
}
...
void parseVideoCodec(string videoBuffer, int index)
{
   ...
   uint8 uMaxProfile = (uint8)atoi((getValueForTag(videoBuffer, "Profile")).c_str());
   uint8 uMaxLevel = (uint8)atoi((getValueForTag(videoBuffer, "Level")).c_str());

    limitCodecParams(uMaxLevel,codecName);
   ...
   MM_MSG_PRIO4(MM_GENERAL,MM_PRIO_HIGH,"user enterted resolution for codecNum = %d [%d x %d %d fps]",index,uMaxHRes,uMaxVRes,fpsSupported);
    ...
    limitCapabilites(uMaxHRes, uMaxVRes, fpsSupported);
...
/*==============================================================================
         ...
         FUNCTION:    parseVideoCodecHeader
...
*//*==========================================================================*/
void parseVideoCodecHeader(string buffer)
{
...
*//*==========================================================================*/
void parseVideoFormats(string buffer)
{
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"parseVideoFormats");

    string tagValue;
    /*Parse VideoCodecHeader*/
    tagValue = getValueForTag(buffer,"VideoCodecHeader");
    if(tagValue.length())
        parseVideoCodecHeader(tagValue);
    int nNumCodecs = 0;
    ...
    if(pReadCfgCapabilities->numAVFormatsExtention > 0)
    {
        ...
        for(int index = 1; index <= pReadCfgCapabilities->numAVFormatsExtention; index++)
        {
            ...
            MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_LOW,
                "parseVideoFormats: Parsing codec Tag = %s",aCodecTag);

            tagValue = getValueForTag(buffer,aCodecTag);
            if(tagValue.length())
            {
                parseVideoCodec(tagValue,index-1);
                nNumCodecs++;
            }
        }
...
/*==============================================================================
         ...
         FUNCTION:    parseExtVideoCodec

         DESCRIPTION:
         @brief        Parse XML for extended Video codec
*//**
...
void parseExtVideoCodec(std::string buffer)
{
     pReadCfgCapabilities->pCfgCapabilities->ext_video_capability.codecList.emplace_back
      (
        getValueForTag(          buffer, "CodecName"),
        std::stoi(getValueForTag(buffer, "Profile")),
        std::stoi(getValueForTag(buffer, "Level")),
        std::stoi(getValueForTag(buffer, "Latency")),
        std::stoi(getValueForTag(buffer, "FrameRateControlSupp")),
        std::stoi(getValueForTag(buffer, "MinimumSliceSize")),
        std::stoi(getValueForTag(buffer, "SliceEncodingParams")),
        std::stoi(getValueForTag(buffer, "Width")),
        std::stoi(getValueForTag(buffer, "Height")),
        std::stoi(getValueForTag(buffer, "Fps"))
      );
}

/*==============================================================================

         FUNCTION:    parseExtVideoFormats

         DESCRIPTION:
         @brief        Parse XML for extended Video formats
*//**
...
*//*==========================================================================*/
void parseExtVideoFormats(std::string buffer)
{
    MM_MSG_PRIO(MM_GENERAL, MM_PRIO_HIGH,"parseExtVideoFormats");
    ...
    do
    {
        std::string codecTag = "ExtVideoCodec" + std::to_string(numCodecs);
        MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_LOW,
            "parseVideoFormats: Parsing codec Tag = %s", codecTag.c_str());

        tagValue = getValueForTag(buffer,codecTag);
        if(tagValue.length())
        {
            parseExtVideoCodec(tagValue);
            numCodecs++;
        }
                         ...
                         && ( ( channels%2 ) == 0 ) ) )
         {
            switch(channels)
            {
             ...
             case AUDIO_4_CHANNELS:
                  pReadCfgCapabilities->pCfgCapabilities->audio_config.aac_codec.supported_modes_bitmap = (AAC_48_16_2 | AAC_48_16_4);
             break;
             case AUDIO_6_CHANNELS:
                  pReadCfgCapabilities->pCfgCapabilities->audio_config.aac_codec.supported_modes_bitmap = (AAC_48_16_2 | AAC_48_16_4 | AAC_48_16_6);
             break;
             case AUDIO_MAX_CHANNELS:
                  pReadCfgCapabilities->pCfgCapabilities->audio_config.aac_codec.supported_modes_bitmap = (AAC_48_16_2 | AAC_48_16_4 | AAC_48_16_6 | AAC_48_16_8);
             break;
            }
...
void parseHWVer()
{
    ...
    char targetVariant[PROPERTY_VALUE_MAX] = {0};

    if(GetContext() == OSAL_VND_CTX) {
    ...
    if(GetContext() == OSAL_VND_CTX) {
        property_get("vendor.media.target_variant", targetVariant, "default");
    } else {
        property_get("vendor.sys.media.target_variant", targetVariant, "default");
    }
    ...
    switch(mnHW_ID)
    {
        ...
        case 366:
            ...
            else if(!(strncmp(value, "2", PROPERTY_VALUE_MAX)))
            {
                mnHW_Ver = 1; //Lower Spec
            }
            break;
        ...
        case 435:
        case 459:
            if(!(strncmp(value, "2", PROPERTY_VALUE_MAX)))
            {
                mnHW_Ver = 0; //Higher Spec
            }
            else if(!(strncmp(value, "3", PROPERTY_VALUE_MAX)))
            {
                mnHW_Ver = 1; //Lower Spec
            }
            break;
        case 450:
            if(!(strncmp(targetVariant, "_shima_v2", PROPERTY_VALUE_MAX)))
            {
                mnHW_Ver = 1; //Lower Spec
            }
            else
            {
                //For all other target variant, consider higher spec
                mnHW_Ver = 0; //Higher Spec
            }
            break;
        case 591:
            if(!(strncmp(targetVariant, "_ukee", PROPERTY_VALUE_MAX)))
            {
                mnHW_Ver = 0; //Higher Spec
            }
            else
            {
                //For all other target variant, consider lower spec
                mnHW_Ver = 1; //Lower Spec
            }
            break;
        case 475:
        ...
        case 499:
        case 515:
            if(!(strncmp(targetVariant, "_yupik_v1", PROPERTY_VALUE_MAX)))
            {
                mnHW_Ver = 1; //Lower Spec
            }
            else
            ...
            break;
        case 537:
           if(!(strncmp(targetVariant, "_parrot_v0", PROPERTY_VALUE_MAX)))
           {
               mnHW_Ver = 0; //Higher Spec
           }
           else
        ...
        case 547:
        case 506:
           if(!(strncmp(targetVariant, "_diwali_v2", PROPERTY_VALUE_MAX)))
           {
               mnHW_Ver = 1; //Lower Spec
           }
           else
           {
               //For all other target variant, consider higher spec
               //Since no difference in capabilities
               mnHW_Ver = 0; //Higher Spec
           }
           break;
        default:
...
void  parseXml(string cfgBuffer)
{
   ...
   /*Parse WFD AV Formats*/
   tagValue = getValueForTag(cfgBuffer, "VideoFormats");
   if(tagValue.length())
   {
      parseVideoFormats(tagValue);
   }
   else
   {
   ...
   /*Parse Extended WFD Video Formats*/
   tagValue = getValueForTag(cfgBuffer, "ExtVideoFormats");
   if(tagValue.length())
   {
      parseExtVideoFormats(tagValue);
   }
   ...
   tagValue = getValueForTag(cfgBuffer, "DisplayEdidSupport");
   if (tagValue.length())
   {
      parseDisplayEdidSupport(tagValue);
   }

   tagValue = getValueForTag(cfgBuffer, "CursorSupport");
   if (tagValue.length())
   {
      parseCursorSupport(tagValue);
   }
...
//Parse for configurable items here itself in order to cache for later use
   ...
   {
      {
         ...
         if(str.length() > 0)
         {
            ...
            updateVar(RTCP_RR_NOTIFICATION_ENABLED_KEY, str);
         }
      }

      {
         ...
         if(str.length() > 0)
         {
            ...
            updateVar(RTCP_RR_ENABLED_KEY, str);
         }
      }

      {
         ...
         if(str.length() > 0)
         {
            ...
            updateVar(RTCP_RR_INTERVAL_MS_KEY, str);
         }
      }
   }
   ...
   if(tagValue.length())
   {
     updateVar(CURSOR_ENABLE_KEY,tagValue);
   }

   tagValue = getValueForTag(cfgBuffer, VIDEO_CUSTOM_SIZE_MODE_KEY);
   if(tagValue.length())
   {
     updateVar(VIDEO_CUSTOM_SIZE_MODE_KEY, tagValue);
   }

   tagValue = getValueForTag(cfgBuffer, EXT_VIDEO_SUPPORT_KEY);
   if(tagValue.length())
   {
     updateVar(EXT_VIDEO_SUPPORT_KEY, tagValue);
   }
...
void updateVar(const char* key, const string& keyValue)
{
  if(keyValue.length() && key)
  {
    auto ret = cfgItemsStr.insert(std::make_pair(key, keyValue));
    if(ret.second == false)
    {
        MM_MSG_PRIO2(MM_GENERAL,MM_PRIO_HIGH,"updateVar:: Element %s already exists with %s", (ret.first)->first.c_str(), (ret.first)->second.c_str());
    }
    else
    {
        MM_MSG_PRIO2(MM_GENERAL,MM_PRIO_HIGH,"updateVar:: Element %s newly inserted with %s", (ret.first)->first.c_str(), (ret.first)->second.c_str());
    }
  }
  else
  {
...
/*=========================================================
   ...
   FUNCTION:    isWiFiDisplaySupported

   DESCRIPTION:
   @brief     Used to check if target supports WiFi Display feature
...
bool isWiFiDisplaySupported()
{
    ...
    /* HW_ID, HW_Ver & arch parameters can be used in future to disable
       support for WiFi Display from here */
...
void resetCfgItems()
{
    cfgItemsStr.clear();
    wfdDevice = SOURCE;
    MM_MSG_PRIO1(MM_GENERAL,MM_PRIO_LOW,"Resetting configurable itemss cfgItemsStr to %zd", cfgItemsStr.size());
}
...
/*=========================================================
   ...
   DESCRIPTION:
   @brief     Used to get the values of configurable
                 item of config file
...
{
    for(auto const& elem: cfgItemsStr){
        configItems.push_back(elem.first);
        configItems.push_back(elem.second);
    }
    return 0;
}
#endif

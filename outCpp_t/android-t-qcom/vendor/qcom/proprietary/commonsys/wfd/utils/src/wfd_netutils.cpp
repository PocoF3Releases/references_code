...
int getPeerMacAddress( char* pMacAddr, char* pPeerIpAddr, int size, NetType netType )
{
    ...
    while( fgets( line, sizeof(line), arpFP ) )
    {
        ...
        if( strncmp( hwa, "00:00:00:00:00:00", 18  ) == 0 ||
             ...
             * Found!
             */
        if( strncmp( ip, pPeerIpAddr, strlen(pPeerIpAddr) ) == 0 )
        {
...
uint64_t getLinkSpeed(char *macaddr, unsigned int len, bool& queryIfName, NetType netType) {
    ...
    MM_MSG_PRIO3(MM_GENERAL, MM_PRIO_MEDIUM,
                 "NetUtils:Macaddress = %s Linkspeed result = %d, linkspeed = \
                  %llu", inputstring, result, (linkspeed * (uint64_t)500000));

    close(nSocket);

    return (linkspeed * (uint64_t)500000);
}
...
int response_handler(struct nl_msg *msg, void *arg)
{
    ...
    struct nla_policy policy[QCA_WLAN_VENDOR_ATTR_LINK_PROPERTIES_MAX + 1];
                        ...
                        unsigned int *enable_4k)
{
    ...
    if (bOk)
    {
        ...
        /* 1x1 hardware can also support b/w requirements for 4K.
           Hence relaxing check for NSS from 2x2 to 1x1 */
        if((sinfo.nss >= 1) && (sinfo.freq > 5000)) /* Review the condition for enabling 4k

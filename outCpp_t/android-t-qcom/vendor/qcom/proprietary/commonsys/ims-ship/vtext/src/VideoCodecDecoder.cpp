...
#define MEDIA_KEY_EXTRADATA_CONCEALMB (char*)"vendor.qti-ext-dec-info-mbi.numMbs\0"
...
QPE_VIDEO_ERROR VideoCodecDecoder::DeInit()
{
  ...
  CRITICAL5("PlayerStateTransition:%d DeInit Called: m_ePlayerState %d, m_format %p, mCodec %p",
            iPlayerState, m_ePlayerState, m_format, mCodec);
                                        ...
                                        QP_VIDEO_CONFIG *pCodecConfig)
{
  ...
  CRITICAL3("PlayerStateTransition:%d Init Done: m_ePlayerState %d",
            iPlayerState, m_ePlayerState);
  memset((void *)m_frameStateSet, 0,
         (sizeof(FrameStateMap)*FRAME_STATE_SET_SIZE));
  mTransform = 0;
  m_PlayerThreadCreated = 0;
  m_frameStateIndex = 0;
  m_height = 0;
  m_width = 0;
  m_preDecoderTstamp = 0;
  m_preRTPTstamp = 0;
  m_uNumFramesRendered = 0;
  return VIDEO_ERROR_OK;
}
...
QPE_VIDEO_ERROR VideoCodecDecoder::Configure(QP_VIDEO_CONFIG tCodecConfig)
{
  ...
  QP_MULTIMEDIA_FRAME OobSpsPpsData;
  CRITICAL5("Configure: m_ePlayerState %d, m_format %p, mCodec %p, iPlayerState %d",
            m_ePlayerState, m_format, mCodec, iPlayerState);
  ...
  if (m_format != NULL) {
    CRITICAL1("Configure: Decoder MediaFormat is clearedup");
    AMediaFormat_delete(m_format);
    m_format = NULL;
  }

  if (mCodec != NULL) {
    CRITICAL1("Configure: Decoder MediaCodec is clearedup");
  ...
  AMediaFormat_setInt32(m_format, MEDIA_KEY_EXTRADATA_CONCEALMB,
                        0); //register for concealmb
  ...
  if (eVideoStateActive == prevState) {
    ...
    if (AMEDIA_OK != mstatus) {
      ...
      /*feed the OOB SPS|PPS to decoder after reconfiguration */
      if ( m_pRxNalInfo && (m_pRxNalInfo->nalLength > 0) ) {
        memset((void *) &OobSpsPpsData, 0, sizeof(OobSpsPpsData));
        OobSpsPpsData.cvoInfo = 0;
        OobSpsPpsData.iDataLen = m_pRxNalInfo->nalLength;
        OobSpsPpsData.iMaxBuffLen = m_pRxNalInfo->nalLength;
        OobSpsPpsData.ullFrameTimeUs = getCurrentSystemTime(NULL) * 1000;
        OobSpsPpsData.pData = m_pRxNalInfo->rawnal;
        CRITICAL3("PlayerStateTransition:%d: Feed OOB SPSPPS to decoder with len %d",
                  iPlayerState, OobSpsPpsData.iDataLen);
        VideoPlayFrame(&OobSpsPpsData, 0);
      }
...
QPE_VIDEO_ERROR VideoCodecDecoder::Start()
{
  ...
  do {
    ...
    /* Feed the oob sps|pps to the decoder */
    if ( m_pRxNalInfo && (m_pRxNalInfo->nalLength > 0) ) {
      memset((void *) &OobSpsPpsData, 0, sizeof(OobSpsPpsData));
      OobSpsPpsData.cvoInfo = 0;
      OobSpsPpsData.iDataLen = m_pRxNalInfo->nalLength;
      ...
      OobSpsPpsData.ullFrameTimeUs = getCurrentSystemTime(NULL) * 1000;
      OobSpsPpsData.pData = m_pRxNalInfo->rawnal;
      CRITICAL3("PlayerStateTransition:%d: Feed the OOB SPSPPS to decoder with len %d",
                iPlayerState, OobSpsPpsData.iDataLen);
      VideoPlayFrame(&OobSpsPpsData, 0);
    }
  } while (0);
...
void VideoCodecDecoder::videoPlayFrameLoop()
{
  ...
  while (eVideoStateActive == iPlayerState) {
    ...
    /*Reset ulDataVal to avoid it affecting next (complete) frame.*/
    ulDataVal = 0;
    ...
    while (status == AMEDIACODEC_INFO_OUTPUT_FORMAT_CHANGED) {
      ...
      if (m_height != iHeight || m_width != iWidth) {
        ...
        /*should not reset transform which is used for CVO change only but not resolution change*/
        /* Reset tranform variable so that rendering surface can be updated after
           decoder resolution update */
        /*mTransform = 0;*/
      }

      AMediaFormat_getInt32(format, MEDIA_KEY_EXTRADATA_CONCEALMB, &concealmb);

      if (0 != concealmb) {
        CRITICAL2("VideoPlayFrame: pExtraData conceal mb %d", concealmb);
        ulDataVal = concealmb;
      }
    ...
    if (status >= 0) {
      ...
      CRITICAL2("VideoPlayFrame: unexpected info code: %ld", (signed long)status);
      SendEventsToThinclient(VIDEO_MSG_ERROR);
    ...
    int belongs_to_drop_set)
{
  ...
  MINIMAL6_STR("VideoPlayFrame: input buffer %ld timestamp: = [%lu][%lu], dropset %d, size %d",
               (signed long)bufidx, Hi64(pFrame->ullFrameTimeUs),
               Low64(pFrame->ullFrameTimeUs), belongs_to_drop_set, pFrame->iDataLen);
  ...
  if (bufidx >= 0) {
    ...
    ERROR2("VideoPlayFrame: input buffer index is invalid : %ld",
           (signed long)bufidx);
    ...
    long long int /*app_tstamp*/)
{
  ...
  uint64_t system_time_decode_video_frame = 0;

  if ((pFrameData->ullFrameTimeUs != 0) && (ullCurSysTime != 0)
      && (pFrameData->ullSysTime != 0) ) {
    system_time_decode_video_frame = (uint64_t)
                                     (pFrameData->ullFrameTimeUs / 1000 - (ullCurSysTime - pFrameData->ullSysTime));
  } else {
    MINIMAL4_STR("Error in timestamps decode times %llu %llu %llu",
                 (unsigned long long)ullCurSysTime,
                 (unsigned long long)pFrameData->ullSysTime,
  ...
  DEBUG6_STR("AddToFrameStateMap seqno %d frameStateIndex %d source_time_video_frame %u, av_tstamp_in_us = [%lu][%lu]",
             ...
             pRxInfo->source_time_video_frame,
             Hi64(m_frameStateSet[nIdx].av_tstamp_in_us),
             Low64(m_frameStateSet[nIdx].av_tstamp_in_us));

  /* feed the frame to decoder with proper source time interval */
  if ((pRxInfo->source_rtp_timestamp > m_preRTPTstamp) &&
      ((pRxInfo->source_rtp_timestamp - m_preRTPTstamp) < 90000)) {
    /* find the difference between previous and current rtp timestamps
       and add that to previous decoder time value */
    m_frameStateSet[nIdx].decoder_tstamp = m_preDecoderTstamp +
                                           ((pRxInfo->source_rtp_timestamp - m_preRTPTstamp) / 90) * 1000;
  } else {
    /* cases where there is jump in rtp timestamps due to ssrc or any other usecase,
       we dont have to jump decoder time as decoder is not dependent on
       the rtp timestamp jump values */
    m_frameStateSet[nIdx].decoder_tstamp = m_preDecoderTstamp + 33000;
  }

  m_preDecoderTstamp = m_frameStateSet[nIdx].decoder_tstamp;
  m_preRTPTstamp = pRxInfo->source_rtp_timestamp;
  /* replacing the original video timestamp with decoder time */
  pFrameData->ullFrameTimeUs = m_preDecoderTstamp;
  DEBUG3_STR("AddToFrameStateMap feeding to decoder at [%lu][%lu] in microsecond",
             Hi64(pFrameData->ullFrameTimeUs),
             Low64(pFrameData->ullFrameTimeUs));
                                      ...
                                      int8_t& retreive_index, QP_VIDEO_PLAYOUT_INFO *playoutInfo)
{
  for (int i = 0; i < FRAME_STATE_SET_SIZE; i++) {
    if ((m_frameStateSet[i].decoder_tstamp == (uint64_t)av_tstamp) ||
        ((m_frameStateSet[i].decoder_tstamp / 1000) == (uint64_t)(
           av_tstamp / 1000))) {
      DEBUG6_STR(
        "DropFrameCheck belongs_to_drop_set %d  frameStateIndex %d source_time_video_frame %llu, av_tstamp_in_us = [%lu][%lu]",
        m_frameStateSet[i].belongs_to_drop_set, i,
        (unsigned long long)m_frameStateSet[i].source_time_video_frame,
        Hi64(m_frameStateSet[i].av_tstamp_in_us),
        Low64(m_frameStateSet[i].av_tstamp_in_us));

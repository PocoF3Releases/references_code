...
#endif
/*====*====*====*====*====*====*====*====*====*====*====*====*====*====*====*
# Copyright (c) 2014, 2016, 2018-2022, Qualcomm Technologies, Inc.
# All Rights Reserved.
...
#define DCI_MAX_LOG_EQUIP_ID 16
#define DCI_MAX_ITEMS_PER_LOG_CODE 512
#define DCI_MAX_EVENT_ITEMS 512

struct dci_log_equip {
	uint8 equip_id;
	uint8 dirty;
	uint8 items[DCI_MAX_ITEMS_PER_LOG_CODE];
} __packed;

struct dci_log_mask {
	struct dci_log_equip equips[DCI_MAX_LOG_EQUIP_ID];
} __packed;

struct dci_event_mask {
	uint8 mask[DCI_MAX_EVENT_ITEMS];
} __packed;
 ...
 * @handle: the handle returned back to the client
 ...
 * @requested_logs: caches the clients log requests
 * @requested_events: caches the clients event requests
 */
struct diag_dci_client_tbl {
	...
	struct dci_log_mask requested_logs;
	struct dci_event_mask requested_events;
};

...
void SystemResource::serviceDied(uint64_t cookie __unused,
             const android::wp<IBase>& who __unused){
  ALOGE("%s: trustedui killed.. releasing surface ",__func__);
  for (auto const& surface : surfaceMap) {
    if (!SystemResource::releaseSurface(surface.first)) break;
  }
  if(mTUIIntf != nullptr) {
    mTUIIntf->unlinkToDeath(this);
    mTUIIntf = nullptr;
  }
}
...
Return<void> SystemResource::acquireSurface(uint32_t width, uint32_t height,
  ...
  /* Register to trustedui hal service and death notification. This
     notification in turn allows systemhelper service to cleanup the surface */
  /* TBD: This functionality can also be achieved by having a signal handler
          which calls cleanup in trustedui listener */
  if(mTUIIntf == nullptr) {
     mTUIIntf = ITrustedUI::getService();
     mTUIIntf->linkToDeath(this, 0);
  }
  result = 0;

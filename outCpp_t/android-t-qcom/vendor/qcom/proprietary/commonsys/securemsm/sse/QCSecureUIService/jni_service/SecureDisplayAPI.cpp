/*
 * Copyright (c) 2012-2013, 2015, 2017-2019, 2021-2022 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
int32_t SecDisp::allocateNativeBuffers(uint32_t buffersForHeap) {
    ...
    if (mConfig.usageFlags & BufferUsage::PROTECTED) {
    ...
    if (minUndequeuedBufs > MAX_DEQUEUE_COUNT) {
        LOGE("NATIVE_WINDOW_MIN_UNDEQUEUED_BUFFERS count is greater"
             "than maximum supported buffers for SUI use case");
        return FAILURE;
    }

    numAllocatedBufs = numRequestedBufs + minUndequeuedBufs;
    if(numAllocatedBufs > MAX_DEQUEUE_COUNT)
    {
        numAllocatedBufs = MAX_DEQUEUE_COUNT;
    }
...
int32_t sec_ui_get_disp_properties(uint32_t u32Height, uint32_t u32Width) {
    ...
    // update the globals
    g_pSd->mDisplayHeight = u32Height;
    g_pSd->mDisplayWidth = u32Width;
    ret = SUCCESS;
...
int32_t sec_ui_start_disp(bool bDisableOverlay, uint32_t colorFormat,
    ...
    config.usageFlags = (uint32_t)BufferUsage::PROTECTED;
    config.usageFlags |= GRALLOC_USAGE_PRIVATE_UNCACHED;
...
static android::hardware::hidl_handle wrapFd(int fd) {
    if (fd >= 0) {
        ...
        if (NULL != handle) {
            ...
            /* as we are doing dup while passing the handle in dequeue so fd ownership
               should not be transfered*/
            hidl_handle.setTo(handle, false /*do not take ownership*/);
            return hidl_handle;
        }
    }
    return {};

...
#define is_cmd(str) ((strlen(str) == strlen(cmd)) &&\
...
void do_bap_connect_converged (char *p);
void do_bap_connect_and_start(char *p);
...
void do_bap_update_in_starting(char *p);
...
const t_cmd console_cmd_list[] =
{
    ...
    { "bap_connect", do_bap_connect,
"cmd :: bap_connect <CodecConfig> <AudioConfig> <BdAddr> <profile> <direction> <context>", 0 },
    { "bap_connect_converged", do_bap_connect_converged, "cmd ::\
bap_connect_converged <CodecConfig> <AudioConfig> <BdAddr> <profile> <direction> <context>", 0 },
    { "bap_connect_and_start", do_bap_connect_and_start, "cmd ::\
bap_connect_and_start <CodecConfig> <AudioConfig> <BdAddr> <profile> <direction> <context>", 0 },
    { "bap_disconnect", do_bap_disconnect,
"cmd :: bap_disconnect <BdAddr> <profile> <direction> <context>", 0 },
    ...
    { "bap_disc_in_connecting", do_bap_disc_in_connecting, "cmd ::\
bap_disc_in_connecting <CodecConfig> <AudioConfig> <BdAddr> <profile> <direction> <context>", 0 },
    { "bap_disc_in_starting", do_bap_disc_in_starting,
"cmd :: bap_disc_in_starting <BdAddr> <profile> <direction> <context>", 0 },
    { "bap_disc_in_stopping", do_bap_disc_in_stopping,
"cmd :: bap_disc_in_stopping <BdAddr> <profile> <direction> <context>", 0 },
    { "bap_stop_in_starting", do_bap_stop_in_starting,
"cmd :: bap_stop_in_starting <BdAddr> <profile> <direction> <context>", 0 },
    { "bap_update_in_starting", do_bap_update_in_starting,
"cmd :: do_bap_update_in_starting <BdAddr> <profile> <direction> <context>", 0 },
    { "bap_update_stream", do_bap_update_stream,
"cmd :: bap_update_stream <BdAddr> <profile> <direction> <new context>", 0 },
    /* last entry */
    {NULL, NULL, "", 0},
};
...
remote_bd_addr, bt_bond_state_t state, int fail_reason)
{
    g_PairState = state;
}

static void acl_state_changed(bt_status_t status, RawAddress* remote_bd_addr,
bt_acl_state_t state, int transport_link_type, bt_hci_error_code_t hci_reason)
{
...
void do_pacs_discovery(char *p)
{
  ...
  sPacsClientInterface->Connect(pacs_client_id, pac_bd_addr, true);
  while(!pacsConnectionComplete) sleep(1);
  printf("%s going for discovery press Enter to continue \n", __func__);
  std::cin >> del;
...
void do_ascs_discovery(char *p)
{
  ...
  sleep(1);
  printf("%s going for connect\n", __func__);
  sAscsClientInterface->Connect(ascs_client_id, ascs_bd_addr, true);
...
std::map<std::string, AudioConfigSettings> audioConfigMap = {
 ...
 {"3_1", {1, 1, {{ASE_DIRECTION_SRC|ASE_DIRECTION_SINK, 0}}}},
 // EB Call Mono Bi-Dir CIS
 {"4_1", {1, 1, {{ASE_DIRECTION_SINK, ASE_SINK_STEREO}}}},
 // Stereo Headset stereo streaming
 {"5_1", {1, 1, {{ASE_DIRECTION_SRC|ASE_DIRECTION_SINK, ASE_SINK_STEREO}}}},
 // EB Call with speaker stereo  mono mic
 {"6_1", {1, 2, {{ASE_DIRECTION_SINK, 0}, {ASE_DIRECTION_SINK, 0}}}},
 // TWM Streaming
 {"6_2", {2, 2, {{ASE_DIRECTION_SINK, 0}, {ASE_DIRECTION_SINK, 0}}}},
 // EBP Streaming same as 1_1
 {"7_1", {1, 2, {{ASE_DIRECTION_SINK, 0}, {ASE_DIRECTION_SRC, 0}}}},
  // EB Call with dual CIS ( same as 3_1)
 {"7_2", {2, 2, {{ASE_DIRECTION_SINK, 0}, {ASE_DIRECTION_SRC, 0}}}},
  // EBP Call with speaker on EB1 and mic on EB2
 {"8_1", {1, 2, {{ASE_DIRECTION_SINK, 0}, {ASE_DIRECTION_SRC|ASE_DIRECTION_SINK, 0}}}},
 // Headset Call with single mic
 {"8_2", {2, 2, {{ASE_DIRECTION_SRC|ASE_DIRECTION_SINK, 0}, {ASE_DIRECTION_SINK, 0}}}},
 // EBP Call with mic from one EB
...
{"11_1", {1, 2, {{ASE_DIRECTION_SRC|ASE_DIRECTION_SINK, 0},
 {ASE_DIRECTION_SRC|ASE_DIRECTION_SINK, 0}}}}, // TWM Call
{"11_2", {2, 2, {{ASE_DIRECTION_SRC|ASE_DIRECTION_SINK, 0},
 {ASE_DIRECTION_SRC|ASE_DIRECTION_SINK, 0}}}}}; // EBP Call
...
{"32_1_2", 75},{"48_1_2", 75},{"48_3_2", 75},{"48_5_2", 75},
                           ...
                           uint8_t server_count, uint8_t total_servers)
{
    ...
    for (uint8_t i = 0; i < acs->num_cises; i++) {
        ...
        memset(&ascs_config, 0, sizeof(ascs_config));
        int max_sdu_m_to_s;
        int max_sdu_s_to_m;
        cis_config.cis_id = i;
        ...
        printf("server count %d total_servers :%d \n", server_count, total_servers);
        if(total_servers == 1) {
          if(acs->num_cises == 1) {
            codec_qos_config->qos_config.ascs_configs.push_back(ascs_config);
            printf("Filling ASCS cfg for CIS Id %d \n", ascs_config.cis_id);
            printf("Populate in Condition num_cises = 1 \n");
          } else if(acs->num_cises == 2) {
            if(acs->audio_type[i % acs->num_cises].audio_dir ==
              acs->audio_type[(i + 1) % acs->num_cises].audio_dir) {
              codec_qos_config->qos_config.ascs_configs.push_back(ascs_config);
              printf("Filling ASCS cfg for CIS Id %d \n", ascs_config.cis_id);
              printf("Populate in Condition mod dir equal \n");
            } else if( i == server_count) {
              codec_qos_config->qos_config.ascs_configs.push_back(ascs_config);
              printf("Filling ASCS cfg for CIS Id %d \n", ascs_config.cis_id);
              printf("Populate in Condition i == server_count \n");
            } else if( acs->audio_type[i].audio_dir == (ASE_DIRECTION_SINK|ASE_DIRECTION_SRC)) {
              codec_qos_config->qos_config.ascs_configs.push_back(ascs_config);
              printf("Filling ASCS cfg for CIS Id %d \n", ascs_config.cis_id);
              printf("Populate in Condition dual dir support \n");
            }
          }
        } else if(total_servers == 2) {
           printf("In total server 2 condition\n");
           if(i == server_id) {
             printf("Filling ASCS cfg for CIS Id %d \n", ascs_config.cis_id);
             printf("Populate in Condition i == server_id \n");
             codec_qos_config->qos_config.ascs_configs.push_back(ascs_config);
           }
        }
    }
...
void do_bap_connect (char *p)
{
    ...
    for (uint8_t i = 0; i < args.serv.size(); i++) {
        ...
        printf("%s going for connect\n", __func__);
        sPacsClientInterface->Connect(pacs_client_id, bap_bd_addr, true);
        while(!pacsConnectionComplete) sleep(1);
        pacsConnectionComplete = 0;
...
void do_bap_connect_converged (char *p)
{
    ...
    for (uint8_t i = 0; i < args.serv.size(); i++) {
        ...
        codec_qos_config_2.qos_config.ascs_configs.clear();
        if(FALSE == GetBdAddr(args.serv[i].bdAddr, &bap_bd_addr)) return;
        if (i == 0 && args.serv[i].direction & ASE_DIRECTION_SINK)
        {
            set_codec_qos_config(&codec_qos_config, args.codecConfig,
                                 &acs, ASE_DIRECTION_SINK, args.serv[i].context,
                                 i, 0,
        ...
        if (i == 0 && args.serv[i].direction & ASE_DIRECTION_SRC)
        {
            set_codec_qos_config(&codec_qos_config_2, args.codecConfig,
                                 &acs, ASE_DIRECTION_SRC, args.serv[i].context,
                                 i, 1,
...
void do_bap_connect_and_start (char *p)
{
    ...
    for (uint8_t i = 0; i < args.serv.size(); i++) {
        RawAddress bap_bd_addr;
        if(FALSE == GetBdAddr(args.serv[i].bdAddr, &bap_bd_addr)) return;
        std::vector<bluetooth::bap::ucast::StreamType> start_streams;
        if (args.serv[i].direction & ASE_DIRECTION_SINK) {
        ...
        std::vector<RawAddress> address;
        address.push_back(bap_bd_addr);
        sUcastClientInterface->Start(address, start_streams, false);
    }
}
...
void do_bap_start (char *p)
{
    ...
    for (uint8_t i = 0; i < ((args.cnt)/4); i++) {
        ...
        std::vector<RawAddress> address;
        address.push_back(bap_bd_addr);
        sUcastClientInterface->Start(address, streams, false);
    }
}
...
{
    bdt_log("INIT BT ");
    status = sBtInterface->init(&bt_callbacks, false, false, 0, nullptr, false, NULL);
    sleep(1);
    if (status == BT_STATUS_SUCCESS) {

...
class CallControllerInterfaceImpl
  ...
  uint8_t callUri[MAX_URI_LENGTH];
 ...
 bt_status_t Init(CallControllerCallbacks* callbacks, Uuid uuid, int max_ccs_clients,
    ...
    BearerProviderInfo =(tCCS_BEARER_PROVIDER_INFO* ) malloc(sizeof(tCCS_BEARER_PROVIDER_INFO));
 ...
 bt_status_t UpdateBearerName(uint8_t* operator_str) {
    ...
    memcpy(BearerProviderInfo->name, operator_str, strlen((char*)operator_str)+1);
    do_in_bta_thread(FROM_HERE,Bind(&CallController::BearerInfoName,
           Unretained(CallController::Get()), BearerProviderInfo->name));
    return BT_STATUS_SUCCESS;
 }

 void Cleanup() {
   LOG(INFO) << __func__ ;
   free(BearerProviderInfo);
...
bt_status_t  UpdateSupportedBearerList(uint8_t* supportedbearer_list) {
 ...
 memcpy(BearerProviderInfo->bearer_schemes_list, supportedbearer_list, strlen((char*)supportedbearer_list)+1);
 LOG(INFO) << __func__ << "bearer_list: " << BearerProviderInfo->bearer_schemes_list;
 do_in_bta_thread(FROM_HERE,Bind(&CallController::UpdateSupportedBearerList,
      Unretained(CallController::Get()), BearerProviderInfo->bearer_schemes_list));
 return BT_STATUS_SUCCESS;
...
void UpdateActiveProfile(const RawAddress& address, int state) override {
    LOG(INFO) << __func__ << ": device "<< address;
    do_in_jni_thread(FROM_HERE, Bind(&call_active_profile_info,
          address, BTAPM_VOICE_AUDIO));
}

 ...
 public:
          ...
          [](AscsClientCallbacks *callback, uint8_t client_id, uint8_t status) {
            ...
            if (instance) {
              LOG(WARNING) << ": ASCS gatt_client_id "
                           << loghex(instance->gatt_client_id);
  ...
  void OnGattConnected(tGATT_STATUS status, uint16_t conn_id,
    ...
    if(dev->state == DevState::CONNECTING) {
      if (status != GATT_SUCCESS) {
        LOG(ERROR) << __func__ << ": Failed to connect to ASCS device";
  ...
  void OnGattDisconnected(tGATT_STATUS status, uint16_t conn_id,
    ...
    if (dev->conn_id) {
      DeregisterAllAscsNotifcations(gatt_client_id, dev);
      // Removes all registrations for connection.
      GattOpsQueue::Clean(dev->conn_id);
      BTA_GATTC_Close(dev->conn_id);
  ...
  void DeregisterAllAscsNotifcations(uint16_t conn_id,
                                         AscsDevice* dev) {
    ...
    for (auto it = dev->sink_ase_list.begin();
                    it != dev->sink_ase_list.end(); it++) {
      LOG(WARNING) << __func__
                   << ": Deregister sink ase notif for handle: "
                   << loghex(it->ase_handle);
      if (it->ase_handle != 0xFFFF) {
        DeregisterAscsNotification(conn_id, dev,
                                      it->ase_ccc_handle,
                                      it->ase_handle);
      }
    }
    ...
    for (auto it = dev->src_ase_list.begin();
                    it != dev->src_ase_list.end(); it++) {
      LOG(WARNING) << __func__
                   << ": Deregister src ase notif for handle: "
                   << loghex(it->ase_handle);
      if (it->ase_handle != 0xFFFF) {
        DeregisterAscsNotification(conn_id, dev,
                                      it->ase_ccc_handle,
    ...
    if (dev->ase_cp_handle != 0xFFFF) {
      LOG(WARNING) << __func__
                   << ": Deregister ase cp notif for handle: "
                   << loghex(dev->ase_cp_handle);
      DeregisterAscsNotification(conn_id, dev,
  ...
  void DeregisterAscsNotification(uint16_t conn_id, AscsDevice* dev,

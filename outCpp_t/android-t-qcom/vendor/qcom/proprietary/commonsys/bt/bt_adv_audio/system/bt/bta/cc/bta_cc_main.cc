...
static bool gIsPtsTestEnabled = false;
static bool gPtsIncomingCallIndicator = false;
...
const char* bta_cc_event_str(uint32_t event) {
  switch (event) {
      ...
      CASE_RETURN_STR(CCS_FRIENDLY_NAME_UPDATE)
  ...
  std::vector<CallControllerDeviceList> FindNotifyDevices(uint16_t handle) {
    ...
    for (size_t it = 0; it != devices.size(); it++){
      if(ccsControlServiceInfo.bearer_provider_name_handle == handle &&
        ...
        notify_devices.push_back(devices[it]);
      } else if(ccsControlServiceInfo.bearer_uri_schemes_supported_handle == handle &&
        devices[it].bearer_uri_schemes_supported_notify) {
        notify_devices.push_back(devices[it]);
      }
    }
...
class CallControllerImpl : public CallController {
 ...
 void SetActiveDevice(const RawAddress& address, int setId) {
  ...
  void CallState(int len, std::vector<uint8_t> call_state_list) {
    ...
    char Tmap_Call_State[PROPERTY_VALUE_MAX] = {'\0'};
    uint8_t Strm_state = 0;
    property_get("persist.vendor.btstack.tmap_CallState", Tmap_Call_State, "0");
    int res = sscanf(Tmap_Call_State, "%hhu", &Strm_state);
    LOG(INFO) <<__func__ <<": res: " << res;
    LOG(INFO) <<__func__ <<": Strm_state: " << Strm_state;
    ...
    for (int k=0; k<len; k++) {
        ...
        if (res == 1 && Strm_state == 2) { //outgoing
           if (call_state_list[3*k + 0] == 1) {
              LOG(INFO) << __func__ << "index 1 is ignored ";
              continue;
          }
        }
	    ...
	    if (gIsPtsTestEnabled) {
              //update call friendly name with incoming uri
              memcpy(FriendlyName.name, IncomingCallInfo.incoming_uri, len);
              HandleCcsEvent(CCS_FRIENDLY_NAME_UPDATE, &FriendlyName);
            }
        } else {
            BlccInfo.list_length = 3;
        }
        if (gIsPtsTestEnabled) {
            if (CallStateOp.state == CCS_STATE_LOCAL_HELD) {
                CallStateOp.state = CCS_STATE_REMOTELY_HELD;
            }
            if (CallStateOp.state == CCS_STATE_ALERTING) {
                CallStateOp.state =  CCS_STATE_DIALING;
            }
            LOG(INFO) << __func__ << "Call State set to " << CallStateOp.state;
        }
        if (CallStateOp.state == CCS_STATE_DISCONNECTED) {
            ...
            if (gIsPtsTestEnabled) {
                if (FriendlyName.index == CallStateOp.index) {
                    FriendlyName.index = 0;
                    memset(FriendlyName.name, 0, MAX_FRIENDLY_NAME_LEN);
                }
                HandleCcsEvent(CCS_FRIENDLY_NAME_UPDATE, &FriendlyName);
            }
    ...
    if (Strm_state == 0 || Strm_state == 2) { //PTS Outgoing or Normal Scenario
       HandleCcsEvent(CCS_CALL_STATE_UPDATE, &CallStatelist);
       HandleCcsEvent(CCS_BEARER_CURRENT_CALL_LIST_UPDATE, &BlccInfolist);
    } else if (Strm_state == 1) { //PTS Incoming
       if (CallStateOp.state == 3) {
         LOG(INFO) << __func__ << " active call update";
         gPtsIncomingCallIndicator = true;
         for (int k =0; k<2; k++) {
           LOG(INFO) << __func__ << " Sending Incoming Call state again before active state";
           HandleCcsEvent(CCS_CALL_STATE_UPDATE, &CallStatelist);
           usleep(100000);
         }
       } else {
 ...
 void BearerInfoName(uint8_t* name) {
   ...
   if (gIsPtsTestEnabled) {
     char schemes_list[20] = "skype";
     LOG(INFO) << __func__ << " pts test enabled,update statusflags and schemes";
     UpdateStatusFlags(1);
     UpdateSupportedBearerList((uint8_t*)schemes_list);
   }
 }
 ...
 void UpdateBearerTechnology(int tech_type) {
   ...
   tCCS_BEARER_PROVIDER_INFO bearerProviderOp;
   bearerProviderOp.technology_type = tech_type;
 ...
 void UpdateIncomingCall(int index, uint8_t* Uri) {
    string Uri_str = (char *)Uri;
    if(Uri_str.length() < 4){
      LOG(INFO) << __func__ << " Uri: " << Uri;
    }
    else if(Uri_str.length() < 8){
      string Uri_str_mask((Uri_str.length()-4),'*');
      string Uri_append_mask = Uri_str.substr(0,4) + Uri_str_mask;
      uint8_t *Uri_mask = (uint8_t *)Uri_append_mask.c_str();
      LOG(INFO) << __func__ << " Uri: " << Uri_mask;
    }
    else{
      string Uri_str_mask((Uri_str.length()-8),'*');
      string Uri_append_mask = Uri_str.substr(0,4) + Uri_str_mask + Uri_str.substr((Uri_str.length()-4),4);
      uint8_t *Uri_mask = (uint8_t *)Uri_append_mask.c_str();
      LOG(INFO) << __func__ << " Uri: " << Uri_mask;
    }
 ...
 void ContentControlId(uint32_t ccid) {
    ...
    ContentControlIdOp.ccid = (uint8_t)ccid;
 ...
 void CallControlInitializedCallback(uint8_t state) {
    ...
    if (state == 0) {
        ...
        BearerProviderInfo.technology_type = 2;
    }
 }
...
void  CallController::Initialize(bluetooth::call_control::CallControllerCallbacks* callbacks,
  ...
  char ptsCC[PROPERTY_VALUE_MAX] = "false";
  if(osi_property_get("persist.vendor.btstack.cc.certification", ptsCC, "false") &&
        !strcmp(ptsCC, "true")) {
    gIsPtsTestEnabled = true;
  }
}
...
static void SSReportingTimerExpired(void *data) {
    LOG(INFO) << __func__ ;
    int conn_id = (uintptr_t)data;
    CallControllerDeviceList *dev = cc_instance->remoteDevices.FindByConnId((uint16_t)conn_id);
    if (dev == nullptr) {
...
static void SetupSSReportingInterval (CallControllerDeviceList* dev) {
    ...
    if (dev->signal_strength_report_interval != 0) {
       alarm_set(dev->signal_strength_reporting_timer,
                       ...
                       (void*)(uintptr_t)(dev->conn_id));
    }
}

bool UpdateSSReportingInterval(const RawAddress& address, uint8_t ssrInterval) {
    bool ret = false;
    CallControllerDeviceList *dev = cc_instance->remoteDevices.FindByAddress(address);
...
void HandleCcsEvent(uint32_t event, void* param) {
  ...
  switch (event) {
    ...
    case BTA_GATTS_DISCONNECT_EVT: {
      ...
      CallControllerDeviceList *remoteDevice;
      remoteDevice = cc_instance->remoteDevices.FindByConnId(p_data->conn.conn_id);
      if((!remoteDevice) ) {
        LOG(INFO) << __func__ << " remote device not found ";
        status = BT_STATUS_FAIL;
        break;
      }
    ...
    case BTA_GATTS_READ_DESCRIPTOR_EVT: {
      ...
      if(p_data->req_data.p_data->read_req.handle ==
        ...
        data = remoteDevice->bearer_signal_strength_report_interval_notify;
          ...
          ccsControlServiceInfo.call_status_flags_desc) {
        LOG(INFO) << __func__ << " call status_flag_desc read";
        data = remoteDevice->status_flags_notify;
      }  else if(p_data->req_data.p_data->read_req.handle ==
          ccsControlServiceInfo.incoming_call_target_bearerURI_desc) {
        LOG(INFO) << __func__ << " incoming_call_target_bearerURI_desc read";
        data = remoteDevice->incoming_call_target_URI_notify;
      }  else if(p_data->req_data.p_data->read_req.handle ==
          ccsControlServiceInfo.incoming_call_desc) {
        LOG(INFO) << __func__ << " incoming_call_desc read";
        data = remoteDevice->incoming_call_state_notify;
      } else {
    ...
    case BTA_GATTS_WRITE_DESCRIPTOR_EVT: {
      ...
      if(req.handle ==
        ...
        handle = ccsControlServiceInfo.bearer_signal_strength_handle;
          ...
          handle = ccsControlServiceInfo.bearer_uri_schemes_supported_handle;
      } else if(req.handle ==
          ccsControlServiceInfo.bearer_list_currentcalls_desc) {
        std::map<uint8_t, tCCS_BEARER_LIST_CURRENT_CALLS>::iterator it;
        ...
        LOG(INFO) << __func__ << " bearer_technology_type: " << BearerProviderInfo.technology_type;
        if ( BearerProviderInfo.technology_type == 0) {
           BearerProviderInfo.technology_type = 2;
        }
    ...
    case CCS_CALL_STATE_UPDATE:
    {
      ...
      for (it = CallStatelist.begin(); it != CallStatelist.end(); it++){
        tCCS_CALL_STATE obj = it->second;
        if ((gPtsIncomingCallIndicator == true) && (obj.state == 3)) {
           LOG(INFO) << __func__ << " faking incoming update";
          _data.push_back(1);
          _data.push_back(0);
          _data.push_back(0);
          gPtsIncomingCallIndicator = false;
        } else {
    ...
    case CCS_CCID_UPDATE:
    {
      ...
      CcidInfo.ccid = (uint8_t)data->ccid;
      rsp->event = CCS_NOTIFY_ALL;
    ...
    case CCS_FRIENDLY_NAME_UPDATE:
    {
      LOG(INFO) << __func__ << " CCS_FRIENDLY_NAME_UPDATE";
      tCCS_FRIENDLY_NAME *data = (tCCS_FRIENDLY_NAME*) param;
      FriendlyName.index = data->index;
      uint16_t len = strlen((char*)data->name);
      ReverseByteOrder(data->name, len);
      memcpy(FriendlyName.name, data->name, len);
      rsp->event = CCS_NOTIFY_ALL;
      rsp->handle = ccsControlServiceInfo.call_friendly_name_handle;
      _data.push_back(FriendlyName.index);
      _data.insert(_data.end(), FriendlyName.name, FriendlyName.name + len);
      isCallControllerOpUsed = true;
      break;
    }
    default:
 ...
 bool DeviceStateConnectionHandler(uint32_t event, void* param) {
  ...
  switch (event) {
    ...
    case CCS_READ_RSP:
      ...
      GattsOpsQueue::SendResponse(p_data->remoteDevice->conn_id, p_data->oper.ReadOp.trans_id,
    ...
    case CCS_DESCRIPTOR_READ_RSP:
      ...
      GattsOpsQueue::SendResponse(p_data->remoteDevice->conn_id, p_data->oper.ReadDescOp.trans_id,
          BT_STATUS_SUCCESS, &p_data->rsp_value);
      break;
    ...
    case CCS_DESCRIPTOR_WRITE_RSP:
    {
      ...
      if (p_data->oper.WriteDescOp.need_rsp) {
        //send rsp to write
        LOG(INFO) << __func__ << " gett send rsp";
        GattsOpsQueue::SendResponse(p_data->remoteDevice->conn_id,p_data->oper.WriteDescOp.trans_id,
...
bool CCSActiveProfile(RawAddress addr) {
    ...
    activeProfile = get_active_profile(addr, 0);
    LOG(INFO) << __func__ << " activeProfile "<< activeProfile <<" for " << addr;
    if (activeProfile != 0x0002) {
        LOG(INFO) << __func__ << " CCService is active profile";
        isCCSActiveProfile = true;
    } else {
        LOG(INFO) << __func__ << " HFP is active profile";
    }
      ...
      if(handle == ccsControlServiceInfo.call_control_point_handle) {
           ...
           switch(CallControllerOps.operation) {
            ...
            case CALL_ORIGINATE: {
                  ...
                  if (res == 1 && Strm_state == 2) {
                     //when pts needs to initiate outgoing call, the below
                     //string needs to be updated to the required number
                     char OutgoingCallNumber[]  = "tel:+918074125086";
                     strlcpy(URI, OutgoingCallNumber, strlen(OutgoingCallNumber));
                  } else {
            ...
            UpdateSSReportingInterval(device->peer_bda, BearerProviderInfo.signal_report_interval);
        }
        break;
    }

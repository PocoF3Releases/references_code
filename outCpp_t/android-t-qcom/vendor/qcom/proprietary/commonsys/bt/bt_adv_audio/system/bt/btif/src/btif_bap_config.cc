...
#define BAP_AAR3_VS_METADATA           "AAR3VSMetadata"
#define BAP_AAR3_CODEC_ENCODER_VER_NUM "AAR3CodecEncoderVersionNumber"
#define BAP_AAR3_CODEC_DECODER_VER_NUM "AAR3CodecDecoderVersionNumber"
...
static bool btif_bap_update_AAR3_codec_info(char *section, CodecConfig *record,
  ...
  if(rec_type == REC_TYPE_CAPABILITY) {
    ...
    if(record->codec_type == CodecIndex::CODEC_INDEX_SOURCE_APTX_ADAPTIVE_LE) {
      config_set_string(config, section, BAP_CODEC_TYPE_KEY, "AAR3");
    }
    // update freqs
    config_set_uint16(config, section, BAP_SAMP_FREQS_KEY ,
                     static_cast<uint16_t>(record->sample_rate));
    ...
    // std::vector<uint8_t> vs_meta_data_AAR3 = GetVendorSpecificMetaDataAAR3(record);
    //all 4 bytes separately
    config_set_uint16(config, section, BAP_AAR3_CODEC_ENCODER_VER_NUM,
        static_cast<uint16_t> (GetCapaVendorMetaDataCodecEncoderVer(record)));

    config_set_uint16(config, section, BAP_AAR3_CODEC_DECODER_VER_NUM,
        static_cast<uint16_t> (GetCapaVendorMetaDataCodecDecoderVer(record)));

    config_set_uint16(config, section, BAP_AAR3_MIN_SUPP_FRAME_DUR,
        static_cast<uint16_t> (GetCapaMinSupFrameDur(record)));

    config_set_uint16(config, section, BAP_AAR3_FEATURE_MAP,
...
bool btif_bap_add_audio_loc(const RawAddress& bd_addr,
  ...
  if(direction == CodecDirection::CODEC_DIR_SRC) {
    config_set_uint16(config, section, BAP_SRC_LOCATIONS_KEY, audio_loc);
  } else {
    config_set_uint16(config, section, BAP_SINK_LOCATIONS_KEY, audio_loc);
  }
  btif_bap_config_save();
  return true;
}
...
bool btif_bap_add_num_src_sink(const RawAddress& bd_addr,
  ...
  strlcpy(section, bdstr, sizeof(section));
  strlcat(section, index, sizeof(section));
  config_set_uint16(config, section, BAP_NUM_SINK_KEY, num_sink);
  config_set_uint16(config, section, BAP_NUM_SRC_KEY, num_src);
  btif_bap_config_save();
  return true;
}
...
bool btif_bap_add_channel_count(const RawAddress& bd_addr,
  ...
  if (dir == CodecDirection::CODEC_DIR_SINK) {
      config_set_uint16(config, section, BAP_SINK_CHANNEL_COUNT_KEY, channel_count);
  }
  if (dir == CodecDirection::CODEC_DIR_SRC) {
      config_set_uint16(config, section, BAP_SRC_CHANNEL_COUNT_KEY, channel_count);
  }
  btif_bap_config_save();
  return true;
}
...
bool btif_bap_add_device_type(const RawAddress& bd_addr,
  ...
  strlcpy(section, bdstr, sizeof(section));
  strlcat(section, index, sizeof(section));
  config_set_uint16(config, section, BAP_SINK_DEVICE_TYPE_KEY,
    static_cast<uint16_t>(sink_device_type));
  config_set_uint16(config, section, BAP_SRC_DEVICE_TYPE_KEY,

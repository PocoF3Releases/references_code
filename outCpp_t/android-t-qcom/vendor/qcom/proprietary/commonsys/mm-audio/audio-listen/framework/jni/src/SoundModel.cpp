/*============================================================================
 * Copyright (c) 2013-2019, 2021-2022 Qualcomm Technologies, Inc.
 * All Rights Reserved.
                                      ...
                                      listen_sound_model_info * pModelInfo)
{
   ...
   if (isLsmHidlClientSupported())
       status = lsm_querySoundModel(pKeywordModel, pModelInfo);
   else
                             ...
                             const char *        userStr )
{
   ...
   if (isLsmHidlClientSupported())
       status = lsm_getSoundModelHeader(pModel, &modelHeader);
   else
   ...
   if (keywordStr != NULL) {
      ...
      if (isLsmHidlClientSupported())
          status = lsm_getKeywordPhrases(pModel, &numKWs, keywordPhrases);
      else
   ...
   if (userStr != NULL) {
      ...
      if (isLsmHidlClientSupported())
          status = lsm_getUserNames(pModel, &numUsers, userNames);
      else
                 ...
                 jobject                   soundModelInfo)  // ListenTypes$SVASoundModelInfo
{
   ...
   numKeyphrases = numKeyphrasesHeader;
      if (isLsmHidlClientSupported())
          status = lsm_getKeywordPhrases(pModel, &numKeyphrases, keywordPhrases);
      else

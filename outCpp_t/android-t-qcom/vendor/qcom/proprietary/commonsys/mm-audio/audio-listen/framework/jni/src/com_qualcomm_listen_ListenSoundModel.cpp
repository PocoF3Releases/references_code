...
#define SML_VERSION_V7 0x0700040100000000  // lib version 7.0_4.1.0
 ...
 * Param [out] version - contains SML version information
 *
 * Return - LISTEN_SUCCESS for success. Negative error is returned for failures.
 */
JNIEXPORT jint JNICALL Java_com_qualcomm_listen_ListenSoundModel_getSMLVersion
    (JNIEnv *env, jclass thiz __unused, jobject version)
{
    sml_version_t smlVersion;
    listen_status_enum smlStatus = kSucess;
    listen_status_enum_t status = LISTEN_SUCCESS;
    uint32_t retSize = 0;
    jclass clazz;
    jfieldID versionId;

    ALOGV("_getSMLVersion: enter");

    if (!isLsmHidlClientSupported()) {
        ALOGV("_getSMLVersion: Implementation not supported");
        return LISTEN_EFUNCTION_NOT_IMPLEMENTED;
    }

    if (version == nullptr) {
        ALOGE("_getSMLVersion: ERROR Null input params");
        return LISTEN_EBAD_PARAM;
    }

    smlStatus = lsm_smlGet(NULL, SML_CONFIG_ID_VERSION, (int8_t *)&smlVersion,
        sizeof(smlVersion), &retSize);
    if ((smlStatus != kSucess) || (retSize != sizeof(smlVersion))) {
        ALOGE("_getSMLVersion: ERROR CONFIG_ID_VERSION, status %d", smlStatus);
        MAP_ERR(smlStatus, status);
        return status;
    }

    clazz = env->FindClass("com/qualcomm/listen/ListenTypes$SMLVersion");
    if (clazz == nullptr) {
       ALOGE("_getSMLVersion: ERROR SMLVersion class not found");
       return LISTEN_EFAILURE;
    }

    versionId = env->GetFieldID(clazz, "version", "J");
    if (versionId == nullptr) {
       ALOGE("_getSMLVersion: ERROR SMLVersion class version field not found");
       return LISTEN_EFAILURE;
    }

    env->SetLongField(version, versionId, smlVersion.version);

    ALOGV("_getSMLVersion: version 0x%llx", smlVersion.version);
    return status;
}
                  ...
                  jobject rec )               // ShortBuffer
{
   ...
   if (modelInfo.version >= SM_VERSION_2p0) {
      ...
      if (isLsmHidlClientSupported())
          status = lsm_verifyUserRecording(&keywordModel, kwString,
...
/*
 * verifyUserRecordingQuality
 *
 * Class:     com_qualcomm_listen_ListenSoundModel
 * Method:    verifyUserRecordingQuality
 ...
 *
 * Given User-Independent keyword model data this method returns the detection
 * algorithm's confidence that the given user-specific recordings matches
 * keyword associated with this given model data.
 * Calls ListenSoundModel library API to determine how closely the recording
 * matches given keyword contained in user-independent keyword-only SoundModel.
 * It supports taking in recordings from clean and noisy conditions indicating
 * to sound model library algorithm for improved user training of the sound model
 * and provides quality stats of the recording input as feedback to client.
 *
 * Param [in]  env - ptr to JNIEnv
 * Param [in]  clazz - java class (not an object instance) of this static method
 * Param [in]  inputModel - contains User-Independent model data
 * Param [in]  keywordPhrase - name of keyword in Sound model to be extended
 * Param [in]  recording  - a single recording of user speaking keyword
 * Param [in]  isNoisySample  - input recording is capatured in noisy (true) or
 *             clean (false) environment
 * Param [out] diagnostics - contains recording quality check result information
 *
 * Return - STATUS_SUCCESS for success. Negative error number is returned
 *     if error occurred.
 */
JNIEXPORT jint JNICALL Java_com_qualcomm_listen_ListenSoundModel_verifyUserRecordingQuality
                 (JNIEnv *env, jclass thiz __unused,
                  ...
                  jobject diagnostics)
{
    listen_model_type keywordModel = {nullptr, 0};
    listen_user_recording userRecording = {nullptr, 0};
    listen_status_enum smlStatus = kSucess;
    listen_status_enum_t status = LISTEN_SUCCESS;
    listen_sound_model_info modelInfo;
    qualityCheckResult_t qCheckInfo;
    keywordId_t kwString = nullptr;
    uint32_t noisy = 0;
    jclass clazz;

    struct {
        jfieldID isEpdFilteredSegmentSet;
        jfieldID isLowSnrSet;
        jfieldID epdSnr;
        jfieldID epdStart;
        jfieldID epdEnd;
        jfieldID exactEpdStart;
        jfieldID exactEpdEnd;
        jfieldID keywordConfidenceLevel;
        jfieldID epdPeakLevel;
        jfieldID epdRmsLevel;
        jfieldID n_epdSamplesClipping;
        jfieldID keywordStart;
        jfieldID keywordEnd;
    } qCheckInfoIds;

    ALOGV("_verifyUserRecordingQuality: enter");

    if (!isLsmHidlClientSupported()) {
        ALOGV("_verifyUserRecordingQuality: Implementation not supported");
        return LISTEN_ENOT_SUPPORTED_FOR_SOUNDMODEL_VERSION;
    }

    if ((inputModel == nullptr) || (rec == nullptr) ||
        (keywordPhrase == nullptr) || (diagnostics == nullptr)) {
        ALOGE("__verifyUserRecordingQuality: ERROR Null input params");
        return LISTEN_EBAD_PARAM;
    }

    status = getSMFromByteBuff(env, inputModel, &keywordModel.size,
                               &keywordModel.data, &modelInfo);
    if (status != LISTEN_SUCCESS) {
        ALOGE("__verifyUserRecordingQuality: ERROR getSMFromByteBuff status %d", status);
        goto exit_1;
    }
    ALOGV("__verifyUserRecordingQuality: user independent SM (ptr,size)=(%p,%d)",
          keywordModel.data, keywordModel.size);

    status = getRecFromShortBuff(env, rec, &userRecording.n_samples,
                                 &userRecording.data);
    if (status != LISTEN_SUCCESS) {
       ALOGE("__verifyUserRecordingQuality: ERROR getSMFromShortBuff status %d", status);
       goto exit_1;
    }
    ALOGV("__verifyUserRecordingQuality: user recording (ptr,size)=(%p,%d)",
          userRecording.data, userRecording.n_samples);

    kwString = (keywordId_t)env->GetStringUTFChars(keywordPhrase, 0);

    noisy = isNoisyRec ? 1 : 0;
    ALOGV("call verifyUserRecordingExt() with %s", kwString);
    smlStatus = lsm_verifyUserRecordingExt(&keywordModel, kwString, NULL,
                                       &userRecording, noisy, &qCheckInfo);

    if (smlStatus != kSucess) {
        ALOGE("__verifyUserRecordingQuality: ERROR verifyUserRecording status %d", smlStatus);
        MAP_ERR(smlStatus, status);
        goto exit_1;
    }
    ALOGI("__verifyUserRecordingQuality confidence score %d",
          qCheckInfo.keywordConfidenceLevel);
    ALOGV("__verifyUserRecordingQuality keyword [start end] = [%d %d]",
          qCheckInfo.keywordStart, qCheckInfo.keywordEnd);

    clazz = env->FindClass("com/qualcomm/listen/ListenTypes$QualityCheckInfo");
    if (clazz == nullptr) {
       ALOGE("__verifyUserRecordingQuality: ERROR QualityCheckInfo class not found");
       status = LISTEN_EFAILURE;
       goto exit_1;
    }

    qCheckInfoIds.isEpdFilteredSegmentSet = env->GetFieldID(clazz, "isEpdFilteredSegmentSet", "Z");
    qCheckInfoIds.isLowSnrSet = env->GetFieldID(clazz, "isLowSnrSet", "Z");
    qCheckInfoIds.epdSnr = env->GetFieldID(clazz, "epdSnr", "F");
    qCheckInfoIds.epdStart = env->GetFieldID(clazz, "epdStart", "I");
    qCheckInfoIds.epdEnd = env->GetFieldID(clazz, "epdEnd", "I");
    qCheckInfoIds.exactEpdStart = env->GetFieldID(clazz, "exactEpdStart", "I");
    qCheckInfoIds.exactEpdEnd = env->GetFieldID(clazz, "exactEpdEnd", "I");
    qCheckInfoIds.keywordConfidenceLevel = env->GetFieldID(clazz, "keywordConfidenceLevel", "S");
    qCheckInfoIds.epdPeakLevel = env->GetFieldID(clazz, "epdPeakLevel", "F");
    qCheckInfoIds.epdRmsLevel = env->GetFieldID(clazz, "epdRmsLevel", "F");
    qCheckInfoIds.n_epdSamplesClipping = env->GetFieldID(clazz, "n_epdSamplesClipping", "I");
    qCheckInfoIds.keywordStart = env->GetFieldID(clazz, "keywordStart", "I");
    qCheckInfoIds.keywordEnd = env->GetFieldID(clazz, "keywordEnd", "I");

    if ((qCheckInfoIds.isEpdFilteredSegmentSet == nullptr) ||
        (qCheckInfoIds.isLowSnrSet == nullptr) || (qCheckInfoIds.epdSnr == nullptr) ||
        (qCheckInfoIds.epdStart == nullptr) || (qCheckInfoIds.epdEnd == nullptr) ||
        (qCheckInfoIds.exactEpdStart == nullptr) || (qCheckInfoIds.exactEpdEnd == nullptr) ||
        (qCheckInfoIds.keywordConfidenceLevel == nullptr) || (qCheckInfoIds.epdPeakLevel == nullptr) ||
        (qCheckInfoIds.epdRmsLevel == nullptr) || (qCheckInfoIds.n_epdSamplesClipping == nullptr) ||
        (qCheckInfoIds.keywordStart == nullptr) || (qCheckInfoIds.keywordEnd == nullptr)) {
        ALOGE("__verifyUserRecordingQuality: ERROR QualityCheckInfo class fields not found");
        status = LISTEN_EFAILURE;
        goto exit_1;
    }

    env->SetBooleanField(diagnostics, qCheckInfoIds.isEpdFilteredSegmentSet, qCheckInfo.isEpdFilteredSegmentSet);
    env->SetBooleanField(diagnostics, qCheckInfoIds.isLowSnrSet, qCheckInfo.isLowSnrSet);
    env->SetFloatField(diagnostics, qCheckInfoIds.epdSnr, qCheckInfo.epdSnr);
    env->SetIntField(diagnostics, qCheckInfoIds.epdStart, qCheckInfo.epdStart);
    env->SetIntField(diagnostics, qCheckInfoIds.epdEnd, qCheckInfo.epdEnd);
    env->SetIntField(diagnostics, qCheckInfoIds.exactEpdStart, qCheckInfo.exactEpdStart);
    env->SetIntField(diagnostics, qCheckInfoIds.exactEpdEnd, qCheckInfo.exactEpdEnd);
    env->SetShortField(diagnostics, qCheckInfoIds.keywordConfidenceLevel, qCheckInfo.keywordConfidenceLevel);
    env->SetFloatField(diagnostics, qCheckInfoIds.epdPeakLevel, qCheckInfo.epdPeakLevel);
    env->SetFloatField(diagnostics, qCheckInfoIds.epdRmsLevel, qCheckInfo.epdRmsLevel);
    env->SetIntField(diagnostics, qCheckInfoIds.n_epdSamplesClipping, qCheckInfo.n_epdSamplesClipping);
    env->SetIntField(diagnostics, qCheckInfoIds.keywordStart, qCheckInfo.keywordStart);
    env->SetIntField(diagnostics, qCheckInfoIds.keywordEnd, qCheckInfo.keywordEnd);

exit_1:
    if (keywordModel.data != nullptr)
       free(keywordModel.data);
    if (userRecording.data != nullptr)
       free( userRecording.data);
    if (kwString != nullptr)
       env->ReleaseStringUTFChars(keywordPhrase, kwString);

    ALOGV("__verifyUserRecordingQuality: return status %d", status);
    return status;
}

/*
 * SetSoundModelTuningParams
 *
 * Class:     com_qualcomm_listen_ListenSoundModel
 * Method:    SetSoundModelTuningParams
 * Signature: (ILjava/nio/ByteBuffer;)I
 *
 * Set Algo specific Sound Model configuration tuning parameters prior to
 * training the sound model.
 *
 * Param [in]  env - ptr to JNIEnv
 * Param [in]  clazz - java class (not an object instance) of this static method
 * Param [in]  keywordType - SVA_KEYWORD_TYPE_PDK or SVA_KEYWORD_TYPE_UDK
 * Param [in]  tuningPayload - opaque payload interpreted by sml algo
 *
 * Return - percent confidence level that user recording matches keyword
 *          Negative error number is returned if error occurred.
 *          LISTEN_EBAD_PARAM
 *          LISTEN_EFAILURE
 */
JNIEXPORT jint JNICALL Java_com_qualcomm_listen_ListenSoundModel_SetSoundModelTuningParams
                 (JNIEnv *env, jclass thiz __unused,
                  jint keywordType,
                  jobject tuningPayload)
{
    listen_status_enum smlStatus = kSucess;
    listen_status_enum_t status = LISTEN_SUCCESS;
    int8_t *payload = nullptr;
    uint32_t payloadSize = 0;
    struct sml_udk_params *payload_tmp;
    uint32_t retSize = 0;
    sml_version_t smlVersion;
    uint32_t udk_params_size = 0;

    ALOGV("_SetSoundModelTuningParams: enter");
    if (!isLsmHidlClientSupported()) {
        ALOGV("_SetSoundModelTuningParams: Implementation not supported");
        return LISTEN_ENOT_SUPPORTED_FOR_SOUNDMODEL_VERSION;
    }
    if (tuningPayload == nullptr) {
        ALOGE("_SetSoundModelTuningParams: ERROR null tuningPayload");
        return LISTEN_EBAD_PARAM;
    }

    payload = (int8_t *)env->GetDirectBufferAddress(tuningPayload);
    payloadSize = (uint32_t) env->GetDirectBufferCapacity(tuningPayload);
    ...
    ALOGV("_SetSoundModelTuningParams: payload (ptr,size)=(%p,%d)", payload, payloadSize);

    if (keywordType == (jint)SVA_KEYWORD_TYPE_PDK){
        if (payloadSize != sizeof(struct sml_pdk_params)) {
            ALOGE("_SetSoundModelTuningParams: PDK payload size %d is not valid, expected size %lu",
                  payloadSize, sizeof(struct sml_pdk_params));
            return LISTEN_EBAD_PARAM;
        }
        smlStatus = lsm_smlSet(NULL, SML_CONFIG_ID_SET_PDK_PARAMS, payload, payloadSize);
        if (smlStatus != kSucess) {
            ALOGE("_SetSoundModelTuningParams: ERROR SET_PDK_PARAMS, status %d", smlStatus);
            MAP_ERR(smlStatus, status);
            return status;
        }
    } else if (keywordType == (jint)SVA_KEYWORD_TYPE_UDK){
          if (smlVersion.version < SML_VERSION_V7 &&
              payloadSize != sizeof(struct sml_udk_params_v6)) {
              ALOGE("_SetSoundModelTuningParams: UDK payload size %d is not valid, expected size %lu",
                    payloadSize, sizeof(struct sml_udk_params_v6));
              return LISTEN_EBAD_PARAM;
          } else if (smlVersion.version >= SML_VERSION_V7 &&
              payloadSize != sizeof(struct sml_udk_params)) {
              ALOGE("_SetSoundModelTuningParams: UDK payload size %d is not valid, expected size %lu",
                    payloadSize, sizeof(struct sml_udk_params));
              return LISTEN_EBAD_PARAM;
          }
          smlStatus = lsm_smlSet(NULL, SML_CONFIG_ID_SET_UDK_PARAMS, payload, payloadSize);
          if (smlStatus != kSucess) {
              ALOGE("_SetSoundModelTuningParams: ERROR SET_UDK_PARAMS, status %d", smlStatus);
              MAP_ERR(smlStatus, status);
              return status;
          }
    } else {
        ALOGE("_SetSoundModelTuningParams: Invalid keywordType %d", keywordType);
        status = LISTEN_EBAD_PARAM;
    }
    return status;
}
                 ...
                 jstring userName)
{
   ...
   if (modelInfo.version >= SM_VERSION_2p0) {
      ...
      if (isLsmHidlClientSupported())
          status = lsm_getSoundModelHeader(&keywordModel, &modelHeader);
      else
      ...
      if(isLsmHidlClientSupported())
          status = lsm_getUserKeywordModelSize(&keywordModel,
                 ...
                 jobject qualityData)        // ListenTypes$ConfidenceData
{
   ...
   if (modelInfo.version >= SM_VERSION_2p0) {
      ...
      if (isLsmHidlClientSupported())
          status = lsm_createUserKeywordModel(&keywordModel,
                   kwString, userString,
            ...
            jobject jUserRecording) //short buffer
{
    ...
    if (isLsmHidlClientSupported())
        status = lsm_checkUserRecording(&languageModel, NULL,
                &userRecording, &snr, 0); // Passing null for EPD uses default EPD params.
                                          // Passing 0 for maxPhonemeLength as it is optional parameter
    else
            ...
            jobjectArray jUserRecordings)
{
    ...
    ALOGD("_verifyUdkRecording_1: call checkRecordingsQuality");
    if (isLsmHidlClientSupported())
        status = lsm_checkRecordingsQuality(&languageModel, NULL,
    ...
    if (kSucess != status) {
        ...
        jSnr = (jint)snr;
        ALOGD("_verifyUdkRecording_1: checkRecordingsQuality snr %d", (int)jSnr);
    }
...
cleanup_for_verify_rec:
    ...
    if (LISTEN_SUCCESS != retStatus) {
        ALOGE("_verifyUdkRecording_1: returns error %d", retStatus);
        return retStatus;
    }
    return (jint) jSnr;
}
          ...
          jobject jLanguageModel) // ByteBuffer
{
    ...
    if (isLsmHidlClientSupported())
        status = lsm_getUserDefinedKeywordSize(NULL, kwString, userString,
              NULL, numberUdkRecordings, userRecsArray, &languageModel,
              &nUserKeywordModelSize);
    else
          ...
          jobject        jConfidenceData) // ListenTypes$ConfidenceData
{
    ...
    if (isLsmHidlClientSupported())
        status = lsm_createUserDefinedKeywordModel(NULL, kwString, userString,
                NULL, jNumberUserRecordings, userRecsArray, &languageModel,
                &outputModel, &userMatchingScore);
    else
                 ...
                 jobject outDataStruct)          // ListenTypes$VoiceWakeupDetectionData
{
   ...
   if (isLsmHidlClientSupported())
       status = lsm_parseDetectionEventData(
                 ...
                 jobject outDataStruct)          // ListenTypes$VoiceWakeupDetectionDataV2
{
   ...
   memset(&modelHeader, 0, sizeof(listen_sound_model_header));
   if (isLsmHidlClientSupported())
      status = lsm_getSoundModelHeader(&soundModel, &modelHeader);
   else
   ...
   if (isLsmHidlClientSupported())
      status = lsm_getKeywordPhrases(&soundModel, &numKWs, keywordPhrases);
   else
   ...
   if (numUsers > 0) {
      ...
      if (isLsmHidlClientSupported())
          status = lsm_getUserNames(&soundModel, &numUsers, userNames);
      else
                 ...
                 jobjectArray soundModelsArr)    // array of ByteBuffer
{
   ...
   if (isLsmHidlClientSupported())
       status = lsm_getMergedModelSize(numSoundModels, pInputModels,
           &sizeOfMergedModel);
   else
       status = getMergedModelSize(numSoundModels, pInputModels,
           &sizeOfMergedModel);
   if (kSucess != status) {
                 ...
                 jobject      mergedModel)       // ByteBuffer
{
   ...
   if (isLsmHidlClientSupported())
       status = lsm_mergeModels(numSoundModels, pInputModels, &outputModel);
   else
                 ...
                 jstring         user)
{
   ...
   memset(&modelHeader, 0, sizeof(listen_sound_model_header));
   if (isLsmHidlClientSupported())
      status = lsm_getSoundModelHeader(&model, &modelHeader);
   else
   ...
   if (isLsmHidlClientSupported())
       status = lsm_getSizeAfterDeleting(&model,
                 ...
                 jobject         outSoundModel )  // ByteBuffer
{
   ...
   memset(&modelHeader, 0, sizeof(listen_sound_model_header));
   if (isLsmHidlClientSupported())
      status = lsm_getSoundModelHeader(&inputModel, &modelHeader);
   else
   ...
   if (isLsmHidlClientSupported())
       status = lsm_getSizeAfterDeleting(&inputModel,
                   (keywordId_t)keywordChar, (userId_t)userChar,
                   &sizeOfDeletedModel);
   else
   ...
   // call LSMLib function
   if (isLsmHidlClientSupported())
       status = lsm_deleteFromModel(&inputModel,

...
const char *EXT_MEDIA_MIMETYPE_AUDIO_AIFF        = "audio/x-aiff";
const char *EXT_MEDIA_MIMETYPE_AUDIO_AMR_WB_PLUS = "audio/amr-wb-plus";
const char *EXT_MEDIA_MIMETYPE_AUDIO_APE         = "audio/x-ape";
const char *EXT_MEDIA_MIMETYPE_AUDIO_DSD         = "audio/dsd";
const char *EXT_MEDIA_MIMETYPE_AUDIO_DTS         = "audio/dts";
const char *EXT_MEDIA_MIMETYPE_AUDIO_EVRC        = "audio/evrc";
const char *EXT_MEDIA_MIMETYPE_AUDIO_MHAS        = "audio/mhas";
const char *EXT_MEDIA_MIMETYPE_AUDIO_MHM1        = "audio/mhm1";
const char *EXT_MEDIA_MIMETYPE_AUDIO_MHA1        = "audio/mha1";
const char *EXT_MEDIA_MIMETYPE_VIDEO_WMV         = "video/x-ms-wmv";
...
status_t FileSourceWrapper::Init(iStreamPort& aStreamPort,
    ...
    if(iDrmEncrypted) {
        if ((fileformat != FILE_SOURCE_WEBM) &&
            (fileformat != FILE_SOURCE_MPEG4) &&
            (fileformat != FILE_SOURCE_3G2)) {
            LOGE("Encrypted clip but in unsupported container");
            return UNKNOWN_ERROR;
        }

        FileSourceDrmType eDrmType = FILE_SOURCE_NO_DRM;
        if (FILE_SOURCE_SUCCESS != m_pFileSource->GetDRMType(eDrmType)) {
            LOGE("Failed to get DRM type");
            return UNKNOWN_ERROR;
        }
        if (eDrmType != FILE_SOURCE_CENC_DRM) {
            LOGE("Only common encryption schemes are supported");
            return UNKNOWN_ERROR;
        }
    }
...
FileSourceStatus FileSourceWrapper::GetClipPSSHData(uint8* pPsshData,uint32* size) {
    Mutex::Autolock autoLock(mMutex);
    return m_pFileSource->GetClipPSSHData(pPsshData, size);
}
...
const char *MediaType2MIME(FileSourceMnMediaType minorType) {
    ...
    switch (minorType) {
        ...
        case FILE_SOURCE_MN_TYPE_EVRC:
            ...
            mimeType = EXT_MEDIA_MIMETYPE_AUDIO_EVRC;
            break;
        ...
        case FILE_SOURCE_MN_TYPE_APE:
            ...
            mimeType = EXT_MEDIA_MIMETYPE_AUDIO_APE;
            break;
        ...
        case FILE_SOURCE_MN_TYPE_MHAS:
            ...
            mimeType = EXT_MEDIA_MIMETYPE_AUDIO_MHAS;
            break;
        case FILE_SOURCE_MN_TYPE_MHM1:
            LOGV("MediaType2MIME FILE_SOURCE_MN_TYPE_MHM1");
            mimeType = EXT_MEDIA_MIMETYPE_AUDIO_MHM1;
            break;
        case FILE_SOURCE_MN_TYPE_MHA1:
            LOGV("MediaType2MIME FILE_SOURCE_MN_TYPE_MHA1");
            mimeType = EXT_MEDIA_MIMETYPE_AUDIO_MHA1;
            break;
        ...
        case FILE_SOURCE_MN_TYPE_VC1:
            ...
            mimeType = EXT_MEDIA_MIMETYPE_VIDEO_WMV;
            break;
       ...
       case FILE_SOURCE_MN_TYPE_VP8F:
            ...
            mimeType = MEDIA_MIMETYPE_VIDEO_VP8;
            break;
       case FILE_SOURCE_MN_TYPE_VP9:
        ...
        case FILE_SOURCE_MN_TYPE_HEVC:
            ...
            break;
        case FILE_SOURCE_MN_TYPE_DOLBY_VISION:
            LOGV("MediaType2MIME  FILE_SOURCE_MN_TYPE_DOLBY_VISION");
            mimeType =  MEDIA_MIMETYPE_VIDEO_DOLBY_VISION;
            break;
        ...
        case FILE_SOURCE_MN_TYPE_META_TEXT:
            ...
            mimeType = "text/plain";
            break;
        case FILE_SOURCE_MN_TYPE_DATA:
            LOGV("MediaType2MIME FILE_SOURCE_MN_TYPE_DATA");
...
const char *GetFileMimeType(FileSourceFileFormat fileFormat, uint8 tracksPresent) {
    ...
    switch(fileFormat) {
        ...
        case FILE_SOURCE_AIFF:
            mimeType = EXT_MEDIA_MIMETYPE_AUDIO_AIFF;
            break;
...
void MMParserExtractor::setPSSHData()
{
  uint32 uPsshSize = 0;
  m_pFileSourceWrapper->GetClipPSSHData(NULL,&uPsshSize);
  if(uPsshSize)
  {
    uint8* pPsshData = new uint8[uPsshSize];
    if(pPsshData != NULL)
    {
      m_pFileSourceWrapper->GetClipPSSHData(pPsshData,&uPsshSize);
      AMediaFormat_setBuffer(mFileMetaData, AMEDIAFORMAT_KEY_PSSH, pPsshData, uPsshSize);
    }
    delete[] pPsshData;
  }
}
...
void MMParserExtractor::setChannelMask(Track *track,
              uint32_t channelCount,uint32_t hapticChannelCount)
{
  if(hapticChannelCount > 0)
  {
    const uint32_t hapticChannelMask =
        haptic_channel_mask_from_count(hapticChannelCount);
    if((hapticChannelMask == AUDIO_CHANNEL_INVALID)
            || (hapticChannelCount >= channelCount)
            || (channelCount > (hapticChannelCount + 8)))
        ALOGE("Invalid haptic channel count %u", hapticChannelCount);
    else
    {
      const uint32_t audioChannelCount = channelCount - hapticChannelCount;
      const uint32_t channelMask =
        audio_channel_out_mask_from_count(audioChannelCount) | hapticChannelMask;
      AMediaFormat_setInt32(track->meta,
                  AMEDIAFORMAT_KEY_CHANNEL_MASK, channelMask);
      AMediaFormat_setInt32(track->meta,
                  AMEDIAFORMAT_KEY_HAPTIC_CHANNEL_COUNT, hapticChannelCount);
    }
  }
  else
    AMediaFormat_setInt32(track->meta, AMEDIAFORMAT_KEY_CHANNEL_MASK,
...
status_t MMParserExtractor::readMetaData() {
    ...
    for (uint32_t i = 0; i < numTracks; i++){
        ...
        if (m_pFileSourceWrapper->IsDrmProtection()) {
            FS_CONTENT_PROTECTION_INFOTYPE cryptoInfo = {};
            FileSourceStatus status = m_pFileSourceWrapper->GetStreamParameter(track->trackId,
                                                     FS_IndexParamOtherCryptoInfo, &cryptoInfo);
            if (status == FILE_SOURCE_SUCCESS) {
                setCryptoInfo(track->meta, &cryptoInfo);
            }
        }

        if (FILE_SOURCE_MJ_TYPE_VIDEO == majorType) {
            Track* ref = mFirstTrack;
            while (ref != NULL) {
                if ((ref->trackId + 100) == track->trackId) {
                    const char *mime;
                    CHECK(AMediaFormat_getString(ref->meta, AMEDIAFORMAT_KEY_MIME, &mime));
                    if (!strcasecmp(mime, MEDIA_MIMETYPE_VIDEO_DOLBY_VISION)) {
                        AMediaFormat_setInt32(track->meta, AMEDIAFORMAT_KEY_TRACK_ID, ref->trackId);
                        break;
                    }
                }
                ref = ref->next;
    ...
    if(trackPresent & (AUDIO_TRACK | VIDEO_TRACK)) {
        setAlbumArt();
    }
...
void MMParserExtractor::setAudioTrackMeta(Track* track, FileSourceAudioInfo& info) {
    ...
    if (minorType == FILE_SOURCE_MN_TYPE_AAC ||
        ...
        if (m_pFileSourceWrapper->GetWMACodecData(track->trackId, &codecData)) {
            ...
            // C2 vendor keys for WMA
            AMediaFormat_setInt32(meta,"vendor.qti-wma-config.bitsPerSample", codecData.nBitsPerSample);
            AMediaFormat_setInt32(meta,"vendor.qti-wma-config.blkAlign", codecData.nBlockAlign);
            AMediaFormat_setInt32(meta,"vendor.qti-wma-config.channelMask", codecData.nChannelMask);
            AMediaFormat_setInt32(meta,"vendor.qti-wma-config.formatTag", codecData.nFormatTag);
            AMediaFormat_setInt32(meta,"vendor.qti-wma-config.encOptions", codecData.nEncodeOpt);
            if (minorType == FILE_SOURCE_MN_TYPE_WMA_PRO ||
                ...
                // C2 vendor keys for WMA PRO/LOSSLESS
                AMediaFormat_setInt32(meta,"vendor.qti-wma-config.advEncOption", codecData.nAdvEncodeOpt);
                AMediaFormat_setInt32(meta,"vendor.qti-wma-config.advEncOptions2", codecData.nAdvEncodeOpt2);
            }
        }

        enum {kTypeWMA, kTypeWMAPro, kTypeWMALossLess};
        if (minorType == FILE_SOURCE_MN_TYPE_WMA_PRO) {
            AMediaFormat_setInt32(meta, "wma-version", kTypeWMAPro);
            AMediaFormat_setInt32(meta, "vendor.qti-wma-version.value", kTypeWMAPro);
        } else if (minorType == FILE_SOURCE_MN_TYPE_WMA_LOSSLESS) {
            AMediaFormat_setInt32(meta, "wma-version", kTypeWMALossLess);
            AMediaFormat_setInt32(meta, "vendor.qti-wma-version.value", kTypeWMALossLess);
        } else if (minorType == FILE_SOURCE_MN_TYPE_WMA) {
            AMediaFormat_setInt32(meta, "wma-version", kTypeWMA);
            AMediaFormat_setInt32(meta, "vendor.qti-wma-version.value", kTypeWMA);
        }
        ...
        if (24 == info.nBitsPerSample && FILE_SOURCE_MN_TYPE_AIFF_PCM == minorType) {
            ...
            int bitDepth = 24;
            if (AIBinder_getCallingUid() != AID_MEDIA) {
                configStatus = m_pFileSourceWrapper->SetConfiguration(track->trackId, &pitem,
                                              FILE_SOURCE_MEDIA_DISABLE_PCM_SAMPLE_UPGRADE);
                if (configStatus == FILE_SOURCE_SUCCESS)
                    bitDepth = 16;
            }
            AMediaFormat_setInt32(meta, AMEDIAFORMAT_KEY_BITS_PER_SAMPLE, bitDepth);
            AMediaFormat_setInt32(meta, AMEDIAFORMAT_KEY_PCM_ENCODING,
                                  getPcmEncoding((uint32)bitDepth));
        } else if (FILE_SOURCE_MN_TYPE_G711_ALAW == minorType ||
                   FILE_SOURCE_MN_TYPE_G711_MULAW == minorType) {
            AMediaFormat_setInt32(meta, AMEDIAFORMAT_KEY_PCM_ENCODING, kAudioEncodingPcm16bit);
        ...
        ulSize = 12;
        eRet = m_pFileSourceWrapper->GetClipMetaData
                                              ((wchar_t*)pBuf, &ulSize,
                                               FILE_SOURCE_MD_ANDROID_HAPTIC);
        if((FILE_SOURCE_SUCCESS == eRet) && (ulSize != 0)) {
          LOGV("android haptic is present, %s", pBuf);
          std::string s((const char *)pBuf);
          int hapticChannelCount = std::stoi(s);
          int32_t audio_channels;
          AMediaFormat_getInt32(meta, AMEDIAFORMAT_KEY_CHANNEL_COUNT, &audio_channels);
          setChannelMask(track, audio_channels, hapticChannelCount);
        }
    ...
    } else if ((FILE_SOURCE_MN_TYPE_MHM1 == minorType) ||
                (FILE_SOURCE_MN_TYPE_MHA1 == minorType)) {
       AMediaFormat_setInt32(meta, AMEDIAFORMAT_KEY_MPEGH_PROFILE_LEVEL_INDICATION,
                                            info.ucMhacProfileLevel);
       AMediaFormat_setInt32(meta, AMEDIAFORMAT_KEY_MPEGH_REFERENCE_CHANNEL_LAYOUT,
                                            info.ucMhacChannelLayout);
       setCodecSpecificData(track, AMEDIAFORMAT_KEY_CSD_0, FS_FORMAT_DATA_MHAS_CONFIG);
       setCodecSpecificData(track, AMEDIAFORMAT_KEY_CSD_1, FS_FORMAT_DATA_MHAS_SCENEINFO);
       if(info.ulMhapCompatibleSetSize && info.pMhapCompatibleSets)
       {
           AMediaFormat_setBuffer(meta,AMEDIAFORMAT_KEY_MPEGH_COMPATIBLE_SETS,
                                                  info.pMhapCompatibleSets, info.ulMhapCompatibleSetSize);
       }
    }
...
void MMParserExtractor::setVideoTrackMeta(Track* track, FileSourceVideoInfo& info) {
    ...
    //Enable timestamp reorder for AVI and HEVC usecase
    if (FILE_SOURCE_AVI == m_eFileFormat ||
        (FILE_SOURCE_MP2TS == m_eFileFormat && FILE_SOURCE_MN_TYPE_HEVC == minorType)) {
        AMediaFormat_setInt32(meta, "vendor.qti-ext-dec-timestamp-reorder.value", 1);
    }
    ...
    if (FILE_SOURCE_3G2 == m_eFileFormat || FILE_SOURCE_MPEG4 == m_eFileFormat ||
        ...
        if (FILE_SOURCE_MN_TYPE_H264 == minorType ||
            FILE_SOURCE_MN_TYPE_DOLBY_VISION == minorType ||
            FILE_SOURCE_MN_TYPE_HEVC == minorType) {
            bRawCodecData = true;
        }
    }
    if(FILE_SOURCE_MN_TYPE_DOLBY_VISION == minorType)
    {
      if(setCodecSpecificData(track, AMEDIAFORMAT_KEY_CSD_2,
                        FS_FORMAT_DATA_DOLBY_VISION_DVCC))
      {
        const uint8_t* csd;
        size_t csd_size;
        AMediaFormat_getBuffer(track->meta,AMEDIAFORMAT_KEY_CSD_2,(void **)&csd,&csd_size);
        const uint8_t profile = csd[2] >>1;
        if(profile > 1 && profile < 9)
          setCodecSpecificData(track, AMEDIAFORMAT_KEY_CSD_HEVC,
                          FS_FORMAT_DATA_DEFAULT,true);
        else if(profile == 10)
          setCodecSpecificData(track, AMEDIAFORMAT_KEY_CSD_0,
                          FS_FORMAT_DATA_DEFAULT,true);
        else
          setCodecSpecificData(track, AMEDIAFORMAT_KEY_CSD_AVC,
                          FS_FORMAT_DATA_DEFAULT,true);
      }
    }
...
void MMParserExtractor::setCryptoInfo(AMediaFormat* meta, FS_CONTENT_PROTECTION_INFOTYPE *info) {

    if (!info->ucIsEncrypted)
        return;

    int32_t mode = kCryptoModeUnencrypted;
    switch (info->eCryptoMode) {
        case FILE_SOURCE_CRYPTO_MODE_AES_CTR:
            mode = kCryptoModeAesCtr;
            break;
        case FILE_SOURCE_CRYPTO_MODE_AES_CBC:
            mode = kCryptoModeAesCbc;
            break;
        default:
            //Cryto mode might not be set or set to unknown value
            break;
    }

    if (mode != kCryptoModeUnencrypted) {
        AMediaFormat_setInt32(meta, AMEDIAFORMAT_KEY_CRYPTO_MODE, mode);
    }
    AMediaFormat_setBuffer(meta, AMEDIAFORMAT_KEY_CRYPTO_KEY,
                           info->ucKeyID, info->ucKeyIDSize);

    //! If it is patter encryption, either crypt block or skip block will be non-zero
    if (info->ucCryptByteBlock || info->ucSkipByteBlock) {
        AMediaFormat_setInt32(meta, AMEDIAFORMAT_KEY_CRYPTO_ENCRYPTED_BYTE_BLOCK,
            info->ucCryptByteBlock);
        AMediaFormat_setInt32(meta, AMEDIAFORMAT_KEY_CRYPTO_SKIP_BYTE_BLOCK,
            info->ucSkipByteBlock);
    }

    if (info->ucDefaultInitVectorSize) {
        AMediaFormat_setInt32(meta, AMEDIAFORMAT_KEY_CRYPTO_DEFAULT_IV_SIZE,
            info->ucDefaultInitVectorSize);
    }

    if (info->ucInitVectorSize) {
        AMediaFormat_setBuffer(meta, AMEDIAFORMAT_KEY_CRYPTO_IV,
            info->ucInitVector, info->ucInitVectorSize);
    }
}
...
void MMParserExtractor::findThumbnail(Track *track) {
    ...
    int64_t Delta = (duration * 10) / (NUM_SYNC_FRAMES * 100);
    LOGV("Delta value is : %" PRId64, Delta);
    ...
    if (!m_pFileSourceWrapper->IsSeekDenied()) {
        ...
        FileSourceSampleInfo sampleInfo;
        memset(&sampleInfo, 0, sizeof(FileSourceSampleInfo));
        int64_t lastSyncSampleTime_real = 0;

        for (int64_t lastSyncSampleTime = 0, syncSample = 0;
            ...
            //! Never try to seek to negative timestamp
            if (lastSyncSampleTime < 0)
                lastSyncSampleTime = 0;

            //seek to next sync sample
            FileSourceStatus stat = m_pFileSourceWrapper->SeekAbsolutePosition
                    (track->trackId, (uint64)lastSyncSampleTime, true,
                     (uint64)lastSyncSampleTime);
            ...
            lastSyncSampleTime = ((int64_t)sampleInfo.startTime) / 1000 + Delta;
            ...
            if (FILE_SOURCE_DATA_OK != status || bytesRead <= 0){
                ...
                break;
            } else if (lastSyncSampleTime >= 0 &&
                       bytesRead >= maxSyncSampleSize &&
...
MediaTrackHelper* MMParserExtractor::getTrack(size_t index) {
    ...
    //In case of track unselect/select reset back to position 0
    if (track->isPendingReset) {
        m_pFileSourceWrapper->SeekAbsolutePosition(track->trackId, 0, true);
    }
    track->isPendingReset = true;
...
uint32_t MMParserExtractor::flags() const {
    ...
    if (mIsAacformatAdif == true) {
        return (CAN_SEEK_BACKWARD | CAN_PAUSE);
    } else {
...
media_status_t MMParserSource::start() {
    ...
    const uint8_t *key;
    size_t keySize;
    if (AMediaFormat_getBuffer(mFormat, AMEDIAFORMAT_KEY_CRYPTO_KEY,
                               (void**)&key, &keySize)) {
        mIsEncrypted = true;
    }
    ...
    *out = NULL;
    ...
    if (newBuffer && mBuffer) {
        ...
        if (mFileFormat == FILE_SOURCE_MPEG4) {
            ...
            if (sInfo.offset != -1) {
                AMediaFormat_setInt64(meta, "sample-file-offset", sInfo.offset);
            }
            if (sInfo.last_sample_index_in_chunk != -1) {
                AMediaFormat_setInt64(meta, "last-sample-index-in-chunk",
                                      sInfo.last_sample_index_in_chunk);
            }
        }
        ...
        if (mIsEncrypted) {
            setSampleEncryptionMetaData(meta, sInfo);
        }
...
{
    if (mFileFormat == FILE_SOURCE_MPEG4 ||
        mFileFormat == FILE_SOURCE_3G2)
      return true;
    return false;
}

void MMParserSource::setSampleEncryptionMetaData(AMediaFormat *meta, FileSourceSampleInfo &info)
{
    FS_CONTENT_PROTECTION_INFOTYPE *pCP = &info.sSubInfo.sCP;
    if (!pCP->ulEncrSubSampleCount)
        return;

    size_t dataSize = pCP->ulEncrSubSampleCount;
    uint32_t *data = new uint32_t[dataSize];
    if (!data)
        return;

    mMMParserExtractor->setCryptoInfo(meta, pCP);

    for (int i = 0; i < dataSize; i++) {
        data[i] = pCP->sEncSubsampleInfo[i].ulSizeOfClearData;
    }
    AMediaFormat_setBuffer(meta, AMEDIAFORMAT_KEY_CRYPTO_PLAIN_SIZES,
                           data, sizeof(uint32_t) * dataSize);

    for (int i = 0; i < dataSize; i++) {
        data[i] = pCP->sEncSubsampleInfo[i].ulSizeOfEncryptedData;
    }

/* Copyright (c) 2014-2019, 2021-2022 Qualcomm Technologies, Inc.
 * All Rights Reserved.
...
static void onMediaEvent(JNIEnv *e, int32_t eventId, int32_t mediaID) {
    if(e == NULL) {
    ...
    jmethodID mid = e->GetMethodID(gClassImsMedia, "onMediaEvent", "(II)V");
    if (mid == 0) {
        ALOGE("videocall callback: GetMethodID error");
        return;
    }

    e->CallVoidMethod(imsMediaObject, mid, (jint) eventId, (jint) mediaID);
}
...
static void onMediaEvent2(int32_t event, int32_t mediaID) {
    ...
    switch(event) {
        case DATA_USAGE_EVT:
            ...
            break;
        default:
            onMediaEvent(env, event, mediaID);
    }
}
...
static jboolean dpl_haveSameParent(JNIEnv *e, jobject, jobject jSurfaceOne, jobject jSurfaceTwo) {
    ALOGD("%s", __func__);
    return android_view_Surface_haveSameParent(e, jSurfaceOne, jSurfaceTwo) ? JNI_TRUE : JNI_FALSE;
}

static jint dpl_setSharedDisplayParameters(JNIEnv *, jobject, jint width, jint height) {
    ALOGD("%s width = %d height = %d", __func__, width, height);
    jint ret = -1;
    if (imsMediaApis) {
...
static jint dpl_getSurfaceWidth(JNIEnv *e, jobject, jobject jSurface) {
    ALOGD("%s", __func__);
    return static_cast<jint>(android_view_Surface_getSurfaceWidth(e,jSurface));
}

static jint dpl_getSurfaceHeight(JNIEnv *e, jobject, jobject jSurface) {
    ALOGD("%s", __func__);

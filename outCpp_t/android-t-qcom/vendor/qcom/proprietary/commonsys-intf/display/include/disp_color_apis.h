...
enum DISP_APIS_PUBLIC {
  ...
  DISP_APIS_GET_DISPLAY_IDS,
  DISP_APIS_SET_GLOBAL_SIXZONE_THRESHOLDS,
  DISP_APIS_SET_GLOBAL_SIXZONE_CONFIG,
  DISP_APIS_SET_GLOBAL_SIXZONE_PROTECTION,
  DISP_APIS_GET_NUM_RENDER_INTENTS,
  DISP_APIS_GET_RENDER_INTENTS,
  DISP_APIS_GET_GLOBAL_3D_LUT_CONFIG,
  DISP_APIS_SET_GLOBAL_3D_LUT_CONFIG,
  DISP_APIS_SET_GLOBAL_DITHER_STRENGTH_CONFIG,
  DISP_APIS_GET_GLOBAL_DITHER_STRENGTH_CONFIG,
...
#define LUT3D_ENTRIES_COARSE_13_SIZE (13 * 13 * 13)
...
#define DISP_IGC_FLAGS_DITHER_EN (1 << 0)
...
#define DISP_PCC_FLAGS_POS_BEFORE 0x1

/*
struct pcc_coeff_data -- structure for providing per color component coefficients
  flags -- Used to modify PCC behavior. Use DISP_PCC_FLAGS definitions
...
enum {
    ...
    DISPLAY_EXTERNAL_2  = 2,
    DISPLAY_EXTERNAL_3  = 3,
    DISPLAY_EXTERNAL_4  = 4,
...
struct disp_id_config -- structure for storing display IDs
  flags -- Reserved
  disp_id  -- ID for each display
*/
struct disp_id_config {
  uint32_t flags;
  uint64_t disp_id[NUM_DISPLAY_TYPES];
};
...
struct disp_six_zone_threshold {
  ...
  struct disp_range sat_low;
  struct disp_range sat_mid;
  struct disp_range sat_high;
  bool sv_enable;
};

/*
struct disp_six_zone_config_sat_data -- structure for storing six zone data
saturation_mid -- saturation medium setting in units degrees
saturation_high -- Saturation high in units of percentage
brightness -- Brightness setting in units of percentage
*/
struct disp_six_zone_config_sat_data {
...
hue        -- Hue setting in units degrees
saturation -- Saturation low setting in units of percentage
brightness -- Brightness setting in units of percentage
*/
struct disp_six_zone_config_data {
    ...
    double saturation;
    double value;
    struct disp_six_zone_config_sat_data sat_entries;
};
...
struct disp_render_intent {
  uint32_t render_intent;
  char render_string[MAX_STRING_LENGTH];
};
...
struct lut3d_info_coarse {
  ...
  uint32_t uniform;
  uint32_t num_entries;
  struct rgb_entry entries[LUT3D_ENTRIES_COARSE_13_SIZE];
};

/*
Display dither codff data struct
*/
#define DISP_DITHER_FLAGS_LUMA_MODE 0x1
...
/*=========================================================================
FUNCTION
  disp_api_get_display_id()

DESCRIPTION
  This API retrieves display IDs.

  hctx -- Context handle.
  cfg -- display id config, including display id array.

RETURN VALUE
  ZERO (SUCCESS)- success.
  Negative Value - error\failure
=========================================================================*/
int32_t disp_api_get_display_id(DISPAPI_HANDLE hctx, struct disp_id_config *cfg);
...
/*=========================================================================
FUNCTION
  disp_api_get_num_render_intents()

DESCRIPTION
  This API retrieves the number of render intents for the display specified by ID.

  disp_id -- Display ID.
  num -- number of render intents

RETURN VALUE
  ZERO (SUCCESS)- success.
  Negative Value - error\failure
=========================================================================*/
int32_t disp_api_get_num_render_intents(uint32_t disp_id, uint32_t *num);

/*=========================================================================
FUNCTION
  disp_api_get_render_intents()

DESCRIPTION
  This API retrieves render intents for the display specified by ID.

  disp_id -- Display ID.
  num -- number of render intents
  list -- render intents list

RETURN VALUE
  ZERO (SUCCESS)- success.
  Negative Value - error\failure
=========================================================================*/
int32_t disp_api_get_render_intents(uint32_t disp_id, uint32_t num,
                                    struct disp_render_intent *list);
...
RETURN VALUE
  ...
  Negative Value - error\failure
=========================================================================*/
int32_t disp_api_get_global_3dlut_config(DISPAPI_HANDLE hctx, uint32_t disp_id,
                                         uint32_t *enable, struct lut3d_info *cfg,
...
RETURN VALUE
  ...
  Negative Value - error\failure
=========================================================================*/
int32_t disp_api_set_global_3dlut_config(DISPAPI_HANDLE hctx, uint32_t disp_id,
                                         uint32_t enable, struct lut3d_info *cfg,
                                         struct lut3d_info_coarse *cfg_coarse);

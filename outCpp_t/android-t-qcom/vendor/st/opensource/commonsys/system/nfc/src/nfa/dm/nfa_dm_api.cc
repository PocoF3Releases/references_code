...
tNFA_STATUS NFA_Disable(bool graceful) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; (graceful=%i)", __func__, graceful);
...
**
** Description
**                  If deact_type=NFA_DEACTIVATE_TYPE_LPTD:
**                      Deselect the activated device by deactivating into LPTD
**                      Pres check mode.
**
**
** Returns          NFA_STATUS_OK if successfully initiated
...
**
*******************************************************************************/
extern tNFA_STATUS NFA_DeactivateExt(tNFA_DEACTIVATE_TYPE deact_type) {
  tNFA_DM_API_DEACTIVATE* p_msg;

  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; deact_type:%i", __func__, deact_type);
  ...
  if (p_msg != nullptr) {
    p_msg->hdr.event = NFA_DM_API_DEACTIVATE_EVT;
    p_msg->lptd_mode = false;
    p_msg->sleep_mode = false;

    switch (deact_type) {
      // case NFA_DEACTIVATE_TYPE_IDLE:
      //   break;
      // case NFA_DEACTIVATE_TYPE_DISCOVERY:
      //   break;
      case NFA_DEACTIVATE_TYPE_SLEEP:
        p_msg->sleep_mode = true;
        break;
      case NFA_DEACTIVATE_TYPE_LPTD:
        p_msg->lptd_mode = true;
        break;
      default:
        DLOG_IF(ERROR, nfc_debug_enabled) << StringPrintf(
            "%s; deact_type:%i not yet supported", __func__, deact_type);
    }

...
static void add_route_tech_proto_tlv(uint8_t** pp, uint8_t tlv_type,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; nfcee_id: 0x%x, pwr_cfg: 0x%x, tech_proto: 0x%x",
                      __func__, nfcee_id, pwr_cfg, tech_proto);
...
static void add_route_sys_code_tlv(uint8_t** p_buff, uint8_t* p_sys_code_cfg,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; nfcee_id: 0x%x, pwr_cfg: 0x%x", __func__,
...
#ifdef ST21NFC
    // NFC-DEP will be handled elsewhere
    if (power_cfg) {
...
static void nfa_ee_update_route_aid_size(tNFA_EE_ECB* p_cb) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; nfcee_id:0x%x size_aid:%d", __func__, p_cb->nfcee_id,
                      p_cb->size_aid);
}
...
static void nfa_ee_update_route_sys_code_size(tNFA_EE_ECB* p_cb) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; nfcee_id:0x%x size_sys_code:%d", __func__,
  ...
  for (int xx = 0; xx < NFA_EE_NUM_TECH; xx++) {
    ...
    if (power_cfg) {
      ...
      if (block && ((((mask & NFA_TECHNOLOGY_MASK_A) == 0) &&
        ...
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s; Blocking tech routing for tech: %d", __func__,
                            nfa_ee_tech_list[xx]);
...
static void nfa_ee_add_aid_route_to_ecb(tNFA_EE_ECB* p_cb, uint8_t* pp,
  ...
  if (p_cb->aid_entries) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s; Added %d AIDs for NFCEE 0x%x", __func__,
    ...
    for (int xx = 0; xx < p_cb->sys_code_cfg_entries; xx++) {
      ...
      if (p_cb->sys_code_rt_loc_vs_info[xx] & NFA_EE_AE_ROUTE) {
        ...
        if (start_offset >
          ...
          LOG(ERROR) << StringPrintf("%s; start_offset higer than 2", __func__);
          return;
        }
#endif
...
static void nfa_ee_conn_cback(uint8_t conn_id, tNFC_CONN_EVT event,
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s; conn_id: %d, event=0x%02x", __func__, conn_id, event);
...
void nfa_ee_api_add_aid(tNFA_EE_MSG* p_data) {
  ...
  if (p_chk_cb) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
        "%s; The AID entry is already in the database", __func__);
    if (p_chk_cb == p_cb) {
...
void nfa_ee_api_remove_aid(tNFA_EE_MSG* p_data) {
  ...
  if (p_cb && p_cb->aid_entries) {
    ...
    if ((entry + 1) < p_cb->aid_entries) {
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s; len:%d, rest_len:%d", __func__, len, rest_len);
    ...
    // Clear content of NFA_EE_CB_4_DH entry
    p_ecb = &nfa_ee_cb.ecb[NFA_EE_EMPTY_AID_ECB];
    memset(&p_ecb->aid_cfg[0], 0x00, sizeof(p_ecb->aid_cfg));
    memset(&p_ecb->aid_len[0], 0x00, max_aid_entries);
    memset(&p_ecb->aid_pwr_cfg[0], 0x00, max_aid_entries);
    memset(&p_ecb->aid_rt_info[0], 0x00, max_aid_entries);
...
void nfa_ee_api_remove_sys_code(tNFA_EE_MSG* p_data) {
  ...
  if (p_cb && p_cb->sys_code_cfg_entries) {
    ...
    if ((entry + 1) < p_cb->sys_code_cfg_entries) {
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s; rm_len:%d, total_len:%d", __func__, rm_len, total_len);
    ...
    LOG(WARNING) << StringPrintf("%s; The SC entry is not in the database",
                                 __func__);
    evt_data.status = NFA_STATUS_INVALID_PARAM;
  }
...
void nfa_ee_api_lmrt_size(__attribute__((unused)) tNFA_EE_MSG* p_data) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; total size:%d remaining size:%d", __func__,
...
void nfa_ee_api_force_routing(tNFA_EE_MSG* p_data) {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s; nfcee_id = 0x%x, config = 0x%x", __func__,
...
bool nfa_ee_get_forced_routing_status() {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; state:%d", __func__, state);
  return state;
}
#endif
...
void nfa_ee_nci_disc_ntf(tNFA_EE_MSG* p_data) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; cur_ee:%d", __func__, nfa_ee_cb.cur_ee);
...
#endif
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; handle:0x%02x status:%d", __func__, p_rsp->nfcee_id,
                      p_rsp->nfcee_status);
...
void nfa_ee_report_discover_req_evt(void) {
  ...
  if (!nfa_dm_is_active()) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s; DM is not active", __func__);
    return;
  }
  ...
  if (p_rsp->status == NFA_STATUS_OK) {
    if (p_rsp->mode == NFA_EE_MD_ACTIVATE) {
      ...
      if (p_cb->tech_switch_on | p_cb->tech_switch_off |
        ...
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s; nfcee_id: 0x%x, deactivating/still configured. Force update",
            __func__, p_cb->nfcee_id);
#ifdef ST21NFC
        if (p_cb->nfcee_id == 0x10) {
          nfa_ee_report_discover_req_evt();
        }
#endif
      }
...
void nfa_ee_nci_force_routing_rsp(tNFA_EE_MSG* p_data) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; status:%d", __func__, p_rsp->status);
...
void nfa_ee_get_tech_route(uint8_t power_state, uint8_t* p_handles) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s; power_state: 0x%x - [A]: 0x%x, [B]: 0x%x, [F]: 0x%x, [B']: 0x%x",
      __func__, power_state, p_handles[0], p_handles[1], p_handles[2],
      p_handles[3]);
}
...
void nfa_ee_route_add_one_ecb_by_route_order(tNFA_EE_ECB* p_cb, int rout_type,
  ...
  if (more == false) {
    ...
    if (nfa_ee_cb.ee_cfg_sts & NFA_EE_STS_CHANGED_ROUTING) {
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s; nb entries:%d, tlv_size:%d", __func__, num_tlv, tlv_size);
  ...
  for (xx = 0; xx < nfa_ee_cb.cur_ee; xx++, p_cb--) {
    if (p_cb->ee_status == NFC_NFCEE_STATUS_ACTIVE) {
      if (last_active == NFA_EE_INVALID) {
        ...
        DLOG_IF(INFO, nfc_debug_enabled)
            << StringPrintf("%s; active NFCEE: 0x%x", __func__, last_active);
      }
    }
  }
...
void nfa_ee_update_rout(void) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; ee_cfg_sts:0x%02x", __func__, nfa_ee_cb.ee_cfg_sts);
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; ee_cfg_sts:0x%02x ee_cfged:0x%02x", __func__,

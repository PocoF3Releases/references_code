...
void NfcAdaptation::Initialize() {
...
#ifdef ST21NFC
  LOG(INFO) << StringPrintf("%s; ver=%s nfa=%s st=120-20220402-22W12p1",
  ...
  if (NfcConfig::hasKey(NAME_NFA_POLL_BAIL_OUT_MODE)) {
    ...
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s; Overriding NFA_POLL_BAIL_OUT_MODE to use %d",
                        __func__, nfa_poll_bail_out_mode);
  }
...
void NfcAdaptation::InitializeHalDeviceContext() {
...
#ifndef VENDOR_BUILD_FOR_FACTORY
  LOG(INFO) << StringPrintf("%s; INfc::getService()", __func__);
  ...
  LOG(INFO) << StringPrintf("%s; INfc::getService() returned %p (%s)", __func__,
                            mHal.get(),
...
#else
    LOG(INFO) << StringPrintf("%s; Direct access to HAL", __func__);
#endif
}
...
void NfcAdaptation::HalDeviceContextCallback(nfc_event_t event,
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; event=%u", __func__, event);
#ifndef VENDOR_BUILD_FOR_FACTORY
  if (mHalCallback) mHalCallback(event, (tHAL_NFC_STATUS)event_status);
#else
    // not used
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s; event_status=%u", __func__, event_status);
#endif
}
...
void NfcAdaptation::HalDeviceContextDataCallback(uint16_t data_len,
                                                 uint8_t* p_data) {
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s; len=%u", __func__, data_len);
#ifndef VENDOR_BUILD_FOR_FACTORY
  if (mHalDataCallback) mHalDataCallback(data_len, p_data);
#else
    // not used
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s; p_data=%p", __func__, p_data);
#endif
}

...
bool nfcManager_SetMuteTech(JNIEnv* e, jobject o, jboolean muteA,
                            jboolean muteB, jboolean muteF, jbyte mutePollTechMask,
...
/*******************************************************************************
**
** Function:        p2p_prio_logic_multiprotocol
**
** Description:
**
** Returns:         None
...
void reconfigure_poll_cb(union sigval) {
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s; Prio_Logic_multiprotocol timer expire, CallBack Reconfiguring the "
      "POLL to Default",
      __func__);
  clear_multiprotocol();
...
void clear_multiprotocol() {
  ...
  if (thread_ret != 0)
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s; unable to create the thread", __func__);
  pthread_attr_destroy(&attr);
}
...
static void nfcManager_enableActionNtf(JNIEnv* e, jobject o, jboolean enabled) {
  ...
  NfcStExtensions::getInstance().StActionNtfEnable(enabled);
}
...
**
** Description:     listen and polling mode configuration
**                  e: JVM environment.
**                  o: Java object.
**
** Returns:         None.
...
bool nfcManager_SetMuteTech(JNIEnv* e, jobject o, jboolean muteA,
  ...
  bool wasStopped = false;
  LOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("Enter :%s  a:%d b:%d f:%d mutePollTechMask:0x%02X, isCommitNeeded: %d", __func__,
                      muteA, muteB, muteF, mutePollTechMask, isCommitNeeded);

  jint newMask = (muteA ? ST_CE_MUTE_A : 0) | (muteB ? ST_CE_MUTE_B : 0) |
                 (muteF ? ST_CE_MUTE_F : 0);
  newMask |= (mutePollTechMask << 8);
  // if all techs would me mute, we just stop the discovery
  if (muteA && muteB && muteF &&
      ((mutePollTechMask & 0x0F) == 0x0F)) {
  ...
  if (sWalletTechIsMute != newMask) {
    ...
    if ((newMask & ST_CE_MUTE_DISCOVERY) == 0) {
      NFA_SetMuteTech(muteA, muteB, muteF, mutePollTechMask);
  ...
  gIsReconfiguringDiscovery.end();
  LOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "%s; success, sWalletTechIsMute = 0x%04X", __func__, sWalletTechIsMute);
  return true;
}
...
static bool nfcManager_MuteAllTech(JNIEnv* e, jobject o, jboolean doMute) {
  ...
  if (sWalletTechIsMute != newMask) {
    ...
    if ((newMask & ST_CE_MUTE_DISCOVERY) == 0) {
      if (doMute == true) {
        NFA_SetMuteTech(true, true, true, 0x00);
      } else {
        NFA_SetMuteTech(false, false, false, 0x0F);
      }
...
static void nfcManager_enablePollingLoopSpy(JNIEnv* e, jobject o,
                                            jboolean enabled) {
  NfcStExtensions::getInstance().stPollingLoopSpyManagerEnable(enabled);
}
...
static JNINativeMethod gMethods[] = {
    ...
    {"doSetMuteTech", "(ZZZBZ)Z", (void*)nfcManager_SetMuteTech},

...
jmethodID gCachedNfcManagerNotifyException;
static void nfaReporterCallback(std::string value);
...
static void nfaConnectionCallback(uint8_t connEvent,
  ...
  switch (connEvent) {
    ...
    case NFA_DEACTIVATED_EVT:  // NFC link/protocol deactivated
      ...
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("gIsSelectingRfInterface %d",
                                          gIsSelectingRfInterface);
      if(!gIsSelectingRfInterface && !gIsTagDeactivating) {
...
static jboolean nfcManager_initNativeStruc(JNIEnv* e, jobject o) {
  ...
  gCachedNfcManagerNotifyException =
      e->GetMethodID(cls.get(), "notifyException", "(Ljava/lang/String;)V");
...
if (!sP2pActive && eventData->rf_field.status == NFA_STATUS_OK) {
    ...
    case NFA_DM_NFCC_TIMEOUT_EVT: {
      ...
      nfaReporterCallback(NAME_CMD_TIEMOUT);
      abort();
      //////////////////////////////////////////////
    } break;
...
static void nfaReporterCallback(std::string value) {
  ...
  e->CallVoidMethod(nat->manager,
                        android::gCachedNfcManagerNotifyException,srcJavaString.get());
  if (e->ExceptionCheck()) {
    e->ExceptionClear();
...
static jboolean nfcManager_doInitialize(JNIEnv* e, jobject o) {
...

  {
...

    {
      ...
      NFA_registExceptionReporter(nfaReporterCallback);
    }

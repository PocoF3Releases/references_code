...
bool RoutingManager::setRoutingEntry(int type, int value, int route,
  ...
  if (NFA_SET_TECHNOLOGY_ROUTING == type) {
    ...
    if(!isSeIDPresent) {
      ee_handle = 0x400;
      power &= 0x11;
      max_tech_mask = mHostListnTechMask;
    }
    while (value) {
      if (value & 0x01) {
...
void RoutingManager::nfaEeCallback(tNFA_EE_EVT event,
  ...
  switch (event) {
    ...
    case NFA_EE_ACTION_EVT: {
      ...
      //add uicc action recorder
      if ((action.ee_handle != 0x4C0) && (action.ee_handle != 0x400)) {
        routingManager.notifyUiccAction();
      }
...
//add for uicc action recorder
void RoutingManager::notifyUiccAction() {
  JNIEnv* e = NULL;
  ScopedAttach attach(mNativeData->vm, &e);
  if (e == NULL) {
    LOG(ERROR) << StringPrintf("jni env is null");
  ...
  e->CallVoidMethod(mNativeData->manager,
                    android::gCachedNfcManagerRecordUiccAction);
  if (e->ExceptionCheck()) {
    e->ExceptionClear();
    LOG(ERROR) << StringPrintf("fail notify");
  }

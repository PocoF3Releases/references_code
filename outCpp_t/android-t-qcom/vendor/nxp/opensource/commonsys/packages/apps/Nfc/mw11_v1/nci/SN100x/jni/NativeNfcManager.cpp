...
#if (NXP_EXTNS == TRUE)
  LOG(INFO)
    << StringPrintf("#11.59.00 # %s: notifyNfcEvent(nfaDeviceManagementCallback)", __func__);
  NativeJniExtns::getInstance().notifyNfcEvent(
...
static void nfcManager_configNfccConfigControl(bool flag) {
  ...
  if (NFC_GetNCIVersion() != NCI_VERSION_1_0) {
    ...
    LOG(INFO) << StringPrintf(
      "#11.59.00# %s: waiting for sNfaSetConfigEvent", __func__);
    if (status == NFA_STATUS_OK)
      ...
      LOG(ERROR) << StringPrintf("%s:Failed to configure NFCC_CONFIG_CONTROL",
                                 __func__);
    LOG(INFO) << StringPrintf(
      "#11.59.00# %s: sNfaSetConfigEvent done", __func__);
#else
    if (status != NFA_STATUS_OK) {
...
void startRfDiscovery(bool isStart) {
  ...
  if (status == NFA_STATUS_OK) {
    ...
    if (isStart == false && SecureElement::getInstance().mRfFieldIsOn == true) {
      LOG(INFO) << StringPrintf(
        "#11.59.00# %s: set mRfFieldIsOn to false", __func__);

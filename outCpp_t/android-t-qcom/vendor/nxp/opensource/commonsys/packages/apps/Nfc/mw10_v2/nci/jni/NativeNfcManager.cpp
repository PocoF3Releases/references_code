...
jmethodID gCachedNfcManagerRecordUiccAction;
...
void initializeMfcReaderOption() {
  legacy_mfc_reader =
      (NfcConfig::getUnsigned(NAME_LEGACY_MIFARE_READER, 1) != 0) ? true
                                                                  : false;
...
/*JCOP OS Download protection case to be handled*/
  ...
  nfc_jni_native_data* nat =
    ...
    //add for uicc action recorder
    gCachedNfcManagerRecordUiccAction =
        e->GetMethodID(cls.get(), "notifyUiccAction", "()V");

...
void NfcTag::discoverTechnologies(tNFA_DISC_RESULT& discoveryData) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s; mNumDiscTechList=%x", fn, mNumDiscTechList);
#endif
...
void NfcTag::createNativeNfcTag(tNFA_ACTIVATED& activationData) {
  ...
  if(isNfcCombiCard() || !mNumDiscNtf || NfcTag::getInstance().checkNextValidProtocol() == -1) {
    ...
    e->CallVoidMethod(mNativeData->manager, android::gCachedNfcManagerNotifyNdefMessageListeners,
                    tag.get());
#else
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: try notify nfc service", fn);
  ...
  e->CallVoidMethod(mNativeData->manager,
    ...
    DLOG_IF(ERROR, nfc_debug_enabled) << StringPrintf("%s: Selecting next tag", fn);
  }
#endif
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: exit", fn);
}
...
bool NfcTag::isNfcCombiCard() {
  if ((android::nfcManager_isReaderModeEnabled() &&
    isP2pDetected && mNumDiscNtf == 1)) {
    isP2pDetected = false;
    return true;
  } else
    return false;
...
void NfcTag::fillNativeNfcTagMembers4(JNIEnv* e, jclass tag_cls, jobject tag,
  ...
  if(activationData.activate_ntf.protocol & (NFC_PROTOCOL_T1T|NFC_PROTOCOL_T2T|NFC_PROTOCOL_MIFARE|NFC_PROTOCOL_ISO_DEP)) {
    uint8_t merge_sak = 0;
    for (int i = 0; i < mNumTechList; i++) {
...
void NfcTag::fillNativeNfcTagMembers5(JNIEnv* e, jclass tag_cls, jobject tag,
  ...
  if(!mNumDiscNtf)
    mTechListIndex = 0;
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s;mTechListIndex=%x",fn,mTechListIndex);
#endif
}
...
**
** Description:     When reselect intf with current tag, just select it
*to activate.
**
** Returns:         None
**
*******************************************************************************/
void NfcTag::selectCurrentTag() {
  tNFA_INTF_TYPE rf_intf = NFA_INTERFACE_FRAME;
  int id = selectedId;
  int requestedProtocol = mCurrentRequestedProtocol;

  LOG(ERROR) << StringPrintf("%s: sLastSelectedTagId: %d", __func__, id);

  if(id >= mNumDiscTechList || id < 0) {
    LOG(ERROR) << StringPrintf("%s:TagId invalid: %d, select first one, return",
      __func__, id);
    selectFirstTag();
    return;
  }

  if(requestedProtocol != NFC_PROTOCOL_UNKNOWN &&
      mTechLibNfcTypes[id] != requestedProtocol) {
    LOG(ERROR)<<StringPrintf("%s: requestedProtocol:%d, mTechLibNfcTypes[id]:%d",
        __func__, requestedProtocol, mTechLibNfcTypes[id]);

    for (int i = 0; i < mNumDiscTechList; i++) {
      if (mTechLibNfcTypes[i] == requestedProtocol) {
        LOG(ERROR) << StringPrintf("%s: NewTagId: %d", __func__, i);
        id = i;
        break;
      }
    }
  ...
  if (mTechLibNfcTypes[id] == NFA_PROTOCOL_ISO_DEP) {
    rf_intf = NFA_INTERFACE_ISO_DEP;
  } else if (mTechLibNfcTypes[id] == NFC_PROTOCOL_MIFARE) {
    rf_intf = NFA_INTERFACE_MIFARE;
  } else
    rf_intf = NFA_INTERFACE_FRAME;

  tNFA_STATUS stat = NFA_Select(mTechHandles[id],
    mTechLibNfcTypes[id], rf_intf);

  if (stat != NFA_STATUS_OK)
    LOG(ERROR) << StringPrintf("%s: fail select; error=0x%X", __func__, stat);
}
...
int NfcTag::checkNextValidProtocol(void) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter, mNumDiscTechList=%x",
          fn, mNumDiscTechList);
  int foundIdx = -1;
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter,selectedId=%x",
          fn, selectedId);
  for (int i = 0; i < mNumDiscTechList; i++) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
            "%s: nfa target idx=%d h=0x%X; protocol=0x%X", fn, i, mTechHandles [i],
            mTechLibNfcTypes [i]);
...
void NfcTag::selectNextTag () {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: enter, mNumDiscTechList=%x",
          fn, mNumDiscTechList);
  for (int i = 0; i < mNumDiscTechList; i++) {
    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: nfa target idx=%dh=0x%X; protocol=0x%X",
  ...
  if (foundIdx != -1) {
    ...
    if (stat == NFA_STATUS_OK) {
      DLOG_IF(ERROR, nfc_debug_enabled) << StringPrintf("%s: stat=%x; wait for activated ntf", fn, stat);
    } else {
      DLOG_IF(ERROR, nfc_debug_enabled) << StringPrintf("%s: fail select; error=0x%X", fn, stat);
    }
  } else {
    DLOG_IF(ERROR, nfc_debug_enabled) << StringPrintf("%s: only found NFC-DEP technology.", fn);
  }
}
#endif

...
#endif
jmethodID gCachedNfcManagerNotifyRfPhase;
//add for bigdata collection
#define NFC_BIGDATA_TYPE_EXCEPTION 1
#define NFC_BIGDATA_TYPE_RFPHASE 2
jmethodID gCachedNfcManagerReportEEHandle;
jmethodID gCachedNfcManagerNotifyBigData;
static void nfaReporterCallback(int type, std::string value);
...
static jint sMipayUsing = 0; // 1,when mipay changing discovery by interface.
...
static bool prevMutePollingConfigState = false;
#else
...
/*******************************************************************************
 * **
 * ** Function:        isFeliCaSupport
 * **
 * ** Description:     FeliCa Support for JP build
 * **
 * ** Returns:         support FeliCa or Not
 * **
 * *******************************************************************************/
//FeliCa Adaptation
#define ESE_ROUTE_ID (0x01)
int nfcDefaultRoute = ESE_ROUTE_ID;

static bool isFeliCaSupport() {
  static int isSupport = -1;
  if(isSupport == -1) {
    std::string prop("ro.miui.build.region");
    char valueStr[PROPERTY_VALUE_MAX] = {0};

    property_get(prop.data(), valueStr, "");
    isSupport = strcmp(valueStr, "jp") == 0 ? 1 : 0;
        DLOG_IF(INFO, nfc_debug_enabled)
    << StringPrintf("%s:%s = %d", __func__, valueStr, isSupport);
  }
  return isSupport > 0;
}
/*******************************************************************************
...
static void nfaConnectionCallback(uint8_t connEvent,
  ...
  switch (connEvent) {
    ...
    case NFA_DEACTIVATED_EVT:  // NFC link/protocol deactivated
      ...
      if(gIsWaiting4Deact2SleepNtf) {
        if(eventData->deactivated.type == NFA_DEACTIVATE_TYPE_IDLE) {
          ...
          gIsWaiting4Deact2SleepNtf = false;
        } else if(eventData->deactivated.type == NFA_DEACTIVATE_TYPE_DISCOVERY) {
          gGotDeact2DiscNtf = true;
        }
      }
#endif
    ...
    case NFA_PRESENCE_CHECK_EVT:
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("%s: NFA_PRESENCE_CHECK_EVT, status:%d", __func__,eventData->status);
...
static jboolean nfcManager_initNativeStruc(JNIEnv* e, jobject o) {
  ...
  gCachedNfcManagerNotifyTransactionListeners = e->GetMethodID(
    ...
    gCachedNfcManagerNotifyRfPhase =
      e->GetMethodID(cls.get(), "notifyRfPhase", "(I)V");
  //add for bigdata collection
  gCachedNfcManagerReportEEHandle =
      e->GetMethodID(cls.get(), "reportEEHandle", "(I)V");
  gCachedNfcManagerNotifyBigData =
      e->GetMethodID(cls.get(), "notifyNfcBigData", "(ILjava/lang/String;)V");
...
if (!sP2pActive && eventData->rf_field.status == NFA_STATUS_OK) {
    ...
    case NFA_DM_NFCC_TIMEOUT_EVT: {
      ...
      if (recovery_option) {
        ...
        nfaReporterCallback(NFC_BIGDATA_TYPE_EXCEPTION, NAME_CMD_TIEMOUT);
    ...
    case NFA_DM_RF_PHASE_EVT:{
      DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
          "%s: NFA_DM_RF_PHASE_EVT; status=0x%X; phase status=%u", __func__,
          eventData->rf_phase.status, eventData->rf_phase.rf_phase_status);

      if (eventData->rf_phase.status == NFA_STATUS_OK) {
        struct nfc_jni_native_data* nat = getNative(NULL, NULL);
        JNIEnv* e = NULL;
        ScopedAttach attach(nat->vm, &e);
        if (e == NULL) {
        ...
        e->CallVoidMethod(nat->manager,
                        android::gCachedNfcManagerNotifyRfPhase,
                        eventData->rf_phase.rf_phase_status);
      }
    } break;
    default:
      DLOG_IF(INFO, nfc_debug_enabled)
...
static void nfaReporterCallback(int type, std::string value) {
  ...
  ScopedAttach attach(nat->vm, &e);
  if (e == NULL) {
    LOG(ERROR) << StringPrintf("jni env is null for nfaReporterCallback");
    return;
  }
  ScopedLocalRef<jobject> srcJavaString(e, e->NewStringUTF(value.c_str()));
  e->CallVoidMethod(nat->manager,
                        android::gCachedNfcManagerNotifyBigData, type, srcJavaString.get());
  if (e->ExceptionCheck()) {
    e->ExceptionClear();
    LOG(ERROR) << "fail notify for nfaReporterCallback";
  }
}
...
static jboolean nfcManager_doInitialize(JNIEnv* e, jobject o) {
...

  {
...

    {
      ...
      NFA_registerBigDataReporter(nfaReporterCallback);
    }
...
#if (NXP_EXTNS == TRUE)
#define SCREEN_MUTE_POLLING_CONIFG_MASK 0x100
  bool mutePollingConfigState = (screen_state_mask & SCREEN_MUTE_POLLING_CONIFG_MASK);
  LOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("mutePollingConfigState=%d,"
      "screen_state_mask=%d", mutePollingConfigState, screen_state_mask);
#endif
...
#if (NXP_EXTNS == TRUE)
  if(prevScreenState == state && prevMutePollingConfigState == mutePollingConfigState) {
    LOG_IF(INFO, nfc_debug_enabled)<< StringPrintf("Screen state is not changed.");
    return;
  }
  scrnOnLockedPollDisabled = false;
  prevMutePollingConfigState = mutePollingConfigState;
  ...
  if (state == NFA_SCREEN_STATE_OFF_LOCKED ||
    ...
    discovry_param =
       (isFeliCaSupport() && nfcDefaultRoute == ESE_ROUTE_ID) ?
        // DH-NFCEE considered as disabled 0x02
  ...
  if ((state > NFA_SCREEN_STATE_UNKNOWN &&
      ...
      (prevScreenState == NFA_SCREEN_STATE_ON_UNLOCKED ||
       prevScreenState == NFA_SCREEN_STATE_ON_LOCKED) &&
      (!sP2pActive) && (!sSeRfActive) && (!SecureElement::getInstance().isRfFieldOn())) {
...
/*******************************************************************************
 **
 ** Function:        nfcManager_discoveryChangingMode
 **
 ** Description:     enable changing discovery flag for mipay
 **
 ** Returns:         void
 **
 *******************************************************************************/
static void nfcManager_discoveryChangingMode(JNIEnv*, jobject,
                                                 jint flag) {
  sMipayUsing = flag;
  DLOG_IF(INFO, true) << StringPrintf("nfcManager_discoveryChangingMode,sMipayUsing = %d", sMipayUsing);
  return;
}
...
static JNINativeMethod gMethods[] = {
    ...
    {"discoveryChangingMode", "(I)V",
                (void*)nfcManager_discoveryChangingMode},
};

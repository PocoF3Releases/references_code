...
bool RoutingManager::addAidRouting(const uint8_t* aid, uint8_t aidLen,
  ...
  if (!mSecureNfcEnabled) {
    if ((aid == nullptr) && (aidLen == 0x00)) {
      ...
      // if (route == SecureElement::DH_ID) power = HOST_PWR_STATE; // set by app self
      powerState = power;
    }
  }
  tNFA_STATUS nfaStat =
...
void RoutingManager::notifyActivated(uint8_t technology) {
  static const char fn[] = "RoutingManager:notifyactivated";
  JNIEnv* e = NULL;
  LOG(INFO) << StringPrintf("%s: tech=0x%X", fn, technology);
...
void RoutingManager::notifyDeactivated(uint8_t technology) {
  static const char fn[] = "RoutingManager:notifyDeactivated";
  LOG(INFO) << StringPrintf("%s: tech=0x%X", fn, technology);
#if (NXP_EXTNS == TRUE)
...
bool RoutingManager::setRoutingEntry(int type, int value, int route, int power)
{
    ...
    LOG(INFO) << StringPrintf("%s: enter, >>>>>>>> type:0x%x value =0x%x route:%x power:0x%x", fn, type, value ,route, power);
    ...
    {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: exit, handle:%x invalid", fn, ee_handle);
        return nfaStat;
    }
...
bool RoutingManager::clearAidTable ()
{
    ...
    if (nfaStat == NFA_STATUS_OK)
    {
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: removed AID. before", fn);
        RoutingManager::getInstance().mAidAddRemoveEvent.wait();
        DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s: removed AID. after", fn);
        return true;
    } else
    {
...
void RoutingManager::setEmptyAidEntry(int route) {
    ...
    tNFA_HANDLE ActDevHandle = NFA_HANDLE_INVALID;
    static const char fn []   = "RoutingManager::setEmptyAidEntry";

...
typedef enum {
  RFST_IDLE,               /* idle state                     */
  RFST_DISCOVERY,          /* discovery state                */
  RFST_W4_ALL_DISCOVERIES, /* wait for all discoveries state */
  RFST_W4_HOST_SELECT,     /* wait for host selection state  */
  RFST_POLL_ACTIVE,        /* poll mode activated state      */
  RFST_LISTEN_ACTIVE,      /* listen mode activated state    */
  RFST_LISTEN_SLEEP,       /* listen mode sleep state        */
  RFST_LP_LISTEN,          /* Listening in Low Power mode    */
  RFST_LP_ACTIVE           /* Activated in Low Power mode    */
}RFST_t;
...
static bool nfcManager_IsTagConnected();
#endif
...
void initializeGlobalDebugEnabledFlag() {
  ...
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("%s: nfc_debug_enabled=%u from JNI", __func__, nfc_debug_enabled);
}
...
static void handleRfDiscoveryEvent(tNFC_RESULT_DEVT* discoveredDevice) {
  ...
  if (!sReaderModeEnabled && isP2p) {
    ...
    if(gIsSelectingRfInterface) {
      NfcTag::getInstance().selectCurrentTag();
    } else {
...
/*******************************************************************************
**
** Function:        nfcManager_IsTagConnected
**
** Description:     Used to check whether tag is connected or not
**
** Returns:         true/false
**
*******************************************************************************/
static bool nfcManager_IsTagConnected() {
  uint8_t state = NFA_GetRfLinkState();
  if ((state == RFST_W4_ALL_DISCOVERIES) ||
      (state == RFST_W4_HOST_SELECT) ||
      (state == RFST_POLL_ACTIVE))
  {
     return true;
  } else {
     return false;

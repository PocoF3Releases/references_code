...
NfcTag::NfcTag()
  ...
  memset(&mNonStdTagdata, 0 ,sizeof(mNonStdTagdata));
#endif
...
**
*******************************************************************************/
bool NfcTag::isTagDetectedInRefTime(uint32_t reference) {
  ...
  if (timediff < reference) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: Non standard MFC tag detected, sequence-2", __func__);
    isNonStdCard = true;
  }
  return isNonStdCard;
}
...
**
*******************************************************************************/
void NfcTag::updateNdefState(uint8_t protocol, uint8_t more_disc_ntf) {
  bool isMFCDetected = false;
  if (protocol == NFC_PROTOCOL_MIFARE) {
    /*If NonStd MFC tag is detected*/
    if (mIsNonStdMFCTag && isTagDetectedInRefTime(mNonStdCardTimeDiff[MFC])) {
      mIsSkipNdef = true;
    } else {
      clearNonStdMfcState();
    }
    ...
    for (int i = 0; i < mNumTechList; i++) {
      if (mTechLibNfcTypes[i] == NFC_PROTOCOL_MIFARE) {
        isMFCDetected = true;
      }
    }
    if (!isMFCDetected) {
...
**
*******************************************************************************/
void NfcTag::clearNonStdMfcState() {
  mIsSkipNdef = false;
  mIsNonStdMFCTag = false;
  memset(&LastDetectedTime, 0, sizeof(timespec));
}
...
void NfcTag::fillNativeNfcTagMembers4(JNIEnv* e, jclass tag_cls, jobject tag,
  ...
  LOG(INFO)
    << StringPrintf("#11.59.00# %s: mTechListTail=%d", fn, mTechListTail);
  if (mTechListTail == 0) {
    gtechActBytes =
  ...
  if (gtechActBytes != NULL && mTechListTail != 0) {
    LOG(INFO)
      << StringPrintf("#11.59.00# %s: DeleteGlobalRef(gtechActBytes)", fn);
    e->DeleteGlobalRef(gtechActBytes);
    gtechActBytes =
        reinterpret_cast<jobjectArray>(e->NewGlobalRef(techActBytes.get()));
  }
...
void NfcTag::selectCurrentTag() {
  ...
  if(requestedProtocol != NFC_PROTOCOL_UNKNOWN &&
      mTechLibNfcTypesDiscData[id] != requestedProtocol) {
    LOG(ERROR)<<StringPrintf("%s: requestedProtocol:%d, mTechLibNfcTypesDiscData[id]:%d",
        __func__, requestedProtocol, mTechLibNfcTypesDiscData[id]);

    for (int i = 0; i < mNumDiscTechList; i++) {
      if (mTechLibNfcTypesDiscData[i] == requestedProtocol) {
  ...
  sLastSelectedTagId = id;

  if (mTechLibNfcTypesDiscData[id] == NFA_PROTOCOL_ISO_DEP) {
    rf_intf = NFA_INTERFACE_ISO_DEP;
  } else if (mTechLibNfcTypesDiscData[id] == NFC_PROTOCOL_MIFARE) {
    rf_intf = NFA_INTERFACE_MIFARE;
  } else
    rf_intf = NFA_INTERFACE_FRAME;

  tNFA_STATUS stat = NFA_Select(mTechHandlesDiscData[id],
    mTechLibNfcTypesDiscData[id], rf_intf);

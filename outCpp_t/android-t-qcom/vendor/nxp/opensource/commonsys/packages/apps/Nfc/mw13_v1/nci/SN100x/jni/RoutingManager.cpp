...
#define AID_OP_TIMEOUT (10 * 1000)
#define ROUTING_OP_TIMEOUT (4 * 1000) //setting anr timeout is 5s
...
bool RoutingManager::initialize(nfc_jni_native_data* native) {
...
#endif
  {
    ...
    LOG(INFO) << StringPrintf("%s: >>>> mUiccListnTechMask=0x%X", fn, mUiccListnTechMask);
#endif
  ...
  tNFA_STATUS nfaStat = NFA_CeSetIsoDepListenTech(
      mHostListnTechMask & (NFA_TECHNOLOGY_MASK_A | NFA_TECHNOLOGY_MASK_B));
...
void RoutingManager::enableRoutingToHost() {
  ...
  if ((mHostListnTechMask & NFA_TECHNOLOGY_MASK_A) &&
  ...
  if ((mHostListnTechMask & NFA_TECHNOLOGY_MASK_B) &&
  ...
  if ((mHostListnTechMask & NFA_TECHNOLOGY_MASK_F) &&
...
bool RoutingManager::addAidRouting(const uint8_t* aid, uint8_t aidLen,
  ...
  if(android::nfcManager_isNfcDisabling() && !android::nfcManager_isNfcActive()) {
    LOG(ERROR) << StringPrintf("%s: Nfc is turnning Off", __func__);
    return false;
  }
  ...
  if (nfaStat == NFA_STATUS_OK) {
    ...
    mAidAddRemoveEvent.wait(AID_OP_TIMEOUT);
#else
    mRoutingEvent.wait();
  }
...
#endif
  if(android::nfcManager_isNfcDisabling()) {
    LOG(ERROR) << StringPrintf("%s: Nfc is turnning Off", __func__);
  } else {
...
void RoutingManager::nfaEeCallback(tNFA_EE_EVT event,
  ...
  switch (event) {
    ...
    case NFA_EE_ACTION_EVT: {
      ...
      routingManager.reportEEHandle(action.ee_handle);
                                         ...
                                         )
{
  tNFA_STATUS nfaStat = NFA_STATUS_FAILED;
  {
    ...
    if (nfaStat == NFA_STATUS_OK) {
        if(mRoutingEvent.wait(ROUTING_OP_TIMEOUT)) {
            DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("proto routing SUCCESS");
        }
        else {
            LOG(ERROR) << StringPrintf("%s: proto routing set timeout!", __func__);
        }
    } else {
          ...
          if ((max_tech_mask != 0x01) && (max_tech_mask == 0x02) && value) {
            {
              ...
              if (nfaStat == NFA_STATUS_OK)
                if(mRoutingEvent.wait(ROUTING_OP_TIMEOUT)) {
                    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("tech A routing SUCCESS");
                }
                else {
                    LOG(ERROR) << StringPrintf("%s: tech A routing set timeout!", __func__);
                }
              else {
                DLOG_IF(ERROR, nfc_debug_enabled)
                     ...
                     value) {
            {
              ...
              if (nfaStat == NFA_STATUS_OK)
                if(mRoutingEvent.wait(ROUTING_OP_TIMEOUT)) {
                    DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("tech B routing SUCCESS");
                }
                else {
                    LOG(ERROR) << StringPrintf("%s: tech B routing set timeout!", __func__);
                }
              else {
                DLOG_IF(ERROR, nfc_debug_enabled)
                    << StringPrintf("Fail to set tech routing");
        ...
        }
        {
            ...
            if(nfaStat == NFA_STATUS_OK){
                if(mRoutingEvent.wait(ROUTING_OP_TIMEOUT)) {
                    ...
                    LOG(ERROR) << StringPrintf("%s: tech routing set timeout!", __func__);
                }
            }
            else{
...
void RoutingManager::setEmptyAidEntry(int routeAndPowerState) {
    ...
    if ((routeLoc != ROUTE_LOC_HOST_ID) && ((max_tech_mask & 0x03) == 0)) {
      LOG(ERROR) << StringPrintf("%s: return, routeLoc: 0x%2x, max_tech_mask: 0x%2x", __func__, routeLoc, max_tech_mask);
      return;
    }
...
//add for bigdata collection
void RoutingManager::reportEEHandle(tNFA_HANDLE ee_handle) {
  JNIEnv* e = NULL;
  ScopedAttach attach(mNativeData->vm, &e);
  if (e == NULL) {
  ...
  e->CallVoidMethod(mNativeData->manager,
                    android::gCachedNfcManagerReportEEHandle,
                    (int)ee_handle);
  if (e->ExceptionCheck()) {
    e->ExceptionClear();
    LOG(ERROR) << StringPrintf("fail report about CE handle");
  }
}

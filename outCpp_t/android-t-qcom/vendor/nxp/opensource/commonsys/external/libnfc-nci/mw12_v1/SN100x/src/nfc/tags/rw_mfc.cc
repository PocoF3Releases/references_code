...
static void rw_mfc_handle_format_rsp(uint8_t* p_data) {
  ...
  if (!p_data) {
    DLOG_IF(INFO, nfc_debug_enabled)
        << StringPrintf("%s: Empty buffer. Returning!!",__func__);
        return;
  }
  ...
  switch (event) {
    ...
    case NFC_ERROR_CEVT:
      ...
      if ((p_mfc->state == RW_MFC_STATE_NOT_ACTIVATED) ||
        ...
        LOG(ERROR) << __func__ << "the event is NFC_ERROR_CEVT";
        return;
      }
      nfc_stop_quick_timer(&p_mfc->timer);
      LOG(ERROR) << __func__ << "the event is NFC_ERROR_CEVT, nfc stop quick timer";
      return;

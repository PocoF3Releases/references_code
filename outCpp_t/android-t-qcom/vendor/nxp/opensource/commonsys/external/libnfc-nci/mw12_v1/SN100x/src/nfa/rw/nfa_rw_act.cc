...
static void nfa_rw_handle_ndef_detect(tRW_DATA* p_rw_data) {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf(
      "NDEF Detection completed: cur_size=%i, max_size=%i, flags=0x%x, status=%d",
      p_rw_data->ndef.cur_size, p_rw_data->ndef.max_size,
      p_rw_data->ndef.flags, p_rw_data->ndef.status);
...
static tNFC_STATUS nfa_rw_start_ndef_detection(void) {
  ...
  LOG(INFO) << StringPrintf("nfa_rw_start_ndef_detection:  protocol=0x%x", protocol);
...
static bool nfa_rw_op_req_while_busy(tNFA_RW_MSG* p_data) {
  ...
  LOG(ERROR) << StringPrintf("nfa_rw_op_req_while_busy: unable to handle API, op:%d",
    p_data->op_req.op);
  ...
  switch (p_data->op_req.op) {
    ...
    case NFA_RW_OP_SEND_RAW_FRAME:
      event = NFA_DATA_EVT;
      conn_evt_data.data.p_data = NULL;
      conn_evt_data.data.len = 0;
      if (p_data->op_req.params.send_raw_frame.p_data) {
          LOG(ERROR) << StringPrintf("free frame data");
          GKI_freebuf(p_data->op_req.params.send_raw_frame.p_data);
      }
      break;
    default:
      return (freebuf);

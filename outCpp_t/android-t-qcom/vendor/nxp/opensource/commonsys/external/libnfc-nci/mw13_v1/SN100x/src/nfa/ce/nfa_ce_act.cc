    ...
    if ((p_cb->listen_info[listen_info_idx].flags &
      ...
      if (p_cb->listen_info[listen_info_idx].flags &
        ...
        if (listen_mask) {
          ...
          char flag_prop[PROPERTY_VALUE_MAX] = {0};
          __system_property_get("nfc.restart.flag", flag_prop);
          if ((p_cb->listen_info[listen_info_idx].ee_handle == 0x4c0) && (strcmp(flag_prop, "1") != 0)) {
              LOG(ERROR) << StringPrintf("ESE should never be deactivated, restart nfc and update FW");
              __system_property_set("nfc.restart.flag", "1");

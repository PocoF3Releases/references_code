...
void NfcAdaptation::InitializeHalDeviceContext() {
  ...
  LOG(INFO) << StringPrintf("%s: Try INqNfcV2_0::getService()", func);
...
void NfcAdaptation::HalClose() {
  ...
  DLOG_IF(INFO, nfc_debug_enabled) << StringPrintf("%s", func);
  if (mpInstance) mpInstance->CopyNxpTransitConfig();
  mHal->close();
...
/*******************************************************************************
**
** Function:    NfcAdaptation::CopyNxpTransitConfig
**
** Description: Copy transit config from data to data vendor.
**
** Returns:     None.
**
*******************************************************************************/
void NfcAdaptation::CopyNxpTransitConfig() {
  const char* func = "CopyNxpTransitConfig";

  const string path_transit_config = "/data/nfc/libnfc-nxpTransit.conf";
  struct stat file_stat;

  /* Checking whether libnfc-nxpTransit.conf exists */
  if (stat(path_transit_config.c_str(), &file_stat) == 0
      && S_ISREG(file_stat.st_mode)) {
    LOG(INFO) << StringPrintf(
        "%s: /data/nfc/libnfc-nxpTransit.conf exists", func);

    string config;
    bool config_read = ReadFileToString(path_transit_config, &config);
    if (config_read) {
      int len = strlen(config.c_str());
      nfc_nci_IoctlInOutData_t inpOutData;
      inpOutData.inp.data.transitConfig.val =
          (char *) config.c_str();
      inpOutData.inp.data.transitConfig.len = len;
      int ret = HalIoctl(HAL_NFC_IOCTL_SET_TRANSIT_CONFIG, &inpOutData);
      LOG(INFO) << StringPrintf("%s: result = %d", func, ret);
    } else {
      LOG(ERROR) << StringPrintf(
          "%s: Read failed: /data/nfc/libnfc-nxpTransit.conf ", func);
    }
  } else {
    LOG(INFO) << StringPrintf(
        "%s: /data/nfc/libnfc-nxpTransit.conf not exist", func);
  }
}
#endif
/*******************************************************************************

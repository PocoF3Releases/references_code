...
static void nfc_main_hal_data_cback(uint16_t data_len, uint8_t* p_data) {
  ...
  if (data_len == NFC_BIGDATA_LENGTH) {
    if (p_data[0] == NFC_BIGDATA_EXCEPTION) {
      LOG(ERROR) << StringPrintf("receive exception in nfc_main_hal_data_cback");
...
tNFA_EXCEPTION_REPORTER_CBACK* exception_reporter_cback;
void NFA_registExceptionReporter(tNFA_EXCEPTION_REPORTER_CBACK* cback) {
  exception_reporter_cback = cback;
}
...
void reportException(std::string value) {
  if(exception_reporter_cback != NULL) {
    LOG(ERROR) << StringPrintf("%s: value: %s", __func__, value.c_str());
    exception_reporter_cback(value);
  } else {
    LOG(ERROR) << StringPrintf("%s: exception_reporter_cback is NULL.", __func__);
  }
}

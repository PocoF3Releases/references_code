    ...
    case HAL_NFC_OPEN_CPLT_EVT:
      ...
      */
      ...
      /* Updates completed, restart NFC service */
        ...
        reportException(NAME_RESTART_NFC_SERVICE);
        LOG(ERROR)
...
static void nfc_main_hal_data_cback(uint16_t data_len, uint8_t* p_data) {
  ...
  if (data_len == NFC_BIGDATA_LENGTH) {
    if (p_data[0] == NFC_BIGDATA_TYPE_EXCEPTION) {
      LOG(INFO) << StringPrintf("exception received from HAL layer");
      reportException(formatValueFromHal(p_data[1]));
      return;
    }
  }
...
tNFA_BIGDATA_REPORTER_CBACK* nfc_bigdata_reporter_cback;
void NFA_registerBigDataReporter(tNFA_BIGDATA_REPORTER_CBACK* cback) {
  nfc_bigdata_reporter_cback = cback;
}

std::string formatValueFromHal(uint8_t value) {
  switch (value) {
    case ABORT_MAX_RETY:
      return NAME_ABORT_MAX_RETY;
    case ABORT_CLOSE_HAL_ERROR:
      return NAME_ABORT_CLOSE_HAL_ERROR;
    case ABORT_FW_ASSERT:
      return NAME_ABORT_FW_ASSERT;
    case ABORT_WATCHDOG_RESET:
      return NAME_ABORT_WATCHDOG_RESET;
    case ABORT_INPUT_CLOCK_LOST:
      return NAME_ABORT_INPUT_CLOCK_LOST;
    case ABORT_OVER_TEMPERATURE:
      return NAME_ABORT_OVER_TEMPERATURE;
    case ABORT_UNRECOVERABLE_ERROR:
      return NAME_ABORT_UNRECOVERABLE_ERROR;
    case ABORT_UNKNOW_ERROR:
    default:
      return NAME_ABORT_UNKNOW_ERROR;
  }
}

std::string formatToHEXString(uint8_t value) {
    char result[20] = {0};
    sprintf(result, "0x%02X", value);
    return string(result);
}

static std::string formatToIntString(uint8_t value) {
    char result[8] = {0};
    sprintf(result, "%d", value);
    return string(result);
}

void reportException(std::string value) {
  if(nfc_bigdata_reporter_cback != NULL) {
    LOG(INFO) << StringPrintf("%s: value: %s", __func__, value.c_str());
    nfc_bigdata_reporter_cback(NFC_BIGDATA_TYPE_EXCEPTION, value);
  } else {
    LOG(ERROR) << StringPrintf("%s: nfc_bigdata_reporter_cback is NULL.", __func__);
  }
}

void reportRfPhase(std::string value) {
  if(nfc_bigdata_reporter_cback != NULL) {
    LOG(INFO) << StringPrintf("%s: value: %s", __func__, value.c_str());
    nfc_bigdata_reporter_cback(NFC_BIGDATA_TYPE_RFPHASE, value);
  } else {
    LOG(ERROR) << StringPrintf("%s: nfc_bigdata_reporter_cback is NULL.", __func__);
  }
}

void nfc_bigdata_report_rf_phase(uint8_t* p, uint8_t len) {
  if (len != 1) {
    LOG(ERROR) << StringPrintf("%s: length error", __func__);
    return;
  }
  uint8_t value = *p++;
  reportRfPhase(NAME_RF_PHASE_VALUE + formatToIntString(value));
}

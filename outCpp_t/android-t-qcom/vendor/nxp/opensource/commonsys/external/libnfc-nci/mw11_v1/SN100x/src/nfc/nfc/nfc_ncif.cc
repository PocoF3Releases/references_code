...
void nfc_ncif_cmd_timeout(void) {
  ...
  reportException(NAME_CMD_TIEMOUT);
...
#if (NXP_EXTNS == TRUE)
  DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("nfc_ncif_send_cmd()");
  if (p_buf == nullptr) {
    DLOG_IF(INFO, nfc_debug_enabled)
      << StringPrintf("p_buf is NULL.");
    return;
  }
#endif
...
bool nfc_ncif_process_event(NFC_HDR* p_msg) {
  ...
  {/* If we have received NCI_STATUS_SEMANTIC_ERROR, abort the process!!
    * EXCEPTION: CORE_SET_POWER_SUB_STATE_CMD & RF_ISO_DEP_NAK_PRESENCE_CMD */
    ...
    reportException(NAME_SEMANTIC_ERROR);
    abort();
  }
#endif
  ...
  if (p_cback) {
    ...
    switch (evt_data.act_data.trigger) {
      ...
      case NCI_EE_TRIG_RF_TECH_PROP_RSP_NTF:
        LOG(INFO)
          << StringPrintf("#11.59.00# %s: NCI_EE_TRIG_RF_TECH_PROP_RSP_NTF", __func__);
        if (data_len > NFC_MAX_AID_LEN) data_len = NFC_MAX_AID_LEN;
        DLOG_IF(INFO, nfc_debug_enabled)
      ...
      case NCI_EE_TRIG_PROP_RSP_NTF:
        LOG(INFO)
          << StringPrintf("#11.59.00# %s: NCI_EE_TRIG_PROP_RSP_NTF", __func__);
        DLOG_IF(INFO, nfc_debug_enabled)
...
void nfc_ncif_proc_reset_rsp(uint8_t* p, bool is_ntf) {
  ...
  if (*p_len > 2 && is_ntf) {
    ...
    if (status == NCI2_0_RESET_TRIGGER_TYPE_CORE_RESET_CMD_RECEIVED
      ...
      DLOG_IF(INFO, nfc_debug_enabled)
          << StringPrintf("#11.59.00# CORE_RESET_NTF info len = %d, offset = %d", infoLen, fwVerOffset);
      p += fwVerOffset;
...
void nfc_ee_temp_error_delay_timeout(){
  ...
  if(p_cb != nullptr) {
    ...
    if (nfa_ee_cb.p_enable_cback){
      ...
      reportException(NAME_ENABLE_CALLBACK_NULL);
      abort();
    }
  }
}

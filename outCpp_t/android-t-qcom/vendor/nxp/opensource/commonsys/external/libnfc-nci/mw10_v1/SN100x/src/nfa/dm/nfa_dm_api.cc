...
#if (NXP_EXTNS == TRUE)
  } else if (NfcAdaptation::GetInstance().NFA_GetBootMode() ==
             NFA_MINIMUM_BOOT_MODE) {
    nfa_ee_init();
    if (nfa_ee_max_ee_cfg != 0) {
      nfa_dm_cb.get_max_ee = p_hal_entry_tbl->get_max_ee;
      nfa_t4tnfcee_init();
...
/*******************************************************************************
**
** Function         NFA_GetRfLinkState()
**
** Description      This function returns RF communication state

**
** Returns          RF state
**
*******************************************************************************/
uint8_t NFA_GetRfLinkState() {
  return nfa_dm_cb.disc_cb.disc_state;
}
#endif

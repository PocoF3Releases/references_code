...
#endif
      reportException(NAME_CORE_GENERIC_ERROR + formatToHEXString(*pp));
      break;

    case NCI_MSG_CORE_INTF_ERR_STATUS:
      conn_id = *(pp + 1);
      nfc_ncif_error_status(conn_id, *pp);
      reportException(NAME_CORE_INTERFACE_ERROR + formatToHEXString(*pp));
      break;
...
#endif
#endif
    case NCI_MSG_CORE_RF_PHASE: //nfc bigdata feature
      nfc_bigdata_report_rf_phase(pp, len);
      break;
    case NCI_MSG_RF_ISO_DEP_NAK_PRESENCE:
...
void nci_proc_ee_management_ntf(NFC_HDR* p_msg) {
  ...
  switch (op_code) {
    ...
    case NCI_MSG_NFCEE_STATUS:
      ...
      // nfc bigdata start
      if ((nfc_response.nfcee_status.nfcee_id == 0xC0) &&
          (nfc_response.nfcee_status.nfcee_status == 0x91)) {
        LOG(ERROR) << StringPrintf("eSE C091 ERROR!!!");
        reportException(NAME_NFCEE_ESE_REBOOT);
      }
      // nfc bigdata end

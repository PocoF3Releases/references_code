...
bool ConfigFile::isUpdateAllowed(const std::string& key) {
  LOG(INFO) << "ConfigFile - isUpdateAllowed - [" << key << "]";
...
#if(NXP_EXTNS == TRUE)
  if (file_name.find("nxpTransit") == std::string::npos)
    CHECK(config_read);
  else
    if (!config_read) {
      LOG(ERROR) << "ConfigFile - Parsing file '" << file_name << "' failed";
      return ;
    }
#else
    CHECK(config_read);
  ...
  while (getline(ss, line)) {
    ...
    if (cur_file_name_.find("nxpTransit") != std::string::npos) {
        if (!isUpdateAllowed(key)) {
            LOG(INFO) << "ConfigFile - [" << key << "] token restricted. ignore";
            continue;
        }
    }
#endif
    ...
    if (cur_file_name_.find("nxpTransit") == std::string::npos)
      ...
      if (!value_parsed) {
        LOG(ERROR) << "ConfigFile - Parsing [" << key << "]'s value from file '"
            << cur_file_name_ << "' failed";
        cur_file_name_ = "";
        return ;
      }
#else

...
static void uwa_dm_uwb_response_cback(tUWB_RESPONSE_EVT event,
  ...
  switch (event) {
    ...
    case UWB_SET_CORE_CONFIG_REVT: /* 2  Set Config Response */
      /* If this setconfig was due to UWA_SetCoreConfig, then notify the app */
      /* lsb=whether last UCI_SET_CONFIG was due to UWA_SetCoreConfig */
    ...
    case UWB_UWBD_RESP_TIMEOUT_REVT:
      {
        dm_cback_data.status = UWB_STATUS_FAILED;
        (*uwa_dm_cb.p_dm_cback)(UWA_DM_UWBD_RESP_TIMEOUT_EVT, &dm_cback_data);
      }
      break;
    case UWB_SEND_DATA_PACKET_RSP_REVT:
      {
        dm_cback_data.status = p_data->status;
        (*uwa_dm_cb.p_dm_cback)(UWA_DM_SEND_DATA_PACKET_RSP_EVT, &dm_cback_data);
      }
      break;
...
**
** Description      callback handles all RF test responses and notfications
**
** Returns          void
**
*******************************************************************************/
...
bool uwa_dm_register_ext_cb(tUWA_DM_MSG* p_data) {
  tUWA_DM_CBACK_DATA dm_cback_data;
  UCI_TRACE_I("uwa_dm_register_ext_cb ()");

  /* if UWA is enabled, then register for callback */
  if (uwa_dm_cb.flags & UWA_DM_FLAGS_DM_IS_ACTIVE) {
    UWB_RegisterExtCallBack(p_data->ext_cb.p_dm_ext_cback);
    dm_cback_data.status = UWA_STATUS_OK;
    (*(uwa_dm_cb.p_dm_cback))(UWA_DM_REGISTER_EXT_CB_EVT, &dm_cback_data);
  }
  return (true);
}
...
bool uwa_dm_act_send_data_frame(tUWA_DM_MSG* p_data){
  ...
  if(p_data == NULL) {
    ...
    status = UWB_SendData(p_data->send_data_frame.session_id, p_data->send_data_frame.addr_len,
                          p_data->send_data_frame.p_addr, p_data->send_data_frame.data_len,
                          p_data->send_data_frame.p_data);
  }
  if(status == UWB_STATUS_OK) {
    UCI_TRACE_I("uwa_dm_act_send_data_frame(): success , status=0x%X",status);
  } else {
...
bool uwa_dm_act_test_set_config(tUWA_DM_MSG* p_data) {
  ...
  if (p_data->sTest_set_config.length + 2 > 255) {
    /* Total length of TLV must be less than 256 (1 byte) */
    status = UWB_STATUS_FAILED;
  } else {
...
std::string uwa_dm_uwb_revt_2_str(tUWB_RESPONSE_EVT event)
{
  switch (event) {
    ...
    case UWB_SEND_DATA_PACKET_RSP_REVT:
      return "UWB_SEND_DATA_PACKET_RSP_REVT";

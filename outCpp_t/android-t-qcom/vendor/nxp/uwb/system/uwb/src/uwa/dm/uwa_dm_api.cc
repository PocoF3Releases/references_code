...
**                  UWA_DM_TEST_RX_RSP_EVT callback.
**
...
**
*******************************************************************************/
tUWA_STATUS UWA_SendUwbData(uint32_t session_id,uint8_t addr_len, uint8_t* p_addr,
                                           uint16_t data_len, uint8_t* p_data) {
  tUWA_DM_API_SEND_DATA_FRAME* p_msg;
  UCI_TRACE_I("UWA_SendUwbDataFrame: addr_len= %d, data_len=%d", addr_len, data_len);
  if ((data_len == 0) || (p_data == nullptr) || (addr_len == 0) || (p_addr == nullptr))
    return (UWA_STATUS_FAILED);

  p_msg = (tUWA_DM_API_SEND_DATA_FRAME*)phUwb_GKI_getbuf(sizeof(tUWA_DM_API_SEND_DATA_FRAME) + addr_len + data_len);
  if (p_msg != nullptr) {
    p_msg->hdr.event = UWA_DM_API_SEND_DATA_FRAME_EVT;
    p_msg->session_id = session_id;
    p_msg->addr_len = addr_len;
    p_msg->data_len = data_len;
    p_msg->p_addr = (uint8_t*)(p_msg + 1);
    memcpy(p_msg->p_addr, p_addr, addr_len);

    p_msg->p_data = (uint8_t*)(p_msg + 1) + addr_len;
...
tUWA_MW_VERSION UWA_GetMwVersion() {
  ...
  UCI_TRACE_I("0x%x:UWB MW Major Version:", UWB_NXP_MW_VERSION_MAJ);
  UCI_TRACE_I("0x%x:UWB MW Minor Version:", UWB_NXP_MW_VERSION_MIN);

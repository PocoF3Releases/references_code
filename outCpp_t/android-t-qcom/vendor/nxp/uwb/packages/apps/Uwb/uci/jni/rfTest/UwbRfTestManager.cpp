...
const char* PERIODIC_TX_DATA_CLASS_NAME = "com/android/uwb/UwbTestPeriodicTxResult";
const char* PER_RX_DATA_CLASS_NAME = "com/android/uwb/UwbTestRxPacketErrorRateResult";
const char* UWB_LOOPBACK_DATA_CLASS_NAME = "com/android/uwb/UwbTestLoopBackTestResult";
const char* RX_DATA_CLASS_NAME = "com/android/uwb/UwbTestRxResult";

static SyncEvent sUwaRfTestEvent;                       // event for RF test response
static SyncEvent sUwaSetTestConfigEvent;                // event for Set_testConfig
static SyncEvent sUwaGetTestConfigEvent;                // event for Get_testConfig
...
UwbRfTestManager& UwbRfTestManager::getInstance() {
    return mObjTestManager;
}
...
void UwbRfTestManager::onPeriodicTxDataNotificationReceived(uint16_t len, uint8_t* data) {
    ...
    if(len != 0) {
        ...
        jmethodID periodicTxCtor = env->GetMethodID(mPeriodicTxDataClass, "<init>", "(I)V");
        jobject periodicTxObject = env->NewObject(mPeriodicTxDataClass,
                                                  periodicTxCtor,
                                                  (int)sPeriodic_tx_data.status);

        if(mOnPeriodicTxDataNotificationReceived != NULL) {
            env->CallVoidMethod(mObject, mOnPeriodicTxDataNotificationReceived, periodicTxObject);
            if (env->ExceptionCheck()) {
                env->ExceptionClear();
                JNI_TRACE_E("%s: fail to send periodic TX data", __func__);
            }
        } else {
...
void UwbRfTestManager::onPerRxDataNotificationReceived(uint16_t len, uint8_t* data) {
    ScopedJniEnv env(mVm);
    if (env == NULL) {
        JNI_TRACE_E("%s: jni env is null", __func__);
        return;
    ...
    uint8_t* vendorExtnData = NULL;
    jbyteArray extendedVendorSpecInfo = NULL;
    int16_t perRxVendorLen = 0;
    ...
    if(len != 0) {
        ...
        perRxVendorLen = len - PER_RX_NTF_LEN;
        if(perRxVendorLen > 0) {
          STREAM_TO_UINT8(sPer_rx_data.vendor_specific_length, data);
        }
        if(sPer_rx_data.vendor_specific_length > 0)
        {
          STREAM_TO_ARRAY(sPer_rx_data.vendor_specific_info, data, sPer_rx_data.vendor_specific_length);
          vendorExtnData = (uint8_t*) sPer_rx_data.vendor_specific_info;
        }

        /* Vendor Specific data  */
        if(sPer_rx_data.vendor_specific_length > 0)
        {
          extendedVendorSpecInfo = env->NewByteArray(sPer_rx_data.vendor_specific_length);
          env->SetByteArrayRegion(extendedVendorSpecInfo, 0, sPer_rx_data.vendor_specific_length, (jbyte *)sPer_rx_data.vendor_specific_info);
        }

        jmethodID perRxCtor = env->GetMethodID(mPerRxDataClass, "<init>", "(IJJJJJJJJJJJJJ[B)V");
        jobject perRxObject = env->NewObject(mPerRxDataClass,
                                             perRxCtor,
                                             (int)sPer_rx_data.status,
                                             (long)sPer_rx_data.attempts,
                                             (long)sPer_rx_data.ACQ_detect,
                                             (long)sPer_rx_data.ACQ_rejects,
                                             (long)sPer_rx_data.RX_fail,
                                             (long)sPer_rx_data.sync_cir_ready,
                                             (long)sPer_rx_data.sfd_fail,
                                             (long)sPer_rx_data.sfd_found,
                                             (long)sPer_rx_data.phr_dec_error,
                                             (long)sPer_rx_data.phr_bit_error,
                                             (long)sPer_rx_data.psdu_dec_error,
                                             (long)sPer_rx_data.psdu_bit_error,
                                             (long)sPer_rx_data.sts_found,
                                             (long)sPer_rx_data.eof,
                                             extendedVendorSpecInfo
                                             );
        if(mOnPerRxDataNotificationReceived != NULL) {
            env->CallVoidMethod(mObject, mOnPerRxDataNotificationReceived, perRxObject);
            if (env->ExceptionCheck()) {
              env->ExceptionClear();
                JNI_TRACE_E("%s: fail to send per RX data", __func__);
            }
        } else {
...
void UwbRfTestManager::onLoopBackTestDataNotificationReceived(uint16_t len, uint8_t* data) {
    ScopedJniEnv env(mVm);
    if (env == NULL) {
        JNI_TRACE_E("%s: jni env is null", __func__);
        return;
    ...
    if(len != 0) {
        ...
        loopbackVendorLen = len - (LOOPBACK_NTF_LEN + psduDataLen);
        if(sUwb_loopback_data.psdu_data_length > 0)
        {
          STREAM_TO_ARRAY(sUwb_loopback_data.psdu_data, data, sUwb_loopback_data.psdu_data_length);
          psduData = env->NewByteArray(sUwb_loopback_data.psdu_data_length);
          env->SetByteArrayRegion (psduData, 0, sUwb_loopback_data.psdu_data_length, (jbyte *)sUwb_loopback_data.psdu_data);
        }
        if(loopbackVendorLen > 0) {
          STREAM_TO_UINT8(sUwb_loopback_data.vendor_specific_length, data);
        }
        if(sUwb_loopback_data.vendor_specific_length > 0)
        {
          STREAM_TO_ARRAY(sUwb_loopback_data.vendor_specific_info, data, sUwb_loopback_data.vendor_specific_length);
        ...
        if(sUwb_loopback_data.vendor_specific_length > 0)
        {
          extendedVendorSpecInfo = env->NewByteArray(sUwb_loopback_data.vendor_specific_length);
          env->SetByteArrayRegion(extendedVendorSpecInfo, 0, sUwb_loopback_data.vendor_specific_length, (jbyte *)sUwb_loopback_data.vendor_specific_info);
        }

        jmethodID uwbLoopBackCtor = env->GetMethodID(mUwbLoopBackDataClass, "<init>", "(IJIJIIII[B[B)V");
        jobject uwbLoopBackObject = env->NewObject(mUwbLoopBackDataClass,
                                                   uwbLoopBackCtor,
                                                   (int)sUwb_loopback_data.status,
                                                   (long)sUwb_loopback_data.txts_int,
                                                   (int)sUwb_loopback_data.txts_frac,
                                                   (long)sUwb_loopback_data.rxts_int,
                                                   (int)sUwb_loopback_data.rxts_frac,
                                                   (int)sUwb_loopback_data.aoa_azimuth,
                                                   (int)sUwb_loopback_data.aoa_elevation,
                                                   (int)sUwb_loopback_data.phr,
                                                    psduData,
                                                    /* Vendor Specific data */
                                                    extendedVendorSpecInfo
                                                   );
        if(mOnLoopBackTestDataNotificationReceived != NULL) {
            env->CallVoidMethod(mObject, mOnLoopBackTestDataNotificationReceived, uwbLoopBackObject);
            if (env->ExceptionCheck()) {
                env->ExceptionClear();
                JNI_TRACE_E("%s: fail to send rf loopback data", __func__);
            }
        } else {
...
void UwbRfTestManager::onRxTestDataNotificationReceived(uint16_t len, uint8_t* data) {
    ScopedJniEnv env(mVm);
    if (env == NULL) {
        JNI_TRACE_E("%s: jni env is null", __func__);
        return;
    ...
    if(len != 0) {
        ...
        testRxVendorLen = len - (TEST_RX_NTF_LEN + psduDataLen);
        if(sRx_data.psdu_data_length > 0){
          ...
          env->SetByteArrayRegion (psduData, 0, sRx_data.psdu_data_length, (jbyte *)sRx_data.psdu_data);
        }
        if(testRxVendorLen > 0) {
          STREAM_TO_UINT8(sRx_data.vendor_specific_length, data);
        }
        if(sRx_data.vendor_specific_length > 0){
          STREAM_TO_ARRAY(sRx_data.vendor_specific_info, data, sRx_data.vendor_specific_length);
        }

        /* Vendor Specific data to be added/parsed later */

        if(sRx_data.vendor_specific_length > 0){
          extendedVendorSpecInfo = env->NewByteArray(sRx_data.vendor_specific_length);
          env->SetByteArrayRegion(extendedVendorSpecInfo, 0, sRx_data.vendor_specific_length, (jbyte *)sRx_data.vendor_specific_info);
        }

        jmethodID rxDataCtor = env->GetMethodID(mRxDataClass, "<init>", "(IJIIIII[B[B)V");
        jobject rxDataObject = env->NewObject(mRxDataClass,
                                                   rxDataCtor,
                                                   (int)sRx_data.status,
                                                   (long)sRx_data.rx_done_ts_int,
                                                   (int)sRx_data.rx_done_ts_frac,
                                                   (int)aoaFirst,
                                                   (int)aoaSecond,
                                                   (int)sRx_data.toa_gap,
                                                   (int)sRx_data.phr,
                                                    psduData,
                                                    /* Vendor Specific data  */
                                                   extendedVendorSpecInfo
        ...
        if(mOnRxTestDataNotificationReceived != NULL) {
            env->CallVoidMethod(mObject, mOnRxTestDataNotificationReceived, rxDataObject);
            if (env->ExceptionCheck()) {
                env->ExceptionClear();
...
void UwbRfTestManager::doLoadSymbols(JNIEnv* env, jobject thiz) {
    ...
    if (clazz != NULL) {
        ...
        mOnPeriodicTxDataNotificationReceived =
            env->GetMethodID(clazz, "onPeriodicTxDataNotificationReceived", "(Lcom/android/uwb/UwbTestPeriodicTxResult;)V");
        mOnPerRxDataNotificationReceived =
            env->GetMethodID(clazz, "onPerRxDataNotificationReceived", "(Lcom/android/uwb/UwbTestRxPacketErrorRateResult;)V");
        mOnLoopBackTestDataNotificationReceived =
            env->GetMethodID(clazz, "onLoopBackTestDataNotificationReceived", "(Lcom/android/uwb/UwbTestLoopBackTestResult;)V");
        mOnRxTestDataNotificationReceived =
            env->GetMethodID(clazz, "onRxTestDataNotificationReceived", "(Lcom/android/uwb/UwbTestRxResult;)V");

        uwb_jni_cache_jclass(env, PERIODIC_TX_DATA_CLASS_NAME, &mPeriodicTxDataClass);
        uwb_jni_cache_jclass(env, PER_RX_DATA_CLASS_NAME, &mPerRxDataClass);
        uwb_jni_cache_jclass(env, UWB_LOOPBACK_DATA_CLASS_NAME, &mUwbLoopBackDataClass);
...
**
*******************************************************************************/
jbyteArray UwbRfTestManager::setTestConfigurations(JNIEnv* env, jobject o, jint sessionId, jint noOfParams, jint testConfigLen, jbyteArray TestConfig) {
    tUWA_STATUS status;
    ...
    if(testConfigData != NULL) {
        ...
        env->GetByteArrayRegion(TestConfig,0,testConfigLen,(jbyte*)testConfigData);
        setTestConfigRespStatus = false;
        SyncEventGuard guard(sUwaSetTestConfigEvent);
        JNI_TRACE_I("%d: testConfigLen", testConfigLen);
        status = UWA_TestSetConfig(sessionId, noOfParams,testConfigLen, testConfigData);
        free(testConfigData);
        if(status == UWA_STATUS_OK) {
            ...
            if(setTestConfigRespStatus) {
                testConfigArray = env->NewByteArray(sSetTestConfigLen + sizeof(sSetTestConfigStatus) + sizeof(sNoOfTestConfigIds));
                env->SetByteArrayRegion (testConfigArray, 0, 1, (jbyte *)&sSetTestConfigStatus);
                env->SetByteArrayRegion (testConfigArray, 1, 1, (jbyte *)&sNoOfTestConfigIds);
                if(sSetTestConfigLen > 0) {
                    env->SetByteArrayRegion (testConfigArray, 2, sSetTestConfigLen, (jbyte *)&sSetTestConfig[0]);
                }
            }
        } else {
            JNI_TRACE_E("%s: Failed UWA_SetAppConfigCommand", __func__);
            return testConfigArray;
        }
    } else {
...
**
*******************************************************************************/
jbyteArray UwbRfTestManager::getTestConfigurations(JNIEnv* env, jobject o, jint sessionId, jint noOfParams, jint testConfigLen, jbyteArray TestConfig) {
    tUWA_STATUS status;
    jbyteArray testConfigArray = NULL;
    uint8_t* testConfigData = NULL;
    JNI_TRACE_I("%s: Enter", __func__);
    ...
    if(testConfigData != NULL) {
        ...
        status = UWA_TestGetConfig(sessionId, noOfParams, testConfigLen, testConfigData);
        if (status == UWA_STATUS_OK) {
            sUwaGetTestConfigEvent.wait(UWB_CMD_TIMEOUT);
            if(getTestConfigRespStatus) {
                testConfigArray = env->NewByteArray(sGetTestConfigLen + sizeof(sNoOfTestConfigIds) + sizeof(sGetTestConfigStatus));
                env->SetByteArrayRegion (testConfigArray, 0, 1, (jbyte *)&sGetTestConfigStatus);
                env->SetByteArrayRegion (testConfigArray, 1, 1, (jbyte *)&sNoOfTestConfigIds);
                env->SetByteArrayRegion (testConfigArray, 2, sGetTestConfigLen, (jbyte *)&sGetTestConfig[0]);
            }
        }
        free(testConfigData);
    } else {
...
**                  UWA_STATUS_FAILED
*******************************************************************************/
jbyte UwbRfTestManager::startPerRxTest (JNIEnv* env, jobject o, jbyteArray refPsduData) {
    ...
    if(refPsduData != NULL) {
        ...
        if(dataLen > 0) {
          ...
          if(ref_psdu_data != NULL) {
              ...
              env->GetByteArrayRegion(refPsduData, 0, dataLen, (jbyte*)ref_psdu_data);
              ...
              if(UWA_STATUS_OK == status){
                  ...
                  IsRfTestOngoing = false;
                  JNI_TRACE_E("%s: per rx command is  failed", __func__);
              }
              free(ref_psdu_data);
          } else {
              JNI_TRACE_E("%s: Unable to Allocate Memory", __func__);
...
**
*******************************************************************************/
jbyte UwbRfTestManager::startPeriodicTxTest (JNIEnv* env, jobject o, jbyteArray psduData) {
    tUWA_STATUS status = UWA_STATUS_FAILED;
    uint16_t dataLen = 0;
    ...
    if(psduData != NULL){
        ...
        if(dataLen > UCI_MAX_PAYLOAD_SIZE) {
            JNI_TRACE_E("%s: PER TX data size exceeds %d", __func__,UCI_MAX_PAYLOAD_SIZE);
            return status;
        }
        ...
        if(psdu_Data != NULL) {
            ...
            if(UWA_STATUS_OK == status){
                ...
                IsRfTestOngoing = false;
                JNI_TRACE_E("%s: per tx command is  failed", __func__);
            }
            free(psdu_Data);
        } else {
            JNI_TRACE_E("%s: Unable to Allocate Memory", __func__);
...
**
*******************************************************************************/
jbyte UwbRfTestManager::startUwbLoopBackTest (JNIEnv* env, jobject o, jbyteArray psduData) {
    tUWA_STATUS status = UWA_STATUS_FAILED;
    uint16_t dataLen = 0;
    uint8_t* psdu_Data = NULL;
    JNI_TRACE_I("%s: Enter; ", __func__);
    ...
    if(psduData != NULL){
        dataLen = env->GetArrayLength(psduData);
        if(dataLen > UCI_MAX_PAYLOAD_SIZE) {
            JNI_TRACE_E("%s: Loopback data size exceeds %d", __func__,UCI_MAX_PAYLOAD_SIZE);
        ...
        if(psdu_Data != NULL) {
            ...
            if(UWA_STATUS_OK == status){
              ...
              IsRfTestOngoing = false;
              JNI_TRACE_E("%s: loopback test command is  failed", __func__);
            }
            free(psdu_Data);
        } else {
            JNI_TRACE_E("%s: Unable to Allocate Memory", __func__);
...
jbyte UwbRfTestManager::startRxTest (JNIEnv* env, jobject o) {
    ...
    if(UWA_STATUS_OK == status) {
        ...
        IsRfTestOngoing = false;
        JNI_TRACE_E("%s: start Rx command is  failed", __func__);
    }
...
jbyte UwbRfTestManager::stopRfTest (JNIEnv* env, jobject o) {
    ...
    if(UWA_STATUS_OK == status){
        ...
        JNI_TRACE_E("%s: per stop command is  failed", __func__);
    }
...
**
** Description:     Receive Rf Test related device management events from UCI stack
    ...
    switch (dmEvent) {
        case UWA_DM_TEST_SET_CONFIG_RSP_EVT:  // result of UWA_TestSetConfig
            JNI_TRACE_I("%s: UWA_DM_TEST_SET_CONFIG_RSP_EVT", __func__);
            {
                ...
                if(eventData->sTest_set_config.tlv_size > 0){
                    memcpy(sSetTestConfig, eventData->sTest_set_config.param_ids,eventData->sTest_set_config.tlv_size);
                }
        ...
        case UWA_DM_TEST_GET_CONFIG_RSP_EVT: /* Result of UWA_TestGetConfig */
            JNI_TRACE_I("%s: UWA_DM_TEST_GET_CONFIG_RSP_EVT", __func__);
            {
                ...
                if(eventData->sTest_get_config.tlv_size){
                    memcpy(sGetTestConfig, eventData->sTest_get_config.param_tlvs, eventData->sTest_get_config.tlv_size);
                }
        ...
        case UWA_DM_TEST_PERIODIC_TX_NTF_EVT:
            JNI_TRACE_I("%s: UWA_DM_TEST_PERIODIC_TX_NTF_EVT", __func__);
            {
                ...
                if(eventData->rf_test_data.length > 0) {
                    uwbRfTestManager.onPeriodicTxDataNotificationReceived(eventData->rf_test_data.length, &eventData->rf_test_data.data[0]);
                }
            }
            break;
        ...
        case UWA_DM_TEST_PER_RX_NTF_EVT:
            JNI_TRACE_I("%s: UWA_DM_TEST_PER_RX_NTF_EVT", __func__);
            {
                IsRfTestOngoing = false;
                if(eventData->rf_test_data.length > 0) {
                    uwbRfTestManager.onPerRxDataNotificationReceived(eventData->rf_test_data.length, &eventData->rf_test_data.data[0]);
                }
            }
            break;
        ...
        case UWA_DM_TEST_LOOPBACK_NTF_EVT:
            JNI_TRACE_I("%s: UWA_DM_TEST_LOOPBACK_NTF_EVT", __func__);
            {
                IsRfTestOngoing = false;
                if(eventData->rf_test_data.length > 0) {
                    uwbRfTestManager.onLoopBackTestDataNotificationReceived(eventData->rf_test_data.length, &eventData->rf_test_data.data[0]);
                }
            }
            break;
        ...
        case UWA_DM_TEST_RX_NTF_EVT:
            JNI_TRACE_I("%s: UWA_DM_TEST_RX_NTF_EVT", __func__);
            {
                IsRfTestOngoing = false;
                if(eventData->rf_test_data.length > 0) {
                    uwbRfTestManager.onRxTestDataNotificationReceived(eventData->rf_test_data.length, &eventData->rf_test_data.data[0]);
                }
            }
            break;

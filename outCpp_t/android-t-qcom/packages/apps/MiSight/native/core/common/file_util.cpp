 ...
 * Description: misight file folder implement head file
 *
 */
#include "file_util.h"
...
bool CreateDirectory(const std::string& path, const mode_t& mode)
{
    if (access(path.c_str(), F_OK) == 0) {
        MISIGHT_LOGD("%s exist, no need create", path.c_str());
        return true;
    }
    ...
    while ((n = dirPath.find("/", n + 1)) != std::string::npos) {
        ...
        if (access(subPath.c_str(), F_OK) == -1) {
            if (mkdir(subPath.c_str(), FILE_ALL_MODE) == -1) { // 0777: create wiht all rights
                MISIGHT_LOGE("mkdir %s failed, %s, err:%s", subPath.c_str(), path.c_str(), strerror(errno));
                return false;
            }
    ...
    n = std::string::npos;
    while ((n = dirPath.rfind("/")) != std::string::npos) {
        dirPath = dirPath.substr(0, n);
        if (!ChangeMode(dirPath, mode)) {
            MISIGHT_LOGE("change mode %s failed", dirPath.c_str());
            return false;
        }
        if (n <= pos) {
            break;
...
{
    if (!CreateDirectory(path, FILE_EXEC_MODE)) {
        return false;
    }
...
{
    if (fd <= 0) {
        MISIGHT_LOGE("invalid fd:%d", fd);
        return false;
    }

    if (content.empty()) {
        MISIGHT_LOGD("content is empty, no need to save!");
        return true;
    }

    const long len = write(fd, content.c_str(), content.length());
    if (len < 0) {
        MISIGHT_LOGE("write file failed! errno:%d, err:%s", errno, strerror(errno));
        return false;
    }

    if ((unsigned long)len != content.length()) {
        MISIGHT_LOGE("the length write to file is not equal to fileLength!len:%ld, fileLen:%zu",
            len, content.length());
        return false;
    }
...
void DeleteFile(const std::string& filePath)
{
    ...
    if (remove(filePath.c_str()) != 0) {
        MISIGHT_LOGE("remove file %s errno:%d, err:%s", filePath.c_str(), errno, strerror(errno));
    }
}
...
bool MoveFile(const std::string& srcFile, const std::string& dstFile)
{
    ...
    if (rename(srcFile.c_str(), dstFile.c_str()) != 0) {
        MISIGHT_LOGE("move file %s to %s failed, err:%d, %s", srcFile.c_str(), dstFile.c_str(), errno, strerror(errno));
        return false;
    }
    return true;
}
...
bool IsFileLatest(const std::string& srcFile, const std::string& dstFile)
{
    ...
    memset(&dstState, 0, sizeof(struct stat));

    (void)stat(srcFile.c_str(), &srcState);
    (void)stat(dstFile.c_str(), &dstState);
...
size_t GetDirectorySize(std::string& dirString)
{
    ...
    if ((dp = opendir(dir)) == NULL) {
        MISIGHT_LOGE("Cannot open dir: %s\n", dir);
        return -1;
    }

  ...
  while (ptr + NLA_HDRLEN <= end_ptr) {
    ...
    if (header->nla_type == attr_id) {
      if (ptr + NLA_ALIGN(header->nla_len) > end_ptr) {
        ALOGE("Failed to get attribute: broken nl80211 atrribute.");
        return false;
      }
...
bool BaseNL80211Attr::Merge(const BaseNL80211Attr& other_attr) {
  if (!other_attr.IsValid()) {
    ALOGE("Can not merge invalid attribute");
    return false;
  }
  if (GetAttributeId() != other_attr.GetAttributeId()) {
    ALOGE("Can not merge attributes with different ids");
    return false;
  }
...
bool NL80211NestedAttr::GetListOfNestedAttributes(
  ...
  while (ptr + NLA_HDRLEN <= end_ptr) {
    const nlattr* header = reinterpret_cast<const nlattr*>(ptr);
    if (ptr + NLA_ALIGN(header->nla_len) > end_ptr) {
      ALOGE("Failed to get list of nested attributes: invalid nla_len.");
      return false;
    }
...
void NL80211NestedAttr::DebugLog() const {
  ...
  while (ptr + NLA_HDRLEN <= end_ptr) {
    const nlattr* header = reinterpret_cast<const nlattr*>(ptr);
    if (ptr + NLA_ALIGN(header->nla_len) > end_ptr) {
      ALOGE("broken nl80211 atrribute.");
      return;
    }
    ALOGE("Have attribute with nla_type= %d  and nla_len= %d", header->nla_type, header->nla_len);
    if (header->nla_len == 0) {
      ALOGE("0 is a bad nla_len");
      return;
    }
    ptr += NLA_ALIGN(header->nla_len);
  }

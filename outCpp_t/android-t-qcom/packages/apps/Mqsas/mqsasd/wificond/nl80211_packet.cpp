...
NL80211Packet::NL80211Packet(const NL80211Packet& packet) {
  data_ = packet.data_;
 ALOGE("Copy constructor is only used for unit tests");
}
...
bool NL80211Packet::IsValid() const {
  ...
  if (data_.size() < NLMSG_HDRLEN) {
    ALOGE("Cannot retrieve netlink header.");
    return false;
  }
  ...
  if (GetMessageType() >= NLMSG_MIN_TYPE) {
    if (data_.size() < NLMSG_HDRLEN + GENL_HDRLEN ||
      ...
      ALOGE("Cannot retrieve generic netlink header.");
      return false;
    }
  }
  ...
  if (data_.size() < nl_header->nlmsg_len ||
    ...
    ALOGE("Discarding incomplete / invalid message.");
    return false;
  }
  return true;
}

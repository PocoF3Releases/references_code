...
#ifndef MULTI_A2DP_ENABLE
#define MULTI_A2DP_ENABLE
#endif
...
/* Bluetooth AV mulicast states */
typedef enum {
    BTAV_MULTICAST_DISABLE = 0,
    BTAV_MULTICAST_ENABLE,
    BTAV_MULTICAST_PLAYING,
    BTAV_MULTICAST_DISCONNECT,
    BTAV_MULTICAST_DISCONNECTED,
    BTAV_MULTICAST_SWITCH_SLAVE_DEVICE,
} btav_mulicast_state_t;
/*
...
typedef enum {
  ...
  BTAV_A2DP_QVA_CODEC_INDEX_SOURCE_MAX = 10,

#ifdef SUPPORT_LHDC_CODEC
  BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV2 = BTAV_A2DP_QVA_CODEC_INDEX_SOURCE_MAX + 2,
  BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV3,
  BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV1,
  BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV5 = BTAV_A2DP_CODEC_INDEX_SOURCE_LHDCV1 + 5,
...
typedef struct btav_a2dp_codec_config_t {
  ...
  uint8_t codec_max_target_bitrate;
  uint8_t codec_channel_split_mode;
...
typedef void (* btav_multicast_state_callback)(btav_mulicast_state_t state);
...
typedef struct {
  ...
  btav_multicast_state_callback multicast_state_cb;
  #endif
...
typedef struct {
  ...
  #ifdef MULTI_A2DP_ENABLE
  bt_status_t (*enable_multi_a2dp)(const RawAddress& bd_addr);
  bt_status_t (*disable_multi_a2dp)(void);
  bt_status_t (*set_multi_a2dp_device)(const RawAddress& bd_addr,int value,int type);
  int (*get_multi_a2dp_device)(const RawAddress& bd_addr,int type);
  #endif

...
#define LOG_TAG "bt_a2dp_hw_aosp"
...
static int a2dp_read_output_audio_config(
  ...
  if (a2dp_ctrl_receive(common, &common->isLLSupported,
                        sizeof(bool)) < 0) {
    return -1;
  }

  DEBUG("common->codec_index = %d, common->isLLSupported = %d", common->codec_index, common->isLLSupported);
#endif
  ...
  if (update_stream_config) {
    ...
    common->buffer_sz = audio_a2dp_hw_stream_compute_buffer_size(
        ...
        codec_config->channel_mode, common->isLLSupported);
#else
    common->buffer_sz = audio_a2dp_hw_stream_compute_buffer_size(
        codec_config->sample_rate, codec_config->bits_per_sample,
...
#ifdef SUPPORT_LHDC_CODEC
    btav_a2dp_codec_channel_mode_t codec_channel_mode, bool isLLSupported) {
#else
    btav_a2dp_codec_channel_mode_t codec_channel_mode) {
#endif
  size_t buffer_sz = AUDIO_STREAM_OUTPUT_BUFFER_SZ;  // Default value
#ifdef SUPPORT_LHDC_CODEC
  const uint64_t time_period_ms = (isLLSupported == true) ? 11 : 20;                // Conservative 20ms
#else

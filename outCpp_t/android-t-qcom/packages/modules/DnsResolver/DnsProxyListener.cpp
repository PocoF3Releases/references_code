...
DnsProxyListener::DnsProxyListener() : FrameworkListener(SOCKET_NAME) {
    ...
    // MI ADD START:
    registerCmd(new SetOperatorAddressDnsCmd());
    // END
}
...
void DnsProxyListener::GetAddrInfoHandler::run() {
    ...
    LocalRecord::get()->addAddrInfo(mClient->getPid(), mClient->getUid(), mHost.c_str(), result);
...
int DnsProxyListener::GetAddrInfoCmd::runCommand(SocketClient* cli, int argc, char** argv) {
    ...
    // MI ADD START:
    uid_t cli_uid = cli->getUid();
    if(DnsProxyBlocker::getInstance()->isDnsBlocked(cli_uid)) {
        LOG(DEBUG) << "block dns query from uid = " << cli_uid;
        uint32_t rv = 0x07;
        cli->sendBinaryMsg(ResponseCode::DnsProxyOperationFailed, &rv, sizeof(rv));
        return -1;
    }
    // END
    ...
    const pid_t pid = cli->getPid();
    // MIUI ADD:
    if (DnsProxyBlocker::getInstance()->isGetPhoneNumberAddress(name)) {
        LOG(DEBUG) << "operator get phone number！ address: " << name.c_str();
        std::string specialPerm = "MIUI-APPOP;10039;" + std::to_string(uid) + ";" + std::to_string(pid);
        if (!gResNetdCallbacks.check_calling_permission(specialPerm.c_str())) {
             uint32_t rv = 0x07;
             cli->sendBinaryMsg(ResponseCode::DnsProxyOperationFailed, &rv, sizeof(rv));
             return -1;
        }
...
// MI ADD START:
DnsProxyListener::SetOperatorAddressDnsCmd::SetOperatorAddressDnsCmd() :
    FrameworkCommand("setoperatoraddress") {}
int DnsProxyListener::SetOperatorAddressDnsCmd::runCommand(SocketClient* cli, int argc, char** argv) {
    logArguments(argc, argv);
    const uid_t cli_uid = cli->getUid();
    if (cli_uid != 1000) {
        sendCodeAndBe32(cli, ResponseCode::OperationFailed, -EPERM);
        return -1;
    }
    if (argc != 2) {
        LOG(WARNING) << "SetOperatorAddressDnsCmd::runCommand: setoperatoraddress: from UID "
                     << cli_uid << ", invalid number of arguments to setoperatoraddress: " << argc;
        sendCodeAndBe32(cli, ResponseCode::DnsProxyQueryResult, -EINVAL);
        return -1;
    }
    std::string address = argv[1];
    (new SetOperatorAddressDnsHandler(cli, address))->spawn();
    return 0;
}
DnsProxyListener::SetOperatorAddressDnsHandler::SetOperatorAddressDnsHandler(
    SocketClient* c, std::string address) : Handler(c), mAddress(std::move(address)) {}
void DnsProxyListener::SetOperatorAddressDnsHandler::run() {
    if(mAddress.find("misecfirewall") == 0) {
        std::vector<std::string> parameter;
        std::string delimiter = ">";
        size_t pos = 0;
        std::string info;
        while ((pos = mAddress.find(delimiter)) != std::string::npos) {
            info = mAddress.substr(0, pos);
            parameter.push_back(info);
            mAddress.erase(0, pos + delimiter.length());
        }
        parameter.push_back(mAddress);
        int parameterSize = parameter.size();
        if (parameterSize == 2) {
            DnsProxyBlocker::getInstance()->setDefaultNetworkType(std::stoi(parameter[1]));
        } else if (parameterSize == 4) {
            DnsProxyBlocker::getInstance()->updateFirewallForUid(std::stoi(parameter[1]),
                std::stoi(parameter[2]),std::stoi(parameter[3]));
        }
    } else if (!strcmp(mAddress.c_str(), "*clear"))
        DnsProxyBlocker::getInstance()->clearAddress();
    else
        DnsProxyBlocker::getInstance()->addAddress(mAddress);
    mClient->sendMsg(ResponseCode::CommandOkay, "Set dns operator address completed", false);
}
std::string DnsProxyListener::SetOperatorAddressDnsHandler::threadName() {
    return makeThreadName(0, mClient->getUid());
}

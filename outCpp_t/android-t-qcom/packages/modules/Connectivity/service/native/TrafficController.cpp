...
const std::string uidMatchTypeToString(uint32_t match) {
    ...
    // MIUI ADD:
    FLAG_MSG_TRANS(matchType, AUROGON_MATCH, match);
    if (match) {
...
// MIUI ADD: START
Status TrafficController::updateAurogonUidRule(uint32_t uid, bool allow) {
    std::lock_guard guard(mMutex);

    if (allow) {
        RETURN_IF_NOT_OK(addRule(uid, AUROGON_MATCH));
    } else {
        RETURN_IF_NOT_OK(removeRule(uid, AUROGON_MATCH));
    }
    return netdutils::status::ok;
}
// END

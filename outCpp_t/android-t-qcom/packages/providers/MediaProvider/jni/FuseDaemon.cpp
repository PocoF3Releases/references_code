...
const std::string LOG_STATE = android::base::GetProperty("log.tag.FuseDaemon", "DEBUG");

const bool IS_FUSEDAEMON_INFO = LOG_STATE.compare("VERBOSE") == -1;
// logging macros to avoid duplication.
#define TRACE_NODE(__node, __req)                                                  \
    if (IS_FUSEDAEMON_INFO) { } else LOG(VERBOSE) << __FUNCTION__ << " : " << #__node << " = [" << get_name(__node) \
                 << "] (uid=" << (__req)->ctx.uid << ") "
...
void FuseDaemon::InvalidateFuseDentryCache(const std::string& path) {
    LOG(VERBOSE) << "Invalidating FUSE dentry cache path: "<< path;
    if (active.load(std::memory_order_acquire)) {
        string name;
        fuse_ino_t parent;
        fuse_ino_t child;

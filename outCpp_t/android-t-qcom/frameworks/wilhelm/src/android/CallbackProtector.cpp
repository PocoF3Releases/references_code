...
CallbackProtector::~CallbackProtector() {
    ...
    if(mCallTrack && mCallTrack->mAudioTrack){
        mCallTrack->mAudioTrack->stopAndJoinCallbacks();
    }
...
void CallbackProtector::getTrackPlayer(sp<android::TrackPlayerBase> mTrack){
        mCallTrack = mTrack;
}

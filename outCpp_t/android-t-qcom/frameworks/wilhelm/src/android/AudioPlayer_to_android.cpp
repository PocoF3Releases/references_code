...
#define FADE_IN_COUNT 10
#define APP_CACHE_BUFFER_TIME_MS 120
int curUid = 0;
int totalMuteCount = 0;
...
bool isInWhiteList() {
    ...
    if (curUid == uid) {
        ...
        android::PermissionController permissionController;
        android::Vector<android::String16> pkgs;
        //It takes 2ms
        permissionController.getPackagesForUid(uid, pkgs);
        if (!pkgs.isEmpty()) {
            std::string curPkgName = android::String8(pkgs[0]).string();
            if (PackageNames.find(curPkgName) != PackageNames.end()) {
                curUid = uid;
                ret = true;
            }
        }
...
size_t audioTrack_handleMoreData_lockPlay(CAudioPlayer* ap,
    ...
    size_t bytesWritten = 0;
        ...
        if (ap->mBufferQueue.mState.count != 0) {
            ...
            //Workround for fix noise when the game screen recording start
            if (ap->mOnNewIAudioTrack &&
                ...
                //Init totalMuteCount
                if (totalMuteCount == 0) {
                    int queueFrameCount = 0;
                    if ((df_pcm->bitsPerSample != 0) && (df_pcm->numChannels != 0)) {
                        queueFrameCount = ap->mBufferQueue.mFront->mSize /
                            (df_pcm->bitsPerSample/8) / df_pcm->numChannels;
                    }
                    if ((queueFrameCount > 0) && (df_pcm->samplesPerSec != 0)) {
                        int startThresholdBufferCount =
                            ap->mTrackPlayer->mAudioTrack->getStartThresholdInFrames() /
                                queueFrameCount;
                        int appCacheBufferCount = APP_CACHE_BUFFER_TIME_MS /
                            (queueFrameCount / (sles_to_android_sampleRate(df_pcm->samplesPerSec) / 1000));
                        int queueBufferCount = ap->mBufferQueue.mNumBuffers;
                        totalMuteCount = startThresholdBufferCount +
                            appCacheBufferCount +
                            queueBufferCount;
                    }
                }
                int remainingMuteCount = totalMuteCount - ap->mMuteQueueCount;
                if (remainingMuteCount >= 0) {
                    //Fadein
                    if (remainingMuteCount <= FADE_IN_COUNT) {
                        float v = (FADE_IN_COUNT-remainingMuteCount) * (1.0f / FADE_IN_COUNT);
                        ap->mTrackPlayer->mAudioTrack->setVolume(v);
                    } else {
                        //Mute dirty buffer
                        ap->mTrackPlayer->mAudioTrack->setVolume(0);
                    }
                } else {
                    ap->mMuteQueueCount = 0;
                    ap->mOnNewIAudioTrack = false;
                    ap->mTrackPlayer->mAudioTrack->setVolume(1);
                }
            } else {
                ap->mMuteQueueCount = 0;
                ap->mOnNewIAudioTrack = false;
            }
            //Workround for fix noise when the game screen recording end
            ...
            // stop the track so it restarts playing faster when new data is enqueued,But treat mxplayer differently
            if (android::String16("com.mxtech.videoplayer.ad").compare(ap->mTrackPlayer->mAudioTrack->getPackagenameInMcbf())) {
                ALOGD("Stop AudioTrack from opensl es.");
...
void android_audioPlayer_create(CAudioPlayer *pAudioPlayer) {
    ...
    pAudioPlayer->mCallbackProtector->getTrackPlayer(pAudioPlayer->mTrackPlayer);
    ...
    case AUDIOPLAYER_FROM_PCM_BUFFERQUEUE: {
        ...
        if (channelMask == AUDIO_CHANNEL_INVALID
            ...
            if (FeatureManager::isFeatureEnable(AUDIO_DOLBY_ENABLE) &&
                ...
                if (df_pcm->channelMask == SL_ANDROID_SPEAKER_5DOT1DOT2) {
                    channelMask = AUDIO_CHANNEL_OUT_5POINT1POINT2;
                }
            }
            // MIUI END
        ...
        audio_attributes_t* attr = nullptr;
        audio_attributes_t attr_init = AUDIO_ATTRIBUTES_INITIALIZER;
        if (df_pcm->channelMask == SL_ANDROID_SPEAKER_5DOT1DOT2 &&
            FeatureManager::isFeatureEnable(AUDIO_DOLBY_ENABLE) &&
            FeatureManager::isFeatureEnable(AUDIO_DOLBY_ATMOS_GAME)) {
            policy = (audio_output_flags_t)(policy & ~AUDIO_OUTPUT_FLAG_RAW);
            attr_init.flags = AUDIO_FLAG_NEVER_SPATIALIZE;
            attr_init.usage = AUDIO_USAGE_GAME;
            attr = &attr_init;
        }
        //MIUI ADD: END

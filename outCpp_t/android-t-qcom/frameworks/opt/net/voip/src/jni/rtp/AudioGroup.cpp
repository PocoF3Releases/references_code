...
// MIUI ADD: START
bool AudioGroup::setRelay(int socket) {
    if (mMode == RELAY && mSocket == socket) {
        return true;
    }
    mDeviceThread->requestExitAndWait();
    close(mSocket);
    mSocket = socket;
    ALOGD("group[%d] connected to relay socket[%d]", mDeviceSocket, mSocket);
    mMode = RELAY;
    return mDeviceThread->start();
}
// END
...
bool AudioGroup::DeviceThread::threadLoop()
{
    ...
    // MIUI ADD: START
    if (mode == RELAY) {
        output *= audio_bytes_per_sample(AUDIO_FORMAT_PCM_16_BIT);
        input *= audio_bytes_per_sample(AUDIO_FORMAT_PCM_16_BIT);
        int socket = mGroup->mSocket;
        setsockopt(deviceSocket, SOL_SOCKET, SO_RCVBUF, &input, sizeof(input));
        setsockopt(deviceSocket, SOL_SOCKET, SO_SNDBUF, &output, sizeof(output));
        setsockopt(socket, SOL_SOCKET, SO_RCVBUF, &input, sizeof(input));
        setsockopt(socket, SOL_SOCKET, SO_SNDBUF, &output, sizeof(output));
        ...
        epoll_event event;
        event.events = EPOLLIN;
        event.data.fd = deviceSocket;
        if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, deviceSocket, &event) == -1) {
        ...
        if (epoll_ctl(epoll_fd, EPOLL_CTL_ADD, socket, &event) == -1) {
            ALOGE("epoll_ctl: socket");
            close(epoll_fd);
            return false;
        }

        ALOGD("relay loop enter");
        while (!exitPending()) {
            ...
            if (n == 0) continue;
            if (event.data.fd == deviceSocket) {
                int16_t buffer[output];
                size_t len = recv(deviceSocket, buffer, sizeof(buffer), MSG_DONTWAIT);
                if (len > 0) {
                    send(socket, buffer, len, MSG_DONTWAIT);
                }
            } else {
                int16_t buffer[input];
                size_t len = recv(socket, buffer, sizeof(buffer), MSG_DONTWAIT);
                if (len > 0) {
                    send(deviceSocket, buffer, len, MSG_DONTWAIT);
                }
            }
        }
        ALOGD("relay loop exit");
        close(epoll_fd);
        return true;
    }
    // END
...
// MIUI ADD: START
void setRelay(JNIEnv *env, jobject thiz, jint socket)
{
    AudioGroup *group = (AudioGroup *)env->GetLongField(thiz, gNative);
    if (group && !group->setRelay(socket)) {
        jniThrowException(env, "java/lang/IllegalArgumentException", NULL);
    }
}
// END
...
JNINativeMethod gMethods[] = {
    ...
    // MIUI ADD:
    {"nativeSetRelay", "(I)V", (void *)setRelay},

...
LayoutPiece::LayoutPiece(const U16StringPiece& textBuf, const Range& range, bool isRtl,
                         ...
                         EndHyphenEdit endHyphen, const U16StringPiece& fullTextBuf /*MIUI ADD*/) {
    ...
    // MIUI ADD
    Range fullRange(0, fullTextBuf.size());
    MLEOptimizer& opt = MLEOptimizer::getInstance();
    unsigned int charAcc = 0;
    opt.initSpace(isRtl, paint, startHyphen, endHyphen);
    // END
    ...
    for (int run_ix = isRtl ? items.size() - 1 : 0;
        ...
        for (ssize_t scriptRunStart = run.start; scriptRunStart < run.end;
            ...
            //ALOGE("MLETEST_1 numGlyphs:%u", numGlyphs);
            for (unsigned int i = 0; i < numGlyphs; i++) {
                ...
                // MIUI ADD:
                bool miui_match = false;
                bool hans_num = false;
                uint32_t charIndex = start + charAcc + i;
                if (i == 0) { // Hans Word maybe
                    std::pair<bool, bool> match = opt.isMatch(opt.getLastPieceTail(fullTextBuf), textBuf.at(charIndex));
                    miui_match = match.first;
                    hans_num = match.second;
                }
                if (i != 0) {
                    std::pair<bool, bool> match = opt.isMatch(textBuf.at(charIndex - 1), textBuf.at(charIndex));
                    miui_match = match.first;
                    hans_num = match.second;
                }
                 float miui_gap = opt.getMiuiGap(hans_num);
                // END
                float xoff = HBFixedToFloat(positions[i].x_offset);
                // MIUI ADD:
                xoff += miui_match ? miui_gap : 0.0;
                // END
                ...
                xAdvance += miui_match ? miui_gap : 0.0;
                // if (textBuf.at(charIndex) == opt.getSpaceCode()) { // space
                //     xAdvance = miui_gap;
                // }
                // END
                ...
                // MIUI ADD:
                if (numGlyphs > 0) {
                    opt.resetLastPieceTail(fullTextBuf, textBuf.at(charIndex));
                }
                // END
            }
            if (numGlyphs) {
...
// MIUI ADD: MIUILayoutPiece
MIUILayoutPiece::MIUILayoutPiece(const U16StringPiece& textBuf, const Range& range, bool isRtl,
                         const MinikinPaint& paint, StartHyphenEdit startHyphen, EndHyphenEdit endHyphen, bool measureSpace) {
    const uint16_t* buf = textBuf.data();
    const size_t start = range.getStart();
    const size_t count = range.getLength();
    const size_t bufSize = textBuf.size();
    ...
    for (int run_ix = isRtl ? items.size() - 1 : 0;
        ...
        for (ssize_t scriptRunStart = run.start; scriptRunStart < run.end;
            ...
            [[maybe_unused]] const uint32_t clusterStart =
                    addToHbBuffer(buffer, buf, start, count, bufSize, scriptRunStart, scriptRunEnd,
                                  startHyphen, endHyphen, hbFont);
            ...
            if (measureSpace) {
                positions = hb_buffer_get_glyph_positions(buffer.get(), NULL);
            }

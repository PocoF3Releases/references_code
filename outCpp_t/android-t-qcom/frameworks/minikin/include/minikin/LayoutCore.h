...
public:
    LayoutPiece(const U16StringPiece& textBuf, const Range& range, bool isRtl,
                ...
                const U16StringPiece& fullTextBuf /*MIUI ADD*/);
...
private:
    ...
    //MIUI ADD:
    bool isOpted = false; // for cache check
    //
...
// MIUI ADD:
// this class is a wrapper of  hb, used to get all glyph info of the fullTextBuf
class MIUILayoutPiece {
public:
    MIUILayoutPiece(const U16StringPiece& textBuf, const Range& range, bool isRtl,
                const MinikinPaint& paint, StartHyphenEdit startHyphen, EndHyphenEdit endHyphen, bool measureSpace /*Perf Opt*/);

    // Helper accessors
    uint32_t glyphCount() const { return mGlyphIds.size(); }
    const FakedFont& fontAt(int glyphPos) const { return mFonts[mFontIndices[glyphPos]]; }
    uint32_t glyphIdAt(int glyphPos) const { return mGlyphIds[glyphPos]; }
    float advanceAt(int glyphPos) const { return mAdvances[glyphPos]; }

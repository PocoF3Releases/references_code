...
public:
    ...
    void getOrCreate(const U16StringPiece& text, const Range& range, const MinikinPaint& paint,
        ...
        if (paint.skipCache() || range.getLength() >= LENGTH_LIMIT_CACHE) {
            LayoutPiece piece = LayoutPiece(text, range, dir, paint, startHyphen, endHyphen, text/*MIUI ADD*/);
        ...
        key.copyText();
        ValueExtractor ve;
        LayoutCache::getInstance().getOrCreate(text, range, paint, dir, startHyphen, endHyphen, text/*MIUI ADD*/,ve);

...
public:
    ...
    bool operator==(const LayoutCacheKey& o) const {
        return mId == o.mId && mStart == o.mStart && mCount == o.mCount && mStyle == o.mStyle &&
               ...
               mEndHyphen == o.mEndHyphen && mIsRtl == o.mIsRtl && mIsMLEOpted == o.mIsMLEOpted /* MIUI ADD */ &&
               mNchars == o.mNchars &&
...
public:
    ...
    void getOrCreate(const U16StringPiece& text, const Range& range, const MinikinPaint& paint,
                     bool dir, StartHyphenEdit startHyphen, EndHyphenEdit endHyphen, const U16StringPiece& fullText /*MIUI ADD*/,F& f) {
        LayoutCacheKey key(text, range, paint, dir, startHyphen, endHyphen);
        if (paint.skipCache() || range.getLength() >= LENGTH_LIMIT_CACHE) {
            f(LayoutPiece(text, range, dir, paint, startHyphen, endHyphen, fullText/*MIUI ADD*/), paint);
            return;
        }
        // MIUI ADD
        MLEOptimizer& opt = MLEOptimizer::getInstance();
        bool miui_match = false;
         auto match= opt.isMatch(opt.getLastPieceTail(fullText), text.at(range.getStart()));
...

        {
            ...
            if (layout != nullptr) {
                // MIUI ADD:
                uint32_t lastCharIndex = range.getEnd() - 1;
                opt.resetLastPieceTail(fullText, text.at(lastCharIndex));
                // END
                f(*layout, paint);
                return;
            }
        ...
        std::unique_ptr<LayoutPiece> layout =
                std::make_unique<LayoutPiece>(text, range, dir, paint, startHyphen, endHyphen, fullText /*MIUI ADD*/);
        f(*layout, paint);
        {
            std::lock_guard<std::mutex> lock(mMutex);

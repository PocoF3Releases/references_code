...
//  float w[nfft*5/4];
    float * w = (float *)malloc(nfft * 5 / 4 * sizeof(float));

    int isgn = 1;
    rdft(nfft, isgn, input, ip, w);
    for(int i = 0; i < nfft/2; i++)
    {
        if(i == 0)
        {
            ...
            output[0].imag = 0;
            output[nfft/2].real = input[1];
            output[nfft/2].imag = 0;
        }
        else
        {
            output[i].real = input[2*i];
            output[i].imag = input[2*i+1];
            output[nfft-i].real = output[i].real;
            output[nfft-i].imag = output[i].imag;
        }
...
//  float w[nfft*5/4];
    ...
    float * w = (float *)malloc(nfft * 5 / 4 * sizeof(float));
    memset(ip,0,(NMAXSQRT + 2)*sizeof(int));

    int isgn = -1;
//  float input_ptr[nfft];
    float * input_ptr = (float *)malloc(nfft * sizeof(float));
    memset(input_ptr,0,nfft*sizeof(float));

    for(int i = 0; i < nfft/2 ; i++)
    {
        if(i==0)
        {
            input_ptr[0] = input[0].real;
            input_ptr[1] = input[nfft/2].real;
            //printf("input_ptr[%d] = %f+%fi \n",i, input_ptr[2*i],0);
            //printf("input_ptr[%d] = %f+%fi \n",nfft/2, input_ptr[2*i+1],0);
        }
        else
        {
            input_ptr[2*i] = input[i].real;
            input_ptr[2*i+1] = input[i].imag;
            //printf("input_ptr[%d] = %f+%fi \n",i, input_ptr[2*i],input_ptr[2*i+1]);
        }
    }


    rdft(nfft, isgn, input_ptr, ip, w);

    for(int i = 0; i < nfft; i++)
    {
        output[i] = input_ptr[i]*2/nfft;
        //output[nfft-i] = output[i];
    }
/*
    for(int i = 0; i < nfft; i++)
    {
        printf("input_ptr[%d] = %f \n",i, input_ptr[i]*2/nfft);
    }
*/

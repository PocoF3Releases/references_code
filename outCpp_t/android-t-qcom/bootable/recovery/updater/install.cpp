...
void uiPrintf(State* _Nonnull state, const char* _Nonnull format, ...) {
  ...
  va_end(ap);
  state->updater->UiPrint(error_msg);
}
...
Value* PackageExtractFileFn(const char* name, State* state,
  ...
  if (argv.size() == 2) {
    ...
    // MIUI ADD: START
    LOG(DEBUG) << "package_extract_file to "<< dest_path << " took " << getdifftime(t) << "s";
    uiPrintf(state, "package_extract_file to %s took %.2lds.\n", dest_path.c_str(), getdifftime(t));
    // END
    ...
    // MIUI ADD: START
    LOG(DEBUG) << "package_extract_file took " << getdifftime(t) << "s";
    uiPrintf(state, "package_extract_file from %s to memory took %.2lds.\n", zip_path.c_str(), getdifftime(t));
    // END
...
Value* PatchPartitionCheckFn(const char* name, State* state,
  ...
  // MIUI ADD: START
  LOG(DEBUG) << "PatchPartitionCheckFn took " << getdifftime(t) << "s";
  uiPrintf(state, "PatchPartitionCheckFn took %.2lds.\n", getdifftime(t));
  // END
...
Value* MountFn(const char* name, State* state, const std::vector<std::unique_ptr<Expr>>& argv) {
  ...
  // MIUI ADD
  LOG(DEBUG) << "MountFn took " << getdifftime(t) << "s";
...
Value* RunProgramFn(const char* name, State* state, const std::vector<std::unique_ptr<Expr>>& argv) {
  ...
  // MIUI ADD
  LOG(INFO) << "RunProgramFn took " << getdifftime(t) << "s";
...
// MIUI ADD: START
#ifdef MTK_VENDOR
  Register_librecovery_updater_mtk();
#endif
// END
}

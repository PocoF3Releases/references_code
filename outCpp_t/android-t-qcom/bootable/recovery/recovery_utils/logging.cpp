...
FILE* fopen_path(const std::string& path, const char* mode, selabel_handle* sehandle) {
...
std::vector<saved_log_file> ReadLogFilesToMemory() {
  ...
  //MIUI ADD: START
  std::string wipe_confirmed = android::base::GetProperty("vendor.wipecache.menu.confirmed", "");
  if (strcmp(wipe_confirmed.c_str(),"yes"))
  {
    LOG(INFO) << "wipe cache is not selected from recovery menu,so save logs";
    struct dirent *mi_de;
    struct stat mi_sb;
    for (const auto& str : EraseCacheWhitelist){
      std::unique_ptr<DIR, decltype(&closedir)> mi_d(opendir(str.c_str()), closedir);
      if (mi_d) {
        while ((mi_de = readdir(mi_d.get())) != nullptr) {
          if (strncmp(mi_de->d_name, ".", 1)) {
            std::string mi_path = android::base::StringPrintf("%s/%s", str.c_str(), mi_de->d_name);
            LOG(INFO) << "cache files: " << mi_path.c_str();
            if (stat(mi_path.c_str(), &mi_sb) == 0) {
              if(S_ISDIR(mi_sb.st_mode)) continue;

              std::string mi_data(mi_sb.st_size, '\0');
              FILE* f = fopen(mi_path.c_str(), "rb");
              fread(&mi_data[0], 1, mi_data.size(), f);
              fclose(f);

              log_files.emplace_back(saved_log_file{ mi_path, mi_sb, mi_data });
            }
          }
        }
      }
...
bool RestoreLogFilesAfterFormat(const std::vector<saved_log_file>& log_files) {
  ...
  //MIUI ADD: START
  for (const auto& str : EraseCacheWhitelist){
    if (mkdir_recursively(str.c_str(), 0777, false, logging_sehandle) != 0) {
      PLOG(ERROR) << "Failed to create dir " << str.c_str();
      return false;
    }
  }
  // END

...
// MIUI ADD: START
[[maybe_unused]]static void gr_init_font(void) {
  ...
  if (gr_font == nullptr) {
    printf("gr_font calloc blk fail\n");
    return;
  }
  gr_font->texture = static_cast<GRSurface*>(malloc(sizeof(*gr_font->texture)));
  if (gr_font->texture == nullptr) {
    free(gr_font);
    printf("gr_font->texture malloc blk fail\n");
    return;
  }
  gr_font->texture->width = FONT_S.width;
  gr_font->texture->height = FONT_S.height;
  gr_font->texture->row_bytes = FONT_S.width;
  gr_font->texture->pixel_bytes = 1;

  unsigned char* bits = static_cast<unsigned char*>(malloc(FONT_S.width * FONT_S.height));
  if (bits == nullptr) {
    free(gr_font->texture);
    free(gr_font);
    printf("gr_font->texture->data malloc blk fail\n");
    return;
  }
  //gr_font->texture->data = bits;
  gr_font->texture = GRSurface::Create(FONT_S.width, FONT_S.height, FONT_S.width, 1).release();

  unsigned char data;
  unsigned char* in = FONT_S.rundata;
  while ((data = *in++)) {
    memset(bits, (data & 0x80) ? 255 : 0, data & 0x7f);
    bits += (data & 0x7f);
  }

  gr_font->char_width = FONT_S.char_width;
  gr_font->char_height = FONT_S.char_height;
}
// END

...
// MIUI ADD : START
int internal_res_create_display_surface(const char* language, const char* name,
                                        GRSurface** pSurface);
// END
...
int res_create_display_surface_from_rgb_data(const char* data, unsigned int width, unsigned int height, GRSurface** pSurface);

// MIUI ADD: START
// mi add for multi-display
// Initializes the graphics backend and loads font file. Returns 0 on success, or -1 on error. Note
// that the font initialization failure would be non-fatal, as caller may not need to draw any text
// at all. Caller can check the font initialization result via gr_sys_font() as needed.
int gr_init_mi();

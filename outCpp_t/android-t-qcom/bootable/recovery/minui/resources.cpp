...
// MIUI ADD: START
static const int LOC_NUM = 78;     // the number of recovery supported languages
static const int LOC_LEN_FULL = 6; // the full length of locale
static const int LOC_LEN_PART = 2; // the prefix length before underscore, will be used in several similar languages case such as "en_GB","en_US","en_IN".
static const char RECOVERY_SUPPORT_LOCALES[LOC_NUM][LOC_LEN_FULL] = {
        "en",
        "az_AZ",
        "ms_MY",
        "bs_BA",
...
PngHandler::PngHandler(const std::string& language, const std::string& name) {
  std::string res_path = g_resource_dir + "/" + name + ".png";
  // MIUI ADD: START
  if (!language.empty()) {
    res_path = android::base::StringPrintf("%s/%s/%s.png", g_resource_dir.c_str(), language.c_str(), name.c_str());
  }
  // END
  ...
  // MIUI ADD: START
    ...
    // 8-bit ARGB images: great, nothing to do.
  // END
    ...
    fprintf(stderr, "%s: minui doesn't support PNG depth %d channels %d color_type %d\n", name.c_str(), bit_depth_,
            channels_, color_type_);
    // END
    error_code_ = -7;
  }
...
int res_create_display_surface(const char* name, GRSurface** pSurface) {
  return internal_res_create_display_surface("", name, pSurface);
}

int internal_res_create_display_surface(const char* language, const char* name, GRSurface** pSurface) {
  *pSurface = nullptr;

  PngHandler png_handler(language, name);
  // MIUI MODIFY: END
...
// MIUI ADD : START
static int miui_matches_locale(const char* locale, int loc_seq)
{
  int flag = 0;
  int j;
  ...
  for (j = 0; j < LOC_LEN_FULL-1; j++) {
    if (locale[j] == RECOVERY_SUPPORT_LOCALES[loc_seq][j])
      flag++;
    else
      break;
  }
  if ((flag == LOC_LEN_FULL-1) || (flag == LOC_LEN_PART)) {
    return 1;
  }
...
int res_create_localized_alpha_surface(const char* name,
  ...
  *pSurface = nullptr;
  ...
  int w = (int)width;
  int h = w / 18;
  if (strncmp(name, "installing_tip", 14) == 0 ) {
    if (w < 1080) {
      h = 141;
    } else if (w > 1080) {
      h = 282;
  ...
  for (png_uint_32 y = 0; loc_seq <= (LOC_NUM - 1); ++y) {
    std::vector<unsigned char> row(width);
    if (miui_matches_locale(locale, loc_seq) || loc_seq == (LOC_NUM - 1)) {
      printf("  %20s: loc_seq:%d (%d x %d @ %d)\n", name, loc_seq, w, h, y);
      std::unique_ptr<GRSurface> surface = GRSurface::Create(w, h, w, 1);
      if (!surface) {
        return -9;
      }
...
int res_create_display_surface_from_rgb_data(const char* data, unsigned int width, unsigned int height,
                                        GRSurface** pSurface) {
    png_byte channels = 4;
    int result = 0;
    unsigned char* p_row;
...
// MIUI ADD : END
    ...
    for (y = 0; y < height; ++y) {
        p_row = (unsigned char*)data + width*channels*y;
        TransformRgbToDraw(p_row, surface->data() + y * surface->row_bytes, channels, width);
    }

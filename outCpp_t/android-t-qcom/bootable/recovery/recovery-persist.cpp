...
constexpr const char* LAST_LOG_FILE_NOT_IN_CACHE = "/data/misc/recovery/last_log";
...
constexpr const char* LAST_KMSG_FILE_NOT_IN_CACHE = "/data/misc/recovery/last_kmsg";
...
ssize_t logsave(
    ...
    rotate_logs(LAST_LOG_FILE_NOT_IN_CACHE, LAST_KMSG_FILE_NOT_IN_CACHE);
    rotated = true;
...
int main(int argc, char **argv) {
    ...
    if (!has_cache) {
      // MIUI MOD:
      if (access(LAST_INSTALL_FILE_NOT_IN_CACHE, F_OK) && unlink(LAST_INSTALL_FILE_NOT_IN_CACHE) == -1) {
        PLOG(ERROR) << "Failed to unlink " << LAST_INSTALL_FILE_NOT_IN_CACHE;
      }
    }
    ...
    if (rotated) {
        if (!access(LAST_CONSOLE_FILE, R_OK)) {
            // MIUI MOD: START
            copy_file(LAST_CONSOLE_FILE, LAST_KMSG_FILE_NOT_IN_CACHE);
        } else if (!access(ALT_LAST_CONSOLE_FILE, R_OK)) {
            copy_file(ALT_LAST_CONSOLE_FILE, LAST_KMSG_FILE_NOT_IN_CACHE);
            // END
        }
    }

...
// MIUI ADD: START
extern const char* updater_secure_msg;
extern char* sideload_secure_msg;
static constexpr float VERIFICATION_PROGRESS_FRACTION = 0.05;
// END
...
static InstallResult TryUpdateBinary(Package* package, bool* wipe_cache,
  ...
  *wipe_cache = false;
  ...
  while (fgets(buffer, sizeof(buffer), from_child) != nullptr) {
    // MIUI ADD: START
    // reset powerkey after 20min
    if (!enableReboot && t.duration() > 1200) {
      enableReboot = true;
      ui->SetEnableReboot(enableReboot);
    }
    // END
  ...
  fclose(from_child);
  // MIUI ADD
  LOG(ERROR) << "Try update binary took:" << std::to_string(t.duration());
  int status;
...
static InstallResult VerifyAndInstallPackage(Package* package, bool* wipe_cache,
                                             std::vector<std::string>* log_buffer, int retry_count,
                                             int* max_temperature, RecoveryUI* ui, bool needs_mount = false) {
  ui->SetBackground(RecoveryUI::INSTALLING_UPDATE);
  // Give verification half the progress bar...
  ui->SetProgressType(RecoveryUI::DETERMINATE);
  ui->ShowProgress(VERIFICATION_PROGRESS_FRACTION, VERIFICATION_PROGRESS_TIME);

  // MIUI ADD: START for verify package
  Timer t;
  bool bypass_verify = false;
  ...
  MemMapping *map;
  int map_ret = 0;
  if(package->GetType() == PackageType::kMemory) {
    MemoryPackage* mem_package;
    if((mem_package = dynamic_cast<MemoryPackage*>(package)) == nullptr) {
      LOG(ERROR) << "failed to dynamic_cast to MemoryPackage\n";
      return INSTALL_CORRUPT;
    }
    map = mem_package->GetMemMapping();
    if (map == nullptr) {
      LOG(ERROR) << "failed to create memmapping";
      return INSTALL_CORRUPT;
    }
  } else if(package->GetType() == PackageType::kFile) {
    map = new MemMapping();
    if (!map->MapFile(FUSE_SIDELOAD_HOST_PATHNAME)) {
  ...
  if (needs_mount) {
    if(check_identity(updater_secure_msg, map) != INSTALL_SUCCESS) {
      LOG(ERROR) << "secureboot check_identity failed.";
      log_buffer->push_back(android::base::StringPrintf("error: %d", kZipVerificationFailure));
      signal(SIGBUS, SIG_DFL);
      if(package->GetType() == PackageType::kFile) {
        delete map; // here the destructor will unmap
      }
      return INSTALL_VERIFY_FAILURE;
    }
  } else {
    // returns 1 on need verify package, -1 on verify failure, 0 on skip verify package.
    int verify = check_verify(sideload_secure_msg, map);
    if(verify < 0) {
      LOG(ERROR) << "signature check_verify failed.";
      log_buffer->push_back(android::base::StringPrintf("error: %d", kZipVerificationFailure));
      signal(SIGBUS, SIG_DFL);
      if(package->GetType() == PackageType::kFile) {
        delete map; // here the destructor will unmap
  ...
  // Verify package.
  if (!bypass_verify && !verify_package(package, ui, package->GetType() == PackageType::kFile)) {
    // END
    log_buffer->push_back(android::base::StringPrintf("error: %d", kZipVerificationFailure));
    // MIUI MOD: START
    signal(SIGBUS, SIG_DFL);
  ...
  // MIUI ADD:
  LOG(ERROR) << "Open update package took:" << std::to_string(t.duration());
  signal(SIGBUS, SIG_DFL);
  ...
  auto result =
      TryUpdateBinary(package, wipe_cache, log_buffer, retry_count, max_temperature, ui);
  ui->SetEnableReboot(true);
  ui->Print("\n");
...
InstallResult InstallPackage(Package* package, const std::string_view package_id,
                             bool should_wipe_cache, int retry_count, RecoveryUI* ui, std::string install_file, bool needs_mount) {
  ...
  if (!package) {
    ...
    result = VerifyAndInstallPackage(package, &updater_wipe_cache, &log_buffer, retry_count,
                                     &max_temperature, ui, needs_mount);
  ...
  if(install_file.empty())
    install_file = Paths::Get().temporary_install_file();
  // END
...
bool verify_package(Package* package, RecoveryUI* ui, bool from_fuse) {
  ...
  if(from_fuse) {
    if (setjmp(jb) == 0) {
      ...
      err = verify_file(package, loaded_keys);
    } else {
      LOG(INFO) << "Verify file get SIGBUS when the host aborts the transfer.";
      err = VERIFY_FAILURE;
    }
  } else {
      LOG(INFO) << "Verify file.";
...
bool SetupPackageMount(const std::string& package_path, bool* should_use_fuse) {
  ...
  *should_use_fuse = true;
  ...
  // MIUI ADD: for lazy sdcard mount
  if (android::base::StartsWith(package_path, "/sdcard/")) {
    ensure_dev_ready("/sdcard");
  }
  // MIUI ADD END

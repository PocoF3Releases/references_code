...
UpdateState ReadSnapshotMergeState()
{
  ...
  if (!sm) {
    printf("Could not create SnapshotManager.\n");
    return UpdateState::None;
  }

  double merge_progress;
  auto mergestate = sm->GetUpdateState(&merge_progress);
  printf("the snapshot update state is %d.\n",mergestate);
  return mergestate;
}
// END

...
// MIUI ADD: START
int apply_from_adb(bool* wipe_cache, Device* device) {
  ...
  for (int i = 0; i < ADB_INSTALL_TIMEOUT; ++i) {
    ...
    auto package =
            Package::CreateFilePackage(FUSE_SIDELOAD_HOST_PATHNAME,
                                       std::bind(&RecoveryUI::SetProgress, ui, std::placeholders::_1));
    result = InstallPackage(package.get(), FUSE_SIDELOAD_HOST_PATHNAME, wipe_cache, 0, ui, Paths::Get().temporary_install_file());
    break;
  }

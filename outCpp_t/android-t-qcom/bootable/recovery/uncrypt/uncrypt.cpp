...
// MIUI ADD: START
static const std::string RESCUE_BLOCK_MAP = "/mnt/rescue/recovery/block.map";
static const std::string RESCUE_UNCRYPT_PATH_FILE = "/mnt/rescue/recovery/uncrypt_file";
static const std::string RESCUE_UNCRYPT_STATUS = "/mnt/rescue/recovery/uncrypt_status";
static const std::string APP_SPLICED_PATH = "/data/miui/apps.block";
static const std::string REAL_APP_SPLICED_PATH = "/data/media/0/apps.block";
static const off64_t BUFF_COUNT = 256;
static std::string blockMap = CACHE_BLOCK_MAP;
static std::string uncryptPathFile = UNCRYPT_PATH_FILE;
static std::string uncryptStatus = UNCRYPT_STATUS;
static bool isNewFactoryReset = false;
// END
...
// MIUI ADD:
static void addFromFindResult(std::list<std::string>& paths) {
    const std::string F2FS_PATH = "/sys/fs/f2fs";
    DIR *dir = opendir(F2FS_PATH.c_str());
    struct dirent *dirp = nullptr;

    if (!dir) {
        LOG(ERROR) << "Can't open dir: " << F2FS_PATH << " " << strerror(errno);
        return;
    }
    ...
    while ((dirp = readdir(dir)) != nullptr) {
        ...
        if (dirp->d_type == DT_DIR && !strncmp(dirp->d_name, "dm", 2)) {
            paths.push_back(F2FS_PATH + "/" + dirp->d_name);
        }
    }
...
#define F2FS_IOC_GET_PIN_FILE	_IOR(F2FS_IOCTL_MAGIC, 14, __u32)
    ...
    if (f2fs_fs) {
        // MIUI ADD: START
        const char* gc_pin_file_thresh_str = "1572864"; // 6 GiB
        const int GC_PIN_FILE_THRESH = 1572864;
        std::list<std::string> paths;

        addFromFindResult(paths);

        for (const auto& path : paths) {
            std::string gc_pin_files;
            android::base::ReadFileToString(path + "/gc_pin_file_thresh", &gc_pin_files);
            if (std::stoi(gc_pin_files) != GC_PIN_FILE_THRESH) {
                PLOG(WARNING) << "gc pin file thresh " << std::stoi(gc_pin_files);
                if (!android::base::WriteStringToFile(gc_pin_file_thresh_str, path + "/gc_pin_file_thresh"))
                    PLOG(WARNING) << "write gc pin failed on " << path;
            }
        }
        // END
    ...
    if (isNewFactoryReset) {
        ...
        pos = 0;
        off64_t blockSize = static_cast<off64_t>(sb.st_blksize);
        off64_t fileSize = static_cast<off64_t>(sb.st_size);
        unsigned char *buffer = (unsigned char*)malloc(sizeof(char) * blockSize * BUFF_COUNT);
        for (size_t i = 0; i < ranges.size(); i += 2) {
            off64_t blockStartPos = ranges[i] * blockSize;
            off64_t blockEndPos = ranges[i + 1] * blockSize;
            while (blockStartPos < blockEndPos && pos != fileSize) {
                size_t to_read = static_cast<size_t>(
                    std::min(blockSize * BUFF_COUNT, blockEndPos - blockStartPos));
                pos += to_read;
                if (pos > fileSize) {
                    pos -= to_read;
                    to_read = fileSize - pos;
                    pos += to_read;
                }
                if (!android::base::ReadFully(fd, buffer, to_read)) {
                    PLOG(ERROR) << "failed to write: " << path << ", total size: " << fileSize
                                << ", rewriteSize: " << pos << ", to_read: " << to_read;
                    free(buffer);
                    return kUncryptReadError;
                }
                if (write_at_offset(buffer, to_read, wfd,
                        static_cast<off64_t>(blockStartPos)) != 0) {
                    free(buffer);
                    return kUncryptWriteError;
                }
                blockStartPos += to_read;
            }
        }
        free(buffer);
        // block map file lose
...
static void log_uncrypt_error_code(UncryptErrorCode error_code) {
    if (!android::base::WriteStringToFile(android::base::StringPrintf(
            "uncrypt_error: %d\n", error_code), uncryptStatus)) {
        PLOG(WARNING) << "failed to write to " << uncryptStatus;
    }
}

// MIUI ADD: START
static bool set_factoryreset_command() {
    std::vector<std::string> options;
    options.push_back("--factory_test_reset");
    options.push_back("--apps_block=@/cache/recovery/block.map");
    std::string err;
...
static bool uncrypt_wrapper(const char* input_path, const char* map_file, const int socket) {
    ...
    if (input_path == nullptr) {
      if (!FindUncryptPackage(uncryptPathFile, &package)) {
      ...
      // MIUI ADD: START
      if (APP_SPLICED_PATH.find(package) != std::string::npos) {
        isNewFactoryReset = true;
        std::thread writeMiscCommand(set_factoryreset_command);
        if (!copyFile(APP_SPLICED_PATH, REAL_APP_SPLICED_PATH)) {
          write_status_to_socket(-1, socket);
          log_uncrypt_error_code(kUncryptFileOpenError);
          LOG(ERROR) << "Copy file from " << APP_SPLICED_PATH << " to " << REAL_APP_SPLICED_PATH << " failed!";
          return false;
        }
        package = REAL_APP_SPLICED_PATH;
        writeMiscCommand.join();
      }
      // END
    ...
    if (status != 0) {
        ...
        if (!android::base::WriteStringToFile(uncrypt_message, uncryptStatus)) {
            PLOG(WARNING) << "failed to write to " << uncryptStatus;
        }
...
int main(int argc, char** argv) {
    // MIUI ADD: add support for VAB
    if (android::base::GetBoolProperty("ro.build.ab_update", false)) {
        blockMap = "/mnt/rescue/recovery/block.map";
        uncryptPathFile = "/mnt/rescue/recovery/uncrypt_file";
        uncryptStatus = "/mnt/rescue/recovery/uncrypt_status";
    }
    // END;

    enum { UNCRYPT, SETUP_BCB, CLEAR_BCB, UNCRYPT_DEBUG } action;
    const char* input_path = nullptr;
    const char* map_file = blockMap.c_str();

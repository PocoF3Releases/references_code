...
bool PatchPartition(const Partition& target, const Partition& source, const Value& patch,
  ...
  if (ReadPartitionToBuffer(target, &target_file, false)) {
    ...
    // MIUI ADD: START
    std::string err;
    if (!android::base::RemoveFileIfExists(Paths::Get().cache_temp_source(), &err)) {
      LOG(ERROR) << "failed to remove the file " << Paths::Get().cache_temp_source() << " " << err;
    }
    // END
...
static size_t GetFileSize(std::string file_name) {
  struct stat st;
  if (stat(file_name.c_str(), &st) == 0 && S_ISREG(st.st_mode)) {
    return st.st_size;
  }
  return 0;
}
...
static bool GenerateTarget(const Partition& target, const FileContents& source_file,
  ...
  // MIUI MOD: START
  std::string err;
  if (backup_source && !android::base::RemoveFileIfExists(Paths::Get().cache_temp_source(), &err)) {
    LOG(ERROR) << "failed to remove the file " << Paths::Get().cache_temp_source() << " " << err;
  }
  size_t cache_temp_source_size = GetFileSize(Paths::Get().cache_temp_source());
  

  // We write the original source to cache, in case the partition write is interrupted.
  if (backup_source && !CheckAndFreeSpaceOnCache(source_file.data.size() - cache_temp_source_size)) {
  // END

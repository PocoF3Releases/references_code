...
static bool check_permission(const char* name) {
  ...
  // ### white list                                                                           ###
  // ############################################################################################
  // ### WARNNING: Modification of the white list needs to be reviewed by the security group! ###
  // ############################################################################################
  std::vector<std::string> whiteList;
  whiteList.push_back("/cache/");
  whiteList.push_back("/tmp/");
  whiteList.push_back("/dev/block/by-name/oops");

  // prevent such as /cache/../dev/block/boot path
  if (strstr(name, "..") || strstr(name, "//") || name[0] == '~') {
    goto checkerr;
  }

  for (size_t i = 0; i < whiteList.size(); i++) {
    if (strncmp(name, whiteList[i].c_str(), whiteList[i].length()) == 0) {
      return true;
    }
  }
...
static bool handle_sync_command(int fd, std::vector<char>& buffer) {
  ...
  bool checkPermission = check_permission(name);
  syncmsg msg = {};
  switch (request.id) {
    case ID_LSTAT_V1:
      if (false == checkPermission) {
        ...
        msg.stat_v1.size = 0;
        msg.stat_v1.mtime = 0;
        return WriteFdExactly(fd, &msg.stat_v1, sizeof(msg.stat_v1));
      }
    ...
    case ID_STAT_V2:
      if (false == checkPermission) {
        msg.stat_v2.id = request.id;
        msg.stat_v2.error = errno_to_wire(errno);
        return WriteFdExactly(fd, &msg.stat_v2, sizeof(msg.stat_v2));
      }
      if (!do_stat_v2(fd, request.id, name)) return false;
      break;
    case ID_LIST_V1:
      if (false == checkPermission) {
        msg.dent_v1.id = ID_DONE;
        return WriteFdExactly(fd, &msg.dent_v1, sizeof(msg.dent_v1));
      }
      if (!do_list_v1(fd, name)) return false;
      break;
    case ID_LIST_V2:
      if (false == checkPermission) {
        msg.dent_v2.id = ID_DONE;
        return WriteFdExactly(fd, &msg.dent_v2, sizeof(msg.dent_v2));
      }
      if (!do_list_v2(fd, name)) return false;
      break;
    case ID_SEND_V1:
      // Prevent the data of flash memory from being modified, so disable send operation.
      //
      // if (!do_send_v1(fd, name, buffer)) return false;
      // break;
    case ID_SEND_V2:
      // if (!do_send_v2(fd, name, buffer)) return false;
      SendSyncFail(fd, "push file not supported");
      break;
    case ID_RECV_V1:
      if (false == checkPermission) {
        SendSyncFailErrno(fd, "no permission!");
        return false;
      }
...
void file_sync_service(int fd, const std::string& arg) {
  std::vector<char> buffer(SYNC_DATA_MAX);
  if (!arg.empty()) printf("file_sync_service arguments: %s\n", arg.c_str());

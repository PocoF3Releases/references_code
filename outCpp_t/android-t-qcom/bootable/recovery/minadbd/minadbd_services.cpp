...
static void sideload_host_service(int sfd, const std::string& args) {
  ...
  if (pieces.size() != 4 || !android::base::ParseInt(pieces[0], &file_size) || file_size <= 0 ||
      !android::base::ParseInt(pieces[1], &block_size) || block_size <= 0 ) {
    LOG(ERROR) << "bad sideload-host arguments: " << args;
    exit(1);
  }

  std::string verification = android::base::StringPrintf("%s:%s", pieces[2].c_str(), pieces[3].c_str());
  LOG(DEBUG)<< "sideload-host file size " << file_size << " block size " << block_size << " verification " << verification;
  // MIUI ADD : START
  sideload_verification(verification.c_str());
  // END
  unique_fd ufd(sfd);
  auto adb_data_reader =
          std::make_unique<FuseAdbDataProvider>(std::move(ufd), file_size, block_size);
  int result = run_fuse_sideload(std::move(adb_data_reader));

  printf("sideload_host finished: result %d\n", result);
  //MIUI MOD:
  // exit(result == 0 ? 0 : 1);
  read_from_father(sfd);
}
// END
...
//       !android::base::ParseInt(pieces[1], &block_size) || block_size <= 0 ) {
//     LOG(ERROR) << "bad sideload-host arguments: " << args;
//     return kMinadbdArgumentsParsingError;
//   }
...
unique_fd daemon_service_to_fd(std::string_view name, atransport* /* transport */) {
  ...
  if (name.starts_with("sideload:")) {
    ...
    std::string args(name);
    ret = create_service_thread(sideload_host_service, args.c_str());
    // END
  }
  // MIUI ADD:
  else {
    ret = service_mi_fd(create_service_thread, std::string(name).c_str());
  }
  if (ret >= 0) {
    close_on_exec(ret);
  }

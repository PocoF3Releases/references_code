...
#include <recovery_ui/include/recovery_ui/screen_ui.h>
...
// MIUI ADD: START
#include "minadbd/minadbd_services.h"
...
// MIUI ADD:
// store extension parameters for recovery
static std::map<std::string, std::string> extensionParameters;
...
static std::vector<std::string> get_args(const int argc, char** const argv, std::string* stage) {
  ...
  // MIUI ADD: START
  //
  // read extension parameters to extensionParameters
  std::fstream lastExtensionParameters;
  if (ensure_path_mounted(LAST_EXTENSION_PARAMETERS_FILE) != 0) {
    LOG(ERROR) << "Can't mount " << LAST_EXTENSION_PARAMETERS_FILE;
  }
  lastExtensionParameters.open(LAST_EXTENSION_PARAMETERS_FILE);
  if (lastExtensionParameters) {
    LOG(INFO) << "Start to read extension parameters";
    std::string context;
    while (lastExtensionParameters >> context) {
      LOG(INFO) << context;
      // use to split '=' and save parameter to extensionParameters
      for (size_t i = 0; i < context.length(); i++) {
        if (context[i] == '=') {
          extensionParameters[context.substr(0, i)] = context.substr(i + 1, context.length());
          break;
        }
      }
    };
    lastExtensionParameters.close();
  }
  // END
...
int main(int argc, char** argv) {
  ...
  // MIUI ADD: START
  if (argc == 4 && strcmp(argv[1], "--adbd") == 0) {
    argv += 2;
    int to_child = atoi(argv[0]);
    int from_father = atoi(argv[1]);
    minadbd_main(to_child, from_father);
    return 0;
  }
  // END
  ...
  if (locale.empty()) {
    ...
    // MIUI ADD: START
    if (locale.empty()) {
      locale = android::base::GetProperty("ro.product.locale", "en_US");
      locale.replace(2, 1, "_");
    }
    // END
  ...
  // MIUI ADD: START
  //
  // use extension parameters in the area
  if (extensionParameters["secure_password"] == "1") {
    ui->SetShowMaintenanceMode(true);
  }
  if (extensionParameters["disable_factoryreset"] == "1") {
    set_disable_factoryreset();
  }
  // END
  ...
  // MIUI ADD : START
  // Mapping language and defualt english
  if (langToResource[locale] != "") {
    ui->InitLanguageSpecificResource(langToResource[locale].c_str());
  } else {
    ui->InitLanguageSpecificResource("en");
  }
  // END
  ...
  // MIUI ADD: START Restore persist
  if (ensure_path_mounted(PERSIST_ROOT) == 0) {
    int ret = selinux_android_restorecon(PERSIST_ROOT, SELINUX_ANDROID_RESTORECON_RECURSE | SELINUX_ANDROID_RESTORECON_FORCE);
    if (ret != 0) {
      printf("restore persist secontext label fail, ret: %d\n", ret);
    }
    ensure_path_unmounted(PERSIST_ROOT);
  } else {
    printf("mount persist fail\n");
  }
  // END
  ...
  while (true) {
    ...
    // MIUI ADD: START
    if(fastboot) ensure_path_unmounted(CACHE_ROOT);
    ...
    switch (ret) {
      ...
      case Device::REBOOT_RECOVERY:
        ...
        // MIUI add for miassistant
        android::base::SetProperty("sys.usb.config_disable", "0");
        if(!android::base::WaitForProperty("sys.usb.config_disable", "0"))
          LOG(ERROR) << "failed to setproperty sys.usb.config_disable to 0";
      ...
      case Device::ENTER_FASTBOOT:
        if (android::fs_mgr::LogicalPartitionsMapped()) {
          ...
          Reboot("fastboot");
          // MIUI MOD:
        } else if (android::base::GetBoolProperty("ro.boot.dynamic_partitions", false)) {
      ...
      case Device::ENTER_RECOVERY:
        ...
        // MIUI ADD: START for miassistant
        android::base::SetProperty("sys.usb.config_disable", "0");
        if(!android::base::WaitForProperty("sys.usb.config_disable", "0"))
          LOG(ERROR) << "failed to setproperty sys.usb.config_disable to 0";
        ui->SetCurrentState(false);

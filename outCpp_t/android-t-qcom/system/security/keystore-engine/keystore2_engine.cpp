...
extern "C" ssize_t keystore2_get(const char* key, size_t keyLength, uint8_t** value) {
    std::string alias(key, keyLength);
    ::ndk::SpAIBinder keystoreBinder(AServiceManager_checkService(keystore2_service_name));
    auto keystore2 = ks2::IKeystoreService::fromBinder(keystoreBinder);
    if (!keystore2) {
        LOG(WARNING) << "Unable to connect to Keystore 2.0.";
        return {};
    }
    ...
    auto cert = *response.metadata.certificateChain;
    *value = reinterpret_cast<uint8_t*>(malloc(cert.size()));
    memcpy(*value, &(cert[0]), cert.size());

    return cert.size();
}

extern "C" char** keystore2_list(const char* key, size_t keyLength, int *count) {
    ::ndk::SpAIBinder keystoreBinder(AServiceManager_checkService(keystore2_service_name));
    auto keystore2 = ks2::IKeystoreService::fromBinder(keystoreBinder);
    if (!keystore2) {
        LOG(WARNING) << "Unable to connect to Keystore 2.0.";
    ...
    auto rc = keystore2->listEntries(ks2::Domain::SELINUX, KS2_NAMESPACE_WIFI, &key_list);
    if (!rc.isOk()) {
        LOG(ERROR) << AT <<  "keystore2_list failed: " << rc.getDescription();
        return {};
    }

    std::vector<std::string> aliases;
    std::string prefix(key, keyLength);
    for (auto k : key_list) {
        if (k.alias->find(prefix) == 0) {
            aliases.push_back(k.alias->substr(prefix.length()));
        }
    }
    *count = aliases.size();
    char **results = (char**)malloc(sizeof(char*)*(*count));
    if (!results) {
        LOG(ERROR) << AT << "keystore2_list malloc aliases error";
        return {};
    }

    int index = 0;
    for (auto it = aliases.begin(); it != aliases.end(); ++it) {
        int temp_len = it->length();
        char *alias = (char *)malloc(temp_len+1);
        if (!alias) {
            LOG(ERROR) << AT << "keystore_list malloc alias error";
            return {};
        }
        memset(alias, 0, temp_len+1);
        memcpy(alias, &((*it)[0]), temp_len);

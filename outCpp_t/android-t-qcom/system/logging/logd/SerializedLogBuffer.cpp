...
bool SerializedLogBuffer::ShouldLog(log_id_t log_id, const char* msg, uint16_t len) {
    ...
    bool limitLog = false;
    if ((LOG_ID_SYSTEM == log_id) && (mLogLevel != ANDROID_LOG_VERBOSE) && (prio < mLogLevel)) {
        limitLog = true;
    }
    return !limitLog && __android_log_is_loggable_len(prio, tag, tag_len, ANDROID_LOG_VERBOSE);
    // END
}
...
// MIUI ADD:
static int is_debug_key() {
    if ((android::base::GetProperty("ro.build.tags", "") == "test-keys")
            || (android::base::GetProperty("ro.debuggable", "") == "1")) {
        return 1;
    }
    return 0;
}

void SerializedLogBuffer::initLogLevel() {
    std::string value = android::base::GetProperty("persist.logd.limit", "");
    ...
    if (c == 'D') {
        mLogLevel = ANDROID_LOG_DEBUG;
    } else if (c == 'I') {
        mLogLevel = ANDROID_LOG_INFO;
    } else if (c == 'W') {
        mLogLevel = ANDROID_LOG_WARN;
    } else if (c == 'E') {
        mLogLevel = ANDROID_LOG_ERROR;
    } else if (c == 'O') {
        //select off log
        mLogLevel = ANDROID_LOG_FATAL;
    } else if (c == 'S') {
        //no use
        mLogLevel = ANDROID_LOG_SILENT;
    } else {
        mLogLevel = ANDROID_LOG_VERBOSE;
    }
}
// END
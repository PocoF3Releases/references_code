...
// MIUI Add
template <typename TKey, typename TEntry>
class LogFilterHashtable {
  private:
    std::unordered_map<TKey, TEntry> map;

    /*
     * For not cost to much memory, we set MAX size 512
     * When table size reach MAX, we only keep top RESERVE value.
     */
    static const int MAX = 512;
    static const int RESERVE = 64;

    void dropSmallValues() {
        std::string output;
        std::array<const TKey*, MAX> sorted_keys;
        std::array<const LogFilterPidEntry*, MAX> sorted_entries;
        MaxLogFilterEntries(sorted_keys, sorted_entries);

        // Drop small value, only keep top RESERVE in map.
        for (size_t index = sorted_keys.size() - 1; index > RESERVE; index--) {
            const TKey& key = *sorted_keys[index];
            map.erase(key);
        }
    }
  ...
  public:
    ...
    void ClearMap(bool is_filtered, log_time realtime) {
        if (allowClearMapFlag && is_filtered) {
            int time_gap = realtime.tv_sec - allowClearMapTime.tv_sec;
            if (time_gap > 1 || time_gap < -1) {
                map.clear();
                allowClearMapFlag = false;
            }
        }
    }

    void SetAllowClearMap() {
        allowClearMapFlag = true;
        log_time now(CLOCK_REALTIME);
        allowClearMapTime = now;
    }

    // Returns a sorted array of up to len highest entries sorted by size.  If fewer than len
    // entries are found, their positions are set to nullptr.
    template <size_t len>
    void MaxLogFilterEntries(std::array<const TKey*, len>& out_keys,
                    std::array<const LogFilterPidEntry*, len>& out_entries) const{
        out_keys.fill(nullptr);
        out_entries.fill(nullptr);
        for (const auto& [key, entry] : map) {
            int total_filter_log_num = entry.get_total_filter_log_num();
            ssize_t index = len - 1;
            while ((!out_entries[index] || total_filter_log_num > out_entries[index]->get_total_filter_log_num()) && --index >= 0)
                ;
            if (++index < (ssize_t)len) {
                size_t num = len - index - 1;
                if (num) {
  ...
  private:
    bool allowClearMapFlag = false;
    log_time allowClearMapTime;
...
// MIUI Add
class LogFilterPidEntry : public EntryBaseDropped {
  public:
    explicit LogFilterPidEntry(pid_t pid)
        : EntryBaseDropped(),
          uid_(android::pidToUid(pid)),
          name_(android::pidToName(pid)) {
          }

    explicit LogFilterPidEntry(const LogStatisticsElement& element)
        : EntryBaseDropped(element), uid_(element.uid), name_(android::pidToName(element.pid)) {
        }

    LogFilterPidEntry(const LogFilterPidEntry& entry)
        : EntryBaseDropped(entry),
          uid_(entry.uid_),
          name_(entry.name_ ? strdup(entry.name_) : nullptr) {
          }

    ~LogFilterPidEntry() { free(name_); }
    ...
    void Add(const LogStatisticsElement& element) {
        ...
        if (element.is_filtered) {
            total_filter_count_ = total_filter_count_ + 1;
        }

        if (element.filter_log_num > 0) {
            total_filter_log_num_ = total_filter_log_num_ + element.filter_log_num;
            if (element.filter_log_num > max_filter_log_num_) {
                max_filter_log_num_ = element.filter_log_num;
                max_filter_timestamp_sec_ = element.realtime.tv_sec;
            }
        }
        EntryBaseDropped::Add(element);
    }

    int get_total_filter_log_num() const {
        return total_filter_log_num_;
    }
  ...
  private:
    ...
    int total_filter_log_num_ = 0;
    int total_filter_count_ = 0;
    int max_filter_log_num_ = 0;
    int max_filter_timestamp_sec_ = 0;
};
// MIUI End
...
class LogStatistics {
    ...
    // MIUI Add
    // Log filter pid to uid list
    typedef LogFilterHashtable<pid_t, LogFilterPidEntry> logFilterPidTable_t;
    logFilterPidTable_t logFilterPidTable GUARDED_BY(lock_);
    // MIUI End
  ...
  public:
    ...
    // MIUI mod: drop the "const"
    std::string Format(uid_t uid, pid_t pid, unsigned int logMask) EXCLUDES(lock_);
  ...
  private:
    ...
    // MIUI Add
    template <typename TKey, typename TEntry>
    std::string FormatLogFilterTable(const LogFilterHashtable<TKey, TEntry>& table, const std::string& name = std::string(""),
                                                        log_id_t id = LOG_ID_MAX) const REQUIRES(lock_);
    // MIUI End

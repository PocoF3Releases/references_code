...
void LogStatistics::Add(LogStatisticsElement element) {
    ...
    // MIUI Add
    logFilterPidTable.ClearMap(element.is_filtered, element.realtime);
    logFilterPidTable.Add(element.pid, element);
    // MIUI End
...
// MIUI Add
std::string LogFilterPidEntry::formatHeader(const std::string& name,
                                   log_id_t /* id */) const {
    return formatLine(name, std::string(""), std::string("")) +
           formatLine(std::string("  PID/UID   COMMAND LINE"),
                      std::string("Total Count Max Time"), std::string(""));
}

std::string LogFilterPidEntry::format(const LogStatistics& stat, log_id_t, pid_t pid) const
        REQUIRES(stat.lock_) {
    std::string name = android::base::StringPrintf("%5u/%u", pid, uid_);

    std::string size = android::base::StringPrintf("%d %d %d %d", total_filter_log_num_, total_filter_count_, max_filter_log_num_, max_filter_timestamp_sec_);
...
//MIUI Add
template <typename TKey, typename TEntry>
std::string LogStatistics::FormatLogFilterTable(const LogFilterHashtable<TKey, TEntry>& table, const std::string& name, log_id_t id) const
        REQUIRES(lock_) {
    static const size_t maximum_sorted_entries = 32;
    std::string output;
    std::array<const TKey*, maximum_sorted_entries> sorted_keys;
    std::array<const LogFilterPidEntry*, maximum_sorted_entries> sorted_entries;
    table.MaxLogFilterEntries(sorted_keys, sorted_entries);
    bool header_printed = false;

    for (size_t index = 0; index < maximum_sorted_entries; ++index) {
        const LogFilterPidEntry* entry = sorted_entries[index];
        if (!entry) {
            break;
        }
        if (entry->get_total_filter_log_num() <= (sorted_entries[0]->get_total_filter_log_num() / 100)) {
            break;
        }
        if (!header_printed) {
            output += "\n\n";
...
std::string LogStatistics::Format(uid_t uid, pid_t pid, unsigned int logMask) {
    auto lock = std::lock_guard{lock_};
    ...
    // MIUI Add
    size_t logFilterPidTableSize = logFilterPidTable.size();
    if (logFilterPidTableSize > 0) {
        name = "Log Filter Top PIDs:";
        name += android::base::StringPrintf(" (Total size:%zu)", logFilterPidTableSize);
        output += FormatLogFilterTable(logFilterPidTable, name);
        logFilterPidTable.SetAllowClearMap();
    }
    // MIUI End
    return output;
}

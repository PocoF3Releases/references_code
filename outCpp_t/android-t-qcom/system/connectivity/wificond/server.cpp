...
// MIUI ADD : START
Status Server::createP2pInterface(const std::string& iface_name,
                                 sp<IP2pInterface>* created_interface) {
  InterfaceInfo interface;
  uint32_t wiphy_index;
  ...
  LOG(INFO) << "createP2pInterface: wiphy_index " << wiphy_index << " iface_name " << iface_name;

  unique_ptr<P2pInterfaceImpl> p2p_interface(new P2pInterfaceImpl(
      interface.name,
      interface.if_index,
      if_tool_.get(),
      netlink_utils_,
      scan_utils_));
  *created_interface = p2p_interface->GetBinder();
  p2p_interfaces_[iface_name] = std::move(p2p_interface);
  iface_to_wiphy_index_map_[iface_name] = wiphy_index;
  return Status::ok();
}

Status Server::tearDownP2pInterface(const std::string& iface_name,
                                       bool* out_success) {
  *out_success = false;
  const auto iter = p2p_interfaces_.find(iface_name);
  if (iter != p2p_interfaces_.end()) {
    p2p_interfaces_.erase(iter);
    *out_success = true;
  }
  ...
  if (iter_wi != iface_to_wiphy_index_map_.end()) {
    int wiphy_index = iter_wi->second;
    LOG(INFO) << "tearDownP2pInterface: erasing wiphy_index for iface_name " << iface_name;
    iface_to_wiphy_index_map_.erase(iter_wi);
    if (hasNoIfaceForWiphyIndex(wiphy_index)) {
      EraseBandWiphyIndexMap(wiphy_index);
    } else {
...
status_t Server::dump(int fd, const Vector<String16>& /*args*/) {
  ...
  // MIUI ADD : START
  for (const auto& iface : p2p_interfaces_) {
    iface.second->Dump(&ss);
  }
  // MIUI ADD : END

...
bool NetlinkUtils::GetInterfaceInfo(uint32_t interface_index,
                                    const std::array<uint8_t, ETH_ALEN>& mac_address,
                                    InterfaceInfo* out_iface_info) {
  ...
  for (auto& packet : response) {
    ...
    string name;
    if (!packet->GetAttributeValue(NL80211_ATTR_IFNAME, &name)) {
      LOG(WARNING) << "Failed to get ifname";
      name = "unknown";
    }

    string if_type;
    if (!packet->GetAttributeValue(NL80211_ATTR_IFTYPE, &if_type)) {
      LOG(WARNING) << "Failed to get iftype";
      if_type = "unknown";
    }

    uint32_t freq;
    if (!packet->GetAttributeValue(NL80211_ATTR_WIPHY_FREQ, &freq)) {
      LOG(WARNING) << "Failed to get frequency";
      freq = 0;
    }

    uint32_t width_serial;
    uint32_t width;
    if (!packet->GetAttributeValue(NL80211_ATTR_CHANNEL_WIDTH, &width_serial)) {
      LOG(WARNING) << "Failed to get width";
      width_serial = -1;
    }
    switch(width_serial) {
      case 0: // 20MHz(not HT)
    ...
    LOG(INFO) << "bandwidth: " << width;

    uint32_t if_index;
    if (!packet->GetAttributeValue(NL80211_ATTR_IFINDEX, &if_index)) {
      LOG(WARNING) << "Failed to get if_index";
      if_index = -1;
    }
    if (if_index != interface_index)
      continue;

    LOG(INFO) << "Interface " << name << " found";
    *out_iface_info = InterfaceInfo(interface_index, -1, name, mac_address,
                                  if_type, freq, width);
    return true;
  }
...
void NetlinkUtils::handleBandFreqAttributes(const NL80211NestedAttr& freqs_attr,
  ...
  for (auto& freq : freqs) {
    ...
    if (frequency_value > k2GHzFrequencyLowerBound &&
      ...
      // Put indoor only channels into the dfs category.
      // This aligns with what framework always assumes.
      if (freq.HasAttribute(NL80211_FREQUENCY_ATTR_INDOOR_ONLY)) {
        out_band_info->band_dfs.push_back(frequency_value);
        continue;
      }
...
bool NetlinkUtils::GetStationInfo(uint32_t interface_index,
  ...
  // MIUI ADD : START
  uint8_t station_bandwidth = 0;
  station_bandwidth = tx_bitrate_attr.HasAttribute(NL80211_RATE_INFO_40_MHZ_WIDTH) ?
    40 : station_bandwidth;
  station_bandwidth = tx_bitrate_attr.HasAttribute(NL80211_RATE_INFO_80_MHZ_WIDTH) ?
    80 : station_bandwidth;
  station_bandwidth = tx_bitrate_attr.HasAttribute(NL80211_RATE_INFO_80P80_MHZ_WIDTH) ?
    160 : station_bandwidth;
  station_bandwidth = tx_bitrate_attr.HasAttribute(NL80211_RATE_INFO_160_MHZ_WIDTH) ?
    160 : station_bandwidth;
  station_bandwidth = tx_bitrate_attr.HasAttribute(NL80211_RATE_INFO_10_MHZ_WIDTH) ?
    10 : station_bandwidth;
  station_bandwidth = tx_bitrate_attr.HasAttribute(NL80211_RATE_INFO_5_MHZ_WIDTH) ?
    5 : station_bandwidth;
  *out_station_info = StationInfo(tx_good, tx_bad, tx_bitrate, current_rssi, rx_bitrate, station_bandwidth);
  // MIUI ADD : END
  return true;
}

...
struct StationInfo {
  ...
  StationInfo(uint32_t station_tx_packets_,
      ...
      : station_tx_packets(station_tx_packets_),
        ...
        // MIUI ADD
        station_bandwidth(station_bandwidth_) {}
  ...
  // MIUI ADD : Current bandwidth.
  uint32_t station_bandwidth;
...
class NetlinkUtils {
 public:
  ...
  // Get interface info from kernel.
  // |interface_info| returns a vector of InterfaceInfo struct
  // with information of the interface.
  // returns true on success.
  virtual bool GetInterfaceInfo(uint32_t interface_index,
                                const std::array<uint8_t, ETH_ALEN>& mac_address,

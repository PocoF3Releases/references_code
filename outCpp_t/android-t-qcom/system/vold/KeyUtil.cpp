...
bool backupKeyExists(const std::string& path, int keymaster) {
    if (access((path + "/encrypted_key").c_str(), F_OK) != 0) return false;
    if (access((path + "/secdiscardable").c_str(), F_OK) != 0) return false;
    if (access((path + "/stretching").c_str(), F_OK) != 0) return false;
    if (access((path + "/version").c_str(), F_OK) != 0) return false;

    if (keymaster) {
        if (access((path + "/keymaster_key_blob").c_str(), F_OK) != 0) return false;
    }

    return true;
}

bool retrieveBackupKey(const std::string& key_path, const std::string& backup_key_path,
                       const KeyAuthentication& key_authentication, KeyBuffer* key) {
    if (pathExists(backup_key_path)) {
        LOG(DEBUG) << "Backup Key exists, using: " << backup_key_path;
        // copy backup key to tmp dir, then retrieveKey
        backupKey(backup_key_path, key_path);

        // retrieve key from tmp dir
        if (retrieveKey(key_path, key_authentication, key)) {
            property_set("sys.vold.backup", "1");
        } else {
            return false;
        }
    } else {
        LOG(ERROR) << "No Backup Key in: " << key_path;
        return false;
    }
...
void backupKey(const std::string& key_path_dir, const std::string& key_path_dir_back) {
    copyFile(key_path_dir + "/encrypted_key", key_path_dir_back + "/encrypted_key");
    copyFile(key_path_dir + "/keymaster_key_blob", key_path_dir_back + "/keymaster_key_blob");
    copyFile(key_path_dir + "/secdiscardable", key_path_dir_back + "/secdiscardable");
    copyFile(key_path_dir + "/stretching", key_path_dir_back + "/stretching");
    copyFile(key_path_dir + "/version", key_path_dir_back + "/version");
}

...
int VolumeManager::fixupAppDirRecursive(const std::string& path, int32_t appUid, int32_t gid,
        int32_t flag /*bit0-sync(1)/async(0), bit1-not chmod(1)/chmod(0)*/) {
    return VoldStub::fixupAppDirRecursive(path, appUid, gid, flag);
}
// MIUI END

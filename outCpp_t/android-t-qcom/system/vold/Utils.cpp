...
status_t ForceUnmount(const std::string& path) {
    ...
    // MIUI ADD: START
    // The original design here was to unmount a volume in a safer way, but some
    // apps restart so quickly even after we killed them and they open files
    // immediately after started, so umount2 would fail with EBUSY and we waste all
    // the time for nothing
    // Use MNT_DETACH to fristly make the mount point unavailable for new processes,
    // then kill those who already hold open fds to the underlying disk
    PLOG(ERROR) << "ForceUnmount failed. Trying MNT_DETACH " << path;
    if (UnmountTree(path) == OK) {
        // No SIGTERM, just kill those bad apps

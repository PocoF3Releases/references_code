...
// MIUI ADD: START
static void syncFsBeforeUnmount(const std::string volId) {
    std::string path;
    {
        ACQUIRE_LOCK;
        auto vol = VolumeManager::Instance()->findVolume(volId);
        ...
        if (vol->getType() != VolumeBase::Type::kPublic
                || vol->getState() != VolumeBase::State::kMounted) {
            return;
        }
        path = vol->getInternalPath();
    }
    // Avoid holding open fd to the underlying disk inside vold
    int pid = fork();
    if (pid == 0) {
        android::base::unique_fd fd(open(path.c_str(), O_RDONLY|O_CLOEXEC));
        if (fd == -1) {
            PLOG(ERROR) << "Failed to open " << path << ", skipped syncing " << volId;
            return;
        }
        LOG(INFO) << "Syncing " << volId;
        int err = syncfs(fd);
        if (err == 0) {
            LOG(INFO) << "Synced " << volId;
            _exit(0);
        } else {
            PLOG(ERROR) << "Failed to Sync " << volId;
            _exit(1);
        }
    } else if (pid == -1) {
        PLOG(ERROR) << "Failed to fork, skipped syncing " << volId;
    } else {
...
binder::Status VoldNativeService::runExtMFlush(int pageCount, int flushLevel,
        const android::sp<android::os::IVoldTaskListener>& listener) {
    ENFORCE_SYSTEM_OR_ROOT;
    ACQUIRE_LOCK;

    std::thread([=]() {
        VoldStub::runExtMFlush(pageCount, flushLevel, listener);
    }).detach();
    return Ok();
}

binder::Status VoldNativeService::stopExtMFlush(
        const android::sp<android::os::IVoldTaskListener>& listener) {
    ENFORCE_SYSTEM_OR_ROOT;
    ACQUIRE_LOCK;

    std::thread([=]() {
        VoldStub::stopExtMFlush(listener);
    }).detach();
    return Ok();
}

// MIUI ADD: START
binder::Status VoldNativeService::fixupAppDirRecursive(const std::string& path,
                                                       int32_t appUid, int32_t gid, int32_t flag) {
    ENFORCE_SYSTEM_OR_ROOT;
    CHECK_ARGUMENT_PATH(path);
    ACQUIRE_LOCK;
    VoldStub::fixupAppDirRecursiveNative(path, appUid, gid, flag);
    return Ok();
}

binder::Status VoldNativeService::moveStorageQuickly(const std::string& from, const std::string& to,
                             const android::sp<android::os::IVoldTaskListener>& listener,
                             int32_t flag, int32_t* _aidl_return) {
    ENFORCE_SYSTEM_OR_ROOT;
    CHECK_ARGUMENT_PATH(from);
    CHECK_ARGUMENT_PATH(to);
    ACQUIRE_LOCK;
    VoldStub::moveStorageQuickly(from, to, listener, flag, _aidl_return);
    return Ok();
}
// END

binder::Status VoldNativeService::gcBoosterControl(const std::string& enable) {
    ENFORCE_SYSTEM_OR_ROOT;
    ACQUIRE_LOCK;
    VoldStub::gcBoosterControl(enable);
    return Ok();
}

binder::Status VoldNativeService::setCldListener(
        const android::sp<android::os::IVoldTaskListener>& listener) {
    ENFORCE_SYSTEM_OR_ROOT;
    ACQUIRE_LOCK;
    VoldStub::SetCldListener(listener);
    return Ok();
}

binder::Status VoldNativeService::getCldFragLevel(int32_t* _aidl_return) {
    ENFORCE_SYSTEM_OR_ROOT;
    ACQUIRE_CRYPT_LOCK;
    *_aidl_return = VoldStub::GetCldFragLevel();
    return Ok();
}

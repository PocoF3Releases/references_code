...
static bool read_key(const std::string& metadata_key_dir, const KeyGeneration& gen,
    ...
    auto temp = metadata_key_dir + "/tmp";
        auto dir_back = metadata_key_dir + "/key_back";
    if (!retrieveOrGenerateKey(dir, temp, kEmptyAuthentication, gen, key)) {
        LOG(DEBUG) << "retrieveOrGenerateKey failed, try backup";
        if (!retrieveBackupKey(dir, dir_back, kEmptyAuthentication, key)) {
            LOG(ERROR) << "retrieveBackupKey failed, return.";
            return false;
        }
    } else {
        if (!backupKeyExists(dir_back, 1)) {
            if (TEMP_FAILURE_RETRY(mkdir(dir_back.c_str(), 0700)) == -1) {
                LOG(ERROR) <<"mkdir backup dir failed";
            } else {
                LOG(ERROR) <<"start backup";
                std::thread(backupKey, dir, dir_back).detach();
            }
        }
    }
    return true;

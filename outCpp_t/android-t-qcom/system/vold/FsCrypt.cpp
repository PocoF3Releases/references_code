...
using android::vold::retrieveBackupKey;
using android::vold::backupKey;
...
// MIUI ADD: START
const std::string device_key_path_backup = device_key_dir + "/key_back";
// END
...
static bool read_and_fixate_user_ce_key(userid_t user_id,
    ...
    for (auto const ce_key_path : paths) {
        ...
        auto back_ce_key_path =  directory_path +"/miuser_backup";
        if (retrieveKey(ce_key_path, auth, ce_key)) {
            LOG(DEBUG) << "Successfully retrieved key";
            fixate_user_ce_key(directory_path, ce_key_path, paths);
            // get ce key success, check whether has backup.
            if  (!android::vold::backupKeyExists(back_ce_key_path, 0)) {
                if (TEMP_FAILURE_RETRY(mkdir(back_ce_key_path.c_str(), 0700)) == -1) {
                    LOG(ERROR) <<"mkdir backup dir failed";
                } else {
                    LOG(DEBUG) <<"start backup, src =  "<< ce_key_path << " dst = "<<back_ce_key_path;
                    std::thread(backupKey, ce_key_path, back_ce_key_path).detach();
                }
            }
            return true;
        } else {
            LOG(DEBUG) <<"get ce key failed for user: "<<user_id << "try backup key: "<< back_ce_key_path;
            if (retrieveBackupKey(ce_key_path, back_ce_key_path, auth, ce_key)) {
                LOG(DEBUG) << "retrieveBackupKey success";
                return true;
            }
        }
    }
...
static bool load_all_de_keys() {
    ...
    for (;;) {
        ...
        KeyBuffer de_key;
        auto key_path_backup = key_path+"_backup";
        if (!retrieveKey(key_path, kEmptyAuthentication, &de_key)) {
            LOG(DEBUG) << "retrieveKey user 0  failed, try backup";
            if (!retrieveBackupKey(key_path, key_path_backup, kEmptyAuthentication, &de_key)) {
                // This is probably a partially created user, so ignore
                if (user_id != 0) continue;
                LOG(ERROR) << "retrieveBackupKey failed, return";
                return false;
            }
        } else {
            if (!android::vold::backupKeyExists(key_path_backup, 1)) {
                if (TEMP_FAILURE_RETRY(mkdir(key_path_backup.c_str(), 0700)) == -1) {
                    LOG(ERROR) <<"mkdir backup dir failed";
                } else {
                    LOG(INFO) <<"start backup";
                    std::thread(backupKey, key_path, key_path_backup).detach();
                }
            }
        }
        EncryptionPolicy de_policy;
...
bool fscrypt_initialize_systemwide_keys() {
    ...
    if (!retrieveOrGenerateKey(device_key_path, device_key_temp, kEmptyAuthentication,
        ...
        // MIUI ADD: START
        LOG(INFO) << "retrieveOrGenerateKey failed, try backup key";
        if (!retrieveBackupKey(device_key_path, device_key_path_backup, kEmptyAuthentication, &device_key)) {
            LOG(ERROR) << "retrieveBackupKey failed";
            return false;
        }//END
    } else {  //get key success, backup it, if not backup
        if (!android::vold::backupKeyExists(device_key_path_backup, 1)) {
            LOG(ERROR) <<"start backup";
            if (TEMP_FAILURE_RETRY(mkdir(device_key_path_backup.c_str(), 0700)) == -1) {
                LOG(ERROR) <<"mkdir backup dir failed";
            } else {
                std::thread(backupKey, device_key_path, device_key_path_backup).detach();
            }
        }
    }
...
bool fscrypt_destroy_user_key(userid_t user_id) {
    ...
    if (it != s_ephemeral_users.end()) {
        ...
        //remote user id ce key backup
        auto ce_key_path_backup  = get_ce_key_directory_path(user_id) + "/miuser_backup";
        if (android::vold::pathExists(ce_key_path_backup)) {
            success &= android::vold::destroyKey(ce_key_path_backup);
        } else {
            LOG(INFO) << "Not present so not erasing: " << ce_key_path_backup;
        }
        ...
        // remove user id de key backup
        auto de_key_path_backup = de_key_path+"_backup";
        LOG(DEBUG) << "erasing de backup key: " << de_key_path_backup;
        if (android::vold::pathExists(de_key_path_backup)) {
            success &= android::vold::destroyKey(de_key_path_backup);
        } else {
            LOG(INFO) << "Not present so not erasing: " << de_key_path_backup;
        }
    }
    return success;
}
...
static bool fscrypt_rewrap_user_key(userid_t user_id, int serial,
    ...
    std::string back_ce_key_path = directory_path +  "/miuser_backup";
    if(0 == std::remove((back_ce_key_path+"/keymaster_key_blob").c_str())) {
        LOG(INFO) << "Successfully remove keymaster_key_blob";
    } else {
        LOG(ERROR) << "Failed to remove keymaster_key_blob";
    }
    std::thread(backupKey, ce_key_path, back_ce_key_path).detach();
    return true;
}

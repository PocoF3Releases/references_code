    ...
    for (const auto& id : privateIds) {
        ...
        if (vol != nullptr && vol->getState() == VolumeBase::State::kMounted) {
            if (path_type == PathTypes::kMountPoint) {
                ...
                if ((fs_type == "f2fs" || fs_type == "mifs") &&
                     (Realpath(vol->getRawDmDevPath(), &gc_path) ||
...
static void addFromFstab(std::list<std::string>* paths, PathTypes path_type, bool only_data_part) {
    ...
    for (const auto& entry : fstab_default) {
        ...
        if (path_type == PathTypes::kMountPoint) {
            ...
            std::string path;
            if ((entry.fs_type == "f2fs" || entry.fs_type == "mifs") &&
...
void Trim(const android::sp<android::os::IVoldTaskListener>& listener) {
    ...
    // MIUI ADD: START
    // Collect both fstab and vold volumes of discard
    std::list<std::string> paths_discard;
    addFromFstab(&paths_discard, PathTypes::kBlkDevice,false);
    addFromVolumeManager(&paths_discard, PathTypes::kBlkDevice);
    // END

    for (const auto& path : paths) {
        // MIUI ADD: START
        if (VoldStub::ShouldAbort()) {
            PLOG(WARNING) << "Trim stopped";
            break;
        }
        // END
    ...
    // MIUI ADD:START
    /*
     * The function RunUrgentGcIfNeeded apply to the version of kernel 5.10 and above,
     * need to trigger urgent gc to issue the discard request after fstrim.
     * Discard timeing policy:
     * PENDING_DISCARD_THRESHOLD = 128 discard_cmd;
     * PENDING_DISCARD_TIMEOUT_SEC = 7 min;
     */
    VoldStub::RunUrgentGcIfNeeded(paths_discard);
    // END
...
int AbortIdleMaint(const android::sp<android::os::IVoldTaskListener>& listener) {
    ...
    // MIUI ADD:
    // Stop trim operation if it's in progress
    VoldStub::StopTrimThread();

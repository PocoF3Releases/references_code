...
class VoldNativeService : public BinderService<VoldNativeService>, public os::BnVold {
  public:
    ...
    // MIUI ADD: START
    binder::Status runExtMFlush(int pageCount, int flushLevel,
                const android::sp<android::os::IVoldTaskListener>& listener);
    binder::Status stopExtMFlush(
                const android::sp<android::os::IVoldTaskListener>& listener);
    // MIUI ADD: START
    binder::Status gcBoosterControl(const std::string& enable);
    binder::Status fixupAppDirRecursive(const std::string& path, int32_t appUid, int32_t gid, int32_t flag);
    binder::Status moveStorageQuickly(const std::string& from, const std::string& to,
                             const android::sp<android::os::IVoldTaskListener>& listener,
                             int32_t flag, int32_t* _aidl_return);
    binder::Status setCldListener(const android::sp<android::os::IVoldTaskListener>& listener);
    binder::Status getCldFragLevel(int32_t* _aidl_return);
    // END

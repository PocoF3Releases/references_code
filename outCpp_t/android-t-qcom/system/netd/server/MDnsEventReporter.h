...
class MDnsEventReporter final {
  public:
    ...
    // threadsafe.
    const EventListenerSet& getEventListeners() const ACQUIRE(mMutex);
    void putEventListeners() const RELEASE(mMutex);
  ...
  private:
    ...
    int removeEventListenerImpl(
            const android::sp<android::net::mdns::aidl::IMDnsEventListener>& listener)
            EXCLUDES(mMutex);
    const EventListenerSet& getEventListenersImpl() const ACQUIRE(mMutex);
    void putEventListenersImpl() const RELEASE(mMutex);
};

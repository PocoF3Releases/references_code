...
int FwmarkServer::processClient(SocketClient* client, int* socketFd) {
    ...
    *socketFd = received_fds[0].release();
    ...
    // MIUI ADD: START
    int rc = 0;
    if (command.cmdId != FwmarkCommand::TAG_SOCKET && command.cmdId != FwmarkCommand::UNTAG_SOCKET) {
        if ((rc = com::android::internal::net::OemNetd::checkMiuiNetworkFirewall(client, socketFd, command.netId))) {
           return rc;
        }
    }
    // END

...
const std::string WriteProtectPartitions[] =
{
    "fsg", "fsc", "modemst1", "modemst2", "persist"
};

static bool IsWpPartition(const std::string& PartitionName)
{
    uint32_t i = 0;
    uint32_t ArrayNum = sizeof(WriteProtectPartitions)/sizeof(WriteProtectPartitions[0]);

    if (PartitionName.empty())
        return false;
    for (i = 0; i < ArrayNum; i++) {
        if (WriteProtectPartitions[i] == PartitionName)
        return true;
    }
    return false;
}
...
bool EraseHandler(FastbootDevice* device, const std::vector<std::string>& args) {
    ...
    if (IsWpPartition(args[1])) {
        return device->WriteStatus(FastbootResult::FAIL,"Partition is write protected");
    }

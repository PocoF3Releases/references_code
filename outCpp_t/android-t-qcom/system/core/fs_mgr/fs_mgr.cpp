...
static bool is_mifs(const std::string& fs_type) {
    return fs_type == "mifs";
}

static bool is_f2fs_or_mifs(const std::string& fs_type) {
...
extern int get_dev_sz(const std::string& fs_blkdev, uint64_t* dev_sz);
static std::string get_resize_ovp_option(const std::string& blk_device) {
    ...
    if (get_dev_sz(blk_device, &dev_sz) == 0) {
        ...
        if (flash_ovp_map.find(flash) != flash_ovp_map.end()){
            ovp_option = "-o " + flash_ovp_map[flash];
            LINFO << "Setting resize overprovision ratio to " << flash_ovp_map[flash] << " for " << blk_device << "(" << flash << "G)";
        }
    }
...
static void check_fs(const std::string& blk_device, const std::string& fs_type,
    ...
    if (fs_type == "vfat") {
    ...
    } else if (is_f2fs_or_mifs(fs_type)) {
        std::string ovp_option = get_resize_ovp_option(blk_device);
        ...
        // reserve 1M(256*4096) for footer which actually need 16K at this time
        const char* f2fs_resize_argv[] = {F2FS_RESIZE_BIN, "-r 256", blk_device.c_str()};
        const char* f2fs_resize_argv_with_ovp[] = {F2FS_RESIZE_BIN, "-r 256", ovp_option.c_str(), blk_device.c_str()};
        ...
        LINFO << "Running " << F2FS_RESIZE_BIN << realpath(blk_device);
        if (ovp_option.empty())
            ret = logwrap_fork_execvp(ARRAY_SIZE(f2fs_resize_argv),f2fs_resize_argv,
                                        &status, true, LOG_KLOG | LOG_FILE, true, FSCK_LOG_FILE);
        else
            ret = logwrap_fork_execvp(ARRAY_SIZE(f2fs_resize_argv_with_ovp),f2fs_resize_argv_with_ovp,
                                        &status, true, LOG_KLOG | LOG_FILE, true, FSCK_LOG_FILE);
        if (ret < 0) {
            /* No need to check for error in fork, we can't really handle it now */
...
static int prepare_fs_for_mount(const std::string& blk_device, const FstabEntry& entry,
    ...
    } else if (is_f2fs(entry.fs_type) || is_mifs(entry.fs_type)) {
...
static int __mount(const std::string& source, const std::string& target, const FstabEntry& entry) {
    ...
    bool try_f2fs_gc_allowance = is_f2fs_or_mifs(entry.fs_type) && entry.fs_checkpoint_opts.length() > 0;
  ...
  private:
    bool UpdateCheckpointPartition(FstabEntry* entry, const std::string& block_device) {
        if (entry->fs_mgr_flags.checkpoint_fs) {
            if (is_f2fs_or_mifs(entry->fs_type)) {
...
static int fs_mgr_do_mount_helper(Fstab* fstab, const std::string& n_name,
    ...
    for (auto i {fstab->size()}; i--;) {
        auto &fstab_entry = (*fstab)[i];

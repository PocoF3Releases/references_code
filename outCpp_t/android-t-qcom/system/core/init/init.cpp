...
static void LoadBootScripts(ActionManager& action_manager, ServiceList& service_list) {
    ...
    if (bootscript.empty()) {
        ...
        if (!parser.ParseConfig("/mnt/vendor/mi_ext/etc/init")) {
            late_import_paths.emplace_back("/mnt/vendor/mi_ext/etc/init");
        }
    } else {
...
int SecondStageMain(int argc, char** argv) {
    ...
    //Update debug policy status
    std::string dp_flag;
    android::base::ReadFileToString("/proc/dp_flag", &dp_flag);
    if(!dp_flag.empty()){
       SetProperty("ro.boot.dp", dp_flag.c_str());
    }

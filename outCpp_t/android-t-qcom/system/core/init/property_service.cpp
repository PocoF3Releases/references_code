...
static uint32_t PropertySet(const std::string& name, const std::string& value, std::string* error) {
    ...
    if (pi != nullptr) {
#ifdef MIUI_INIT_HOOK
      if (!should_skip_check(name.c_str())) {
#endif
...
#ifdef MIUI_INIT_HOOK
            if (loading_cust && !should_skip_check(key)) {
                LOG(ERROR) << key << " should not be set as cust prop";
                continue;
            }
#endif // MIUI_INIT_HOOK
...
static void property_initialize_ro_product_props() {
    ...
    const char* RO_PRODUCT_PROPS[] = {
            "brand", "device", "manufacturer", "model", "real_model", "name", "marketname", "cert",
    };
...
#ifdef MIUI_INIT_HOOK
    loading_cust = 1;
    // Load miui cust prop to make read-only
    // "ro.*" and "permanent.*" properties force using cust value
    const char* cust_prop_file_path = get_cust_prop_file_path();
    if (cust_prop_file_path != nullptr && access(cust_prop_file_path, R_OK) == 0) {
        load_properties_from_file(cust_prop_file_path, nullptr, &properties);
    }
    const char* cts_prop_file_path = get_cts_prop_file_path();
    if (cts_prop_file_path != nullptr && access(cts_prop_file_path, R_OK) == 0) {
        load_properties_from_file(cts_prop_file_path, nullptr, &properties);
    }
    loading_cust = 0;
#endif

//ccm.prop should be the last prop
#ifdef MIUI_INIT_HOOK
    load_properties_from_file("/product/etc/ccm.prop", nullptr, &properties);
#endif

//read property from mi_ext
#ifdef MIUI_INIT_HOOK
    if (access("/mnt/vendor/mi_ext/etc/build.prop", R_OK) == 0) {
        load_properties_from_file("/mnt/vendor/mi_ext/etc/build.prop", nullptr, &properties);
    }
#endif
    for (const auto& [name, value] : properties) {
        std::string error;
...
constexpr auto MIUIRO_PREFIX = "miuiro."sv;

static void ProcessKernelCmdline() {
    ImportKernelCmdline([&](const std::string& key, const std::string& value) {
        if (StartsWith(key, ANDROIDBOOT_PREFIX)) {
            InitPropertySet("ro.boot." + key.substr(ANDROIDBOOT_PREFIX.size()), value);
        } else if (StartsWith(key, MIUIRO_PREFIX)) {
            InitPropertySet("ro." + key.substr(MIUIRO_PREFIX.size()), value);
	}
    });
}
...
static void HandleInitSocket() {
    ...
    switch (init_message.msg_case()) {
        case InitMessage::kLoadPersistentProperties: {
            ...
            loading_cust = 1;
            std::map<std::string, std::string> properties;
            // Load miui cust prop to make modified
            // "persistent.*", etc, properties force using cust value
            const char* cust_prop_file_path = get_cust_prop_file_path();
            if (cust_prop_file_path != nullptr && access(cust_prop_file_path, R_OK) == 0) {
                load_properties_from_file(cust_prop_file_path, nullptr, &properties);
            }
            ...
            // Load miui_customized.prop
            const char* miui_customized_prop_file_path = get_miui_customized_prop_file_path();
            if (miui_customized_prop_file_path != nullptr && access(miui_customized_prop_file_path, R_OK) == 0) {
                load_properties_from_file(miui_customized_prop_file_path, nullptr, &properties);
            }
            // Load cota carrier prop to make modified
            const char* carrier_common_prop_file_path = get_carrier_common_prop_file_path();
            if (carrier_common_prop_file_path != nullptr && access(carrier_common_prop_file_path, R_OK) == 0) {
                load_properties_from_file(carrier_common_prop_file_path, nullptr, &properties);
            }
            const char* carrier_region_prop_file_path = get_carrier_region_prop_file_path();
            if (carrier_region_prop_file_path != nullptr && access(carrier_region_prop_file_path, R_OK) == 0) {
                load_properties_from_file(carrier_region_prop_file_path, nullptr, &properties);
            }
            // Load business.prop
            const char* business_prop_file_path = get_business_prop_file_path();
            if (business_prop_file_path != nullptr && access(business_prop_file_path, R_OK) == 0) {
                 load_properties_from_file(business_prop_file_path, nullptr, &properties);
            }
            loading_cust = 0;
            for (const auto& [name, value] : properties) {
                std::string error;
                if (PropertySet(name, value, &error) != PROP_SUCCESS) {
                    LOG(ERROR) << "Could not set '" << name << "' to '" << value
                            << "' while loading miui cust prop files" << error;
                }
            }
#endif
            ...
            // Load netflixchannel prop
            std::string netflixchannel = GetProperty("persist.sys.netflix.channel", "");
            if (!netflixchannel.empty()) {
                InitPropertySet("ro.netflix.channel", netflixchannel);
            }
            // Load facebookid prop
            std::string facebookid = GetProperty("persist.sys.facebook.partnerid", "");
            if (!facebookid.empty()) {
                InitPropertySet("ro.facebook.partnerid", facebookid);
            }

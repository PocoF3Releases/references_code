...
static int get_sched_policy_from_group(const std::string& group, SchedPolicy* policy) {
    if (group.empty()) {
        ...
        *policy = SP_AUDIO_APP;
    } else if (group == "camera-background"
            || group == "camera-background/background"
            || group == "camera-background/limit") {
        *policy = SP_CAMERA_BACKGROUND;
    } else {
        errno = ERANGE;
        return -1;
    }
...
//MIUI ADD: START
int get_cpuset_policy(int tid, SchedPolicy* policy) {
    if (tid == 0) {
        tid = GetThreadId();
    }
    ...
    if (cpusets_enabled()) {
        ...
        // Wipe invalid group to fallback to cpu
        if (!group.empty()) {
            if (get_sched_policy_from_group(group, policy) < 0) {
                group.clear();
            } else {
    ...
    if (schedboost_enabled() && (getCGroupSubsys(tid, "schedtune", group) < 0)
        && (getCGroupSubsys(tid, "cpu", group) < 0)) {
        LOG(ERROR) << "Failed to find cpu cgroup for tid " << tid;
        return -1;
    }
...
int get_cpuset_policy(int, SchedPolicy* policy) {
    *policy = SP_SYSTEM_DEFAULT;
    return 0;
}
#endif
...
const char* get_sched_policy_name(SchedPolicy policy) {
    ...
    static const char* const kSchedPolicyNames[] = {
            ...
            [SP_RT_APP] = "rt",     [SP_RESTRICTED] = "rs", [SP_UNTRUSTEDAPP_BACKGROUND] = "ubg",
            [SP_CAMERA_BACKGROUND] = "cbg"
    };
...
const char* get_cpuset_policy_profile_name(SchedPolicy policy) {
    ...
    static constexpr const char* kCpusetProfiles[SP_CNT + 1] = {
            ...
            "CPUSET_SP_CAMERA_LIMIT_BACKGROUND"};

...
static constexpr const char* CGROUPS_DESC_FILE_V1 = "/etc/cgroups_v1.json";
...
static bool ReadDescriptors(std::map<std::string, CgroupDescriptor>* descriptors) {
    ...
    if (base::GetBoolProperty("persist.sys.millet.cgroup1", false)) {
        if (!ReadDescriptorsFromFile(CGROUPS_DESC_FILE_V1, descriptors)) {
            return false;
    }
    } else {

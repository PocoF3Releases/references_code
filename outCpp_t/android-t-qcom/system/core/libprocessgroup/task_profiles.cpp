...
static constexpr const char* TASK_PROFILE_DB_FILE_V1 = "/etc/task_profiles_v1.json";
...
TaskProfiles::TaskProfiles() {
    // load system task profiles
     if (android::base::GetBoolProperty("persist.sys.millet.cgroup1", false)) {
        if (!Load(CgroupMap::GetInstance(), TASK_PROFILE_DB_FILE_V1)) {
            LOG(ERROR) << "Loading " << TASK_PROFILE_DB_FILE_V1 << " for [" << getpid() << "] failed";
        }
     }
     else {

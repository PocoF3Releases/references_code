...
static int debuggerd_dispatch_pseudothread(void* arg) {
  // MIUI Add, block signals, signal 33 for systemserver SIGSEGV
  sigset64_t sigset;
  memset(&sigset, 0xff, sizeof(sigset64_t));
  if (0 != __rt_sigprocmask(SIG_SETMASK, &sigset, NULL, sizeof(sigset64_t))) {
	  async_safe_format_log(ANDROID_LOG_ERROR, "libc", "__rt_sigprocmask failed: %s", strerror(errno));
  }
  // MIUI end
  ...
  if (fcntl(output_write.get(), F_SETPIPE_SZ, expected) < static_cast<int>(expected)) {
    ...
    async_safe_format_log(ANDROID_LOG_FATAL, "libc",
                          "failed to set pipe buffer size to %zd: %s", expected, strerror(errno));
    return 1;
    // END
  }
  ...
  MiDebuggerdStub::dumpUnreachableMemory(input_read.get());

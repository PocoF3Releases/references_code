  ...
  regs->IterateRegisters([&thread, memory_dump, maps, memory](const char* name, uint64_t value) {
    ...
    *thread.add_registers() = r;
    ...
    if (memory_dump) {
      ...
      size_t kNumBytesAroundRegister = 256; // default size to give dumping
      if (strcmp(name, "sp") == 0)
      kNumBytesAroundRegister = 4096;
      else if (dump.mapping_name().empty() || dump.mapping_name().c_str()[0] == '[') // anonymous area
      kNumBytesAroundRegister = 1024; // TODO: tell more different mapping names/types
      size_t kNumTagsAroundRegister = kNumBytesAroundRegister / kTagGranuleSize;
      // END
...
void engrave_tombstone_proto(Tombstone* tombstone, unwindstack::Unwinder* unwinder,
  ...
  *result.mutable_signal_info() = sig;
  ...
  if (android::base::GetBoolProperty("ro.debuggable", false)) {
    // MIUI MOD: START
    //Camera stress test will set log buffer to be 256M,
    //catch such big logcat in tombstone will cause debuggerd timeout.
    bool logcatEn = android::base::GetBoolProperty("debug.debuggerd.tombstone_logcat_en", true);
    if (logcatEn) {
      dump_logcat(&result, main_thread.pid);
    } else {
      async_safe_format_log(ANDROID_LOG_INFO, LOG_TAG, "tombstone logcat %d is set",logcatEn);
    }
    // END
  }

...
// MIUI ADD:
#if defined(__LP64__)
#define MIUINDBG_PATH "system/lib64/libmiuindbg.so"
#else
#define MIUINDBG_PATH "system/lib/libmiuindbg.so"
#endif
// END
...
static bool activity_manager_notify(pid_t pid, int signal, const std::string& amfd_data) {
  ...
  {
    PLOG(ERROR) << "cannot ignore SIGPIPE";
  }
  // MIUI END
  ...
  {
    PLOG(ERROR) << "default SIGPIPE action";
  }
  // MIUI END
...
int main(int argc, char** argv) {
  ...
  MiDebuggerdStub::ptraceThreadsForUnreachable(target_process, pseudothread_tid, output_pipe.get(), dump_type);
  ...
  if (!backtrace && fatal_signal) {
    typedef void (*funcPtr) (pid_t, pid_t);

    void *handle = dlopen(MIUINDBG_PATH, RTLD_NOW);
    if (!handle) {
      PLOG(ERROR) << "cannot open " << MIUINDBG_PATH;
    } else {
      funcPtr func = (funcPtr)dlsym(handle, "miui_native_debug_process_O");
      if (func) {
        func(target_process, g_target_thread);
      } else {
        PLOG(ERROR) << "cannot find miui_native_debug_process_O";
      }
      dlclose(handle);
    }
  }

...
// MIUI ADD:
static inline bool validateHeapAddr(void* ptr, const char* caller, int callat) {
    if (sizeof(void*) != 8) // only effect on 64-bits
        return true;

    if (ptr == nullptr) {
        ALOGE("[%s:%d] null pointer\n", caller, callat);
        return false;
    }

    uintptr_t addr = reinterpret_cast<uintptr_t>(ptr); // check address alignment
    if (addr % 8 != 0) {
        ALOGE("[%s:%d] wrong alignment: %p\n", caller, callat, ptr);
        return false;
    }

    uintptr_t top = addr & (0xffULL << 56); // check address value's top-byte
    /*if (top == 0) {
        ALOGE("[%s:%d] invalid top-byte: %p\n", caller, callat, ptr);
        return false;
    }*/ // TODO: some app steals top-byte, so cannot validate it - fix the case later

    addr -= top;
    if (addr >= 0x7fffffffff) {  // check address range
        ALOGE("[%s:%d] bad address: %p\n", caller, callat, ptr);
        return false;
    }

    // TODO: check addr in heap partition

    return true;
}

#define VALIDATE_HEAP_ADDR(ptr) validateHeapAddr(ptr, __func__, __LINE__)
// END
...
{
    // MIUI ADD: workaround to improve robustness in case of possbile uaf issue
    if (!VALIDATE_HEAP_ADDR(mRefs))
        return;
    // END
...
RefBase::~RefBase()
{
    ...
    if ((flags & OBJECT_LIFETIME_MASK) == OBJECT_LIFETIME_WEAK) {
        ...
        if (mRefs->mWeak.load(std::memory_order_relaxed) == 0) {
            ...
            int weak_cnt = mRefs->mWeak.load(std::memory_order_relaxed);
            LOG_ALWAYS_FATAL("RefBase: destruct object %p, which holds %d WEAK ref.", this, weak_cnt);
        } // END
        ...
        int strong_cnt = mRefs->mStrong.load(std::memory_order_relaxed);
        if (strong_cnt > 0) { // the ref. was increased
            LOG_ALWAYS_FATAL("RefBase: destruct object %p, which holds %d STRONG ref.", this, strong_cnt);
        }
    } // END

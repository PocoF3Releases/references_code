...
struct inner_proc {
    ...
    struct inner_proc* pidhash_next;
};

#define PIDHASH_SZ 1024
static struct inner_proc** pidhash = NULL;
#define pid_hashfn(x) ((((x) >> 8) ^ (x)) & (PIDHASH_SZ - 1))

static struct inner_proc* pid_lookup(int pid) {
    struct inner_proc* procp;
...
static void proc_insert(struct inner_proc* procp) {
    if (!pidhash) {
        pidhash = static_cast<struct inner_proc**>(calloc(PIDHASH_SZ, sizeof(*pidhash)));
    }
...
void stats_store_taskname(int pid, const char* taskname) {
    ...
    struct inner_proc* procp = pid_lookup(pid);
    if (procp != NULL) {
    ...
    procp = static_cast<struct inner_proc*>(malloc(sizeof(struct inner_proc)));
    procp->pid = pid;
...
const char* stats_get_task_name(int pid) {
    struct inner_proc* inner_proc = pid_lookup(pid);
    return inner_proc ? inner_proc->taskname : NULL;
}

...
#define PSI_SCRIT_COMPLETE_STALL_MS (50)
#else
...
bool use_inkernel_interface = true;
bool has_inkernel_module;
...
#ifdef QCOM_FEATURE_ENABLE
int direct_reclaim_pressure = 45;
int reclaim_scan_threshold = 1024;
bool force_use_old_strategy;
#endif
bool use_minfree_levels;
static bool per_app_memcg;
bool enhance_batch_kill;
bool enable_adaptive_lmk;
bool enable_userspace_lmk;
bool enable_watermark_check;
...
#ifdef QCOM_FEATURE_ENABLE
bool enable_preferred_apps =  false;
bool last_event_upgraded = false;
static int count_upgraded_event;
static long pa_update_timeout_ms = 60000; /* 1 min */
int wmark_boost_factor = 1;
int wbf_step = 1, wbf_effective = 1;
#endif
static int kpoll_fd;
#ifdef QCOM_FEATURE_ENABLE
int psi_cont_event_thresh = PSI_CONT_EVENT_THRESH;
static bool use_perf_api_for_pref_apps;
/* PSI window related variables */
int psi_window_size_ms = PSI_WINDOW_SIZE_MS;
int psi_poll_period_scrit_ms = PSI_POLL_PERIOD_SHORT_MS;
...
/* camera */
#define MEMCG_MEMORY_CAMERA_APP "/dev/memcg/camera/app/cgroup.procs"
#define CAMERA_APP "com.android.camera"
static uid_t cameraUid;
...
#else
const char* const vmstat_field_names[MI_FIELD_COUNT] =
{
    "nr_free_pages",
    "nr_inactive_file",
    "nr_active_file",
...
union vmstat s_crit_base;
bool s_crit_event = false;
#endif
 ...
 */
char *reread_file(struct reread_data *data) {
...
struct proc *pid_lookup(int pid) {
    struct proc *procp;
...
void proc_insert(struct proc *procp) {
...
long get_time_diff_ms(struct timespec *from,
                                    struct timespec *to) {
...
char *proc_get_name(int pid, char *buf, size_t buf_size) {
    static char path[PATH_MAX];
    int fd;
    char *cp;
    ssize_t ret;
...
static void cmd_procprio(LMKD_CTRL_PACKET packet, int field_count, struct ucred *cred) {
    ...
    if (cameraUid == 0) {
        ...
        if (!strcmp(path, CAMERA_APP)) {
            cameraUid = params.uid;
            writefilestring(MEMCG_MEMORY_CAMERA_APP, std::to_string(params.pid).c_str(), true);
        }
    }
    ...
    if (!procp) {
        ...
        if (cameraUid == procp->uid) {
            writefilestring(MEMCG_MEMORY_CAMERA_APP, std::to_string(procp->pid).c_str(), true);
        }
    } else {
...
static void ctrl_command_handler(int dsock_idx) {
    ...
    default:
#ifdef QCOM_FEATURE_ENABLE
        if (LmkdStub::ext_command_handler(cmd, packet, nargs)) {
            break;
        }
#endif
...
bool vmstat_parse_line(char *line, union vmstat *vs) {
    char *cp;
    char *ap;
    char *save_ptr;
    int64_t val;
...
int vmstat_parse(union vmstat *vs) {
    static struct reread_data file_data = {
...
static void killinfo_log(struct proc* procp, int min_oom_score, int rss_kb,
                         ...
                         struct wakeup_info *wi, struct timespec *tm, struct psi_data *pd, char *taskname) {
    /* log process information */
    android_log_write_int32(ctx, procp->pid);
    android_log_write_string8(ctx, taskname);
...
static struct proc *proc_get_heaviest(int oomadj) {
    ...
    while (curr != head) {
        ...
        long tasksize = LmkdStub::proc_get_size(pid);
        if (tasksize < 0) {
            ...
            } else if (tmp_taskname != NULL && LmkdStub::is_protected_task(oomadj, tmp_taskname)) {
                // protect last recent app for camera forground.
                ALOGV("%s, skip %s in oomadj %d", __func__, tmp_taskname, oomadj);
            } else {
                if (tasksize > maxsize) {
                    maxsize = tasksize;
    ...
    if (maxsize > 0) {
        return maxprocp;
    } else if (maxsize_pa > 0) {
        return maxprocp_pa;
    }
    return (struct proc *)LmkdStub::proc_get_heaviest_extend(oomadj);
#else
    return maxprocp;
#endif
}
...
static void watchdog_callback() {
    ...
    for (int oom_score = OOM_SCORE_ADJ_MAX; oom_score >= 0;) {
        ...
        char buf[LINE_MAX];
        proc_get_name(target.pid, buf, sizeof(buf));
        if (!strcmp(buf, "com.phonetest.application:CameraMemoryWatcher")) {
            oom_score--;
            prev_pid = 0;
            continue;
        }
        if (reaper.kill({ target.pidfd, target.pid, target.uid }, true) == 0) {
            ALOGW("lmkd watchdog killed process %d, oom_score_adj %d %s", target.pid, oom_score, buf);
            killinfo_log(&target, 0, 0, 0, NULL, NULL, NULL, NULL, NULL, NULL);
            break;
        }
        prev_pid = target.pid;
    }
...
static int kill_one_process(struct proc* procp, int min_oom_score, struct kill_info *ki,
    ...
    #ifdef QCOM_FEATURE_ENABLE
    LmkdStub::mem_report(procp->oomadj,data_sock);
    #endif
    ...
    #ifdef OPT_AOSP_LMKD_REAPER
    reaper.async_mrelease({ pidfd, pid, uid });
    set_process_prio(pid, ANDROID_PRIORITY_HIGHEST);
    #endif
    ...
    killinfo_log(procp, min_oom_score, rss_kb, swap_kb, ki, mi, wi, tm, pd, taskname);
...
#ifdef QCOM_FEATURE_ENABLE
    LmkdStub::extend_reclaim_check(mi);

    if (!killed_size && !min_score_adj && is_userdebug_or_eng_build) {
        killed_size = LmkdStub::proc_get_script();
    }
#endif
 ...
 */
#if defined(QCOM_FEATURE_ENABLE) && defined(MI_PERF_FEATURE)
enum zone_watermark get_lowest_watermark(union meminfo *mi __unused,
...
void calc_zone_watermarks(struct zoneinfo *zi, struct zone_meminfo *zmi, int64_t *pgskip_deltas) {
    ...
    for (int node_idx = 0; node_idx < zi->node_count; node_idx++) {
        ...
        for (int zone_idx = 0; zone_idx < node->zone_count; zone_idx++) {
            ...
            if (debug_process_killing) {
                ALOGE("%s pgskip_deltas_val: %" PRId64 " pgskip_deltas[PGSKIP_IDX(i++)]: %"
                        PRId64, __func__, pgskip_deltas_val, pgskip_deltas[PGSKIP_IDX(i++)]);
            }
    ...
    LmkdStub::log_zone_watermarks(zi, watermarks);
}
#else
...
static void mp_event_psi(int data, uint32_t events, struct polling_params *poll_params) {
...
#if defined(QCOM_FEATURE_ENABLE) && defined(MI_PERF_FEATURE)
    struct memory_pressure_policy_pc pc;
#endif
...
#if defined(QCOM_FEATURE_ENABLE) && defined(MI_PERF_FEATURE)
    LmkdStub::mi_count_cont_psi_event(events, level, &curr_tm);
#endif
...
#ifdef QCOM_FEATURE_ENABLE
    LmkdStub::fill_log_pgskip_stats(&vs, init_pgskip, pgskip_deltas);
#endif
...
#if defined(QCOM_FEATURE_ENABLE) && defined(MI_PERF_FEATURE)
    LmkdStub::mi_do_early(vs, events, pgskip_deltas, mi, level);
#endif
    ...
    wmark = get_lowest_watermark(&mi, &watermarks, level);
#endif
...
#if defined(QCOM_FEATURE_ENABLE) && defined(MI_PERF_FEATURE)
    pc.in_compaction = in_compaction,
    pc.swap_is_low = swap_is_low,
    pc.cycle_after_kill = cycle_after_kill,
    pc.reclaim = reclaim,
    pc.thrashing_limit = thrashing_limit,
    pc.thrashing_limit_pct = thrashing_limit_pct,
    pc.thrashing_critical_pct = thrashing_critical_pct,
    pc.events = events,
    pc.thrashing = thrashing,
    pc.swap_low_threshold = swap_low_threshold,
    pc.min_score_adj = &min_score_adj,
    pc.kill_reason = kill_reason,
    pc.curr_tm = &curr_tm,
    pc.kill_desc = kill_desc,
    pc.wmark = wmark,
    pc.level = level,
    pc.mi = mi,
    pc.zi = zi,
    pc.zone_mem_info = zone_mem_info,
    pc.psi_info = psi_data,
    LmkdStub::mi_memory_pressure_policy(&pc);
    kill_reason = pc.kill_reason;
#endif
...
// v1 cgroup hierarchy.
void mp_event_common(int data, uint32_t events, struct polling_params *poll_params) {
...
#ifdef QCOM_FEATURE_ENABLE
    if (!s_crit_event) {
        level = LmkdStub::upgrade_vmpressure_event(level);
    }
#endif
...
#ifdef QCOM_FEATURE_ENABLE
                min_score_adj = LmkdStub::zone_watermarks_ok(level);
                if (min_score_adj == OOM_SCORE_ADJ_MAX + 1) {
    ...
    while (1) {
		...
		if (force_use_old_strategy) {
			...
			if (s_crit_event &&
				...
				vmstat_parse(&poll2);
				if ((nevents > 0 && LmkdStub::have_psi_events(events, nevents)) ||
        ...
        for (i = 0, evt = &events[0]; i < nevents; ++i, evt++) {
            ...
            if (evt->data.ptr) {
                ...
                if ((handler_info->handler == mp_event_common ||
                    ...
                    LmkdStub::check_cont_lmkd_events(handler_info->data);
                }
#endif
#if defined(QCOM_FEATURE_ENABLE) && defined(MI_PERF_FEATURE)
                LmkdStub::poll_params_reset(&poll_params);
#endif
...
#endif
    thrashing_critical_pct = std::max(0, property_get_int32("ro.lmk.thrashing_limit_critical",

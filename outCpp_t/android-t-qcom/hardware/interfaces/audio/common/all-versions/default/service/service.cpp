...
// AudioCpuSched start
void reScheduleCpu(int cpuSet, int cpus, pid_t tid)
{
    cpu_set_t set;
    ALOGD("[APM]-HAL %s() cpuSet=%d tid=%d", __func__, cpuSet, tid);
    CPU_ZERO(&set);
...
// AudioCpuSched start
    int64_t isAudioCpuSchedEnabled = property_get_int64("vendor.audio.cpu.sched", 0 /* default_value */);
    if (isAudioCpuSchedEnabled & AUDIO_SCHED_HB) {
        int cpus = property_get_int32("vendor.audio.cpu.sched.cpus", 8 /* default_value */);
        int audioCpuSet = property_get_int32("vendor.audio.cpu.sched.cpuset.hb", 0xff /* default_value */);

...
// AudioCpuSched start
    ...
    audioCpuSet = property_get_int32("vendor.audio.cpu.sched.cpuset.he", 0xff /* default_value */);
// AudioCpuSched end
}
...
void reScheduleCpu(int cpuSet, int cpus, pid_t tid)
{
    ...
    ALOGD("[APM]-HalEffect %s() cpuSet=%d tid=%d", __func__, cpuSet, tid);
    CPU_ZERO(&set);
...
// AudioCpuSched start
    if (isAudioCpuSchedEnabled & AUDIO_SCHED_HE) {
        reScheduleCpu(audioCpuSet, cpus, mProcessThread->getTid());
    }
// AudioCpuSched end
    return Void();
}

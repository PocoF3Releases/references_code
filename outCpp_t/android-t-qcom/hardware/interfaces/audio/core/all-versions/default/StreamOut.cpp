...
// AudioCpuSched start
    ...
    audioCpuSet = property_get_int32("vendor.audio.cpu.sched.cpuset.hso", 0xff /* default_value */);
// AudioCpuSched end
}
...
void reScheduleCpu(int cpuSet, int cpus, pid_t tid)
{
    ...
    ALOGD("[APM]-HalSteam %s() cpuSet=%d tid=%d", __func__, cpuSet, tid);
    CPU_ZERO(&set);
...
Return<void> StreamOut::prepareForWriting(uint32_t frameSize, uint32_t framesCount,
    ...
    _hidl_cb(Result::OK, *mCommandMQ->getDesc(), *mDataMQ->getDesc(), *mStatusMQ->getDesc(),
             threadInfo);
// AudioCpuSched start
    if (isAudioCpuSchedEnabled & AUDIO_SCHED_HSO) {
        reScheduleCpu(audioCpuSet, cpus, mWriteThread->getTid());
    }
// AudioCpuSched end
    return Void();
}

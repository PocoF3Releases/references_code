...
static void* phTmlUwb_TmlReaderThread(void* pParam) {
  ...
  UNUSED(pParam);
  NXPLOG_TML_D("SR100 - Tml Reader Thread Started................\n");
  ...
  while (gpphTmlUwb_Context->bThreadDone) {
    ...
    if (1 == gpphTmlUwb_Context->tReadInfo.bEnable) {
      NXPLOG_TML_D("SR100 - Read requested.....\n");
      /* Set the variable to success initially */
      wStatus = UWBSTATUS_SUCCESS;
      ...
      if (NULL != gpphTmlUwb_Context->pDevHandle) {
        NXPLOG_TML_D("SR100 - Invoking SPI Read.....\n");
        dwNoBytesWrRd =
        ...
        if (-1 == dwNoBytesWrRd) {
          NXPLOG_TML_E("SR100 - Error in SPI Read.....\n");
          ...
          NXPLOG_TML_E("SR100 - Empty packet Read, Ignore read and try new read...\n");
          pthread_mutex_lock(&gpphTmlUwb_Context->read_abort_lock);
          if(gpphTmlUwb_Context->is_read_abort) {
            pthread_cond_signal(&gpphTmlUwb_Context->read_abort_condition);
            gpphTmlUwb_Context->is_read_abort = false;
          ...
          NXPLOG_TML_D("SR100 - SPI Read successful.....\n");
          gpphTmlUwb_Context->tReadInfo.bEnable = 0;
          ...
          NXPLOG_TML_D("SR100 - Posting read message.....\n");
        ...
        NXPLOG_TML_D("SR100 -gpphTmlUwb_Context->pDevHandle is NULL");
      }
    } else {
      NXPLOG_TML_D("SR100 - read request NOT enabled");
      usleep(10 * 1000);
    }
  } /* End of While loop */
...
static void* phTmlUwb_TmlWriterThread(void* pParam) {
  ...
  static phLibUwb_Message_t tMsg;
  UNUSED(pParam);
  NXPLOG_TML_D("SR100 - Tml Writer Thread Started................\n");

  /* Writer thread loop shall be running till shutdown is invoked */
  while (gpphTmlUwb_Context->bThreadDone) {
    NXPLOG_TML_D("SR100 - Tml Writer Thread Running................\n");
    if (sem_wait(&gpphTmlUwb_Context->txSemaphore) != 0) {
      NXPLOG_TML_E("Failed to wait semaphore ");
    }
    /* If Tml write is requested */
    if (1 == gpphTmlUwb_Context->tWriteInfo.bEnable) {
      NXPLOG_TML_D("SR100 - Write requested.....\n");
      /* Set the variable to success initially */
      wStatus = UWBSTATUS_SUCCESS;
      if (NULL != gpphTmlUwb_Context->pDevHandle) {
        gpphTmlUwb_Context->tWriteInfo.bEnable = 0;
        ...
        NXPLOG_TML_D("SR100 - Invoking SPI Write.....\n");
        ...
        if (-1 == dwNoBytesWrRd) {
          NXPLOG_TML_E("SR100 - Error in SPI Write.....\n");
        ...
        if (UWBSTATUS_SUCCESS == wStatus) {
          NXPLOG_TML_D("SR100 - SPI Write successful.....\n");
        ...
        NXPLOG_TML_D("SR100 - Posting Fresh Write message.....\n");
        phTmlUwb_DeferredCall(gpphTmlUwb_Context->dwCallbackThreadId, &tMsg);
        if (UWBSTATUS_SUCCESS == wStatus) {
          /* TML reader writer callback synchronization mutex lock --- START
             */
      ...
      NXPLOG_TML_D("SR100 - Write request NOT enabled");
      usleep(10000);
    }

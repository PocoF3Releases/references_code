...
public:
    ...
    dng_image* render(int width, int height) {
        ...
        try {
            ...
            //xiaomi add start
            if (fFinalSpace) {
                render.SetFinalSpace(*fFinalSpace);
            } else {
            //xiaomi add end
    ...
    //xiaomi add start
    bool setFinalSpace(const skcms_ICCProfile* prof, const SkColorSpace *cs) {
        if (cs->gammaCloseToSRGB()) {
            auto &v = prof->toXYZD50.vals;
            dng_matrix_3by3 m(v[0][0],v[0][1],v[0][2],
                             v[1][0],v[1][1],v[1][2],
                             v[2][0],v[2][1],v[2][2]);

            auto finalSpace = std::make_unique<dng_space_custom>(
                               dng_space_custom::FnType::fn_type_srgb, m);
            fFinalSpace = std::move(finalSpace);
            SkDebugf("supported icc profile, use P3RGB for dng rendering");
            return true;
        } else {
            SkDebugf("unsupported icc profile, use SRGB for dng rendering");
        }
        return false;
    }
    //xiaomi add end
...
private:
    ...
    bool fIsXtransImage;
    //xiaomi add start
    std::unique_ptr<dng_color_space> fFinalSpace;
    //xiaomi add end
};
...
SkCodec::Result SkRawCodec::onGetPixels(const SkImageInfo& dstInfo, void* dst,
    ...
    const skcms_ICCProfile* srcProfile = this->getEncodedInfo().profile();
    if (auto cs = dstInfo.colorSpace()) {
        cs->toProfile(&dstProfileStorage);
        dstProfile = &dstProfileStorage;
        bool ret = fDngImage->setFinalSpace(dstProfile, cs);
        if (ret) {
            // dng will render to dst color space directly, no need to do CS transform for decoded pixels.

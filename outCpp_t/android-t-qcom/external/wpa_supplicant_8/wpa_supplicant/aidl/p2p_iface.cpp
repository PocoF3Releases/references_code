...
void P2pIface::stopPendingScan()
{
	struct wpa_supplicant* wpa_s = retrieveIfacePtr();
	if (pending_scan_res_join_callback != NULL) {
		wpa_printf(MSG_ERROR, "P2P: Failed to find the group with ssid"
			"for %d times try - stop join attempt",
			wpa_s->p2p_join_scan_count);
	...
	bool joinExistingGroup)
{
	...
	wpa_printf(MSG_DEBUG, "P2P: If freq is not 0, find by scan.if freq is 0, try old logic");
	if (freq == 0) {
		...
		os_get_reltime(&now);
		if (bss != NULL && !os_reltime_expired(&now, &wpa_s->last_scan, 5)) {

...
::ndk::ScopedAStatus StaIface::doSupplicantCommand(
	const std::string& in_command)
{
	return validateAndCall(
		this, SupplicantStatusCode::FAILURE_IFACE_INVALID,
		&StaIface::doSupplicantCommandInternal, in_command);
}
...
ndk::ScopedAStatus StaIface::doSupplicantCommandInternal(
	const std::string& in_command)
{
	std::string reply;
	size_t reply_len = 0;
	char *cmd_buf, *reply_buf = NULL;
	std::string MSCS_CMD = "MSCS";
	std::string WLAN0 = "wlan0";
	std::string WLAN1 = "wlan1";
	std::string P2P0 = "p2p0";
	std::string LATENCY_LEVEL_CMD = "p2p0 SET_LATENCY_LEVEL";
	std::string::size_type idx = in_command.find(MSCS_CMD);
	ndk::ScopedAStatus ret = ndk::ScopedAStatus::ok();
	if (idx != std::string::npos) {
		idx = in_command.find_first_of("-i");//-i wlan0 
		std::string str = in_command.substr(9);
		cmd_buf = (char *)os_malloc(strlen(str.c_str()) + 1);
		strcpy(cmd_buf, str.c_str());
		if (in_command.find(WLAN0) != std::string::npos) {
			reply_buf = wpa_supplicant_ctrl_iface_process(retrieveIfacePtr(WLAN0), cmd_buf, &reply_len);
		} else if (in_command.find(WLAN1) != std::string::npos) {
			reply_buf = wpa_supplicant_ctrl_iface_process(retrieveIfacePtr(WLAN1), cmd_buf, &reply_len);
		} else {
			reply_buf = wpa_supplicant_ctrl_iface_process(retrieveIfacePtr(), cmd_buf, &reply_len);
		}
		if (!reply_buf) {
			reply.append("FAIL\n");
			ret = createStatus(SupplicantStatusCode::FAILURE_UNKNOWN);
		} else {
			std::string str = reply_buf;
			str.resize(reply_len);
			if (str.compare("FAIL\n") == 0) {
				reply.append("FAIL\n");
				ret = createStatus(SupplicantStatusCode::FAILURE_UNKNOWN);
			}
		...
		if (in_command.find(LATENCY_LEVEL_CMD) != std::string::npos) {
			std::string str = in_command.substr(8);
			cmd_buf = (char *)os_malloc(strlen(str.c_str()) + 1);
			strcpy(cmd_buf, str.c_str());
			reply_buf = wpa_supplicant_ctrl_iface_process(retrieveIfacePtr(P2P0), cmd_buf, &reply_len);
		} else {
			cmd_buf = (char *)os_malloc(strlen(in_command.c_str()) + 1);
			strcpy(cmd_buf, in_command.c_str());
			reply_buf = wpa_supplicant_ctrl_iface_process(retrieveIfacePtr(), cmd_buf, &reply_len);
		}
		if (!reply_buf && reply_len == 1) {
			reply.append("FAIL\n");
		} else if (!reply_buf && reply_len == 2) {
			reply.append("OK\n");
		} else if (reply_buf) {
			reply.append(reply_buf);
		}
		if (reply.size() > 0) {
			ret = ndk::ScopedAStatus::ok();
		} else {
			ret = createStatus(SupplicantStatusCode::FAILURE_UNKNOWN);
		}
	}
...
std::pair<MloLinksInfo, ndk::ScopedAStatus> StaIface::getConnectionMloLinksInfoInternal()
{
	...
	memset(&linksInfo, 0, sizeof(linksInfo));
	for (int i = 0; i < MAX_NUM_MLD_LINKS; i++) {
		...
		link.staLinkMacAddress.resize(ETH_ALEN);
		os_memcpy(link.staLinkMacAddress.data(), wpa_s->links[i].addr, ETH_ALEN);
...
wpa_supplicant *StaIface::retrieveIfacePtr(std::string interface)
{
	return wpa_supplicant_get_iface(wpa_global_, interface.c_str());
}
}  // namespace supplicant
}  // namespace wifi
}  // namespace hardware

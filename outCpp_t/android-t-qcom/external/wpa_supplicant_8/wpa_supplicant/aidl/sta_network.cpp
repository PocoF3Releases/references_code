...
#ifdef CONFIG_WAPI
static int WAPI_PSK_TYPE_ASCII   = 0;
static int WAPI_PSK_TYPE_HEX     = 1;
static int WAPI_CERT_MODE_AUTO   = 0;
static int WAPI_CERT_MODE_MANUAL = 1;
#endif
...
{
	wpa_printf(MSG_DEBUG, "setPskPassphraseInternal");
	struct wpa_ssid *wpa_ssid = retrieveNetworkPtr();
	std::string psk = rawPsk;
	int ret=0;
#ifdef CONFIG_WAPI /*INTERFACE*/
	if (wpa_ssid->key_mgmt & WPA_KEY_MGMT_WAPI_PSK) {
		if (rawPsk.size() > 2 && rawPsk.front()== '"' && rawPsk.back() == '"') {
			psk = rawPsk.substr(1, rawPsk.size() - 2);
			wpa_ssid->psk_key_type = WAPI_PSK_TYPE_ASCII;
		} else {
			wpa_ssid->psk_key_type = WAPI_PSK_TYPE_HEX;
		}
	...
	if (isPskPassphraseValid(psk)) {
		wpa_printf(MSG_INFO, "isPskPassphraseValid");
		return createStatus(SupplicantStatusCode::FAILURE_ARGS_INVALID);
	}
	if (wpa_ssid->passphrase &&
	...
	if (setStringKeyFieldAndResetState(
		...
		wpa_printf(MSG_DEBUG, "set wpa psk");
		return createStatus(SupplicantStatusCode::FAILURE_UNKNOWN);
	}
#ifdef CONFIG_WAPI /*INTERFACE*/
	if(setStringFieldAndResetState(psk.c_str(), &(wpa_ssid->wapi_psk), "wapi_psk")) {
		wpa_printf(MSG_DEBUG, "set Wapi psk");
		return createStatus(SupplicantStatusCode::FAILURE_UNKNOWN);
	}
#endif
...
{
#ifdef CONFIG_WAPI /*_INTERFACE*/
	wpa_printf(MSG_DEBUG, "setWapiCertSuiteInternal suite: %s",suite.c_str());
	struct wpa_ssid *wpa_ssid = retrieveNetworkPtr();
	if(suite.compare("auto")==0)
	{
		wpa_ssid->wapi_user_cert_mode = WAPI_CERT_MODE_AUTO;
		wpa_printf(MSG_DEBUG, "WapiCertSelMode Auto: 0x%x", wpa_ssid->wapi_user_cert_mode);
		resetInternalStateAfterParamsUpdate();
	}
	else
	{
		wpa_ssid->wapi_user_cert_mode = WAPI_CERT_MODE_MANUAL;
		wpa_printf(MSG_DEBUG, "WapiCertSelMode Manual: 0x%x", wpa_ssid->wapi_user_cert_mode);
		resetInternalStateAfterParamsUpdate();
		if (setStringFieldAndResetState(
			suite.c_str(), &(wpa_ssid->wapi_user_sel_cert), "wapi_user_sel_cert")) {
			return createStatus(SupplicantStatusCode::FAILURE_UNKNOWN);
		}
	}
	return ndk::ScopedAStatus::ok();
...
#ifdef CONFIG_WAPI
	if (wpa_ssid->key_mgmt & WPA_KEY_MGMT_WAPI_PSK) {
		if (wpa_ssid->psk_key_type == WAPI_PSK_TYPE_HEX) {
			if (!wpa_ssid->wapi_psk) {
				return {"", createStatus(SupplicantStatusCode::FAILURE_UNKNOWN)};
			}
			return {wpa_ssid->wapi_psk, ndk::ScopedAStatus::ok()};
		} else if(wpa_ssid->psk_key_type == WAPI_PSK_TYPE_ASCII) {
			if (!wpa_ssid->wapi_psk) {
				return {"", createStatus(SupplicantStatusCode::FAILURE_UNKNOWN)};
			}
			std::string passphrase;
			passphrase.append("\"");
			passphrase.append(wpa_ssid->wapi_psk);
			passphrase.append("\"");
...
#ifdef CONFIG_WAPI
	struct wpa_ssid *wpa_ssid = retrieveNetworkPtr();
	if(wpa_ssid->wapi_user_cert_mode == WAPI_CERT_MODE_AUTO)
		return {"auto", ndk::ScopedAStatus::ok()};
	else
		return {wpa_ssid->wapi_user_sel_cert, ndk::ScopedAStatus::ok()};
#else
	return {"", createStatus(SupplicantStatusCode::FAILURE_UNKNOWN)};
#endif
}
...
{
	uint32_t psk_max_len = static_cast<uint32_t>(ISupplicantStaNetwork::PSK_PASSPHRASE_MAX_LEN_IN_BYTES);
#ifdef CONFIG_WAPI
	psk_max_len += 1;
#endif
	wpa_printf(MSG_INFO,"psk max len = %u", psk_max_len);
	if (psk.size() <
		static_cast<uint32_t>(ISupplicantStaNetwork::
					  PSK_PASSPHRASE_MIN_LEN_IN_BYTES) ||
		psk.size() > psk_max_len) {
		return 1;
	}

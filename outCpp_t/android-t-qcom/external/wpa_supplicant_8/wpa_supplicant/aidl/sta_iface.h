...
public:
	...
	::ndk::ScopedAStatus startDppEnrolleeResponder(int32_t in_listenChannel) override;
        ::ndk::ScopedAStatus doSupplicantCommand(const std::string& in_command) override;
...
private:
	...
	ndk::ScopedAStatus doSupplicantCommandInternal(const std::string& in_command);
	...
	struct wpa_supplicant* retrieveIfacePtr(std::string interface);

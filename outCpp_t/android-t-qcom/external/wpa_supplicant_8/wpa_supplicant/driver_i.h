...
static inline int wpa_drv_set_sar_limit(struct wpa_supplicant *wpa_s,
				  unsigned int set)
{
	if (!wpa_s->driver->set_sar_limit)
		return -1;
	return wpa_s->driver->set_sar_limit(wpa_s->drv_priv, set);
}

static inline int
wpa_drv_set_latency_level(struct wpa_supplicant *wpa_s,
				uint32_t level)
{
	if (!wpa_s->driver->set_latency_level)
		return -1;
	return wpa_s->driver->set_latency_level(wpa_s->drv_priv,
							level);
}
...
static inline int wpa_drv_set_ant_half(struct wpa_supplicant *wpa_s,
				  unsigned int set)
{
	if (!wpa_s->driver->set_nss_ant)
		return -1;
        unsigned int nss = set ? 1 : 2;
	return wpa_s->driver->set_nss_ant(wpa_s->drv_priv, nss);
}

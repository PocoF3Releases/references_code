	...
	const std::string owe_transition_ifname)
{
	...
	int ocv_config = 2;
	bool bcn_prot_config = true;
	std::string encryption_config_as_string;
	switch (nw_params.encryptionType) {
	case EncryptionType::NONE:
		// no security params
            	bcn_prot_config = false;
		break;
	...
	return StringPrintf(
		...
		ocv_config, bcn_prot_config ? 1 : 0);
}
...
::ndk::ScopedAStatus Hostapd::doHostapdCommand(const std::string& in_command)
{
	return doHostapdCommandInternal(in_command);
}
...
::ndk::ScopedAStatus Hostapd::doHostapdCommandInternal(const std::string& in_command)
{
	std::string reply;
	size_t reply_len = 0;
	...
	wpa_printf(MSG_ERROR, "doHostapdCommandInternal--enter");

	reply_buf = (char *)os_malloc(reply_size);
	if (reply_buf == NULL) {
		return createStatus(HostapdStatusCode::FAILURE_UNKNOWN);
	}
...
// MIUI ADD: START
	...
	set_hostapd_vendor_specific(in_command.c_str());
#endif
// END

	if ((interfaces_->count > 0) && (interfaces_->iface[0]->num_bss > 0))
		hapd = interfaces_->iface[0]->bss[0];
	else
		return createStatus(HostapdStatusCode::FAILURE_UNKNOWN);
	...
	reply_len = hostapd_ctrl_iface_receive_process(hapd, cmd_buf, reply_buf, reply_size, &from, fromlen);
	if (reply_len > 0) {

...
struct wpa_driver_ops {
	...
	/*
	 * set_sar_limit - Set SAR power limit
	 * @priv: Private driver interface data
	 * @set: TX power liminit set in BD file: 0 ~ 4 predefined set number, 5 means disable
	 */
	int (*set_sar_limit)(void *priv, unsigned int set);
	...
	/**
	 * set_latency_level - Set WLAN latency level for firmware preset
	 * strategies. Less latency will consume more power
	 * @priv: Private driver interface data
	 * @level: value from qca_wlan_vendor_attr_config_latency_level.
	 * Returns: 0 on success, -1 on failure
	 */
	int (*set_latency_level)(void *priv, uint32_t level);

	/*
	 * set_nss_ant_mode - Set nss and ant to 1x1 or 2x2
	 * @priv: Private driver interface data
	 * @enable: 1 means 1x1, and 2 means 2x2
	 */
	int (*set_nss_ant)(void *priv, int nss);

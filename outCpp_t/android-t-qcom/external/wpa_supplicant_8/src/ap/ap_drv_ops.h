...
// MIUI ADD: START
static inline int hostapd_drv_set_sar_limit(struct hostapd_data *hapd, unsigned int set)
{
	if (!hapd->driver || !hapd->driver->stop_ap || !hapd->drv_priv)
		return -1;
	return hapd->driver->set_sar_limit(hapd->drv_priv, set);
}
//END

// MIUI ADD: START
static inline int hostapd_drv_set_ant_half(struct hostapd_data *hapd, unsigned int set)
{
	if (!hapd->driver || !hapd->drv_priv || !hapd->driver->set_nss_ant)
		return -1;
        unsigned int nss = set ? 1 : 2;
	return hapd->driver->set_nss_ant(hapd->drv_priv, nss);
}
//END

...
#define  LOG(...)  __android_log_print(ANDROID_LOG_ERROR,"MiuiRenderEngine",__VA_ARGS__)
...
class MiuiRenderEngine {
public:
    uint32_t mVpWidth;
    uint32_t mVpHeight;

    MiuiShaderUtil mShaderUtil;
    ...
    static MiuiRenderEngine* create(float density, uint32_t screenWidth, uint32_t screenHeight);

    MiuiRenderEngine(float density, uint32_t screenWidth, uint32_t screenHeight);
    ~MiuiRenderEngine();
    ...
    void computeBlurParam(float blurRatio);

...
public:
    ...
    int mBlack;
    int mIncreaseSaturation;

    MiuiShaderUtilLowEffect() { };
    MiuiShaderUtilLowEffect(bool useHighPrecision, int black, int increaseSaturation);
    ~MiuiShaderUtilLowEffect() = default;

    // from surfaceflinger miuiblur shader
    const char* getVertexShader();
    const char* simpleFragmentShader();
    const char* saturationFragmentShader();
    void getFragmentShader(bool isX, int radius, int textureSize, String8 &builder);

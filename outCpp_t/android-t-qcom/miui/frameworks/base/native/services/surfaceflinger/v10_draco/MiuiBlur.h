...
public:
    ...
    inline static bool needLayerSkip(const sp<Layer>& layer) {
        ...
        // for draco
        return isEnable() && (isLayerBlur(layer) || isLayerCovered(layer) || isBlurLayerVisibleChild(layer) /*|| isLayerCurve(layer)*/);//set for gpu composer
    };
    static bool isBlurLayerVisibleChild(const sp<const Layer>& layer);
    static bool isLayerCovered(const sp<Layer>& layer);
    static bool isLayerCurve(const sp<const Layer>& layer); // for draco

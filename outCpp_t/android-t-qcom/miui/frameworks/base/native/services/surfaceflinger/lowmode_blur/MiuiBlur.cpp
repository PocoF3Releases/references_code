...
static MiuiRenderEngineLowEffect* sBlurRender = nullptr;
...
bool BlurSurface::step(int64_t now, uint8_t alpha) {
    ...
    ratio = mLayerDrawInfo.mDrawInfo.mBlurRatio * 2.0f;
    if (ratio >1.0f) {
        ratio = 1.0f;
    }
...
bool BlurSurface::computeBlurParam(float blurRatio, int blurMode) {
    ...
    mLayerDrawInfo.mDrawInfo.mBlurTextureWidth = Blur::sScreenWidth >> SCALE_SHIFT_BIT;
    mLayerDrawInfo.mDrawInfo.mBlurTextureHeight = Blur::sScreenHeight >> SCALE_SHIFT_BIT;
...
void Blur::initialize(float density, bool unused, uint32_t screenWidth, uint32_t screenHeight) {
    ...
    sBlurRender = MiuiRenderEngineLowEffect::create(density, screenWidth, screenHeight);
}
...
/*
    ...
    for (size_t i=0; i<blurLayers.size(); i++) {
        ...
        sBlurRender->computeBlurParam(blurLayer->miuiBlurRatio, blurLayer->miuiBlurMode);

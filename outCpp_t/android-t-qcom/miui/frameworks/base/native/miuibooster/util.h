...
static char const * TAG = "MiuiBooster"; // default log tag
void static setTag(char const *tag){
    TAG = tag;
}

static bool DEBUG = false; // global debug flag for native code default(false)
...
#if defined(ANDROID_NATIVE)

#include <android/log.h>
#include <fstream>

static int (*logFunc)(int prio, const char *tag,  const char *fmt, ...) = &__android_log_print;

#if 0

#include "xlogger.h"

static int __ComLogV(int level, const char *tag, const char* fmt, ...) {
...
#define perr(format, ...) (*logFunc)(ANDROID_LOG_ERROR , TAG, #format, ##__VA_ARGS__);
#define pwrn(format, ...) (*logFunc)(ANDROID_LOG_WARN , TAG, #format, ##__VA_ARGS__);
#define pdbg(format, ...) if (DEBUG)((*logFunc)(ANDROID_LOG_DEBUG , TAG, #format, ##__VA_ARGS__));
...
static string sockaddrToString(struct sockaddr_in addr) {
    char a[32] = {0};
...
static int sockaddrToIpPort(struct sockaddr_in addr, string *ip, int *pPort) {
    char a[32] = {0};
    unsigned char *bytes = (unsigned char *) &addr.sin_addr;

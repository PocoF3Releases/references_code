...
//#define DROP 0 // test packet loss , loss:  DROP != 0 && rand() % DROP == 0
...
//  const static int CONST_DATAQUEUE_SIZE = 10;
    ...
    typedef struct Receiver {
        ...
        map<int64_t, pair<uint32_t, uint8_t *> *> sessionMap;
    ...
    map<int64_t, Sender *> m_mapSender;
    map<int64_t, Receiver *> m_mapReceiver;
    list<TimeoutEvent *> m_eventTimerList;

    int deleteTimeoutEvent(TimeoutEvent *timeoutEvent) {
        for (list<TimeoutEvent *>::iterator it = m_eventTimerList.begin(); it != m_eventTimerList.end(); it++) {
            if (*it == timeoutEvent) {
    ...
    int processTimeout(TimeoutEvent *timeoutEvent) {
        ...
        const string strAddr = sockaddrToString(timeoutEvent->toaddr);
        if (!ss) {
        ...
        sendPackage(&timeoutEvent->toaddr, timeoutEvent->udpPkg->type, timeoutEvent->udpPkg->seq, timeoutEvent->udpPkg);
        return 0;
    }
    ...
    int cleanReceiver(Receiver *rs) {
        ...
        for (map<int64_t, pair<uint32_t, uint8_t *> *>::iterator it = rs->sessionMap.begin(); it != rs->sessionMap.end(); it++) {
            pair<uint32_t, uint8_t *> *p = (*it).second;
            delete[]p->second;
            delete p;
        }
        delete rs;
    ...
    int processReceive() {
        ...
        const string strAddr = sockaddrToString(sender);
        ...
        if (udpPkg.type == PACKAGE_SYN) {
            ...
            udpPkg.datalen = 0;
            sendPackage(&sender, PACKAGE_ACK, udpPkg.seq, &udpPkg);
            return 0;
        }
...
//        }
        ...
        if (ss->status == SYN_SENT) {
            if (ss->synTimeoutEvent.udpPkg == NULL || udpPkg.seq != ss->synTimeoutEvent.udpPkg->seq) {
                pwrn("S_IGNORERecvSEQ [%d,%d] %s SS:%s", udpPkg.seq, ss->synTimeoutEvent.udpPkg->seq, udpPkg.toString().c_str(),
                     ss->toString().c_str());
                return 0;
            }
    ...
    string getWindowSeqList(TimeoutEvent *window) {
        stringstream s;
    ...
    TimeoutEvent *getWindowBySeq(TimeoutEvent *window, uint32_t seq) {
        for (uint32_t i = 0; i < CONST_WINDOW; i++) {
            if (window[i].udpPkg && window[i].udpPkg->seq == seq) return &window[i];
        }
        return NULL;
    }
    ...
    TimeoutEvent *checkEmptyWindow(TimeoutEvent *window, int expectSeq) {
        ...
        int emptyIndex = -1;
        for (int i = 0; i < CONST_WINDOW; i++) {
            if (!window[i].udpPkg) emptyIndex = i;
            else if (minSeq > window[i].udpPkg->seq) minSeq = window[i].udpPkg->seq;
        }

        if (emptyIndex == -1)return NULL;
        if ((minSeq != INT_MAX) && (  (int)(minSeq + CONST_WINDOW - 1) < expectSeq)) return NULL;
        return &window[emptyIndex];
    }

    int windowAllEmpty(TimeoutEvent *window) {
        for (uint32_t i = 0; i < CONST_WINDOW; i++) {
            if (window[i].udpPkg) return 0;
        }
        return 1;
    }
    ...
    int sendPackage(struct sockaddr_in *addr, uint8_t type, u_int32_t seq, UdpPkg *udpPkg) {
        ...
        const string strAddr = sockaddrToString(*addr);
        ...
        if (udpPkg->type == PACKAGE_ACK) return sendRet; // Set a timeout event if the packet isn't an ACK
    ...
    int dataRecv(UdpPkg *udpPkg, Receiver *rs) {
        pair<uint32_t, uint8_t *> *sessionPair = rs->sessionMap.count(udpPkg->sessionid) ? rs->sessionMap[udpPkg->sessionid] : NULL;
        ...
        if (!sessionPair) {
            sessionPair = new pair<uint32_t, uint8_t *>(0, new uint8_t[udpPkg->totallen]);
    ...
    int checkSendQueue() {
        ...
        for (map<int64_t, Sender *>::iterator it = m_mapSender.begin(); it != m_mapSender.end(); it++) {
            Sender *ss = it->second;
    ...
    int loop() {
        while (m_fd > 0 || m_eventTimerList.size()) {
            ...
            if (m_eventTimerList.size() && timeout <= 0) { //timeout
                const list<TimeoutEvent *>::iterator it = m_eventTimerList.begin();
            ...
            const int selectRet = select(max(m_fd, m_pipefd[0]) + 1, &fdset, NULL, NULL, pdiff);

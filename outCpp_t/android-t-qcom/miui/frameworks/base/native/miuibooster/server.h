...
private :
    ...
    typedef struct RecvCache {
        ...
        ~RecvCache(){
            reqHeaderOffset = 0;
            if(reqPayload) delete[] reqPayload;
            reqPayload = NULL;
            reqPayloadOffset = 0;
        }
    ...
    map<int, RecvCache*> mapRecvCache;

    int recvEvent(Event event, int fd, uid_t uid, const char *path, uint8_t *recvData, int recvDataLen) {
        ...
        if (event == EVENT_DATA) {
            ...
            while (const uint32_t readLeft = (recvDataLen - (readPtr - recvData))) {
                ...
                if (recvCache->reqPayloadOffset < pHeader->bodylen) {
                    if (recvCache->reqPayload == NULL) recvCache->reqPayload = new uint8_t[pHeader->bodylen];

                    const int shouldRead = min(pHeader->bodylen - recvCache->reqPayloadOffset, readLeft );
                ...
                int ret = pp->processReceive(uid, &fd, pHeader, recvCache->reqPayload, pHeader->bodylen, inst);
                if (ret < 0) delete pp;
                else respQueue.push(pp);
        ...
        if (event == EVENT_SYSTEM) {
            if (recvDataLen == 1) {
                inst->onEvent(*recvData);
            }
            return 0;
        }
...
public:
    ...
    int onCallback(int uid, int funcid, uint64_t requestid, uint64_t timestamp, int retCode, uint8_t *data, int len) {
		UNUSED(requestid);
		UNUSED(timestamp);
		UNUSED(retCode);
        ...
        int ret = pp->processSend(funcid, &fd, data, len);
        pdbg("callback funcid:%d, len:%d ret:%d", funcid, len, ret);
        respQueue.push(pp);
        return 0;
    }
...
private :
    ...
    int recvEvent(Event event, struct sockaddr_in *addr, uint64_t sessionid, uint8_t *data, int len) {
        ...
        if (event == EVENT_DATA) {
            ...
            int ret = pp->processReceive(uid, addr, (AMCReqHeader *) data, data + sizeof(AMCReqHeader), len - sizeof(AMCReqHeader), inst);
            if (ret < 0) delete pp;
            else respQueue.push(pp);
